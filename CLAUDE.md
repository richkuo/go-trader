...
- `planHLCloseOrder`=SOLE size+mode of execute/`type=manual` SIZED close (not sole-owner whole/`ForceFullClose`/open cleanup). Short fill books fill; re-arm size ONLY `resolveHLCloseRemainderStop`, never `hlSLEffectiveQty`; Q=0 verify-first cancels unconfirmed SL/TP; unverified/unbacked/unplaced=ONE CRITICAL; `order_outcome`=SOLE known outcome.
