...
- `executor.go`/`shutdown.go`: side-effect wrappers use `runPythonSideEffect`, NEVER `runPython`. `confirmHyperliquidExecuteFill` gates each live HL book: finite `AvgPx>0`+`TotalSz>0` in `Execution.Fill`, else no book; `check_hyperliquid.py execute` exits 1 on no fill.
- `server.go`/`ui_*.go`: lock order `mu > strategiesMu`. Loopback only. `/tuning` never writes config; `ui_tuning.go` uses `spawnPythonProcessWithEnv` (NEVER `runPython*`); `POST /api/tuning/apply` = sole promotion. `ui_partition.go`: `uiPartitionParam` = sole `?partition=` resolver for `/api/strategies(/overview)?`, `/api/leaderboard`, `/api/strategies/dead`, `/api/diagnostics`; unparseable=400, unowned=404; diagnostics filters by `SourceRole` before paging+total; cash flow stays `live_owned`; correlation/portfolio-risk under a selection NEVER fall back to the untagged legacy field.
- `config*.go`: `CurrentConfigVersion=19`, `MinSupportedConfigVersion=13`; 7 exclusive HL stop fields (none = `DefaultStopLossATRMult=1.0`); `close_strategy` canonical, unknown-key guard. On-chain TP gate != `len(tiers)>0`. `CircuitBreaker *bool` ONLY via accessors. `portfolio_risk.paper`/`paper_sources[].portfolio_risk`: evaluators use `partitionRiskConfig`, nested `paper` rejected. `portfolio_risk.include_paused_in_warning` (default false, root>paper>source, false never overrides an enabled layer): `portfolio_warning.go` excludes a paused strategy from the Top Contributors block ONLY when flat (`Quantity==0` on regular AND option); an open position stays visible under every partition regardless of pause.
...
- `portfolio_scope.go`/`risk_partition.go`: `PortfolioScope` from `isLiveArgs` = SOLE mode classifier; `RiskPartition{Scope,Source}` (`live`/`paper`/`paper:<id>`; `paper_source` names a `paper_sources` id, never on a live strategy) = SOLE risk+storage owner. New surface: `activePartitions`, `filterStatesByPartition`/`strategiesInPartition`, never the roster.
- `state.go`/`db.go`: SQLite-only, idempotent migrations. `AppState.PortfolioRisk`/`CorrelationSnapshot` = per-`RiskPartition` maps read only via `partitionRisk`/`partitionRiskIfPresent`/`partitionCorrelation`. `initial_capital` only via `SetInitialCapital`.
- `state_store*.go`/`storage_*.go`: identity map immutable; 1 file per partition, pairwise-distinct, order primary>paper>sources by id = lock+save order. EVERY DB caller via `StateStore` (`dbForStrategy`, live-only `liveFile`); ids translate INSIDE `StateDB`; load maps (file,scope)->partition; diagnostics route by `SourceRole` NEVER scope. New table: SKILL.md Storage Ownership row. Manual acks by ROW ID in-tx, NEVER a high-water mark; book+queued row = ONE tx; failing rows end; unknown ownership errs BEFORE mutation; combined reads fail whole.
- `merge-paper-instance.sh`: binaries run on config COPIES; `inspect`/`storage-inspect` = `LoadConfigReadOnly` (no rewrite); both locks held preflight>apply except `--diff` (config-only); `--align-to-live` writes `.aligned` never source; compose lists ALL root-key conflicts then refuses; alias unconditional via `paper_alias.py`; `--once`/`--probe-only` never proof, zero paper override inherits. `channels`/`trade_alert_channels` `-paper` keys skip when the merged bare key already routes there (mirrors `resolveChannel`); `dm_channels` `-paper`/`-paper:<id>` keys NEVER skip (`tradeAlertRoutes` reads that literal key, no fallback, `scheduler/notifier.go`); `--diff` previews as `channel-plan` lines. `inspect` DM view = same exact-key lookup (`resolveDMKeyOverMaps`, `scheduler/inspect_cmd.go`); proof refuses a staged paper DM route the send path disagrees with.
- `risk.go`: `CheckRisk` skips `manual`. Corrupt position (qty<=0 or avgCost<=0) = zero-PnL `*_corrupt` leg, cash untouched. Latch: ONE owner per cycle+partition; `DrawdownReadingSubstituted` labelled everywhere, untrusted over-limit defers, never vetoes. Paper `equityTrusted` always true. `ResetPortfolioKillSwitchManual` = sole DM reset; `AutoResetConfirmedFlatKillSwitch`/`ClearLatchedKillSwitchSharedWallet` `ScopeLive` only.
- `daily_loss.go`: hold-only, UNLATCHED pure read, PRE-FEE realized PnL, never force-closes, per partition. New `portfolio_risk` gate copies this shape.
...
- `hyperliquid_balance.go`: reconciliation-close alerts outside `mu`; hedge-leg rows never public. `pause.go`: paused is NOT a `dueStrategies` skip. `pausedBlocksSignal` holds position-increasing signals at all 6 regime-gated dispatch sites (+per-scope persistence hold); closes/trailing SL/ratchet/protection pass.
- Regime (`regime*.go`): stamp all 5 execute dispatches; strip `sl_after` before ATR parse; `regime_unified.go` owns SL; directional policy DEFAULT-OFF; profile allocation flat-only; dynamic/divergence HL-live-only. Trade-alert span = bundle spec (`tradeAlertRegimeConfig`): `regime.timeframe` else `Args[2]`; options = own fixed spec (window+period+tf); NEVER `sc.Timeframe`, never `cfg.Regime` for options.
- `trailing_tp_ratchet.go`: NO on-chain TP; HL perps+`manual`. `hedge.go`: HL perps only; ONE reconciler (`hedgeTargetDecision`+`runHedgeSync`), ownership `Position.HedgeFor` ONLY; hedge PnL `RecordHedgeTradeResult`, never `RecordTradeResult`; fail-closed unwind + CRITICAL DM. Both hot-reload-blocked while open; hedge also backtester-rejected.
...
- `scale_in.go`: geometry frozen via `RiskAnchorPrice`, never blended `AvgCost`. `manual*.go`: kill-switch+CB gated; SL edits queue `PendingManualAction`, never a bare book write; only re-arm/restore write it; rejected close never assumes an unconfirmed cancel spared the trigger; verify-first restore of SL+cancelled TPs, NEVER a 2nd placement path; unverified = CRITICAL; adopted SL edits never gate; `force-close` live HL perps only.
...
- `hyperliquid_protection.go`: reduce-only, on-chain TP needs `strategyUsesTieredTPATRClose`+live (paper never); `hyperliquid_open_trailing.go` arms SL at open. `hyperliquid_shared_close_floor.go` cycle: <$10 shared-coin close escalates ONLY if each peer flat on-chain (refetched)+in book, before SL blocks, else ONE alert+hold, `venue_rejected` never resends; re-arm (7 owners) same-cycle ONLY after a SUBMITTED cancel; 2nd refusal holds, any wording; manual guard skips a queued-row symbol; failed-close owner re-arms SL leg ONLY, 3 blocks=CRITICAL; unknown TP place=armed, never re-placed. Operator floor (`decideOperatorSharedCloseFloor`): same test+same side+no larger else refuse (not hold); unreadable mark skips escalation.
...
- `kill_switch_limit_orders.go`: cancel each `pending_limit_orders` row BEFORE flatten (keyed on ROW); **never gate `reconcilePendingLimitOrders` on kill-switch**; cancel is not adoption, never auto-delete an unadopted fill.
- `orphan_limit_cancel_alerts.go`: cancel-only lane, status-FIRST finalize, books NO fill; `orphanLimitCancelState` = SSoT (off-book fill = UNTRACKED POSITION). `limit_fill_exposure.go`: books a limit fill ONLY once live exposure confirms; per-coin aggregate, never per-row greedy; fail-closed same-dir+contained, `unreadable`/`unbacked` refuse book+block delete.
...
- Locking: `mu sync.RWMutex`, 6 phases (RLock > Lock(CheckRisk) > no-lock subprocess > Lock(execute) > marks > RLock(status)); symbol locks > `mu`. Skip-reason checks BEFORE spawn; capture `posSide`+`posQty` in Phase 1; `liveExecFailed` guards live exec.
...
- HL stops: `EffectiveStopLossPct` 7 exclusive owners; scalar-regime swap blocked while open. `risk_per_trade_pct` fails closed on unresolvable stop, exclusive vs sizing_leverage/margin/scale_in. Trailing SL replace only past `TrailingStopMinMovePct`; `hlSLEffectiveQty=min(virtual,onChain)`, snapshot = full protection surface. Peers share `margin_mode`+`leverage`, `update_leverage` when flat.
- SIGHUP `validateHotReloadCompatible` blocks add/remove, script/args/type/platform/HTFFilter, kill-switch identity, `db_file`/`paper_db_file`/`paper_sources`/`paper_source`, effective `storage_strategy_id`, `max_notional_usd` (per partition), `market_feed`.
- New per-strategy flag: field, `run*Check` CLI, Python parse, InitOptions/wizard; runtime-required: probe argvs.
- Notifications: `MultiNotifier`; paper routes via `resolveChannelKey` (`<platform>-paper:<id>` before `-paper` before bare). `SendToPartitionChannels` builds each partition's destinations from its own roster (`resolveTradeChannel`, rebuilt on `ReloadConfig`), never a suffix scan; `SendToAllChannels` only when that set is empty.
...
## Testing
- Each feature/bug fix needs a test guarding a behavior contract (money, state, protection, subprocess, migration, backtest parity); assert outcomes, pin only operator-decision wording, no constants/round-trips, table-driven.
- **Test budget.** Only that contract list; max 1 table-driven test per new function. `check_test_budget.py` fails CI on a wording-only test outside `scripts/test_budget_baseline.json` or a stale entry; entries only for operator-decision wording; `--write-baseline` after a delete.
...
- Pytest: `uv run --no-sync python -m pytest shared_strategies/ shared_tools/ platforms/ backtest/`; `shared_scripts/test_*.py` by path; Registry/sys.path tests: FULL suite. CI `-n auto`: never bare-`import` an ambiguous name; intermittent fail = isolation, not flake.
