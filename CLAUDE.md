...
- systemd units: `ProtectSystem=strict`, no `PATH`/`UV_CACHE_DIR` injection; secrets in `/opt/go-trader/.env`. Deployed config OUT of tree at `/var/lib/go-trader[/<instance>]/config.json`; `scheduler/config.json` = transition symlink.
...
- `executor.go`/`shutdown.go`: **new side-effecting wrapper → `runPythonSideEffect`, NEVER `runPython`.** `confirmHyperliquidExecuteFill` gates every live HL book: `Execution.Fill` with finite `AvgPx>0`+`TotalSz>0`, else no book. `check_hyperliquid.py execute` exits 1 on no confirmed fill.
- `server.go`/`ui_*.go`: **lock order `mu → strategiesMu`** (reusing `mu` deadlocks SIGHUP). **Loopback only.** `/tuning` never writes config; `ui_tuning.go` → `spawnPythonProcessWithEnv` (NEVER `runPython*`); `POST /api/tuning/apply` = sole promotion.
...
- `state_store*.go`/`storage_*.go`: identity map immutable. EVERY DB caller via `StateStore` (`dbForStrategy`; live-only `liveFile`); ids translate INSIDE `StateDB`. New table → a SKILL.md § Storage Ownership row. Manual acks by ROW ID in the persisting tx, NEVER a high-water mark. Unknown ownership errors BEFORE mutation; combined reads fail whole.
- `merge-paper-instance.sh`: read-only inspection via each binary ONLY; both locks held preflight→apply; NEVER the migrating writer, `--once` or `--probe-only` as proof; zero paper override inherits.
...
- `shared_scripts/`: every check script takes `--regime-payload-json`, probed at startup. `platforms/<name>/adapter.py`: one `*ExchangeAdapter` per file; HL `_sz_decimals()` via `name_to_asset(symbol)`. `funding_fetcher.py`: `merge_asof` backward, DISJOINT `funding_coverage`; `regime.py` ATR pinned to `simple`.
- `shared_strategies/`: open SSoT `open/registry.py`; **`open/{spot,futures}/strategies.py` = shims, never edit.** Close via `close/registry.py`, imported `from close_registry_loader import …`, never bare `import registry`. `hurst_exponent` (DFA) = SSoT.
...
- Subprocess contract: JSON on stdout and exit 1 on error; Go parses regardless.
...
## Testing
- New functionality and each bug fix need a test guarding a behavior contract (money, state, protection, subprocess contracts, migration, backtest parity); assert outcomes; table-driven variants.
- **Test budget.** Only the contract list; max one table-driven test per new function; none asserting only wording, a constant or a round-trip (operator-decision wording aside). `check_test_budget.py` fails CI on a wording-only test not in `scripts/test_budget_baseline.json` or a stale entry; entries only for operator-decision wording; `--write-baseline` after a delete.
...
- Pytest: `uv run --no-sync python -m pytest shared_strategies/ shared_tools/ platforms/ backtest/`; `shared_scripts/test_*.py` explicitly. Registry/sys.path tests → FULL suite. CI `-n auto`: never bare-`import` an ambiguous name; intermittent failure = isolation, never a flake.
