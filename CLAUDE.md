...
- `hyperliquid_balance.go`: reconciliation-close alerts outside `mu`; hedge-leg rows never reach public route. `pause.go`: paused is NOT a `dueStrategies` skip. `pausedBlocksSignal` holds position-increasing signals at all 6 regime-gated dispatch sites (+per-scope persistence hold); closes/trailing SL/ratchet/protection pass.
...
## Testing
- Each new feature/bug fix needs a test guarding a behavior contract (money, state, protection, subprocess contracts, migration, backtest parity). Assert outcomes; pin only operator-decision wording; no constants or round-trips; table-driven variants.
