...
- New platform: SKILL.md § Custom Platform Integration lists every touchpoint. Adapters load via `importlib`, class `endswith("ExchangeAdapter")`; check scripts use public methods only.
...
- `.github/workflows/claude.yml`: least-privilege split; mode routing fail-closed (untrusted/fork → review); no-execution ban in the agent, commit/push implement-mode only; assembled prompt must not contain `"`, `` ` ``, or `$`; `.github/scripts/` holds ONLY `test_workflow_logic.py`. SKILL.md § The `@claude` GitHub workflow.
...
## Build & Deploy
- **Update only with `bash scripts/update.sh --restart`. Never rebuild Go alone**: Go and Python share an argv contract at one SHA.
- Startup probe: non-zero `--probe-only` → `os.Exit(78)`; both unit files set `RestartPreventExitStatus=78`. Service-file edits need `daemon-reload`.
- Post-update: SKILL.md § Post-Update Agent Protocol. After a Python-launcher change, smoke with `./go-trader --once`.

## Backtest
- Harness map `docs/backtesting-registry.md`: update its row in the PR adding or deprecating a harness.
...
## Testing
- New functionality and each bug fix need tests guarding a behavior contract (money, state, protection, subprocess contracts, migration, backtest parity). Assert outcomes; pin log or DM wording only when it drives an operator decision; no constants or struct round-trips; table-driven for variants.
- **Test budget.** Tests only for that contract list, at most one table-driven test per new function, none asserting only wording, a constant, or a round-trip (operator-decision wording excepted); reviewers flag others. `scripts/check_test_budget.py` fails CI on a wording-only test outside `scripts/test_budget_baseline.json` or a stale entry; add an entry only for operator-decision wording; `--write-baseline` after deleting one.
...
- Pytest: `uv run --no-sync python -m pytest shared_strategies/ shared_tools/ platforms/ backtest/`; `shared_scripts/test_*.py` invoked explicitly. Registry/sys.path tests → the FULL suite. CI uses `-n auto`: never bare-`import` an ambiguous module name; an intermittent failure is test isolation, never a flake.
