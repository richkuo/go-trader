...
- `planHLCloseOrder`=SOLE execute/`type=manual` SIZED close (not whole/ForceFullClose); refetch fail=defer unless uncapped `reduce_only`. Short fill books fill. rearm ONLY `resolveHLCloseRemainderStop`; Q=0 verify-first cancels SL/TP; ONE CRITICAL; `order_outcome`=outcome.
