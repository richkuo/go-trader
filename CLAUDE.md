...
- `planHLCloseOrder`=SOLE size+mode of execute/`type=manual` SIZED close (not sole-owner whole/`ForceFullClose`/open cleanup); refetch fail=defer unless uncapped `reduce_only`. Re-arm ONLY `resolveHLCloseRemainderStop`; Q=0 verify-first cancels unconfirmed SL/TP; unverified/unbacked/unplaced=ONE CRITICAL; `order_outcome`=SOLE known outcome.
