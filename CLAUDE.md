...
- Per-strategy circuit-breaker pending close (#356/#359/#363): `RiskState.PendingCircuitCloses map[string]*PendingCircuitClose` holds queued closes keyed by platform constant (e.g. `PlatformPendingCloseHyperliquid = "hyperliquid"`, NOT the strategy-ID prefix). Use `setPendingCircuitClose` / `clearPendingCircuitClose` / `getPendingCircuitClose` accessors — never write the map directly. `CheckRisk` takes `*PlatformRiskAssist`; all five `set*CircuitBreakerPending` helpers must be called at BOTH CB-fire sites (max-drawdown AND consecutive-losses blocks) — a missing call at either site silently skips the close. DB column: `risk_pending_circuit_closes_json`.
- Operator-gap pending closes (#363 phase 5): OKX spot and Robinhood options have no safe auto-close primitive. `PendingCircuitClose.OperatorRequired bool` (`json:"operator_required,omitempty"`) flags entries that the scheduler will never auto-close. Keys: `PlatformPendingCloseOKXSpot = "okx_spot"`, `PlatformPendingCloseRobinhoodOptions = "robinhood_options"` — deliberately distinct from the portfolio-kill `"okx"` / `"robinhood"` keys so auto-close drains never dequeue them. `setOperatorRequiredCircuitBreakerPending(sc, s)` enqueues; `drainOperatorRequiredPendingCloses(state, notifier, mu)` emits a CRITICAL warning each cycle (stdout + Discord + Telegram) without clearing the pending — it stays set until the operator flattens manually. `scheduler/operator_required_close.go` — `planOperatorRequiredWarning` (pure, testable), `formatOperatorRequiredWarningMessage`, `operatorRequiredNotifier` interface (implemented by `*MultiNotifier`). Portfolio kill switch clears operator-required pendings alongside auto-close pendings to avoid double-notify.
