...
- Per-strategy circuit-breaker pending close (#356/#359): `RiskState.PendingCircuitCloses map[string]*PendingCircuitClose` holds queued reduce-only closes keyed by `StrategyConfig.Platform` (e.g. `"hyperliquid"` — use constant `PlatformPendingCloseHyperliquid`, NOT the strategy-ID prefix). Use `setPendingCircuitClose` / `clearPendingCircuitClose` / `getPendingCircuitClose` accessor methods — never write the map directly. `CheckRisk` takes `*PlatformRiskAssist` (renamed from `*HLRiskAssist` in #359); only HL fields populated today, OKX/TS/RH added in later phases. DB column: `risk_pending_circuit_closes_json` (renamed from `risk_pending_hl_close_json`).
