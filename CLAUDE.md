...
- Open/close split (#480/#483): `StrategyConfig.OpenStrategy` overrides the entry strategy name from `Args[0]`; `CloseStrategies` is an ordered list of exit evaluators, with max `close_fraction` winning; `DisableImplicitClose` suppresses the legacy signal-reversal close when no explicit close list is configured. Composition lives in `scheduler/strategy_composition.go` (Go) and `shared_tools/strategy_composition.py` (Python); the Python helper holds the implicit-close fallback. Old configs omit these fields and stay on the exact legacy subprocess path. Opted-in spot/perps/futures check scripts emit `open_action`, `close_fraction`, `close_strategy`, and the composed legacy `signal`; close composition runs before opening. Options strategies do not use this split yet.
...
- Per-trade stop-loss: `EffectiveStopLossPct(sc)` (HL perps only) returns the price-% trigger, picking from `StopLossPct` (explicit price %) or `StopLossMarginPct/Leverage` (margin-aware) or falling through to `min(MaxDrawdownPct, 50)` when both are nil — but only for sole-owner strategies; `LoadConfig` calls `normalizeHyperliquidPeerStopLosses` to rewrite omitted fields to explicit `0` for same-coin peer groups, so the auto-derive never fires for peers (#484/#490/#494). Explicit `0` on either field opts out entirely. Reduce-only trigger OID stored on `Position.StopLossOID`; `HyperliquidLiveCloser` close path takes `cancelStopLossOIDs []int64`. HL exchange caps open trigger orders at 1000 per account (not 10/day, #481).
