...
- Adding a new spot/futures strategy (no new platform): (1) add `@register_strategy` function to `shared_strategies/spot/strategies.py`, (2) add same to `shared_strategies/futures/strategies.py`, (3) add short name to `knownShortNames` in `scheduler/init.go`, (4) add param grid entry to `DEFAULT_PARAM_RANGES` in `backtest/optimizer.py` or CI's `test_param_ranges_cover_every_registered_strategy` will fail — auto-discovery handles all platform configs
...
- Fetch latest claude[bot] review on a PR: `gh api repos/richkuo/go-trader/issues/<N>/comments --jq '[.[] | select(.user.login=="claude[bot]")] | last | .body'` (inline review comments via `/pulls/<N>/comments`, review summaries via `/pulls/<N>/reviews`)
