...
Guardrails only. Mechanism: `SKILL.md` § Subsystem Mechanism Reference; operator flows: `SKILL.md`, `docs/POST_UPDATE_HISTORY.md`. Keep under 15,000 bytes; never split (agents auto-load only the root `CLAUDE.md`; `AGENTS.md` symlinks it). Shorten wording, never drop a guardrail.

## Environment
- Go 1.26.2 (`/opt/homebrew/bin/go`). Python via `uv run --no-sync python`; scheduler calls `.venv/bin/python3`. `uv sync` per worktree.
- systemd units: `ProtectSystem=strict`, no `PATH`/`UV_CACHE_DIR` injection; secrets in `/opt/go-trader/.env`. Deployed config lives OUT of tree at `/var/lib/go-trader[/<instance>]/config.json`; `scheduler/config.json` = transition symlink.

## Decision Priorities
- **Always the absolute best technical solution.** Cost, compute, time, effort, tests, code volume never narrow the option space; never override the branch+PR workflow, verifying issue claims against code, or destructive-action safety.
...
- `shared_scripts/`: check scripts take `--regime-payload-json`, startup-probed. `check_hyperliquid.py` close gate: floor by lot, never a full close, unknown lot = no gate. `platforms/<name>/adapter.py`: one `*ExchangeAdapter` each; HL `_sz_decimals()` via `name_to_asset`. `funding_fetcher.py`: `merge_asof` backward, DISJOINT `funding_coverage`; `regime.py` ATR = `simple`.
