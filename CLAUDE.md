...
- `planHLCloseOrder`=SOLE size+mode per execute-lane/`type=manual` close (not open/flip/open cleanup), pre-send refetch (fail=defer unless uncapped `reduce_only`). Short fill books fill; re-arm size=`resolveHLCloseRemainderStop` ONLY: min(remainder, own-side chain-net same-side peers), post-close read first, no `hlSLEffectiveQty`; Q=0 verify-first cancels unconfirmed SL/TP; unverified/unbacked/unplaced=ONE CRITICAL, nil=UNVERIFIED; `order_outcome`=SOLE known outcome.
