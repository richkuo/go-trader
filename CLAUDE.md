...
- `planHLCloseOrder`=SOLE size+mode execute/`type=manual` SIZED (not whole/ForceFullClose); refetch fail=only uncapped reduce_only. Short fill books. rearm ONLY `resolveHLCloseRemainderStop`; Q=0 verify-first cancels unconfirmed SL/TP; ONE CRITICAL; `order_outcome`=SOLE outcome.
