...
- Open/close split (#480): `StrategyConfig.OpenStrategy` overrides the entry strategy name from `Args[0]`; `CloseStrategies` is an ordered list of exit evaluators, with max `close_fraction` winning; `DisableImplicitClose` suppresses the legacy signal-reversal close when no explicit close list is configured. Old configs omit these fields and stay on the exact legacy subprocess path. Opted-in spot/perps/futures check scripts emit `open_action`, `close_fraction`, `close_strategy`, and the composed legacy `signal`; close composition runs before opening. Options strategies do not use this split yet.
