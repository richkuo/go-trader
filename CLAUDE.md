...
Guardrails only. Mechanism: `SKILL.md` § Subsystem Mechanism Reference; operator flows: `SKILL.md`, `docs/POST_UPDATE_HISTORY.md`. Under 15,000 bytes; never split (agents auto-load only root `CLAUDE.md`; `AGENTS.md` symlinks it). Shorten wording; drop no guardrail.

## Environment
- Go 1.26.2 (`/opt/homebrew/bin/go`). Python via `uv run --no-sync python`; scheduler calls `.venv/bin/python3`. `uv sync` per worktree.
- systemd units: `ProtectSystem=strict`, no `PATH`/`UV_CACHE_DIR` injection; secrets in `/opt/go-trader/.env`. Deployed config at `/var/lib/go-trader[/<instance>]/config.json`; `scheduler/config.json` = transition symlink.

## Decision Priorities
- **Always the absolute best technical solution.** Cost, compute, time, effort, tests, code volume never narrow the option space; branch+PR workflow, issue-claim verification against code and destructive-action safety always win.
...
- `executor.go`/`shutdown.go`: **side-effecting wrappers → `runPythonSideEffect`, NEVER `runPython`.** `confirmHyperliquidExecuteFill` gates every live HL book: finite `AvgPx>0`+`TotalSz>0` in `Execution.Fill`, else no book; `check_hyperliquid.py execute` exits 1 on no fill.
- `server.go`/`ui_*.go`: **lock order `mu → strategiesMu`**. **Loopback only.** `/tuning` never writes config; `ui_tuning.go` → `spawnPythonProcessWithEnv` (NEVER `runPython*`); `POST /api/tuning/apply` = sole promotion.
...
- `portfolio_scope.go`: `PortfolioScope` from `isLiveArgs` = **the single mode classifier**. `activeScopes` evaluates ONLY configured scopes. New portfolio-wide surface → subset via `filterStatesByScope`/`strategiesInScope`, never whole roster.
- `state.go`/`db.go`: SQLite-only, idempotent migrations. `AppState.PortfolioRisk`/`CorrelationSnapshot` = per-scope maps, read via `scopeRisk`/`scopeRiskIfPresent`/`scopeCorrelation` only. `initial_capital` only via `SetInitialCapital`.
- `state_store*.go`/`storage_*.go`: identity map immutable. EVERY DB caller via `StateStore` (`dbForStrategy`; live-only `liveFile`); ids translate INSIDE `StateDB`. New table → SKILL.md § Storage Ownership row. Manual acks by ROW ID in the persisting tx, NEVER a high-water mark. Unknown ownership errors BEFORE mutation; combined reads fail whole.
- `merge-paper-instance.sh`: binaries run on config COPIES; `inspect`/`storage-inspect` use `LoadConfigReadOnly` (NEVER a rewrite); both locks held preflight→apply; `--once`/`--probe-only` never proof; zero paper override inherits.
...
- `exposure_cap.go`: **blocking-only, direction-aware**; **single exposure model** `computeAssetDeltas`, shared with `ComputeCorrelation`. `notional_cap.go`: **hold-only via `pausedBlocksSignal`**, never skips a cycle; restart-required.
- `replay_log.go`/`replay_mirror.go`: **DEFAULT-OFF**; HL perps, flat-only hot-reload; one mirror per source (`replayMirrorSourceID`).
- `hl_batch.go`: shared-state failure ⇒ per-strategy fallback same cycle; never blank a close/SL/ratchet/protection/hedge. `GO_TRADER_HL_BATCH=0` disables. `market_feed=websocket`: every check path reads one sealed stdin `marketSnapshot`; missing frame = error, NEVER a private fetch.
...
- `pause.go`: paused is NOT a `dueStrategies` skip. `pausedBlocksSignal` holds position-increasing signals at all 6 regime-gated dispatch sites (per-scope persistence hold too); closes/trailing SL/ratchet/protection pass.
...
- `hyperliquid_protection.go`: reduce-only; on-chain TP only when `strategyUsesTieredTPATRClose` AND live (paper never). `hyperliquid_open_trailing.go` arms SL at open.
...
- `orphan_limit_cancel_alerts.go`: cancel-only lane, status-FIRST finalize, books NO fill; `orphanLimitCancelState` = SSoT (off-book fill = UNTRACKED POSITION). `limit_fill_exposure.go`: book a limit fill ONLY after live exposure confirms; per-coin aggregate, never per-row greedy; fail-closed same-direction+contained; `unreadable`/`unbacked` refuse book AND block delete.
- `shared_scripts/`: check scripts take `--regime-payload-json`, probed at start. `check_hyperliquid.py` close gate: live-only, lot-floored, never a full close, no lot = no gate; shared-coin full close <$10 (`hyperliquid_shared_close_floor.go`): escalate only on a refetched flat peer, else ONE alert + held; venue-rejected hold never resends. `platforms/<name>/adapter.py`: one `*ExchangeAdapter`; HL `_sz_decimals()` via `name_to_asset`. `funding_fetcher.py`: `merge_asof` backward, DISJOINT `funding_coverage`; `regime.py` ATR `simple`.
...
## Pull Requests
- `Closes #<N>` in body; never bare `#N` in lists. Title `type(#<N>): summary [C<score>, <model>, <effort>]` (`, fableplan` after a Fable plan). Body: `## Plain simple English` (<55 words) first, then `## Summary` and verification.
...
- Review findings: restate as an invariant, list states breaking it (inverse, compound), add class tests.
- `.github/workflows/claude.yml`: least-privilege split; mode routing fail-closed (untrusted/fork → review); no-execution ban in the agent, commit/push implement-mode only; prompt never holds `"`, `` ` `` or `$`; `.github/scripts/` keeps ONLY `test_workflow_logic.py`.
...
## Build & Deploy
- **Update only with `bash scripts/update.sh --restart`. Never rebuild Go alone**: Go+Python share one argv contract per SHA. `update_resolve_db_exclude` lists every state file.
- Exit codes: probe 78, singleton 79, storage 80; units set `RestartPreventExitStatus=78 79 80`. Ownership over ALL files (incl. `--once`) precedes any migration or startup write. Unit edits need `daemon-reload`.
- Post-update: SKILL.md § Post-Update Agent Protocol. After a Python-launcher change, smoke `./go-trader --once` (daemon stopped).

## Backtest
- Harness map `docs/backtesting-registry.md`: update its row in the PR that adds/deprecates a harness.
...
## Testing
- New functionality and each bug fix need a test guarding a behavior contract (money, state, protection, subprocess contracts, migration, backtest parity). Assert outcomes; pin wording only when it drives an operator decision; no constants or round-trips; table-driven variants.
...
- `go build`/`go test ./...` from repo root; `gofmt -w` after edits. Tabbed Go edits via Python `replace(old,new,1)`.
