...
- Multi-strategy HL perps on same coin (#491): `LoadConfig` requires peer strategies on the same coin to share `margin_mode` + `leverage` and limits at most one peer to carry a non-zero `EffectiveStopLossPct` (reduce-only triggers race on a shared on-chain position). `check_hyperliquid.py` calls `adapter.get_position_leverage(symbol)` and skips `update_leverage` when current state already matches; falls back to `update_leverage` on info-API errors. `validateHotReloadCompatible` mirrors the peer check. Sub-account isolation is the only correct path for full per-strategy independence.
