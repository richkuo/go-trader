...
from adapter import OptionContract, OptionPosition, OptionType, OptionSide, Greeks
from risk import OptionsRiskManager, OptionsRiskConfig
from strategies import OPTIONS_STRATEGY_REGISTRY, list_options_strategies, get_options_strategy, create_options_strategy, MomentumOptionsStrategy, VolMeanReversionStrategy, ProtectivePutsStrategy, CoveredCallsStrategy
...
def _make_adapter():
    ...
    adapter.get_spot_price.return_value = 50000.0
...
def _make_contract(strike=50000.0, dte=30, option_type=OptionType.CALL, underlying='BTC'):
    return OptionContract(symbol=f'BTC-{strike}-C' if option_type == OptionType.CALL else f'BTC-{strike}-P', underlying=underlying, strike=strike, expiry=datetime.now(timezone.utc) + timedelta(days=dte), option_type=option_type, bid=0.03, ask=0.05, last=0.04, spot_price=50000.0, greeks=Greeks(delta=0.5, gamma=0.01, theta=-5.0, vega=100.0, iv=0.6))

def _make_position(pid='pos1', underlying='BTC', option_type=OptionType.CALL, side=OptionSide.BUY, strike=50000.0, pnl_pct=0.0, dte=20, leg_group=None):
...
class TestOptionsRegistry:
    ...
    def test_strategies_registered(self):
        ...
        assert len(names) >= 4
        for expected in ['momentum_options', 'vol_mean_reversion', 'protective_puts', 'covered_calls']:
...
class TestMomentumOptions:
    ...
    def test_no_signal_returns_none(self):
        adapter = _make_adapter()
        risk = _make_risk()
        strat = MomentumOptionsStrategy(adapter, risk, roc_period=14, threshold=5.0, target_dte=37, profit_target_pct=50.0, stop_loss_pct=30.0, position_size_pct=3.0)
    ...
    def test_manage_positions_profit_target(self):
        ...
        adapter.get_positions.return_value = {pid: pos}
        strat = MomentumOptionsStrategy(adapter, risk, profit_target_pct=50.0, stop_loss_pct=30.0)
...
class TestVolMeanReversion:
    ...
    def test_high_iv_sells_strangle(self):
        ...
        strat = VolMeanReversionStrategy(adapter, risk, high_iv_threshold=75, low_iv_threshold=25, target_dte=30, iv_lookback_days=60, exit_iv_reversion_pct=50.0, position_size_pct=5.0)
        actions = strat.evaluate('BTC')
        assert len(actions) == 1
    ...
    def test_manage_approaching_expiry(self):
        adapter = _make_adapter()
        risk = _make_risk()
        pid, pos = _make_position(pnl_pct=5.0, side=OptionSide.BUY, dte=5, leg_group='straddle_1')
        adapter.get_positions.return_value = {pid: pos}
        strat = VolMeanReversionStrategy(adapter, risk, exit_iv_reversion_pct=50.0)
        actions = strat.manage_positions('BTC')
        assert len(actions) == 1
...
class TestProtectivePuts:
    ...
    def test_buys_otm_put(self):
        ...
        adapter.find_options.return_value = [contract]
        adapter.enrich_contract.return_value = contract
        strat = ProtectivePutsStrategy(adapter, risk, otm_pct=12.0, target_dte=45, roll_dte=10, max_hedge_cost_pct=2.0, spot_holding_usd=5000.0)
        actions = strat.evaluate('BTC')
        assert len(actions) == 1
        assert actions[0]['type'] == 'buy_put'
...
class TestCoveredCalls:
    ...
    def test_sells_otm_call(self):
        ...
        adapter.find_options.return_value = [contract]
        adapter.enrich_contract.return_value = contract
        strat = CoveredCallsStrategy(adapter, risk, otm_pct=12.0, target_dte=21, roll_dte=5, itm_roll_threshold_pct=2.0, spot_holding_usd=5000.0)
        actions = strat.evaluate('BTC')
        assert len(actions) == 1
    ...
    def test_manage_roll_approaching_itm(self):
        adapter = _make_adapter()
        risk = _make_risk()
        pid, pos = _make_position(option_type=OptionType.CALL, side=OptionSide.SELL, strike=50500, dte=15)
        adapter.get_positions.return_value = {pid: pos}
    ...
    def test_manage_roll_approaching_expiry(self):
        adapter = _make_adapter()
        risk = _make_risk()
        pid, pos = _make_position(option_type=OptionType.CALL, side=OptionSide.SELL, strike=56000, dte=3)
        adapter.get_positions.return_value = {pid: pos}
        strat = CoveredCallsStrategy(adapter, risk, roll_dte=5, itm_roll_threshold_pct=2.0)
        actions = strat.manage_positions('BTC')
        assert len(actions) == 1
