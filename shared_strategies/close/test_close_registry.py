...
def test_build_close_registry_filters_valid_platforms(registry):
    ...
    for platform in registry.VALID_PLATFORMS:
        ...
        assert set(built) == {'tiered_tp_pct', 'tiered_tp_atr', 'tiered_tp_atr_live', 'tiered_tp_atr_regime', 'tiered_tp_atr_live_regime', 'tiered_tp_atr_live_regime_dynamic', 'trailing_tp_ratchet', 'trailing_tp_ratchet_regime', 'time_stop', 'atr_stop', 'zscore_target', 'avwap_stop'}
...
def test_tp_at_pct_deprecated_shim(registry):
    long_hit = registry.evaluate('tp_at_pct', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1}, {'mark_price': 103}, {'pct': 0.03})
    short_hit = registry.evaluate('tp_at_pct', {'side': 'short', 'avg_cost': 100, 'current_quantity': 1}, {'mark_price': 97}, {'pct': 0.03})
...
def test_tiered_tp_pct_closes_only_unfilled_tier_amount(registry):
    first = registry.evaluate('tiered_tp_pct', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1}, {'mark_price': 102}, {})
    already_taken = registry.evaluate('tiered_tp_pct', {'side': 'long', 'avg_cost': 100, 'current_quantity': 0.5, 'initial_quantity': 1}, {'mark_price': 102}, {})
    final = registry.evaluate('tiered_tp_pct', {'side': 'long', 'avg_cost': 100, 'current_quantity': 0.5, 'initial_quantity': 1}, {'mark_price': 104}, {})
...
def test_tiered_tp_atr_uses_entry_atr_multiple(registry):
    missing_atr = registry.evaluate('tiered_tp_atr', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1}, {'mark_price': 103}, {})
    hit = registry.evaluate('tiered_tp_atr', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1, 'entry_atr': 2}, {'mark_price': 110}, {})
...
def test_tiered_tp_atr_live_uses_market_atr(registry):
    live_hit = registry.evaluate('tiered_tp_atr_live', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1, 'entry_atr': 2}, {'mark_price': 105, 'atr': 3}, {})
...
def test_tiered_tp_atr_live_falls_back_to_entry_atr(registry):
    fallback = registry.evaluate('tiered_tp_atr_live', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1, 'entry_atr': 2}, {'mark_price': 110}, {})
...
def test_tiered_tp_atr_live_zero_live_atr_falls_back(registry):
    result = registry.evaluate('tiered_tp_atr_live', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1, 'entry_atr': 2}, {'mark_price': 103, 'atr': 0}, {})
...
def test_tiered_tp_atr_live_missing_all_atr_noop(registry):
    result = registry.evaluate('tiered_tp_atr_live', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1}, {'mark_price': 104}, {})
...
def test_tiered_tp_atr_live_entry_source_ignores_market_atr(registry):
    result = registry.evaluate('tiered_tp_atr_live', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1, 'entry_atr': 2}, {'mark_price': 110, 'atr': 10}, {'atr_source': 'entry'})
    assert result == {'close_fraction': 1.0, 'reason': 'tiered_tp_atr_live:entry:5'}

def test_tiered_tp_atr_live_short_side(registry):
    result = registry.evaluate('tiered_tp_atr_live', {'side': 'short', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1, 'entry_atr': 5}, {'mark_price': 90, 'atr': 2}, {})
...
def test_market_atr_wiring_end_to_end(registry):
    ...
    n = 30
    df = pd.DataFrame({'open': [100.0] * n, 'high': [101.5] * n, 'low': [98.5] * n, 'close': [100.0] * n, 'volume': [1.0] * n})
    ...
    result = registry.evaluate('tiered_tp_atr_live', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1, 'entry_atr': 99}, market_ctx, {})
    assert result['reason'].startswith('tiered_tp_atr_live:live:'), f"market_ctx['atr'] not flowing through to evaluator: reason={result['reason']!r}"
...
def test_evaluate_reads_tp_tiers(registry):
    position = {'avg_cost': 100.0, 'current_quantity': 1.0, 'initial_quantity': 1.0, 'entry_atr': 10.0, 'side': 'long'}
...
def test_unified_regime_block_evaluator(registry):
    params = {'trend_regime': {'trending_up': {'stop_loss_atr': 1.5, 'tp_tiers': [{'atr_multiple': 2.0, 'close_fraction': 0.5}, {'atr_multiple': 4.0, 'close_fraction': 1.0}]}, 'trending_down': {'tp_tiers': [{'atr_multiple': 1.8, 'close_fraction': 0.5}, {'atr_multiple': 3.0, 'close_fraction': 1.0}]}, 'ranging': {'tp_tiers': [{'atr_multiple': 1.0, 'close_fraction': 0.5}, {'atr_multiple': 2.0, 'close_fraction': 1.0}]}}}
    base_pos = {'avg_cost': 100.0, 'current_quantity': 1.0, 'initial_quantity': 1.0, 'entry_atr': 10.0, 'side': 'long'}
    market = {'mark_price': 130.0}
