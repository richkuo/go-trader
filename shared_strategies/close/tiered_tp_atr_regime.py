...
from regime_atr import REGIME_TP_TIER_GROUP_DEFAULTS, RegimeTierSpec, close_params_are_unified_regime, parse_regime_tp_tiers, regime_close_default_group, resolve_regime_tier, unified_regime_scalar_params

def _resolve_tiers_for_regime(params: dict, regime: str) -> Tuple[List[Tuple[float, float]], List[str]]:
    ...
    for idx, spec in enumerate(specs):
        ...
        if pair is None:
            return ([], [f'tiered_tp_atr_regime.tiers[{idx}]: regime {regime!r} resolved to no atr/close_fraction (config validation should have caught this)'])
        atr, frac = pair
...
def evaluate(position: dict, market: dict, params: dict) -> dict:
    ...
    return {'close_fraction': close_fraction, 'reason': f'tiered_tp_atr_regime:{regime}:{multiple:g}'}
