...
def test_trail_only_tier_returns_zero_close_fraction(ratchet):
    params = {'tp_tiers': [{'atr_multiple': 1.0, 'close_fraction': 0.0, 'trailing_mult_after': 2.0}, {'atr_multiple': 2.0, 'close_fraction': 0.3, 'trailing_mult_after': 1.0}]}
    pos = {'side': 'long', 'avg_cost': 100, 'current_quantity': 1, 'initial_quantity': 1, 'entry_atr': 10}
...
def test_double_close_guard(ratchet):
    ...
    pos = {'side': 'long', 'avg_cost': 100, 'current_quantity': 0.5, 'initial_quantity': 1, 'entry_atr': 10}
...
def test_rejects_decreasing_cumulative_close_fraction(ratchet):
    params = {'tp_tiers': [{'atr_multiple': 1.0, 'close_fraction': 0.4, 'trailing_mult_after': 2.0}, {'atr_multiple': 2.0, 'close_fraction': 0.0, 'trailing_mult_after': 1.0}]}
...
def test_regime_table_resolution(ratchet):
    params = {'tp_tiers': {'trending_up': [{'atr_multiple': 1.0, 'close_fraction': 0.25, 'trailing_mult_after': 1.5}], 'ranging': [{'atr_multiple': 1.0, 'close_fraction': 0.0, 'trailing_mult_after': 2.0}]}}
    tiers, errs = ratchet.resolve_tiers_for_regime(params, 'ranging', regime_table=True)
    assert errs == []
    assert len(tiers) == 1
...
def test_resolve_tiers_for_regime_group_defaults(ratchet):
    clean, errs = ratchet.resolve_tiers_for_regime({'use_defaults': True}, 'trending_up_clean', regime_table=True)
    assert errs == []
    assert [t[0] for t in clean] == [3.0, 4.5, 6.0]
    assert all((t[1] == 0.0 for t in clean))
    ranging, errs = ratchet.resolve_tiers_for_regime({'use_defaults': True}, 'ranging_quiet', regime_table=True)
    assert errs == []
    assert [t[0] for t in ranging] == [0.75, 1.5, 2.0]
    assert [t[1] for t in ranging] == [0.4, 0.8, 1.0]
    scalar, errs = ratchet.resolve_tiers_for_regime({'use_defaults': True}, '', regime_table=False)
    assert errs == []
    assert [t[0] for t in scalar] == [2.0, 2.5, 3.0]
...
def test_resolve_tiers_for_regime_ranging_substates(ratchet):
    volatile, errs = ratchet.resolve_tiers_for_regime({'use_defaults': True}, 'ranging_volatile', regime_table=True)
    assert errs == []
    assert [t[0] for t in volatile] == [1.0, 2.0, 3.0]
    assert [t[1] for t in volatile] == [0.4, 0.8, 1.0]
    directional, errs = ratchet.resolve_tiers_for_regime({'use_defaults': True}, 'ranging_directional', regime_table=True)
    assert errs == []
    assert [t[0] for t in directional] == [1.0, 2.0, 3.0, 4.5]
    assert [t[1] for t in directional] == [0.25, 0.5, 0.75, 0.75]
    assert [t[2] for t in directional] == [1.0, 1.0, 0.8, 0.6]
    for label in ('ranging_directional_up', 'ranging_directional_down'):
        tiers, errs = ratchet.resolve_tiers_for_regime({'use_defaults': True}, label, regime_table=True)
        assert errs == []
        assert tiers == directional, f'{label} must mirror the ranging_directional ladder'
    adx_ranging, errs = ratchet.resolve_tiers_for_regime({'use_defaults': True}, 'ranging', regime_table=True)
    assert errs == []
