...
import pytest
from .post_tp_sl import SLAfterRule, parse_tp_tier_close_fractions, parse_sl_after_rule, parse_strategy_tp_sl_after_rules, validate_post_tp_stop_loss_rules, validate_sl_after_rule

def _regime(entries):
    return {'trend_regime': {label: {'atr_multiple': atr} for label, atr in entries.items()}}
...
def test_scalar_trail_from_here():
    rule = parse_sl_after_rule({'trail_from_here': {'atr_mult': 1.0}})
    assert rule == SLAfterRule(kind='trail_from_here', trail_atr_mult=1.0)

def test_scalar_trail_from_here_tp_atr_fraction():
    rule = parse_sl_after_rule({'trail_from_here': {'tp_atr_fraction': 0.5}})
...
def test_regime_atr_offset_implicit():
    rule = parse_sl_after_rule(_regime({'trending_up': 0.0, 'trending_down': 0.0, 'ranging': -0.5}))
...
def test_regime_atr_offset_explicit_kind():
    rule = parse_sl_after_rule({'kind': 'atr_offset', **_regime({'trending_up': 0.25, 'trending_down': 0.25, 'ranging': 0.0})})
    assert rule.kind == 'atr_offset'
    assert rule.atr_regime is not None

def test_regime_trail_from_here():
    rule = parse_sl_after_rule({'trail_from_here': _regime({'trending_up': 1.0, 'trending_down': 1.0, 'ranging': 0.5})})
    assert rule.kind == 'trail_from_here'
...
def test_regime_trail_rejects_non_positive(atr):
    with pytest.raises(ValueError):
        parse_sl_after_rule({'trail_from_here': _regime({'trending_up': 1.0, 'trending_down': 1.0, 'ranging': atr})})
...
def test_regime_rejects_missing_labels():
    with pytest.raises(ValueError) as exc:
        parse_sl_after_rule({'trend_regime': {'trending_up': {'atr_multiple': 0.25}, 'ranging': {'atr_multiple': 0.0}}})
    assert 'missing required regime labels' in str(exc.value)

def test_regime_rejects_use_defaults_with_explicit():
    with pytest.raises(ValueError):
        parse_sl_after_rule({'use_defaults': True, 'trend_regime': {'trending_up': {'atr_multiple': 0.25}, 'trending_down': {'atr_multiple': 0.25}, 'ranging': {'atr_multiple': 0.0}}})

def test_regime_rejects_scalar_and_regime_mix():
    with pytest.raises(ValueError) as exc:
        parse_sl_after_rule({'atr_mult': 0.25, 'trend_regime': {'trending_up': {'atr_multiple': 0.25}, 'trending_down': {'atr_multiple': 0.25}, 'ranging': {'atr_multiple': 0.0}}})
    assert 'pick one shape' in str(exc.value)

def test_atr_offset_regime_rejects_stray_trail_atr_mult():
    with pytest.raises(ValueError) as exc:
        parse_sl_after_rule({'kind': 'atr_offset', 'trend_regime': {'trending_up': {'atr_multiple': 0.25}, 'trending_down': {'atr_multiple': 0.25}, 'ranging': {'atr_multiple': 0.0}}, 'trail_atr_mult': 99.0})
    assert 'pick one shape' in str(exc.value)

def test_trail_regime_rejects_stray_atr_offset():
    with pytest.raises(ValueError) as exc:
        parse_sl_after_rule({'trail_from_here': {'trend_regime': {'trending_up': {'atr_multiple': 1.0}, 'trending_down': {'atr_multiple': 1.0}, 'ranging': {'atr_multiple': 0.5}}, 'atr_offset': -3.0}})
    assert 'pick one shape' in str(exc.value)
...
def test_parse_strategy_tp_sl_after_rules_regime():
    close_refs = [{'name': 'tiered_tp_atr_live', 'params': {'sl_after': _regime({'trending_up': 0.0, 'trending_down': 0.0, 'ranging': -0.5}), 'tp_tiers': [{'atr_multiple': 2, 'close_fraction': 0.5, 'sl_after': {'trail_from_here': _regime({'trending_up': 1.0, 'trending_down': 1.0, 'ranging': 0.5})}}, {'atr_multiple': 3, 'close_fraction': 1.0}]}}]
...
def test_parse_strategy_tp_sl_after_rules_regime_composite_labels():
    labels = ('ranging_directional', 'ranging_quiet', 'ranging_volatile', 'trending_down_choppy', 'trending_down_clean', 'trending_up_choppy', 'trending_up_clean')
    close_refs = [{'name': 'tiered_tp_atr_regime', 'params': {'tp_tiers': [{'trend_regime': {label: {'atr_multiple': 2.0} for label in labels}, 'close_fraction': 0.5}, {'trend_regime': {label: {'atr_multiple': 4.0} for label in labels}, 'close_fraction': 1.0}]}}]
    rules, errs = parse_strategy_tp_sl_after_rules(close_refs, regime='trending_up_clean', labels=labels)
    assert errs == []
    assert rules.multiples == [2.0, 4.0]

def test_parse_tp_tier_close_fractions_use_defaults_composite_label():
    close_refs = [{'name': 'tiered_tp_atr_regime', 'params': {'use_defaults': True}}]
    got = parse_tp_tier_close_fractions(close_refs, regime='trending_up_clean')
    assert got == [0.25, 0.5, 0.75, 1.0]

def test_validate_rejects_trail_regime_on_manual():
    close_refs = [{'name': 'tiered_tp_atr_live', 'params': {'sl_after': {'trail_from_here': _regime({'trending_up': 1.0, 'trending_down': 1.0, 'ranging': 0.5})}, 'tp_tiers': [{'atr_multiple': 2, 'close_fraction': 0.5}, {'atr_multiple': 3, 'close_fraction': 1.0}]}}]
    errs = validate_post_tp_stop_loss_rules(close_refs, stop_loss_atr_mult=1.5, strategy_type='manual')
    assert any(('trail_from_here is not supported on manual' in e for e in errs)), errs
...
def test_parse_strategy_tp_sl_after_rules_regime_close_per_tier_sl_after():
    close_refs = [{'name': 'tiered_tp_atr_regime', 'params': {'tp_tiers': [{'trend_regime': {'trending_up': {'atr_multiple': 2.0, 'close_fraction': 0.5}, 'trending_down': {'atr_multiple': 2.0, 'close_fraction': 0.5}, 'ranging': {'atr_multiple': 1.5, 'close_fraction': 0.5}}, 'sl_after': 'breakeven'}, {'trend_regime': {'trending_up': {'atr_multiple': 4.0, 'close_fraction': 1.0}, 'trending_down': {'atr_multiple': 4.0, 'close_fraction': 1.0}, 'ranging': {'atr_multiple': 3.0, 'close_fraction': 1.0}}}]}}]
