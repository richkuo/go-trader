...
from _helpers import clamp_fraction, current_close_fraction, float_from, tier_list_from_params
from regime_atr import CANONICAL_TREND_REGIME_LABELS, regime_close_default_group
DEFAULT_RATCHET_TIERS = [{'atr_multiple': 2.0, 'trailing_mult_after': 1.5, 'close_fraction': 0.0}, {'atr_multiple': 2.5, 'trailing_mult_after': 1.0, 'close_fraction': 0.0}, {'atr_multiple': 3.0, 'trailing_mult_after': 0.8, 'close_fraction': 0.0}]
DEFAULT_RATCHET_TIERS_BY_GROUP = {'clean': [{'atr_multiple': 3.0, 'trailing_mult_after': 1.5, 'close_fraction': 0.0}, {'atr_multiple': 4.5, 'trailing_mult_after': 1.0, 'close_fraction': 0.0}, {'atr_multiple': 6.0, 'trailing_mult_after': 0.8, 'close_fraction': 0.0}], 'choppy': [{'atr_multiple': 2.0, 'trailing_mult_after': 1.5, 'close_fraction': 0.0}, {'atr_multiple': 2.5, 'trailing_mult_after': 1.0, 'close_fraction': 0.0}, {'atr_multiple': 3.0, 'trailing_mult_after': 0.8, 'close_fraction': 0.0}], 'ranging_quiet': [{'atr_multiple': 0.75, 'trailing_mult_after': 1.0, 'close_fraction': 0.4}, {'atr_multiple': 1.5, 'trailing_mult_after': 0.75, 'close_fraction': 0.8}, {'atr_multiple': 2.0, 'trailing_mult_after': 0.75, 'close_fraction': 1.0}], 'ranging_volatile': [{'atr_multiple': 1.0, 'trailing_mult_after': 1.0, 'close_fraction': 0.4}, {'atr_multiple': 2.0, 'trailing_mult_after': 0.75, 'close_fraction': 0.8}, {'atr_multiple': 3.0, 'trailing_mult_after': 0.75, 'close_fraction': 1.0}], 'ranging_directional': [{'atr_multiple': 1.0, 'trailing_mult_after': 1.0, 'close_fraction': 0.25}, {'atr_multiple': 2.0, 'trailing_mult_after': 1.0, 'close_fraction': 0.5}, {'atr_multiple': 3.0, 'trailing_mult_after': 0.8, 'close_fraction': 0.75}, {'atr_multiple': 4.5, 'trailing_mult_after': 0.6, 'close_fraction': 0.75}]}
...
def _validate_monotonic_tiers(tiers: List[Tuple[float, float, float]]) -> List[str]:
    ...
    for idx in range(1, len(tiers)):
        ...
        if cur_trail > prev_trail + 1e-12:
            errs.append(f'tp_tiers[{idx}].trailing distance {cur_trail:g} must be <= tier[{idx - 1}] ({prev_trail:g})')
        if cur_frac + 1e-12 < prev_frac:
            errs.append(f'tp_tiers[{idx}].close_fraction {cur_frac:g} must be >= tier[{idx - 1}] close_fraction {prev_frac:g}')
    return errs
...
def resolve_tiers_for_regime(params: dict, regime: str, *, regime_table: bool) -> Tuple[List[Tuple[float, float, float]], List[str]]:
...
def _evaluate(position: dict, market: dict, params: dict, *, regime_table: bool) -> dict:
    ...
    return {'close_fraction': close_fraction, 'reason': f'{tag}{suffix}:{multiple:g}'}

def maybe_apply_mark_ratchet(tiers: List[Tuple[float, float, float]], *, watermark: int, mark_price: float, avg_cost: float, entry_atr: float, side: str, post_tp_trail_mult: Optional[float], trailing_stop_atr_mult: float) -> tuple[int, Optional[float]]:
