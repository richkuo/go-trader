...
def test_zero_buffer_exits_at_line_touch_without_atr(reg):
    ...
    out = reg.evaluate('avwap_stop', _long_pos(entry_atr=0.0), {'mark_price': 100.0, 'avwap': 100.0}, params)
    assert out['close_fraction'] == 1.0
    out = reg.evaluate('avwap_stop', _long_pos(entry_atr=0.0), {'mark_price': 100.01, 'avwap': 100.0}, params)
    assert out['close_fraction'] == 0.0

def test_atr_source_entry_vs_live(reg):
    pos = _long_pos(entry_atr=1.0)
    mkt = {'mark_price': 99.0, 'avwap': 100.0, 'atr': 4.0}
    assert reg.evaluate('avwap_stop', pos, mkt, {'buffer_atr_mult': 0.5, 'atr_source': 'entry'})['close_fraction'] == 1.0
    assert reg.evaluate('avwap_stop', pos, mkt, {'buffer_atr_mult': 0.5, 'atr_source': 'live'})['close_fraction'] == 0.0

def test_missing_atr_fails_safe_when_buffer_positive(reg):
    out = reg.evaluate('avwap_stop', _long_pos(entry_atr=0.0), {'mark_price': 50.0, 'avwap': 100.0}, {'buffer_atr_mult': 0.5, 'atr_source': 'live'})
    assert out['close_fraction'] == 0.0
    assert out['reason'] == 'noop:missing_live_atr'
    out = reg.evaluate('avwap_stop', _long_pos(entry_atr=0.0), {'mark_price': 50.0, 'avwap': 100.0}, {'buffer_atr_mult': 0.5, 'atr_source': 'entry'})
    assert out['close_fraction'] == 0.0
