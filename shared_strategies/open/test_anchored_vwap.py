...
def test_avwap_matches_hand_computed_prefix_sum():
    ...
    for nbar in (7, 8, 9, 10, 11):
        ...
        assert abs(avwap[nbar] - expected) < 1e-09, (nbar, avwap[nbar], expected)
...
def test_avwap_zero_volume_window_falls_back_to_typical():
    ...
    for nbar in range(7, 12):
        assert abs(avwap[nbar] - tp[nbar]) < 1e-09

def _long_reclaim_df():
    closes = [110, 108, 106, 104, 102, 100, 100.5, 100.2, 99.8, 99.5, 103.5, 104.0, 104.5, 105.0]
...
def test_short_signal_mirrors():
    closes = [90, 92, 94, 96, 98, 100, 99.5, 99.8, 100.2, 100.5, 96.5, 96.0, 95.5, 95.0]
...
def _high_prior_long_reclaim_df():
    closes = [140, 138, 136, 134, 132, 100, 100.5, 100.2, 99.8, 99.5, 103.5, 104.0, 104.5, 105.0]
    return _ohlcv(closes, volume=10.0)
...
def test_rsi_gate_blocks_long_below_level_and_passes_above():
    ...
    blocked = anchored_vwap_core(df, **_GATE_BASE_KW, gate_rsi_period=6, gate_rsi_level=rsi_b + 1.0)
    assert blocked['signal'].iloc[b] == 0
    assert (blocked['signal'] == 0).all()
    passed = anchored_vwap_core(df, **_GATE_BASE_KW, gate_rsi_period=6, gate_rsi_level=rsi_b - 1.0)
...
def test_rsi_gate_warmup_nan_fails_open():
    ...
    base = anchored_vwap_core(df, **_GATE_BASE_KW)
    b = int(np.where(base['signal'].to_numpy() == 1)[0][0])
    gated = anchored_vwap_core(df, **_GATE_BASE_KW, gate_rsi_period=13, gate_rsi_level=100.0)
...
def test_ema_gate_passes_aligned_long_and_short():
    ...
    short_df = _ohlcv([90, 92, 94, 96, 98, 100, 99.5, 99.8, 100.2, 100.5, 96.5, 96.0, 95.5, 95.0], volume=10.0)
...
def test_gated_signal_independent_of_future_bars():
    ...
    for k in range(8, len(df)):
        ...
        assert (partial['signal'].to_numpy() == full['signal'].to_numpy()[:k + 1]).all(), k

def _load_registry():
    here = os.path.dirname(os.path.abspath(__file__))
    spec = importlib.util.spec_from_file_location('_reg_under_test_avwap', os.path.join(here, 'registry.py'))
