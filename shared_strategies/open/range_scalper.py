...
def range_scalper_core(df: pd.DataFrame, bb_period: int=14, bb_std: float=1.5, bw_threshold: float=0.008, vol_ratio: float=0.8, rsi_period: int=7, rsi_ob: float=70, rsi_os: float=30) -> pd.DataFrame:
    result = df.copy()
