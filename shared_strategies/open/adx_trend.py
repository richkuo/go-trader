...
def _compute_adx_components(high: np.ndarray, low: np.ndarray, close: np.ndarray, period: int) -> dict:
    n = len(high)
    tr = np.zeros(n)
    plus_dm = np.zeros(n)
    minus_dm = np.zeros(n)
...
def adx_trend_core(df: pd.DataFrame, adx_period: int=14, adx_threshold: float=25.0) -> pd.DataFrame:
    result = df.copy()
    result['signal'] = 0
    n = len(result)
    if n < adx_period * 2 + 1:
        return result
    components = _compute_adx_components(result['high'].values, result['low'].values, result['close'].values, adx_period)
