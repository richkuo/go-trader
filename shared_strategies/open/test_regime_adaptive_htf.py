import numpy as np
import pandas as pd
from regime_adaptive_htf import _confirm_labels, _RANGING_QUIET, _RANGING_VOLATILE, _TREND_UP_CLEAN, _TREND_DOWN_CLEAN, _WARMUP, regime_adaptive_htf_core
PIN = dict(htf_factor=6, period=14, adx_threshold=20.0, return_eff_threshold=0.05, range_eff_threshold=0.03, efficiency_threshold=0.5, confirm_buckets=2, mr_lookback=20, mr_entry_z=2.0, mr_exit_z=0.0)

def make_ohlcv(closes, noise=0.5, volume=100.0, start='2026-01-01'):
    closes = np.asarray(closes, dtype=float)
    n = len(closes)
    return pd.DataFrame({'open': closes - noise * 0.3, 'high': closes + noise, 'low': closes - noise, 'close': closes, 'volume': np.full(n, volume)}, index=pd.date_range(start, periods=n, freq='1h'))
...
def make_htf_clean_uptrend():
    return np.concatenate([100 + np.random.RandomState(1).randn(150) * 0.3, np.linspace(100, 260, 360)])

def test_columns_present():
    out = regime_adaptive_htf_core(make_ohlcv(make_htf_range()), **PIN)
    for col in ('signal', 'position', 'rah_label', 'rah_raw_label', 'rah_z', 'rah_slow_eff'):
...
def test_short_inputs_return_flat_never_raise():
    ...
    for n in (15, 16, 17, 40):
        out = regime_adaptive_htf_core(make_ohlcv([100.0 + 0.1 * i for i in range(n)]), htf_factor=2, period=8)
        assert len(out) == n
        assert (out['signal'] == 0).all()

def test_range_index_fallback_is_safe():
    closes = make_htf_range()
    df = pd.DataFrame({'open': closes, 'high': closes + 0.5, 'low': closes - 0.5, 'close': closes, 'volume': np.full(len(closes), 100.0)})
...
def test_confirm_labels_requires_streak():
    ...
    assert conf.tolist() == [0, _RANGING_QUIET, _RANGING_QUIET, _RANGING_QUIET, _RANGING_QUIET, _RANGING_QUIET, _RANGING_QUIET]

def test_confirm_labels_switches_after_n():
    raw = np.array([_RANGING_QUIET] * 4 + [_TREND_UP_CLEAN] * 3)
    conf = _confirm_labels(raw, 3)
    assert conf.tolist() == [0, 0, _RANGING_QUIET, _RANGING_QUIET, _RANGING_QUIET, _RANGING_QUIET, _TREND_UP_CLEAN]
...
def test_trend_breakout_mode_enters_clean_trend():
    out = regime_adaptive_htf_core(make_ohlcv(make_htf_clean_uptrend(), noise=0.4), trend_entry='breakout', breakout_lookback=10, **PIN)
    entries = np.where(out['signal'].values == 1)[0]
    assert len(entries) > 0
...
def test_trend_drift_confirm_blocks_without_agreeing_drift():
    ...
    base = regime_adaptive_htf_core(df, trend_entry='breakout', breakout_lookback=10, **PIN)
    gated = regime_adaptive_htf_core(df, trend_entry='breakout', breakout_lookback=10, trend_drift_confirm=99.0, **PIN)
...
def test_trend_drift_confirm_inactive_when_veto_disabled():
    df = make_ohlcv(make_htf_clean_uptrend(), noise=0.4)
    out = regime_adaptive_htf_core(df, trend_entry='breakout', breakout_lookback=10, slow_trend_lookback=0, trend_drift_confirm=99.0, **PIN)
    assert (out['signal'] == 1).any()

def test_trend_transition_mode_enters_at_flip():
    out = regime_adaptive_htf_core(make_ohlcv(make_htf_clean_uptrend(), noise=0.4), trend_entry='transition', transition_window=6, **PIN)
    entries = np.where(out['signal'].values == 1)[0]
    assert len(entries) > 0
    first = entries[0]
...
def test_fade_labels_all_mr_widens_the_gate():
    ...
    narrow = regime_adaptive_htf_core(df, fade_labels='ranging', slow_trend_lookback=0, **PIN)
    wide = regime_adaptive_htf_core(df, fade_labels='all_mr', slow_trend_lookback=0, **PIN)
...
def test_long_only_default_never_short():
    out = regime_adaptive_htf_core(make_ohlcv(-make_htf_range() + 300), **PIN)
...
def test_impossible_entry_z_blocks_fades():
    out = regime_adaptive_htf_core(make_ohlcv(make_htf_range()), **{**PIN, 'mr_entry_z': 50.0})
    assert (out['position'] == 0).all()
...
def test_slow_trend_veto_blocks_bear_market_fades():
    ...
    vetoed = regime_adaptive_htf_core(df, slow_trend_lookback=60, slow_veto_threshold=0.05, fade_labels='all_mr', **PIN)
...
def test_slow_trend_veto_inactive_in_flat_range():
    ...
    on = regime_adaptive_htf_core(df, slow_trend_lookback=100, slow_veto_threshold=0.05, **PIN)
...
def test_prefix_consistency_no_lookahead():
    ...
    for k in range(n - 13, n):
        ...
        pd.testing.assert_series_equal(prefix, full.iloc[:k], check_names=False, obj=f'prefix k={k}')
