...
def _load_registry():
    spec = importlib.util.spec_from_file_location('_registry_supertrend', os.path.join(_HERE, 'registry.py'))
...
def make_ohlcv(closes, index=None, noise=0.5):
    ...
    n = len(closes)
    df = pd.DataFrame({'open': closes - noise * 0.3, 'high': closes + noise, 'low': closes - noise, 'close': closes, 'volume': np.full(n, 100.0)})
    if index is not None:
        df.index = index
    return df

def _three_leg_trend_df():
    closes = np.concatenate([np.linspace(100, 200, 100), np.linspace(200, 120, 100), np.linspace(120, 220, 100)])
