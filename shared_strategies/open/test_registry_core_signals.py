...
def _load_registry():
    spec = importlib.util.spec_from_file_location('_registry_core_signals', os.path.join(_HERE, 'registry.py'))
...
def test_ema_crossover_buy_on_bullish_cross(registry):
    ...
    assert out.iloc[prev]['ema_fast'] <= out.iloc[prev]['ema_slow'], 'Buy must land exactly on the crossover bar'
...
def test_volume_weighted_buy_needs_price_cross_and_volume_spike(registry):
    ...
    assert out['signal'].iloc[40] == 1, 'Cross above price SMA on high volume must fire a buy on the cross bar'
...
def test_heikin_ashi_ema_buy_in_clean_uptrend(registry):
    ...
    df = pd.DataFrame({'open': closes - 1.0, 'high': closes + 0.1, 'low': closes - 1.1, 'close': closes, 'volume': np.full(120, 100.0)})
...
def test_heikin_ashi_ema_sell_in_clean_downtrend(registry):
    ...
    df = pd.DataFrame({'open': closes + 1.0, 'high': closes + 1.1, 'low': closes - 0.1, 'close': closes, 'volume': np.full(120, 100.0)})
    out = _apply(registry, 'heikin_ashi_ema', df)
    sells = out.index[out['signal'] == -1]
...
def test_ichimoku_buy_on_tk_cross_above_cloud(registry):
    prices = np.concatenate([np.linspace(110, 100, 80), np.linspace(100, 180, 120)])
...
def test_ichimoku_sell_on_tk_cross_below_cloud(registry):
    prices = np.concatenate([np.linspace(100, 110, 80), np.linspace(110, 30, 120)])
    df = make_ohlcv(prices, noise=0.5)
    out = _apply(registry, 'ichimoku_cloud', df)
    sells = out.index[out['signal'] == -1]
...
def _squeeze_then_breakout(direction=1):
    ...
    return pd.DataFrame({'open': closes, 'high': closes + noise, 'low': closes - noise, 'close': closes, 'volume': np.full(n, 100.0)})
...
def test_order_blocks_buy_on_retest_of_bullish_ob(registry):
    ...
    df = pd.DataFrame({'open': opens, 'high': highs, 'low': lows, 'close': closes, 'volume': np.full(n, 100.0)})
