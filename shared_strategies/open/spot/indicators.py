...
def rsi(df: pd.DataFrame, period: int=14, overbought: float=70, oversold: float=30) -> pd.DataFrame:
    result = df.copy()
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame({'open': prices, 'high': prices + abs(np.random.randn(100)), 'low': prices - abs(np.random.randn(100)), 'close': prices + np.random.randn(100) * 0.5, 'volume': np.random.randint(1000, 10000, 100).astype(float)}, index=dates)
