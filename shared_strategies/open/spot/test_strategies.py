...
_spec = importlib.util.spec_from_file_location('spot_strategies', os.path.join(_spot_dir, 'strategies.py'))
...
_conftest_spec = importlib.util.spec_from_file_location('conftest_helpers', os.path.join(_shared_dir, 'conftest.py'))
...
class TestRegistry:
    ...
    def test_strategies_registered(self):
        ...
        assert len(names) >= 10
        for expected in ['mean_reversion_pro', 'liquidity_sweeps', 'anchored_vwap', 'chart_pattern', 'momentum_pro', 'atr_band_revert']:
            assert expected in names, f'{expected} not registered'
        for quarantined in ['sma_crossover', 'ema_crossover', 'rsi', 'macd', 'momentum', 'bollinger_bands', 'mean_reversion', 'supertrend', 'parabolic_sar', 'tema_cross', 'regime_adaptive']:
...
def _assert_valid_signals(result):
    ...
    assert set(signals.unique()).issubset({-1.0, 0.0, 1.0}), f'Unexpected signal values: {set(signals.unique())}'
...
class TestRSI:
    ...
    def test_buy_on_oversold_recovery(self):
        closes = list(np.linspace(100, 100, 20)) + list(np.linspace(100, 60, 20)) + list(np.linspace(60, 85, 30))
    ...
    def test_sell_on_overbought_drop(self):
        closes = list(np.linspace(100, 100, 20)) + list(np.linspace(100, 160, 20)) + list(np.linspace(160, 130, 30))
        result = _run_strategy('rsi', closes)
        _assert_valid_signals(result)
...
class TestBollingerBands:
    ...
    def test_buy_at_lower_band(self):
        closes = list(np.linspace(100, 100, 30)) + list(np.linspace(100, 75, 15)) + list(np.linspace(75, 95, 20))
    ...
    def test_sell_at_upper_band(self):
        closes = list(np.linspace(100, 100, 30)) + list(np.linspace(100, 125, 15)) + list(np.linspace(125, 105, 20))
        result = _run_strategy('bollinger_bands', closes)
        _assert_valid_signals(result)
...
class TestMeanReversion:
    ...
    def test_buy_on_dip(self):
        closes = list(np.linspace(100, 100, 40)) + list(np.linspace(100, 80, 10)) + list(np.linspace(80, 95, 20))
...
class TestRSIMACDCombo:
    ...
    def test_loosened_short_gate_catches_more_shorts(self):
        closes = list(np.linspace(80, 160, 50)) + list(np.linspace(160, 100, 20)) + list(np.linspace(100, 115, 15)) + list(np.linspace(115, 60, 40))
    ...
    def test_loosened_long_gate_catches_more_longs(self):
        closes = list(np.linspace(140, 70, 50)) + list(np.linspace(70, 130, 20)) + list(np.linspace(130, 115, 15)) + list(np.linspace(115, 180, 40))
        strict = _run_strategy('rsi_macd_combo', closes)
    ...
    def test_params_forwarded_via_apply_strategy(self):
        ...
        result = _run_strategy('rsi_macd_combo', closes, params={'rsi_short_min': 30, 'rsi_long_max': 70})
        assert 'rsi' in result.columns
        _assert_valid_signals(result)
...
class TestStochRSI:
    ...
    def test_buy_in_oversold(self):
        closes = list(np.linspace(100, 100, 20)) + list(np.linspace(100, 50, 30)) + list(np.linspace(50, 65, 10)) + list(np.linspace(65, 45, 15)) + list(np.linspace(45, 48, 5))
...
class TestOrderBlocks:
    ...
    def test_displacement_produces_signal(self):
        closes = list(np.linspace(100, 98, 20)) + [115] + list(np.linspace(115, 100, 20)) + list(np.linspace(100, 110, 20))
        n = len(closes)
        opens = [c - 0.3 for c in closes]
        opens[20] = 98
        df = pd.DataFrame({'open': opens, 'high': [max(o, c) + 0.5 for o, c in zip(opens, closes)], 'low': [min(o, c) - 0.5 for o, c in zip(opens, closes)], 'close': closes, 'volume': [100.0] * n})
...
class TestRangeScalper:
    ...
    def test_signals_in_range_bound_data(self):
        ...
        result = apply_strategy('range_scalper', df, {'bb_period': 10, 'bb_std': 1.5, 'bw_threshold': 0.02, 'vol_ratio': 1.1, 'rsi_period': 5, 'rsi_ob': 65, 'rsi_os': 35})
        assert 'signal' in result.columns
    ...
    def test_no_signals_during_trend(self):
        ...
        result = apply_strategy('range_scalper', df, {'bb_period': 10, 'bb_std': 1.5, 'bw_threshold': 0.005, 'rsi_period': 7, 'rsi_ob': 70, 'rsi_os': 30})
    ...
    def test_columns_present(self):
        ...
        for col in ['bb_mid', 'bb_upper', 'bb_lower', 'bb_bandwidth', 'vol_sma', 'low_volume', 'in_range', 'rsi', 'signal']:
...
class TestEdgeCases:
    ...
    @pytest.mark.parametrize('name', ['sma_crossover', 'ema_crossover', 'rsi', 'bollinger_bands', 'macd', 'mean_reversion', 'momentum', 'volume_weighted', 'triple_ema', 'rsi_macd_combo', 'stoch_rsi', 'supertrend', 'atr_breakout', 'heikin_ashi_ema', 'parabolic_sar', 'amd_ifvg', 'range_scalper'])
    def test_empty_dataframe(self, name):
        df = pd.DataFrame(columns=['open', 'high', 'low', 'close', 'volume'])
        result = apply_strategy(name, df)
        assert len(result) == 0

    @pytest.mark.parametrize('name', ['sma_crossover', 'ema_crossover', 'rsi', 'bollinger_bands', 'macd', 'mean_reversion', 'momentum', 'volume_weighted', 'triple_ema', 'rsi_macd_combo', 'stoch_rsi', 'atr_breakout', 'heikin_ashi_ema', 'amd_ifvg', 'range_scalper', 'sweep_squeeze_combo'])
...
class TestSweepSqueezeCombo:
    ...
    def test_sweep_with_recovery_produces_buy(self):
        ...
        assert (result['ls_signal'] == 1).any(), 'Expected liquidity sweep buy signal on sweep candle'
    ...
    def test_consensus_signal_with_two_agreeing(self):
        ...
        fake_sr.iloc[25] = 1
        with patch('sweep_squeeze_combo.liquidity_sweep_core', return_value=fake_ls_df), patch('sweep_squeeze_combo._squeeze_signals', return_value=fake_sq), patch('sweep_squeeze_combo._stoch_rsi_signals', return_value=fake_sr):
            result = apply_strategy('sweep_squeeze_combo', df, {'min_agree': 2})
        assert result.loc[result.index[25], 'signal'] == 1, 'Expected consensus buy signal=1 when 2 of 3 sub-signals agree'
    ...
    def test_consensus_sell_signal_with_two_agreeing(self):
        ...
        with patch('sweep_squeeze_combo.liquidity_sweep_core', return_value=fake_ls_df), patch('sweep_squeeze_combo._squeeze_signals', return_value=fake_sq), patch('sweep_squeeze_combo._stoch_rsi_signals', return_value=fake_sr):
            result = apply_strategy('sweep_squeeze_combo', df, {'min_agree': 2})
        assert (result['signal'] == -1).any(), 'Expected consensus sell signal=-1 when 2 of 3 sub-signals agree'
    ...
    def test_consensus_buy_signal_with_sweep(self):
        ...
        df.loc[df.index[55], 'low'] = 93.0
        df.loc[df.index[55], 'close'] = 96.0
        result = apply_strategy('sweep_squeeze_combo', df, {'swing_lookback': 5, 'min_agree': 1})
        assert (result['signal'] == 1).any(), 'Expected consensus buy signal=1 with min_agree=1 and ls_signal firing'
