...
def make_ohlc(opens, highs, lows, closes, index, volume=100.0):
    n = len(closes)
    return pd.DataFrame({'open': np.asarray(opens, dtype=float), 'high': np.asarray(highs, dtype=float), 'low': np.asarray(lows, dtype=float), 'close': np.asarray(closes, dtype=float), 'volume': np.full(n, float(volume))}, index=index)
...
def test_emits_short_on_vwap_rejection_in_bear_trend():
    ...
    assert (result['signal'] == -1).any(), 'Expected at least one short signal on rejection bars after a VWAP/EMA rally'
    last_signals = result['signal'].iloc[-5:]
    assert (last_signals == -1).any(), f'Short signal should land in the rejection window, got {last_signals.tolist()}'
...
def test_rsi_reclaim_above_50_blocks_short():
    ...
    assert (result['signal'] == 0).all(), 'RSI reclaim gate at 0 must veto every short'

def test_buffer_rejects_wick_only_touch():
    df = _bear_setup_with_rally_and_rejection()
    result = vwap_rejection_st_core(df, rally_touch_buffer_pct=0.5)
    assert (result['signal'] == 0).all(), 'Buffer gate at 50% must veto every short — no high realistically overshoots that much'
...
def test_trigger_closes_below_every_rally_magnet():
    ...
    for ts, row in fired.iterrows():
        assert row['close'] < row['vwap'], f"{ts}: close {row['close']} not below VWAP {row['vwap']}"
        assert row['close'] < row['ema_short'], f"{ts}: close {row['close']} not below EMA(short) {row['ema_short']}"
        assert row['close'] < row['ema_mid'], f"{ts}: close {row['close']} not below EMA(mid) {row['ema_mid']}"
