...
_PARAMS = dict(pivot_strength=2, entry_atr_mult=1.0, buffer_atr_mult=0.0, confirm_bars=2, atr_period=3)
...
def test_buffer_blocks_shallow_snap_back():
    out = anchored_vwap_reversion_core(_long_stretch_df(), pivot_strength=2, entry_atr_mult=1.0, buffer_atr_mult=5.0, confirm_bars=2, atr_period=3)
    assert (out['signal'] == 0).all()

def test_nan_atr_warmup_yields_no_signal():
    out = anchored_vwap_reversion_core(_long_stretch_df(), pivot_strength=2, entry_atr_mult=1.0, buffer_atr_mult=0.0, confirm_bars=2, atr_period=99)
    assert (out['signal'] == 0).all()

def _load_registry():
    here = os.path.dirname(os.path.abspath(__file__))
    spec = importlib.util.spec_from_file_location('_reg_under_test_avwap_reversion', os.path.join(here, 'registry.py'))
