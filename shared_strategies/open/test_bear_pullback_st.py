...
def make_ohlcv(closes, noise=0.5):
    ...
    n = len(closes)
    return pd.DataFrame({'open': closes - noise * 0.3, 'high': closes + noise, 'low': closes - noise, 'close': closes, 'volume': np.full(n, 100.0)})
...
def test_emits_short_on_failed_rally_in_bear_trend():
    ...
    assert (result['signal'] == -1).any(), 'Expected at least one short signal on rejection bars after a bear-trend rally'
...
def test_shallow_rally_below_rsi_zone_blocks_short():
    ...
    assert not ((rsi_tail >= 55.0) & (rsi_tail <= 65.0)).any(), f'Shallow-rally setup unexpectedly hit the RSI zone: {rsi_tail.tolist()}'
    assert (result['signal'] == 0).all(), 'Shallow rally must not produce any short signals'
...
def test_buffer_rejects_wick_only_touch():
    ...
    assert (result['signal'] == 0).all(), 'Wick-only touch (no buffer overshoot) should not produce a short signal'
