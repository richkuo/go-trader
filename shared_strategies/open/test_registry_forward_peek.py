...
def _load(name: str, path: str):
    ...
    return mod
_REGISTRY = _load('_registry_forward_peek', os.path.join(_HERE, 'registry.py'))
SKIP_STRATEGIES = {}
EXPECTED_ALL_ZERO_SIGNAL = {'hold'}
N_BARS = 960
_SEED = 20261279
...
def _make_ohlcv(rng: np.random.RandomState, n: int, start: str) -> pd.DataFrame:
    ...
    df['funding_rate'] = 5e-05 + 8e-05 * np.sin(np.arange(n) / 30.0) + rng.randn(n) * 2e-05
    return df
...
def _perturbed_df(df: pd.DataFrame, cut: int) -> pd.DataFrame:
    tail = _make_ohlcv(np.random.RandomState(_SEED + 1), len(df) - cut, str(df.index[cut]))
...
def _range_scalper_fixture() -> pd.DataFrame:
    ...
    return pd.DataFrame({'open': close, 'high': close + 0.02, 'low': close - 0.02, 'close': close, 'volume': volume}, index=idx)
...
def _sweep_squeeze_combo_fixture() -> pd.DataFrame:
    ...
    df = pd.DataFrame({'open': close, 'high': close + 0.4, 'low': close - 0.4, 'close': close, 'volume': np.full(n, 100.0)}, index=pd.date_range('2026-01-01', periods=n, freq='15min'))
    stoch = _REGISTRY.STRATEGIES['stoch_rsi']
    sr = pd.to_numeric(stoch['fn'](df.copy(), **stoch['default_params'])['signal'], errors='coerce').fillna(0).to_numpy()
    ...
    return df
STRATEGY_FIXTURES = {'range_scalper': _range_scalper_fixture, 'sweep_squeeze_combo': _sweep_squeeze_combo_fixture}
_FIXTURE_CACHE = {}
_PERTURBED_CACHE = {}
...
def _signal(fn, params, df: pd.DataFrame) -> np.ndarray:
    ...
    assert len(sig) == len(df), f'signal length {len(sig)} != frame length {len(df)}'
    return sig
...
def _prefix_violations(fn, params, df, cache_key):
    ...
    for cut in _cuts_for(full):
        ...
        if len(bad):
            violations.append(f"cut={cut}: truncation changed signals at bars {bad[:10].tolist()}{('…' if len(bad) > 10 else '')} (dropping future bars must not change an earlier signal)")
        pert = _signal(fn, params, _perturbed_at(cache_key, df, cut))
        bad = _mismatch(full[:cut], pert[:cut])
        if len(bad):
            violations.append(f"cut={cut}: perturbing future bars changed signals at bars {bad[:10].tolist()}{('…' if len(bad) > 10 else '')} (bars >= {cut} must be invisible to earlier signals)")
...
def _sweep_cases():
    ...
    for name, entry in _REGISTRY.STRATEGIES.items():
        ...
        for platform in entry['platforms']:
            merged = {**entry['default_params'], **entry['variants'].get(platform, {}).get('default_params', {})}
            key = repr(sorted(merged.items()))
            seen.setdefault(key, (merged, []))[1].append(platform)
        for merged, platforms in seen.values():
            cases.append(pytest.param(name, merged, id=f"{name}[{'+'.join(platforms)}]"))
    return cases
...
def test_registry_strategy_is_truncation_invariant(name, params):
    ...
    assert not violations, f'{name} appears to read future bars (forward peek):\n  ' + '\n  '.join(violations)
    nonzero = np.nan_to_num(full) != 0
    if not nonzero.any():
        assert name in EXPECTED_ALL_ZERO_SIGNAL, f'{name} produced all-zero signals on the sweep fixture — the truncation-invariance check is vacuous for it. Either enrich the fixture / add a per-strategy input hook, or add it to EXPECTED_ALL_ZERO_SIGNAL with an inline justification.'
...
def test_zero_signal_allowlist_is_exact():
    ...
    for name in sorted(EXPECTED_ALL_ZERO_SIGNAL):
        ...
        assert not emitted, f'{name} now emits signals on the sweep fixture — remove it from EXPECTED_ALL_ZERO_SIGNAL (stale vacuity entry).'
...
def test_skip_list_is_closed_allowlist():
    ...
    for name, reason in SKIP_STRATEGIES.items():
        assert isinstance(reason, str) and reason.strip(), f'SKIP_STRATEGIES[{name!r}] must carry a non-empty justification'
...
def test_sweep_covers_every_registered_strategy():
    ...
    assert any((e.get('backtest_only') for e in _REGISTRY.STRATEGIES.values())), 'expected at least one backtest_only strategy in the sweep (#1138)'
...
def test_harness_detects_forward_peeking_strategy():
    ...
    assert violations, 'forward-peeking strategy passed the truncation-invariance checker — the sweep is not sensitive to look-ahead'
