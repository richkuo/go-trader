...
def make_range_with_spike(base=100.0, quiet=40, spike_height=12.0, spike_len=5, recover=6, seed=7):
    dip = make_range_with_v_dip(base=base, quiet=quiet, dip_depth=spike_height, dip_len=spike_len, recover=recover, seed=seed)
    return 2 * base - dip
...
def test_walking_the_band_does_not_fire_repeatedly():
    ...
    base = 100.0
    seg = list(base + rng.randn(40) * 0.3)
    seg += list(np.linspace(base, base - 20, 25))
    seg += list(np.linspace(base - 20, base - 16, 4))
