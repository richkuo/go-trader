...
def make_ohlcv(closes, volume=None, noise=1.0, freq='1h', start='2024-01-01'):
    ...
    return pd.DataFrame({'open': closes - noise * 0.3, 'high': closes + noise, 'low': closes - noise, 'close': closes, 'volume': np.asarray(volume, dtype=float)}, index=idx)
...
def test_columns_present():
    ...
    for col in ('signal', 'position', 'htf_trend', 'htf_ema_fast', 'htf_ema_slow', 'ltf_ema'):
...
def test_exit_emitted_when_htf_trend_flips():
    ...
    down = make_ohlcv(down_closes, start=up.index[-1] + pd.Timedelta(hours=1))
...
def test_incomplete_trailing_bucket_never_visible():
    ...
    proj = pd.Series(htf['close'].to_numpy(), index=visible_at).reindex(df.index, method='ffill')
