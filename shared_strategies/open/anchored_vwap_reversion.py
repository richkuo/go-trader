...
def anchored_vwap_reversion_core(df: pd.DataFrame, pivot_strength: int=5, entry_atr_mult: float=1.5, buffer_atr_mult: float=0.25, confirm_bars: int=2, atr_period: int=14) -> pd.DataFrame:
    result = df.copy()
    n = len(result)
    result['signal'] = 0
    result['avwap'] = np.nan
    ...
    for nbar in range(n):
        ...
        if low[b] <= lower_band[b] and close[b] >= lower_band[b] + buf and np.all(win_c >= lower_band[b:nbar + 1]) and np.all(win_c < avwap[b:nbar + 1]) and (low[b - 1] > lower_band[b - 1]):
            sig[nbar] = 1
            continue
        if high[b] >= upper_band[b] and close[b] <= upper_band[b] - buf and np.all(win_c <= upper_band[b:nbar + 1]) and np.all(win_c > avwap[b:nbar + 1]) and (high[b - 1] < upper_band[b - 1]):
            sig[nbar] = -1
    result['signal'] = sig
    return result
