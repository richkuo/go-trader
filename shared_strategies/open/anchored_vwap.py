...
def anchored_vwap_core(df: pd.DataFrame, pivot_strength: int=5, buffer_atr_mult: float=0.25, confirm_bars: int=2, atr_period: int=14, gate_rsi_period: int=0, gate_rsi_level: float=50.0, gate_ema_period: int=0) -> pd.DataFrame:
    result = df.copy()
    n = len(result)
    result['signal'] = 0
    result['avwap'] = np.nan
    ...
    for nbar in range(n):
        ...
        if np.all(win_c >= win_v) and close[b] >= avwap[b] + buf and (close[b - 1] < avwap[b - 1]):
            sig[nbar] = 1
            continue
        if np.all(win_c <= win_v) and close[b] <= avwap[b] - buf and (close[b - 1] > avwap[b - 1]):
            sig[nbar] = -1
