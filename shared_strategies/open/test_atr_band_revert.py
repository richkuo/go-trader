...
def _box(n=60, level=100.0, top=101.0, bottom=99.0):
    ...
    c = np.full(n, level)
    return pd.DataFrame({'open': c, 'high': np.full(n, top), 'low': np.full(n, bottom), 'close': c, 'volume': [1.0] * n}, index=idx)
...
def test_long_invariant_holds_everywhere():
    ...
    idx = pd.date_range('2024-01-01', periods=n, freq='1h')
    df = pd.DataFrame({'open': closes, 'high': closes + 1.0, 'low': closes - 1.0, 'close': closes, 'volume': [1.0] * n}, index=idx)
    r = atr_band_revert_core(df, period=20, atr_period=14, k_entry=1.5, allow_short=False)
