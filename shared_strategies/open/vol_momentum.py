...
def vol_momentum_core(df: pd.DataFrame, mom_window: int=24, atr_period: int=14, entry_threshold: float=0.3, exit_threshold: float=0.05, eff_entry: float=0.35, eff_exit: float=0.15, allow_short: bool=False) -> pd.DataFrame:
    result = df.copy()
    n = len(result)
