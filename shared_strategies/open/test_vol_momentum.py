...
def make_uptrend_with_plateau():
    return np.concatenate([100 + np.random.RandomState(1).randn(40) * 0.3, np.linspace(100, 180, 100), 180 + np.random.RandomState(3).randn(40) * 0.3])

def make_downtrend_with_plateau():
    return np.concatenate([100 + np.random.RandomState(2).randn(40) * 0.3, np.linspace(100, 50, 100), 50 + np.random.RandomState(4).randn(40) * 0.3])
...
def test_uptrend_exact_entry_and_decay_exit():
    ...
    assert out['vol_mom'].iloc[50] > 0.3
...
def test_downtrend_allow_short_enters_short():
    out = vol_momentum_core(make_ohlcv(make_downtrend_with_plateau(), noise=0.4), allow_short=True)
    shorts = np.where(out['signal'].values == -1)[0]
    assert shorts.tolist()[0] == 53
    assert int((out['position'] == -1).sum()) == 108
    assert out['vol_mom'].iloc[53] < -0.3
...
def test_impossible_entry_threshold_blocks_entries():
    out = vol_momentum_core(make_ohlcv(make_uptrend_with_plateau(), noise=0.4), entry_threshold=10.0)
    assert (out['position'] == 0).all()

def test_impossible_efficiency_gate_blocks_entries():
    out = vol_momentum_core(make_ohlcv(make_uptrend_with_plateau(), noise=0.4), eff_entry=1.01)
    assert (out['position'] == 0).all()

def test_efficiency_collapse_exits_position():
    out = vol_momentum_core(make_ohlcv(make_uptrend_with_plateau(), noise=0.4), exit_threshold=-10.0, eff_exit=0.15)
