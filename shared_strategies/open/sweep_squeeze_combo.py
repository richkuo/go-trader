...
def _squeeze_signals(df: pd.DataFrame, bb_period: int=20, bb_std: float=2.0, kc_period: int=20, kc_mult: float=1.5, mom_lookback: int=12) -> pd.Series:
    result = df.copy()
...
def _stoch_rsi_signals(df: pd.DataFrame, rsi_period: int=14, stoch_period: int=14, k_smooth: int=3, d_smooth: int=3, overbought: float=80, oversold: float=20) -> pd.Series:
    close = df['close']
...
def sweep_squeeze_combo_core(df: pd.DataFrame, swing_lookback: int=10, confirmation: int=1, bb_period: int=20, bb_std: float=2.0, kc_period: int=20, kc_mult: float=1.5, mom_lookback: int=12, rsi_period: int=14, stoch_period: int=14, k_smooth: int=3, d_smooth: int=3, overbought: float=80, oversold: float=20, min_agree: int=2) -> pd.DataFrame:
    result = df.copy()
