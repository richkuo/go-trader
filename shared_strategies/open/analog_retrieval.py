...
def encode_features(df: pd.DataFrame, feat_window: int=20, atr_period: int=14, vol_baseline: int=100) -> pd.DataFrame:
    ...
    feats = pd.DataFrame({'ret_eff': ret_eff, 'mom_atr': mom_atr, 'atr_pct': atr_pct, 'vol_ratio': vol_ratio, 'trend_atr': trend_atr}, index=df.index)
...
def retrieve_neighbors(index_matrix: np.ndarray, query: np.ndarray, k: int) -> np.ndarray:
...
def analog_retrieval_core(df: pd.DataFrame, feat_window: int=20, atr_period: int=14, vol_baseline: int=100, horizon: int=12, k_neighbors: int=25, min_index: int=200, max_index: int=5000, min_t_stat: float=2.0, min_edge_atr: float=0.25) -> pd.DataFrame:
    result = df.copy()
    n = len(result)
    result['signal'] = 0
    ...
    if n == 0:
        return result
    feats = encode_features(result, feat_window=feat_window, atr_period=atr_period, vol_baseline=vol_baseline)
