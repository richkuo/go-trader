...
import pandas as pd
import pytest
from analog_retrieval import FEATURE_COLUMNS, analog_retrieval_core, encode_features, forward_returns, retrieve_neighbors
...
def _ohlcv(closes, volume=100.0):
    ...
    n = len(closes)
    return pd.DataFrame({'open': closes, 'high': closes + 0.5, 'low': closes - 0.5, 'close': closes, 'volume': np.full(n, float(volume))}, index=_hourly_index(n))
...
_FAST = dict(feat_window=8, atr_period=5, vol_baseline=20, horizon=5, k_neighbors=10, min_index=30, max_index=200, min_t_stat=0.5, min_edge_atr=0.0)
...
def test_encode_features_columns_and_warmup():
    ...
    assert (tail['ret_eff'].abs() <= 1.0 + 1e-09).all()
...
def test_core_gates_suppress_all_signals_when_impossible():
    df = _sawtooth_df(700)
    hard_t = analog_retrieval_core(df, **{**_FAST, 'min_t_stat': 1000000000.0})
    assert (hard_t['signal'] == 0).all()
    hard_edge = analog_retrieval_core(df, **{**_FAST, 'min_edge_atr': 1000000000.0})
...
def test_leakage_prefix_consistency_regression():
    ...
    for cut in (320, 450, 555, 699):
        ...
        pd.testing.assert_frame_equal(full.iloc[:cut][checked_cols], prefix[checked_cols])
