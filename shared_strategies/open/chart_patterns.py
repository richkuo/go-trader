...
def find_swing_points(highs: pd.Series, lows: pd.Series, lookback: int=5) -> tuple:
    window = 2 * lookback + 1
...
def volume_confirmed(volume: pd.Series, index: int, vol_period: int=20, vol_multiplier: float=1.5) -> bool:
    if index < vol_period:
        return True
...
def detect_double_top(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    matches = []
    ...
    for i in range(len(sh_idx) - 1):
        ...
        if abs(h1 - h2) / max(h1, 1e-09) > tolerance:
            continue
        ...
        for j in range(h2_bar + lookback + 1, len(close)):
            if close.iloc[j] < neckline:
                matches.append(PatternMatch(pattern='double_top', signal=-1, bar_index=j, neckline=neckline, confidence=0.7))
                break
    return matches

def detect_double_bottom(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    matches = []
    sl_idx = _get_swing_indices(swing_lows)
    sh_idx = _get_swing_indices(swing_highs)
    if len(sl_idx) < 2:
    ...
    for i in range(len(sl_idx) - 1):
        ...
        if abs(l1 - l2) / max(l1, 1e-09) > tolerance:
            continue
        ...
        for j in range(l2_bar + lookback + 1, len(close)):
            if close.iloc[j] > neckline:
                matches.append(PatternMatch(pattern='double_bottom', signal=1, bar_index=j, neckline=neckline, confidence=0.7))
                break
    return matches

def detect_triple_top(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    matches = []
    sh_idx = _get_swing_indices(swing_highs)
    sl_idx = _get_swing_indices(swing_lows)
    if len(sh_idx) < 3:
    ...
    for i in range(len(sh_idx) - 2):
        ...
        avg_h = (h1 + h2 + h3) / 3
        if any((abs(h - avg_h) / max(avg_h, 1e-09) > tolerance for h in (h1, h2, h3))):
            continue
        ...
        for j in range(h3_bar + lookback + 1, len(close)):
            if close.iloc[j] < neckline:
                matches.append(PatternMatch(pattern='triple_top', signal=-1, bar_index=j, neckline=neckline, confidence=0.8))
                break
    return matches

def detect_triple_bottom(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    matches = []
    sl_idx = _get_swing_indices(swing_lows)
    sh_idx = _get_swing_indices(swing_highs)
    if len(sl_idx) < 3:
    ...
    for i in range(len(sl_idx) - 2):
        ...
        avg_l = (l1 + l2 + l3) / 3
        if any((abs(l - avg_l) / max(avg_l, 1e-09) > tolerance for l in (l1, l2, l3))):
            continue
        ...
        for j in range(l3_bar + lookback + 1, len(close)):
            if close.iloc[j] > neckline:
                matches.append(PatternMatch(pattern='triple_bottom', signal=1, bar_index=j, neckline=neckline, confidence=0.8))
                break
    return matches

def detect_head_and_shoulders(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    matches = []
    sh_idx = _get_swing_indices(swing_highs)
    sl_idx = _get_swing_indices(swing_lows)
    if len(sh_idx) < 3:
    ...
    for i in range(len(sh_idx) - 2):
        ...
        if head <= ls or head <= rs:
            continue
        if abs(ls - rs) / max(ls, 1e-09) > tolerance:
            continue
        ...
        for j in range(rs_bar + lookback + 1, len(close)):
            if close.iloc[j] < neckline:
                matches.append(PatternMatch(pattern='head_shoulders', signal=-1, bar_index=j, neckline=neckline, confidence=0.85))
                break
    return matches

def detect_inverse_head_and_shoulders(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    matches = []
    sl_idx = _get_swing_indices(swing_lows)
    sh_idx = _get_swing_indices(swing_highs)
    if len(sl_idx) < 3:
    ...
    for i in range(len(sl_idx) - 2):
        ...
        for j in range(rs_bar + lookback + 1, len(close)):
            if close.iloc[j] > neckline:
                matches.append(PatternMatch(pattern='inv_head_shoulders', signal=1, bar_index=j, neckline=neckline, confidence=0.85))
                break
    return matches

def _detect_flag(highs: pd.Series, lows: pd.Series, close: pd.Series, volume: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, direction: int, pole_min_bars: int=5, pole_max_bars: int=20, flag_min_bars: int=5, flag_max_bars: int=30, pole_atr_mult: float=2.0, lookback: int=5) -> list:
    matches = []
    n = len(close)
        ...
        for peak_bar in sh_idx:
            ...
            for j in range(breakout_start, min(flag_end + 1, n)):
                if close.iloc[j] > flag_high_level:
                    matches.append(PatternMatch(pattern='bull_flag', signal=1, bar_index=j, neckline=flag_high_level, confidence=0.75))
                    break
    else:
        for trough_bar in sl_idx:
            ...
            for j in range(breakout_start, min(flag_end + 1, n)):
                if close.iloc[j] < flag_low_level:
                    matches.append(PatternMatch(pattern='bear_flag', signal=-1, bar_index=j, neckline=flag_low_level, confidence=0.75))
                    break
    return matches

def detect_bull_flag(highs: pd.Series, lows: pd.Series, close: pd.Series, volume: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    return _detect_flag(highs, lows, close, volume, swing_highs, swing_lows, direction=1, lookback=lookback)

def detect_bear_flag(highs: pd.Series, lows: pd.Series, close: pd.Series, volume: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    return _detect_flag(highs, lows, close, volume, swing_highs, swing_lows, direction=-1, lookback=lookback)
...
def _detect_triangle(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, pattern_type: str, tolerance: float=0.03, min_points: int=4, lookback: int=5) -> list:
    matches = []
    sh_idx = _get_swing_indices(swing_highs)
    sl_idx = _get_swing_indices(swing_lows)
        ...
        if pattern_type == 'ascending':
            ...
            for j in range(breakout_start, breakout_end):
                if close.iloc[j] > resistance_level:
                    matches.append(PatternMatch(pattern='ascending_triangle', signal=1, bar_index=j, neckline=resistance_level, confidence=0.7))
                    break
            ...
            for j in range(breakout_start, breakout_end):
                if close.iloc[j] < support_level:
                    matches.append(PatternMatch(pattern='descending_triangle', signal=-1, bar_index=j, neckline=support_level, confidence=0.7))
                    break
            ...
            for j in range(breakout_start, breakout_end):
                ...
                if close.iloc[j] > upper:
                    matches.append(PatternMatch(pattern='symmetrical_triangle', signal=1, bar_index=j, neckline=upper, confidence=0.65))
                    break
                elif close.iloc[j] < lower:
                    matches.append(PatternMatch(pattern='symmetrical_triangle', signal=-1, bar_index=j, neckline=lower, confidence=0.65))
                    break
    return matches

def detect_ascending_triangle(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    return _detect_triangle(highs, lows, close, swing_highs, swing_lows, 'ascending', tolerance, lookback=lookback)

def detect_descending_triangle(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    return _detect_triangle(highs, lows, close, swing_highs, swing_lows, 'descending', tolerance, lookback=lookback)

def detect_symmetrical_triangle(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    return _detect_triangle(highs, lows, close, swing_highs, swing_lows, 'symmetrical', tolerance, lookback=lookback)

def detect_cup_and_handle(highs: pd.Series, lows: pd.Series, close: pd.Series, swing_highs: pd.Series, swing_lows: pd.Series, tolerance: float=0.03, lookback: int=5) -> list:
    matches = []
    sh_idx = _get_swing_indices(swing_highs)
    sl_idx = _get_swing_indices(swing_lows)
    ...
    for i in range(len(sh_idx) - 2):
        ...
        for j_idx in range(i + 1, len(sh_idx)):
            ...
            if abs(left_rim - right_rim) / max(left_rim, 1e-09) > tolerance:
                continue
            ...
            if cup_depth / max(rim_avg, 1e-09) < 0.1:
                continue
            ...
            for k in range(right_rim_bar + lookback + 1, min(handle_end + 10, len(close))):
                if close.iloc[k] > breakout_level:
                    matches.append(PatternMatch(pattern='cup_and_handle', signal=1, bar_index=k, neckline=breakout_level, confidence=0.8))
                    break
...
def _htf_gate_trend(df: pd.DataFrame, htf_factor: int, ema_fast: int=20, ema_slow: int=40) -> pd.Series:
    ...
    trend_htf = pd.Series(np.where(warm & (fast > slow), 1, np.where(warm & (fast < slow), -1, 0)), index=htf.index)
    return _project_to_native(trend_htf, visible_at, df.index).fillna(0).astype(int)
_VOLUME_DETECTORS = {'bull_flag', 'bear_flag'}
_ALL_DETECTORS = {'double_top': detect_double_top, 'double_bottom': detect_double_bottom, 'triple_top': detect_triple_top, 'triple_bottom': detect_triple_bottom, 'head_shoulders': detect_head_and_shoulders, 'inv_head_shoulders': detect_inverse_head_and_shoulders, 'bull_flag': detect_bull_flag, 'bear_flag': detect_bear_flag, 'ascending_triangle': detect_ascending_triangle, 'descending_triangle': detect_descending_triangle, 'symmetrical_triangle': detect_symmetrical_triangle, 'cup_and_handle': detect_cup_and_handle}

def chart_pattern_core(df: pd.DataFrame, pivot_lookback: int=5, tolerance: float=0.03, vol_multiplier: float=1.5, vol_period: int=20, htf_gate_factor: int=0, htf_gate_mode: str='veto', htf_gate_ema_fast: int=20, htf_gate_ema_slow: int=40) -> pd.DataFrame:
    if htf_gate_mode not in _HTF_GATE_MODES:
        raise ValueError(f'htf_gate_mode must be one of {_HTF_GATE_MODES}, got {htf_gate_mode!r}')
    result = df.copy()
    result['signal'] = 0
    n = len(result)
    if n < 2 * pivot_lookback + 5:
        ...
        return result
    swing_highs, swing_lows = find_swing_points(result['high'], result['low'], pivot_lookback)
    all_matches = []
    for name, detector in _ALL_DETECTORS.items():
        if name in _VOLUME_DETECTORS:
            matches = detector(result['high'], result['low'], result['close'], result['volume'], swing_highs, swing_lows, tolerance, lookback=pivot_lookback)
        else:
            matches = detector(result['high'], result['low'], result['close'], swing_highs, swing_lows, tolerance, lookback=pivot_lookback)
    ...
    if htf_gate_factor and int(htf_gate_factor) > 1:
        trend = _htf_gate_trend(result, int(htf_gate_factor), htf_gate_ema_fast, htf_gate_ema_slow)
