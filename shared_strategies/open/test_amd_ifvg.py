...
LEGACY_UTC = dict(asian_start_hour=0, asian_end_hour=8, london_start_hour=8, london_end_hour=12, session_tz='UTC')

def _make_intraday_df(rows: list) -> pd.DataFrame:
    idx = pd.date_range('2024-01-01 00:00', periods=len(rows), freq='15min', tz='UTC')
    return pd.DataFrame({'open': [r[1] for r in rows], 'high': [r[2] for r in rows], 'low': [r[3] for r in rows], 'close': [r[4] for r in rows], 'volume': [100.0] * len(rows)}, index=idx)
...
class TestLookahead:
    ...
    def test_entry_bar_signal_independent_of_future_close(self):
        ...
        assert out_up.loc[entry_idx, 'signal'] == out_dn.loc[entry_idx, 'signal'], f"signal at entry bar {entry_idx} changed when only future bars varied: up={out_up.loc[entry_idx, 'signal']} dn={out_dn.loc[entry_idx, 'signal']}"
        assert out_up.loc[entry_idx, 'signal'] != 0, 'test setup produced no signal at K — adjust fixture so the bug regression actually exercises a signal-firing bar'
    ...
    def test_truncation_invariant(self):
        ...
        for k in signal_bars:
            ...
            assert partial.loc[k, 'signal'] == full.loc[k, 'signal'], f"signal at {k} differs after truncation: full={full.loc[k, 'signal']} truncated={partial.loc[k, 'signal']}"
...
def _make_utc_df_from_ny(ny_bars: list, day: str, tz='America/New_York') -> pd.DataFrame:
    ...
    return pd.DataFrame({'open': [b[0] for b in ny_bars], 'high': [b[1] for b in ny_bars], 'low': [b[2] for b in ny_bars], 'close': [b[3] for b in ny_bars], 'volume': [100.0] * len(ny_bars)}, index=idx_utc)

def _ny_asian_bars() -> list:
    return [(m - 0.1, m + 0.5, m - 0.5, m) for i in range(16) for m in (100 + 0.5 * (1 if i % 2 else -1),)]

def _ny_london_bars() -> list:
    return [(99.5, 99.6, 98.7, 99.0), (99.5, 100.0, 99.4, 99.9), (100.3, 100.9, 100.1, 100.7), (100.5, 101.3, 100.8, 101.2), (100.4, 100.6, 100.0, 100.4), (100.6, 100.9, 100.3, 100.6), (100.6, 100.9, 100.3, 100.6), (100.6, 100.9, 100.3, 100.6)]
...
def _make_dst_crossing_df(tz='America/New_York') -> pd.DataFrame:
    ...
    rows = asian + london
    return pd.DataFrame({'open': [b[0] for b in rows], 'high': [b[1] for b in rows], 'low': [b[2] for b in rows], 'close': [b[3] for b in rows], 'volume': [100.0] * len(rows)}, index=idx)
...
class TestDSTInvariance:
    ...
    def test_winter_and_summer_setups_match(self):
        ...
        assert np.array_equal(ws, ss), f'DST changed the signal sequence: winter fired at {np.flatnonzero(ws)}, summer at {np.flatnonzero(ss)}'
        assert (ws == 1).any()
    ...
    def test_dst_shifts_utc_hour_of_signal(self):
        ...
        assert (wk.hour - sk.hour) % 24 == 1, f'expected a 1h UTC shift across DST: winter {wk.hour}h, summer {sk.hour}h'
...
class TestSessionDayWrap:
    ...
    def test_setup_straddling_midnight_fires(self):
        ...
        out = amd_ifvg_core(df)
        assert (out['signal'] == 1).any(), 'setup straddling civil midnight failed to fire — session-day grouping likely split the Asian range from the London sweep'
...
class TestTruncationDefaultPath:
    ...
    def _assert_truncation_invariant(self, df):
        ...
        for k in signal_bars:
            ...
            assert partial.loc[k, 'signal'] == full.loc[k, 'signal'], f"signal at {k} changed after truncation at K: full={full.loc[k, 'signal']} truncated={partial.loc[k, 'signal']}"
