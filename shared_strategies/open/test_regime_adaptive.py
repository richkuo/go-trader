...
PIN = dict(period=20, adx_threshold=25.0, return_eff_threshold=0.05, range_eff_threshold=0.03, efficiency_threshold=0.5, breakout_lookback=20, mr_lookback=20, mr_entry_z=1.5, mr_exit_z=0.0)

def make_ohlcv(closes, noise=0.5, volume=100.0):
    closes = np.asarray(closes, dtype=float)
    n = len(closes)
    return pd.DataFrame({'open': closes - noise * 0.3, 'high': closes + noise, 'low': closes - noise, 'close': closes, 'volume': np.full(n, volume)}, index=pd.date_range('2026-01-01', periods=n, freq='1h'))
...
def make_clean_uptrend():
    return np.concatenate([100 + np.random.RandomState(1).randn(60) * 0.3, np.linspace(100, 200, 150)])

def make_clean_downtrend():
    return np.concatenate([100 + np.random.RandomState(2).randn(60) * 0.3, np.linspace(100, 40, 150)])

def test_columns_present():
    out = regime_adaptive_core(make_ohlcv(make_range_with_swings()), **PIN)
    for col in ('signal', 'position', 'ra_label', 'ra_return_eff', 'ra_range_eff', 'ra_efficiency', 'ra_adx', 'ra_z'):
...
def test_short_inputs_return_flat_never_raise():
    ...
    for n in (8, 9, 16):
        out = regime_adaptive_core(make_ohlcv([100.0 + 0.1 * i for i in range(n)]), period=8)
        assert len(out) == n
        assert (out['signal'] == 0).all()
...
def test_downtrend_allow_short_enters_short():
    out = regime_adaptive_core(make_ohlcv(make_clean_downtrend(), noise=0.4), allow_short=True, **PIN)
...
def test_ranging_fades_both_sides_when_short_allowed():
    out = regime_adaptive_core(make_ohlcv(make_range_with_swings()), allow_short=True, **PIN)
    assert (out['position'] == 1).any()
    assert (out['position'] == -1).any()

def test_impossible_efficiency_blocks_breakouts():
    out = regime_adaptive_core(make_ohlcv(make_clean_uptrend(), noise=0.4), **{**PIN, 'efficiency_threshold': 1.01})
    assert (out['position'] == 0).all()

def test_impossible_entry_z_blocks_fades():
    out = regime_adaptive_core(make_ohlcv(make_range_with_swings()), **{**PIN, 'mr_entry_z': 50.0})
    assert (out['position'] == 0).all()
...
def test_slow_trend_veto_blocks_bear_market_fades():
    ...
    vetoed = regime_adaptive_core(df, slow_trend_lookback=60, slow_veto_threshold=0.05, **PIN)
    base_fades = int((base['signal'] == 1).sum())
    vetoed_fades = int((vetoed['signal'] == 1).sum())
    assert base_fades > 0
    assert vetoed_fades < base_fades
    mr_labels = {'ranging_quiet', 'ranging_volatile', 'trending_up_choppy', 'trending_down_choppy'}
...
def test_slow_trend_veto_inverse_blocks_short_fades_in_bull():
    ...
    vetoed = regime_adaptive_core(df, slow_trend_lookback=60, slow_veto_threshold=0.05, allow_short=True, **PIN)
    mr_labels = {'ranging_quiet', 'ranging_volatile', 'trending_up_choppy', 'trending_down_choppy'}
    surviving = np.where(vetoed['signal'].values == -1)[0]
    for i in surviving:
