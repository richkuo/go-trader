...
def test_no_lookahead_bias():
    ...
    df = make_ohlcv(prices)
    result = donchian_breakout_core(df)
    assert result['signal'].iloc[25] == 1, 'Breakout candle should fire buy signal (channel based on prior bars)'
    assert (result['signal'].iloc[:25] == 0).all(), 'No signals expected in the flat region before breakout'
