...
def _ohlcv(closes, highs=None, lows=None, opens=None, volume=100.0):
    ...
    return pd.DataFrame({'open': opens, 'high': highs, 'low': lows, 'close': closes, 'volume': vol}, index=_hourly_index(n))
_PARAMS = dict(pivot_strength=2, buffer_atr_mult=0.0, confirm_bars=2, min_width_atr_mult=0.0, atr_period=3)
...
def test_empty_and_short_df_return_zero_signal():
    ...
    for col in ('avwap_support', 'avwap_resistance', 'anchor_low_index', 'anchor_high_index', 'atr'):
...
def test_avwaps_match_hand_computed_prefix_sums():
    ...
    flat = np.asarray(closes, dtype=float)
    ...
    for nbar in (4, 6, 8, 10, 11):
        ...
        assert abs(resistance[nbar] - expected) < 1e-09, (nbar, resistance[nbar], expected)
    for nbar in (8, 9, 10, 11):
        expected = np.mean(closes[6:nbar + 1])
        assert abs(support[nbar] - expected) < 1e-09, (nbar, support[nbar], expected)
...
def test_zero_volume_window_falls_back_to_typical():
    ...
    for nbar in range(8, 12):
        assert abs(support[nbar] - tp[nbar]) < 1e-09
        assert abs(resistance[nbar] - tp[nbar]) < 1e-09
...
def test_buffer_blocks_shallow_reclaim():
    out = anchored_vwap_channel_core(_long_bounce_df(), pivot_strength=2, buffer_atr_mult=5.0, confirm_bars=2, min_width_atr_mult=0.0, atr_period=3)
    assert (out['signal'] == 0).all()

def test_nan_atr_warmup_yields_no_signal():
    out = anchored_vwap_channel_core(_long_bounce_df(), pivot_strength=2, buffer_atr_mult=0.0, confirm_bars=2, min_width_atr_mult=0.0, atr_period=99)
    assert (out['signal'] == 0).all()
...
def test_min_width_gate_blocks_thin_channel():
    out = anchored_vwap_channel_core(_long_bounce_df(), pivot_strength=2, buffer_atr_mult=0.0, confirm_bars=2, min_width_atr_mult=50.0, atr_period=3)
    assert (out['signal'] == 0).all()

def _load_registry():
    here = os.path.dirname(os.path.abspath(__file__))
    spec = importlib.util.spec_from_file_location('_reg_under_test_avwap_channel', os.path.join(here, 'registry.py'))
