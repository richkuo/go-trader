...
_spec = importlib.util.spec_from_file_location('futures_strategies', os.path.join(_futures_dir, 'strategies.py'))
...
class TestFuturesRegistry:
    ...
    def test_strategies_registered(self):
        ...
        assert len(names) >= 10
        for expected in ['breakout', 'delta_neutral_funding', 'mean_reversion_pro', 'momentum_pro', 'anchored_vwap', 'chart_pattern']:
            assert expected in names, f'{expected} not registered'
        for quarantined in ['sma_crossover', 'ema_crossover', 'bollinger_bands', 'volume_weighted', 'triple_ema', 'rsi_macd_combo', 'momentum', 'mean_reversion', 'rsi', 'macd', 'stoch_rsi', 'supertrend', 'squeeze_momentum', 'ichimoku_cloud', 'atr_breakout', 'heikin_ashi_ema', 'order_blocks', 'parabolic_sar', 'triple_ema_bidir', 'tema_cross_bd', 'funding_skew', 'consolidation_range', 'regime_adaptive']:
...
class TestTripleEMABidir:
    ...
    def test_direct_flip_signal_clamped(self):
        closes = list(make_trending_up(80, start=100, step=1.0)) + list(make_trending_down(80, start=180, step=1.0))
    ...
    def test_custom_params_applied(self):
        ...
        custom = _run('triple_ema_bidir', closes, params={'short_period': 3, 'mid_period': 10, 'long_period': 30})
...
class TestRSIMACDCombo:
    ...
    def test_loosened_short_gate_catches_more_shorts(self):
        ...
        assert (loose['signal'] == -1).sum() > (strict['signal'] == -1).sum(), 'loosening rsi_short_min must allow more short signals'
    ...
    def test_params_forwarded_via_apply_strategy(self):
        ...
        result = _run('rsi_macd_combo', closes, params={'rsi_short_min': 30, 'rsi_long_max': 70})
        assert 'rsi' in result.columns
        _valid_signals(result)
...
class TestDeltaNeutralFunding:
    ...
    def test_entry_on_high_funding(self):
        result = _run('delta_neutral_funding', make_flat(20), {'avg_funding_rate_7d': 0.0005, 'entry_threshold': 0.0001})
        assert result['signal'].iloc[-1] == -1

    def test_exit_on_low_funding(self):
        result = _run('delta_neutral_funding', make_flat(20), {'avg_funding_rate_7d': 3e-05, 'exit_threshold': 5e-05})
        assert result['signal'].iloc[-1] == 1

    def test_zero_funding_no_signal(self):
        result = _run('delta_neutral_funding', make_flat(20), {'avg_funding_rate_7d': 0.0})
        assert (result['signal'] == 0).all()
    ...
    def test_series_shorts_after_full_window_when_funding_rich(self):
        ...
        out = apply_strategy('delta_neutral_funding', df, {'entry_threshold': 0.0001, 'exit_threshold': 5e-05})
    ...
    def test_series_exits_when_funding_cheap(self):
        df = self._series_df([3e-05] * 60)
        out = apply_strategy('delta_neutral_funding', df, {'entry_threshold': 0.0001, 'exit_threshold': 5e-05})
        assert out['signal'].iloc[-1] == 1

    def test_series_holds_in_hysteresis_band(self):
        df = self._series_df([7e-05] * 60)
        out = apply_strategy('delta_neutral_funding', df, {'entry_threshold': 0.0001, 'exit_threshold': 5e-05})
    ...
    def test_series_column_overrides_live_scalar(self):
        df = self._series_df([0.0005] * 60)
        out = apply_strategy('delta_neutral_funding', df, {'avg_funding_rate_7d': 0.0})
        assert out['signal'].iloc[-1] == -1

    def test_series_signals_are_valid(self):
        df = self._series_df([0.0005] * 30 + [1e-05] * 30)
...
class TestEdgeCases:
    ...
    @pytest.mark.parametrize('name', ['sma_crossover', 'ema_crossover', 'bollinger_bands', 'volume_weighted', 'triple_ema', 'triple_ema_bidir', 'rsi_macd_combo', 'momentum', 'mean_reversion', 'rsi', 'macd', 'breakout', 'stoch_rsi', 'supertrend', 'squeeze_momentum', 'atr_breakout', 'heikin_ashi_ema', 'parabolic_sar', 'ichimoku_cloud', 'order_blocks', 'delta_neutral_funding', 'vwap_reversion', 'chart_pattern', 'liquidity_sweeps', 'amd_ifvg'])
    def test_empty_dataframe(self, name):
        df = pd.DataFrame(columns=['open', 'high', 'low', 'close', 'volume'])
        result = apply_strategy(name, df)
        assert len(result) == 0

    @pytest.mark.parametrize('name', ['sma_crossover', 'ema_crossover', 'bollinger_bands', 'volume_weighted', 'triple_ema', 'triple_ema_bidir', 'rsi_macd_combo', 'momentum', 'mean_reversion', 'rsi', 'macd', 'breakout', 'stoch_rsi', 'atr_breakout', 'heikin_ashi_ema', 'supertrend', 'squeeze_momentum', 'ichimoku_cloud', 'order_blocks', 'delta_neutral_funding', 'chart_pattern', 'liquidity_sweeps', 'parabolic_sar', 'amd_ifvg'])
