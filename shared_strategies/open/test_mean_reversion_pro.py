...
def make_ohlcv(closes, noise=0.5, volume=100.0):
    ...
    n = len(closes)
    return pd.DataFrame({'open': closes - noise * 0.3, 'high': closes + noise, 'low': closes - noise, 'close': closes, 'volume': np.full(n, volume)})
...
def test_rsi_confirmation_required():
    out = mean_reversion_pro_core(make_ohlcv(make_choppy_with_extremes()), rsi_oversold=-1.0, rsi_overbought=101.0)
    assert (out['signal'] == 0).all()
...
def test_extra_triggers_still_blocked_by_strong_trend():
    closes = np.linspace(100, 300, 400)
    out = mean_reversion_pro_core(make_ohlcv(closes, noise=0.2), touch_entry=1, turn_entry=1)
    assert (out['signal'] == 0).all()

def test_extra_triggers_respect_adx_max_zero():
    out = mean_reversion_pro_core(make_ohlcv(make_choppy_with_extremes()), adx_max=0.0, touch_entry=1, turn_entry=1)
    assert (out['signal'] == 0).all()

def test_extra_triggers_require_rsi_evidence():
    out = mean_reversion_pro_core(make_ohlcv(make_choppy_with_extremes()), rsi_oversold=-1.0, rsi_overbought=101.0, touch_entry=1, turn_entry=1)
    assert (out['signal'] == 0).all()

def test_extra_triggers_fire_both_sides():
    out = mean_reversion_pro_core(make_ohlcv(make_choppy_with_extremes()), entry_std=1.5, touch_entry=1, turn_entry=1)
