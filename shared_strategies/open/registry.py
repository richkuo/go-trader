...
for _p in (os.path.join(_THIS_DIR, 'spot'), _THIS_DIR):
...
M5_DEPRECATED_EDGE_STRATEGIES = frozenset({'adx_trend', 'amd_ifvg', 'atr_breakout', 'bollinger_bands', 'consolidation_range', 'ema_crossover', 'funding_skew', 'heikin_ashi_ema', 'ichimoku_cloud', 'macd', 'mean_reversion', 'momentum', 'mtf_confluence', 'order_blocks', 'pairs_spread', 'parabolic_sar', 'range_scalper', 'regime_adaptive', 'rsi', 'rsi_macd_combo', 'sma_crossover', 'squeeze_momentum', 'stoch_rsi', 'supertrend', 'sweep_squeeze_combo', 'tema_cross', 'tema_cross_bd', 'triple_ema', 'triple_ema_bidir', 'vol_momentum', 'volume_weighted', 'vwap_reversion'})
DISCOVERY_HIDDEN_STRATEGIES = frozenset({'amd_ifvg', 'analog_retrieval', 'donchian_breakout', 'range_scalper', 'session_breakout', 'vol_momentum'}) | M5_DEPRECATED_EDGE_STRATEGIES
_CONSTRAINT_RE = re.compile('^\\s*([A-Za-z_][A-Za-z0-9_]*)\\s*(<=|>=|<|>)\\s*([A-Za-z_][A-Za-z0-9_]*|[-+]?\\d+(?:\\.\\d+)?)\\s*$')
_CONSTRAINT_OPS = {'<': lambda a, b: a < b, '<=': lambda a, b: a <= b, '>': lambda a, b: a > b, '>=': lambda a, b: a >= b}

def _parse_constraint(name: str, expr: str) -> Tuple[str, str, Any]:
    m = _CONSTRAINT_RE.match(expr)
    if not m:
        raise ValueError(f"{name}: unparseable parameter constraint {expr!r}; expected '<param> <op> <param-or-number>' with op in {sorted(_CONSTRAINT_OPS)}")
    lhs, op, rhs = m.groups()
    try:
        rhs = float(rhs)
    except ValueError:
...
def _enforce_parsed_constraints(name: str, effective: dict, parsed: list) -> None:
    for expr, (lhs, op, rhs) in parsed:
        ...
        try:
            ...
            raise ValueError(f'{name}: invalid parameters: constraint {expr!r} needs numeric values (got {lhs}={a!r}' + ('' if isinstance(rhs, float) else f', {rhs}={b!r}') + ')')
        if not ok:
            raise ValueError(f'{name}: invalid parameters: constraint {expr!r} violated ({lhs}={a!r}' + ('' if isinstance(rhs, float) else f', {rhs}={b!r}') + ')')
...
def validate_params(name: str, params: dict, default_params: Optional[dict]=None) -> None:
    entry = STRATEGIES.get(name)
    if entry is None:
        raise ValueError(f'validate_params: unknown strategy {name!r}; registered: {sorted(STRATEGIES)}')
...
def validate_param_value(name: str, param: str, value) -> None:
    entry = STRATEGIES.get(name)
    if entry is None:
        raise ValueError(f'validate_param_value: unknown strategy {name!r}; registered: {sorted(STRATEGIES)}')
    single = []
...
def register(name: str, description: str, default_params: dict, platforms: Tuple[str, ...]=('spot', 'futures'), variants: Optional[Dict[str, Dict[str, Any]]]=None, backtest_only: bool=False, constraints: Optional[List[str]]=None):
    ...
    if bad:
        raise ValueError(f'{name}: unknown platforms {sorted(bad)}; expected subset of {VALID_PLATFORMS}')
    variants = variants or {}
    bad_v = set(variants) - set(platforms)
    if bad_v:
        raise ValueError(f'{name}: variants keys {sorted(bad_v)} not in platforms {platforms}')
    ...
    for _expr in constraint_list:
        ...
        for _pname in (_lhs, _rhs) if not isinstance(_rhs, float) else (_lhs,):
            if _pname not in known_params:
                raise ValueError(f'{name}: constraint {_expr!r} references unknown parameter {_pname!r} (not in default_params)')

    def decorator(fn):
        wrapped = _validated(name, fn, default_params, constraint_list) if constraint_list else fn
        STRATEGIES[name] = {'fn': wrapped, 'description': description, 'default_params': dict(default_params), 'constraints': constraint_list, 'platforms': platforms, 'variants': variants, 'backtest_only': bool(backtest_only), 'edge_status': 'deprecated_m5' if name in M5_DEPRECATED_EDGE_STRATEGIES else None}
        return fn
    return decorator
...
def build_registry(platform: str, *, include_hidden: bool=False) -> Dict[str, Dict[str, Any]]:
    ...
    if missing_from_order:
        raise RuntimeError(f'PLATFORM_ORDER[{platform!r}] is missing {sorted(missing_from_order)}')
    extra_in_order = set(order) - expected
    if extra_in_order:
        raise RuntimeError(f'PLATFORM_ORDER[{platform!r}] references strategies not tagged for {platform!r}: {sorted(extra_in_order)}')
    ...
    for name in order:
        ...
        out[name] = {'fn': entry['fn'], 'description': variant.get('description', entry['description']), 'default_params': {**entry['default_params'], **variant.get('default_params', {})}, 'constraints': entry['constraints'], 'backtest_only': entry.get('backtest_only', False), 'edge_status': entry.get('edge_status')}
    return out

@register('sma_crossover', 'SMA Crossover — buy when fast SMA crosses above slow SMA', {'fast_period': 20, 'slow_period': 50}, constraints=['fast_period > 0', 'fast_period < slow_period'])
...
@register('ema_crossover', 'EMA Crossover — faster response than SMA crossover', {'fast_period': 12, 'slow_period': 26}, constraints=['fast_period > 0', 'fast_period < slow_period'])
...
@register('rsi', 'RSI — buy at oversold, sell at overbought', {'period': 14, 'overbought': 70, 'oversold': 30}, variants={'futures': {'description': 'RSI — overbought/oversold signals for futures'}}, constraints=['period > 0', 'oversold < overbought'])
...
@register('bollinger_bands', 'Bollinger Bands — mean reversion at band touches', {'period': 20, 'num_std': 2.0}, constraints=['period > 0'])
...
@register('macd', 'MACD — buy/sell on MACD line crossing signal line', {'fast_period': 12, 'slow_period': 26, 'signal_period': 9}, variants={'futures': {'description': 'MACD — momentum crossover for futures'}}, constraints=['fast_period > 0', 'signal_period > 0', 'fast_period < slow_period'])
...
@register('mean_reversion', 'Mean Reversion — buy when price is N std below mean, sell when above', {'lookback': 30, 'entry_std': 1.5, 'exit_std': 0.5}, variants={'futures': {'description': 'Mean Reversion — range-bound index futures trading'}}, constraints=['lookback > 0', 'exit_std < entry_std'])
...
@register('momentum', 'Momentum — buy on strong upward momentum, sell on reversal', {'roc_period': 14, 'threshold': 5.0}, variants={'futures': {'description': 'Momentum — trend following on futures using rate of change', 'default_params': {'threshold': 3.0}}}, constraints=['roc_period > 0'])
...
@register('volume_weighted', 'Volume-Weighted — confirms trend with volume analysis', {'sma_period': 20, 'vol_multiplier': 1.5}, constraints=['sma_period > 0'])
...
@register('triple_ema', 'Triple EMA — trend confirmation using 3 EMAs (short/mid/long)', {'short_period': 8, 'mid_period': 21, 'long_period': 55}, constraints=['short_period > 0', 'short_period < mid_period', 'mid_period < long_period'])
...
@register('triple_ema_bidir', 'Triple EMA Bidirectional — long on bullish stack, short on bearish stack', {'short_period': 8, 'mid_period': 21, 'long_period': 55}, platforms=('futures',), constraints=['short_period > 0', 'short_period < mid_period', 'mid_period < long_period'])
...
@register('tema_cross', 'Triple EMA Crossover — EMA fast/mid cross with long EMA trend filter', {'short_period': 5, 'mid_period': 13, 'long_period': 34}, constraints=['short_period > 0', 'short_period < mid_period', 'mid_period < long_period'])
def tema_cross_strategy(df: pd.DataFrame, short_period: int=5, mid_period: int=13, long_period: int=34) -> pd.DataFrame:
    ...
    bullish_cross = (result['ema_short'] > result['ema_mid']) & (result['ema_short'].shift(1) <= result['ema_mid'].shift(1))
    bearish_cross = (result['ema_short'] < result['ema_mid']) & (result['ema_short'].shift(1) >= result['ema_mid'].shift(1))
...
@register('tema_cross_bd', 'Triple EMA Crossover Bidirectional — long/short on cross with trend filter', {'short_period': 5, 'mid_period': 13, 'long_period': 34}, platforms=('futures',), constraints=['short_period > 0', 'short_period < mid_period', 'mid_period < long_period'])
...
@register('rsi_macd_combo', 'RSI+MACD Combo — dual confirmation for higher quality signals', {'rsi_period': 14, 'rsi_oversold': 35, 'rsi_overbought': 65, 'macd_fast': 12, 'macd_slow': 26, 'macd_signal': 9, 'rsi_short_min': 50, 'rsi_long_max': 50}, constraints=['rsi_period > 0', 'macd_fast > 0', 'macd_signal > 0', 'macd_fast < macd_slow', 'rsi_oversold < rsi_overbought'])
def rsi_macd_combo_strategy(df: pd.DataFrame, rsi_period: int=14, rsi_oversold: float=35, rsi_overbought: float=65, macd_fast: int=12, macd_slow: int=26, macd_signal: int=9, rsi_short_min: float=50, rsi_long_max: float=50) -> pd.DataFrame:
    result = df.copy()
...
@register('stoch_rsi', 'Stochastic RSI — earlier momentum signals via stochastic oscillator on RSI', {'rsi_period': 14, 'stoch_period': 14, 'k_smooth': 3, 'd_smooth': 3, 'overbought': 80, 'oversold': 20}, constraints=['rsi_period > 0', 'stoch_period > 0', 'k_smooth > 0', 'd_smooth > 0', 'oversold < overbought'])
def stoch_rsi_strategy(df: pd.DataFrame, rsi_period: int=14, stoch_period: int=14, k_smooth: int=3, d_smooth: int=3, overbought: float=80, oversold: float=20) -> pd.DataFrame:
    result = df.copy()
    result['rsi'] = wilder_rsi(result['close'], rsi_period)
...
@register('supertrend', 'Supertrend — ATR-based trend following with dynamic support/resistance', {'atr_period': 10, 'multiplier': 3.0}, constraints=['atr_period > 0'])
...
@register('ichimoku_cloud', 'Ichimoku Cloud — trend confirmation via Tenkan/Kijun cross, cloud position, and Chikou span', {'tenkan_period': 9, 'kijun_period': 26, 'senkou_b_period': 52}, constraints=['tenkan_period > 0', 'kijun_period > 0', 'senkou_b_period > 0'])
...
@register('pairs_spread', "Pairs/Spread Trading — trade z-score of price ratio between two assets (needs 'close_b' column)", {'lookback': 30, 'entry_z': 2.0, 'exit_z': 0.5}, platforms=('spot',), constraints=['lookback > 0', 'exit_z < entry_z'])
...
@register('squeeze_momentum', 'Squeeze Momentum — BB inside KC detects coiling, trades breakout with momentum confirmation', {'bb_period': 20, 'bb_std': 2.0, 'kc_period': 20, 'kc_mult': 1.5, 'mom_lookback': 12}, constraints=['bb_period > 0', 'kc_period > 0', 'mom_lookback > 0'])
def squeeze_momentum_strategy(df: pd.DataFrame, bb_period: int=20, bb_std: float=2.0, kc_period: int=20, kc_mult: float=1.5, mom_lookback: int=12) -> pd.DataFrame:
    result = df.copy()
...
@register('breakout', 'Breakout — trade breakouts from overnight/session range', {'lookback': 20, 'atr_period': 14, 'atr_multiplier': 1.5}, platforms=('futures',), constraints=['lookback > 0', 'atr_period > 0'])
...
@register('atr_breakout', 'ATR Breakout — enter on volatility breakout beyond ATR band', {'atr_period': 14, 'multiplier': 1.5}, constraints=['atr_period > 0'])
...
@register('amd_ifvg', 'AMD+IFVG — ICT Accumulation-Manipulation-Distribution with Implied Fair Value Gap (15m, session-aware)', {'asian_start_hour': 20, 'asian_end_hour': 0, 'london_start_hour': 2, 'london_end_hour': 5, 'min_ifvg_pct': 0.05, 'sweep_threshold_pct': 0.01, 'session_tz': 'America/New_York'})
def amd_ifvg_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return amd_ifvg_core(df, **params)

@register('heikin_ashi_ema', 'Heikin Ashi + EMA — smoothed candles with EMA trend filter; 2 consecutive HA candles + price side of EMA', {'ema_period': 21, 'confirmation': 2}, constraints=['ema_period > 0'])
...
@register('order_blocks', 'Order Blocks (ICT/SMC) — institutional supply/demand zones from displacement candles', {'atr_period': 14, 'displacement_mult': 1.5, 'ob_lookback': 20, 'max_ob_age': 50}, constraints=['atr_period > 0', 'ob_lookback > 0'])
def order_blocks_strategy(df: pd.DataFrame, atr_period: int=14, displacement_mult: float=1.5, ob_lookback: int=20, max_ob_age: int=50) -> pd.DataFrame:
    result = df.copy()
...
@register('vwap_reversion', 'VWAP Reversion — buy when price drops below VWAP by N std devs, sell when above', {'entry_std': 1.5, 'exit_std': 0.2}, constraints=['exit_std < entry_std'])
def vwap_reversion_strategy(df: pd.DataFrame, entry_std: float=1.5, exit_std: float=0.2) -> pd.DataFrame:
    ...
    result['vwap_std'] = result.groupby('_day')['close'].transform(lambda x: (x - result.loc[x.index, 'vwap']).expanding().std())
...
@register('chart_pattern', 'Chart Pattern — detects Double Top/Bottom, H&S, Flags, Triangles with volume confirmation', {'pivot_lookback': 5, 'tolerance': 0.03, 'vol_multiplier': 1.5, 'vol_period': 20, 'htf_gate_factor': 0, 'htf_gate_mode': 'veto', 'htf_gate_ema_fast': 20, 'htf_gate_ema_slow': 40}, constraints=['pivot_lookback > 0', 'vol_period > 0'])
def chart_pattern_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return chart_pattern_core(df, **params)

@register('liquidity_sweeps', 'Liquidity Sweeps (ICT) — fades stop-hunt wicks beyond swing highs/lows after price closes back inside range', {'swing_lookback': 20, 'confirmation': 1}, constraints=['swing_lookback > 0'])
def liquidity_sweeps_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return liquidity_sweep_core(df, **params)

@register('parabolic_sar', 'Parabolic SAR — trend-following stop and reverse with accelerating trailing stop', {'iaf': 0.02, 'af_step': 0.02, 'max_af': 0.2})
...
@register('range_scalper', 'Range Scalper — detects low-volatility consolidation via Bollinger bandwidth + volume, then mean-reverts at band touches', {'bb_period': 14, 'bb_std': 1.5, 'bw_threshold': 0.008, 'vol_ratio': 0.8, 'rsi_period': 7, 'rsi_ob': 70, 'rsi_os': 30}, constraints=['bb_period > 0', 'rsi_period > 0', 'rsi_os < rsi_ob'])
def range_scalper_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return range_scalper_core(df, **params)

@register('sweep_squeeze_combo', 'Sweep Squeeze Combo — 2-of-3 consensus (liquidity sweeps + squeeze momentum + stochastic RSI) for high-conviction reversals', {'swing_lookback': 10, 'min_agree': 2}, constraints=['swing_lookback > 0'])
def sweep_squeeze_combo_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return sweep_squeeze_combo_core(df, **params)

@register('adx_trend', 'ADX Trend Rider — enters on DI crossovers when ADX confirms strong trend (>25)', {'adx_period': 14, 'adx_threshold': 25}, constraints=['adx_period > 0'])
...
@register('delta_neutral_funding', 'Delta-Neutral Funding — enter when 7d avg funding rate exceeds threshold, exit when below', {'entry_threshold': 0.0001, 'exit_threshold': 5e-05, 'drift_threshold': 2.0, 'current_funding_rate': 0.0, 'avg_funding_rate_7d': 0.0}, platforms=('futures',))
def delta_neutral_funding_strategy(df: pd.DataFrame, entry_threshold: float=0.0001, exit_threshold: float=5e-05, drift_threshold: float=2.0, current_funding_rate: float=0.0, avg_funding_rate_7d: float=0.0) -> pd.DataFrame:
    result = df.copy()
    result['delta_drift_pct'] = 0.0
    result['rebalance_needed'] = 0.0
    has_series = 'funding_rate' in df.columns and pd.to_numeric(df['funding_rate'], errors='coerce').notna().any()
    if has_series:
...
@register('funding_skew', 'Funding Skew — funding-rate crowding extremes (rolling z-score) with EMA price confirmation: long crowded-short squeezes, short crowded-long breakdowns; flat when funding is unavailable', {'funding_window': 168, 'z_entry': 2.0, 'z_exit': 0.5, 'confirm_ema': 40, 'min_abs_rate': 1e-05, 'allow_short': True}, platforms=('futures',), constraints=['funding_window > 0', 'z_exit < z_entry'])
def funding_skew_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return funding_skew_core(df, **params)

@register('donchian_breakout', 'Donchian Channel Breakout — turtle-trading style entry on new high/low channel breakouts', {'entry_period': 20, 'exit_period': 10}, constraints=['entry_period > 0', 'exit_period > 0'])
def donchian_breakout_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return donchian_breakout_core(df, **params)

@register('bear_pullback_st', 'Bear Pullback Short — short bear-market rallies into EMA20/50 resistance with RSI rebound + ADX trend filter', {'ema_short': 20, 'ema_mid': 50, 'ema_long': 200, 'adx_period': 14, 'adx_threshold': 20.0, 'rsi_period': 14, 'rsi_lower': 55.0, 'rsi_upper': 65.0, 'pullback_window': 5, 'pullback_touch_buffer_pct': 0.001}, platforms=('futures',), constraints=['ema_short > 0', 'adx_period > 0', 'rsi_period > 0', 'pullback_window > 0', 'ema_short < ema_mid', 'ema_mid < ema_long', 'rsi_lower < rsi_upper'])
def bear_pullback_st_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return bear_pullback_st_core(df, **params)

@register('session_breakout', 'Session Breakout — break of prior session (Asian/US open/US close) high/low with volume confirmation', {'session': 'asian', 'lookback': 1, 'volume_threshold': 1.5, 'vol_period': 20, 'atr_period': 14, 'atr_multiplier': 0.0}, platforms=('futures',), constraints=['lookback > 0', 'vol_period > 0', 'atr_period > 0'])
def session_breakout_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return session_breakout_core(df, **params)

@register('vwap_rejection_st', 'VWAP Rejection Short — short weak rallies into session VWAP / EMA20 / EMA50 with RSI sub-50 + bearish EMA50<EMA200 regime', {'ema_short': 20, 'ema_mid': 50, 'ema_long': 200, 'rsi_period': 14, 'rsi_max_reclaim': 50.0, 'rally_window': 5, 'rally_touch_buffer_pct': 0.001}, platforms=('futures',), constraints=['ema_short > 0', 'rsi_period > 0', 'rally_window > 0', 'ema_short < ema_mid', 'ema_mid < ema_long'])
def vwap_rejection_st_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return vwap_rejection_st_core(df, **params)

@register('anchored_vwap', 'Anchored VWAP — single VWAP anchored to the last confirmed swing pivot as dynamic S/R; long on a buffered reclaim above, short on a buffered breakdown below', {'pivot_strength': 5, 'buffer_atr_mult': 0.25, 'confirm_bars': 2, 'atr_period': 14, 'gate_rsi_period': 0, 'gate_rsi_level': 50.0, 'gate_ema_period': 0}, constraints=['pivot_strength > 0', 'atr_period > 0'])
def anchored_vwap_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return anchored_vwap_core(df, **params)

@register('anchored_vwap_channel', 'Anchored VWAP Channel — dual VWAPs anchored to the last confirmed swing low (support) and swing high (resistance); long a buffered bounce off the lower line, short a buffered rejection off the upper', {'pivot_strength': 5, 'buffer_atr_mult': 0.25, 'confirm_bars': 2, 'min_width_atr_mult': 1.5, 'atr_period': 14}, constraints=['pivot_strength > 0', 'atr_period > 0'])
def anchored_vwap_channel_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return anchored_vwap_channel_core(df, **params)

@register('anchored_vwap_reversion', 'Anchored VWAP Reversion — fades an ATR-measured stretch beyond the pivot-anchored VWAP; long a buffered snap-back from below the band, short the mirror above', {'pivot_strength': 5, 'entry_atr_mult': 1.5, 'buffer_atr_mult': 0.25, 'confirm_bars': 2, 'atr_period': 14}, constraints=['pivot_strength > 0', 'atr_period > 0'])
def anchored_vwap_reversion_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return anchored_vwap_reversion_core(df, **params)

@register('analog_retrieval', "RESEARCH, backtest-only (#1138) — k-NN analog retrieval: matches the current bar's scale-free state vector (return efficiency, ATR-normalized momentum, ATR%, vol regime, trend) against strictly-prior windows with realized forward returns and votes them into a t-stat- and ATR-edge-gated direction signal. Refused by every live check script; promotion to live requires explicit human sign-off after parity/Sharpe/M1 checks", {'feat_window': 20, 'atr_period': 14, 'vol_baseline': 100, 'horizon': 12, 'k_neighbors': 25, 'min_index': 200, 'max_index': 5000, 'min_t_stat': 2.0, 'min_edge_atr': 0.25}, backtest_only=True, constraints=['feat_window > 0', 'atr_period > 0', 'horizon > 0', 'k_neighbors > 0'])
def analog_retrieval_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return analog_retrieval_core(df, **params)

@register('momentum_pro', 'Momentum Pro — trend-pullback entries in a stacked-EMA trend, ADX-confirmed, on a volume-backed resumption', {'ema_fast': 20, 'ema_mid': 50, 'ema_long': 200, 'adx_period': 14, 'adx_threshold': 20.0, 'pullback_window': 6, 'pullback_touch_buffer_pct': 0.0, 'vol_period': 20, 'vol_mult': 1.2}, constraints=['ema_fast > 0', 'adx_period > 0', 'vol_period > 0', 'pullback_window > 0', 'ema_fast < ema_mid', 'ema_mid < ema_long'])
def momentum_pro_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return momentum_pro_core(df, **params)

@register('mean_reversion_pro', 'Mean Reversion Pro — z-score reversion gated by a no-trend ADX ceiling + RSI extreme confirmation', {'lookback': 30, 'entry_std': 2.0, 'adx_period': 14, 'adx_max': 25.0, 'rsi_period': 14, 'rsi_oversold': 30.0, 'rsi_overbought': 70.0, 'confirm_window': 3, 'touch_entry': 0, 'turn_entry': 0}, constraints=['lookback > 0', 'adx_period > 0', 'rsi_period > 0', 'rsi_oversold < rsi_overbought'])
def mean_reversion_pro_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return mean_reversion_pro_core(df, **params)

@register('rsi_bb_combo', 'RSI+BB Combo — Bollinger Band mean reversion confirmed by RSI extremes. No inline trend filter BY DESIGN — run behind the composite ranging regime gate (init wires it by default) or it fades trends. NOTE: default params LOSE on BTC 1h (-47% gated w/ tiered TP + 2xATR SL); BTC 4h gated showed edge (+27%, PF 1.42, MaxDD -16%) but the strategy FAILED M1 incumbent-relative validation in both plain and gated shapes — mean_reversion_pro dominates head-to-head (docs/research/1329-rsi-bb-combo-m1.md); NOT a promotion candidate, tune per-market before any use', {'bb_period': 20, 'bb_std': 2.0, 'rsi_period': 14, 'rsi_oversold': 30.0, 'rsi_overbought': 70.0, 'confirm_window': 3}, constraints=['bb_period > 0', 'rsi_period > 0', 'confirm_window > 0', 'rsi_oversold < rsi_overbought'])
def rsi_bb_combo_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return rsi_bb_combo_core(df, **params)

@register('consolidation_range', 'Consolidation Range — enters at the edges of a consolidation box (long near the bottom, short near the top); exit via a trailing ATR stop. NOTE: default params LOSE in run_backtest.py (~-40% to -47% on BTC 4h, see docs/research/consolidation-findings.md) — ship as a tunable baseline, adjust box width / stop / trail per market before live use', {'box_width_pct': 0.05, 'min_bars': 16, 'edge_entry_frac': 0.2}, platforms=('futures',), constraints=['min_bars > 0'])
def consolidation_range_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return consolidation_range_core(df, **params)

@register('atr_band_revert', 'ATR Band Reversion — ranging-market mean reversion: fade ATR-scaled bands around an SMA (long below mid-k*ATR; short above mid+k*ATR on futures). Entries only — pair with allowed_regimes=ranging and tiered_tp_atr / stop_loss_atr_mult for the take-profit-at-mid and range-break exit (see atr_band_revert.py)', {'period': 20, 'atr_period': 14, 'k_entry': 1.5, 'allow_short': False}, variants={'futures': {'description': 'ATR Band Reversion — bidirectional ranging mean reversion: fade ATR-scaled bands around an SMA (long below mid-k*ATR, short above mid+k*ATR). Entries only — pair with allowed_regimes=ranging and tiered_tp_atr / stop_loss_atr_mult for exit', 'default_params': {'allow_short': True}}}, constraints=['period > 0', 'atr_period > 0'])
def atr_band_revert_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return atr_band_revert_core(df, **params)

@register('mtf_confluence', 'MTF Confluence — higher-timeframe EMA trend gate (resampled in-frame, no extra data) over native-frame pullback resumption entries; exits when the HTF trend flips', {'htf_factor': 4, 'htf_ema_fast': 20, 'htf_ema_slow': 40, 'htf_sep_pct': 0.001, 'ltf_ema': 20, 'pullback_window': 6, 'pullback_touch_buffer_pct': 0.0, 'allow_short': False}, variants={'futures': {'description': 'MTF Confluence — bidirectional: HTF EMA trend gate over native-frame pullback resumption entries; shorts mirror the logic in HTF downtrends', 'default_params': {'allow_short': True}}}, constraints=['htf_factor > 0', 'htf_ema_fast > 0', 'ltf_ema > 0', 'pullback_window > 0', 'htf_ema_fast < htf_ema_slow'])
def mtf_confluence_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return mtf_confluence_core(df, **params)

@register('vol_momentum', 'Vol Momentum — volatility-targeted time-series momentum: ATR-normalized N-bar net move with Kaufman efficiency-ratio trend confirmation; hysteresis exit on momentum decay or efficiency collapse', {'mom_window': 24, 'atr_period': 14, 'entry_threshold': 0.3, 'exit_threshold': 0.05, 'eff_entry': 0.35, 'eff_exit': 0.15, 'allow_short': False}, variants={'futures': {'description': 'Vol Momentum — volatility-targeted time-series momentum, bidirectional: long/short on ATR-normalized momentum with Kaufman efficiency confirmation', 'default_params': {'allow_short': True}}}, constraints=['mom_window > 0', 'atr_period > 0'])
def vol_momentum_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return vol_momentum_core(df, **params)

@register('regime_adaptive', 'Regime Adaptive — per-bar composite regime metrics (return/range/Kaufman efficiency + ADX) switch between breakout entries in clean trends and mean-reversion fades in ranges; flat in chop', {'period': 20, 'adx_threshold': 20.0, 'return_eff_threshold': 0.05, 'range_eff_threshold': 0.03, 'efficiency_threshold': 0.5, 'breakout_lookback': 10, 'mr_lookback': 20, 'mr_entry_z': 2.0, 'mr_exit_z': 0.0, 'slow_trend_lookback': 100, 'slow_veto_threshold': 0.05, 'allow_short': False}, variants={'futures': {'default_params': {'allow_short': True}}}, constraints=['period > 0', 'breakout_lookback > 0', 'mr_lookback > 0'])
def regime_adaptive_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return regime_adaptive_core(df, **params)

@register('regime_adaptive_htf', 'Regime Adaptive HTF — selective z-score fades gated by composite regime labels (return/range/Kaufman efficiency + ADX) classified on higher-timeframe buckets with confirmation hysteresis; optional clean-trend entry modes; flat in chop and directional grinds', {'htf_factor': 6, 'period': 14, 'adx_threshold': 20.0, 'return_eff_threshold': 0.05, 'range_eff_threshold': 0.03, 'efficiency_threshold': 0.5, 'confirm_buckets': 2, 'trend_entry': 'off', 'trend_drift_confirm': 0.1, 'transition_window': 6, 'pullback_z': 1.0, 'fade_labels': 'ranging', 'breakout_lookback': 10, 'mr_lookback': 20, 'mr_entry_z': 2.0, 'mr_exit_z': 0.0, 'slow_trend_lookback': 100, 'slow_veto_threshold': 0.05, 'allow_short': False}, constraints=['htf_factor > 0', 'period > 0', 'breakout_lookback > 0', 'mr_lookback > 0'])
def regime_adaptive_htf_strategy(df: pd.DataFrame, **params) -> pd.DataFrame:
    return regime_adaptive_htf_core(df, **params)

@register('hold', 'Hold — always returns signal=0; used internally by type=manual strategies for the close-evaluator loop (#569)', {}, platforms=('spot', 'futures'))
def hold_strategy(df: pd.DataFrame) -> pd.DataFrame:
    result = df.copy()
    result['signal'] = 0
    return result
PLATFORM_ORDER: Dict[str, List[str]] = {'spot': ['sma_crossover', 'ema_crossover', 'rsi', 'bollinger_bands', 'macd', 'mean_reversion', 'momentum', 'volume_weighted', 'triple_ema', 'rsi_macd_combo', 'stoch_rsi', 'supertrend', 'ichimoku_cloud', 'pairs_spread', 'squeeze_momentum', 'atr_breakout', 'amd_ifvg', 'heikin_ashi_ema', 'order_blocks', 'vwap_reversion', 'anchored_vwap', 'anchored_vwap_channel', 'anchored_vwap_reversion', 'chart_pattern', 'liquidity_sweeps', 'parabolic_sar', 'range_scalper', 'sweep_squeeze_combo', 'adx_trend', 'donchian_breakout', 'tema_cross', 'momentum_pro', 'mean_reversion_pro', 'rsi_bb_combo', 'atr_band_revert', 'mtf_confluence', 'vol_momentum', 'regime_adaptive', 'regime_adaptive_htf', 'analog_retrieval', 'hold'], 'futures': ['sma_crossover', 'ema_crossover', 'bollinger_bands', 'volume_weighted', 'triple_ema', 'triple_ema_bidir', 'tema_cross', 'tema_cross_bd', 'rsi_macd_combo', 'momentum', 'mean_reversion', 'rsi', 'macd', 'breakout', 'stoch_rsi', 'supertrend', 'squeeze_momentum', 'ichimoku_cloud', 'atr_breakout', 'amd_ifvg', 'heikin_ashi_ema', 'order_blocks', 'vwap_reversion', 'anchored_vwap', 'anchored_vwap_channel', 'anchored_vwap_reversion', 'chart_pattern', 'liquidity_sweeps', 'parabolic_sar', 'range_scalper', 'sweep_squeeze_combo', 'adx_trend', 'delta_neutral_funding', 'funding_skew', 'donchian_breakout', 'session_breakout', 'bear_pullback_st', 'vwap_rejection_st', 'momentum_pro', 'mean_reversion_pro', 'rsi_bb_combo', 'consolidation_range', 'atr_band_revert', 'mtf_confluence', 'vol_momentum', 'regime_adaptive', 'regime_adaptive_htf', 'analog_retrieval', 'hold']}
