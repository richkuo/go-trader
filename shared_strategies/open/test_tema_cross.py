...
def make_ohlcv(closes, volume=None, noise=0.5):
    ...
    return pd.DataFrame({'open': closes - noise * 0.3, 'high': closes + noise, 'low': closes - noise, 'close': closes, 'volume': np.array(volume, dtype=float)})

def _load_registry():
    spec = importlib.util.spec_from_file_location('_registry_tema_cross', os.path.join(_HERE, 'registry.py'))
...
def test_tema_cross_position_persists_between_crosses(registry):
    ...
    assert in_position.sum() > 5, f'Position only held for {in_position.sum()} bars after entry — should persist until bearish cross'
