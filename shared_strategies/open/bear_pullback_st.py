...
def bear_pullback_st_core(df: pd.DataFrame, ema_short: int=20, ema_mid: int=50, ema_long: int=200, adx_period: int=14, adx_threshold: float=20.0, rsi_period: int=14, rsi_lower: float=55.0, rsi_upper: float=65.0, pullback_window: int=5, pullback_touch_buffer_pct: float=0.001) -> pd.DataFrame:
    result = df.copy()
    result['signal'] = 0
    n = len(result)
    ...
    pullback_touch = (high > result['ema_short'] * touch_mult) | (high > result['ema_mid'] * touch_mult)
    pullback_recent = pullback_touch.shift(1).rolling(window=pullback_window).max().fillna(0).astype(bool)
    rsi_in_zone = (result['rsi'] >= rsi_lower) & (result['rsi'] <= rsi_upper)
    rsi_recent = rsi_in_zone.shift(1).rolling(window=pullback_window).max().fillna(0).astype(bool)
    ...
    short_mask = bearish_regime & strong_trend & pullback_recent & rsi_recent & trigger & confirm
