...
def mean_reversion_pro_core(df: pd.DataFrame, lookback: int=30, entry_std: float=2.0, adx_period: int=14, adx_max: float=25.0, rsi_period: int=14, rsi_oversold: float=30.0, rsi_overbought: float=70.0, confirm_window: int=3, touch_entry: int=0, turn_entry: int=0) -> pd.DataFrame:
    result = df.copy()
    result['signal'] = 0
    n = len(result)
