...
def regime_adaptive_core(df: pd.DataFrame, period: int=20, adx_threshold: float=20.0, return_eff_threshold: float=0.05, range_eff_threshold: float=0.03, efficiency_threshold: float=0.5, breakout_lookback: int=10, mr_lookback: int=20, mr_entry_z: float=2.0, mr_exit_z: float=0.0, slow_trend_lookback: int=100, slow_veto_threshold: float=0.05, allow_short: bool=False) -> pd.DataFrame:
    ...
    labels = np.select([warmup, big_move & up & clean, big_move & up, big_move & clean, big_move, high_adx, wide], [_WARMUP, _TREND_UP_CLEAN, _TREND_UP_CHOPPY, _TREND_DOWN_CLEAN, _TREND_DOWN_CHOPPY, _RANGING_DIRECTIONAL, _RANGING_VOLATILE], default=_RANGING_QUIET)
