...
def _align_funding_records(index: pd.Index, records: list) -> pd.Series:
    ...
    right = pd.DataFrame({'ts': pd.to_datetime([int(r['time']) for r in records], unit='ms', utc=True).astype('datetime64[ns, UTC]'), 'rate': [float(r['rate']) for r in records]}).sort_values('ts')
    merged = pd.merge_asof(left, right, on='ts', direction='backward')
    return pd.Series(merged['rate'].values, index=index)

def funding_skew_core(df: pd.DataFrame, funding_window: int=168, z_entry: float=2.0, z_exit: float=0.5, confirm_ema: int=40, min_abs_rate: float=1e-05, allow_short: bool=True, funding_records: list=None) -> pd.DataFrame:
    result = df.copy()
    n = len(result)
    result['signal'] = 0
    result['position'] = 0
    ...
    for i in range(1, n):
        ...
        pos[i] = cur
    result['position'] = pos
    result['signal'] = pd.Series(pos, index=result.index).diff().fillna(0).clip(-1, 1).astype(int)
    return result
