...
def _make_ohlcv(highs, lows, closes, opens=None):
    ...
    if opens is None:
        opens = closes
    return pd.DataFrame({'open': opens, 'high': highs, 'low': lows, 'close': closes, 'volume': [100.0] * n})
...
class TestLookahead:
    ...
    def test_sweep_never_fires_inside_confirmation_window(self):
        ...
        for sp in swing_positions:
            ...
            assert (inside == 0).all(), f'signal fired inside confirmation window of swing at {sp}: non-zero at {[sp + 1 + i for i in np.where(inside != 0)[0]]}'
    ...
    def test_signal_at_k_independent_of_bars_after_k(self):
        ...
        for k in signal_bars:
            ...
            assert partial['signal'].iloc[k] == full['signal'].iloc[k], f"signal at bar {k} differs after truncation: full={full['signal'].iloc[k]} truncated={partial['signal'].iloc[k]}"
