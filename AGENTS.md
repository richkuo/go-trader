...
- `.github/workflows/Codex.yml` — general-purpose `@Codex` handler for PR/issue comments; handles code review, fixes, etc. — no separate review workflow needed
...
- Fetch latest Codex[bot] review on a PR: `gh api repos/richkuo/go-trader/issues/<N>/comments --jq '[.[] | select(.user.login=="Codex[bot]")] | last | .body'` (top-level review summary lives on the **issues** endpoint, not pulls; inline review comments via `/pulls/<N>/comments`, review-object summaries via `/pulls/<N>/reviews`)
