...
func (ss *StatusServer) handleStatus(w http.ResponseWriter, r *http.Request) {
	...
	type StratStatus struct {
		...
		Hedge                          *HedgeStatus               `json:"hedge,omitempty"`                            // #1159: correlated hedge leg — configured geometry + held leg state; nil when no enabled hedge block
	}
	...
	for id, s := range ss.state.Strategies {
		...
		resp.Strategies[id] = StratStatus{
			...
			Paused:                         sc.Paused,
			Hedge:                          hedgeStatusForStrategy(sc, s),
		}
	}
