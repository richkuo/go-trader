...
func (ss *StatusServer) handleStatus(w http.ResponseWriter, r *http.Request) {
	...
	type StratStatus struct {
		...
		Hedge                          *HedgeStatus               `json:"hedge,omitempty"`                            // #1159: correlated hedge leg config + live state; nil when the strategy neither declares nor holds one. Position rows carry hedge_for so the UI can badge the leg instead of showing it as unmanaged exposure.
	}
