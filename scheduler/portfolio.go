...
type Position struct {
	...
	// primary symbol ("" = not a hedge). Set once at hedge open; startup
	// recovery reads THIS persisted stamp, never coin→configured-symbol
	// inference (#1159 phase-1 constraint 5).
	HedgeFor string `json:"hedge_for,omitempty"`
	// HedgePrimaryQtyBasis is the primary-position quantity this hedge leg was
	// last sized against — the watermark hedge sync diffs to detect primary
	// qty events. Advanced only on a confirmed hedge fill (#1159).
...
func recordClosedPosition(s *StrategyState, pos *Position, closePrice, realizedPnL float64, reason string, closedAt time.Time) {
	...
	// #1159: hedge legs are coupled risk offsets, not independent alpha —
	// skip them so per-strategy trade-quality aggregates stay meaningful.
	if pos.HedgeFor == "" {
