...
type KillSwitchCloseInputs struct {
	...
	// HLHeldHedgeCoins is the set of hedge coins the scheduler CURRENTLY holds
	// a virtual hedge leg on (#1159). Hedge coins are invisible to
	// hyperliquidConfiguredCoin, so without this the kill switch flattens every
	// primary and leaves the hedge legs running on-chain. Built from held legs
	// rather than config so a declared-but-flat hedge coin carrying a foreign
	// position is never liquidated.
...
func planKillSwitchClose(in KillSwitchCloseInputs) KillSwitchClosePlan {
	...
	case hlStateFetched && len(in.HLLiveAll) > 0:
		...
		plan.CloseReport = forceCloseHyperliquidLive(ctx, hlPositions, in.HLLiveAll, in.HLCloser, in.HLStopLossOIDs, in.HLHeldHedgeCoins)
		cancel()
