...
func runManualCloseEval(sc StrategyConfig, ss *StrategyState, cfg *Config, regimePayload RegimePayload, notifier *MultiNotifier, logger *StrategyLogger) (float64, float64, RegimePayload, bool) {
	pos := ss.Positions[sc.Symbol]
	if pos == nil {
	...
	result, _, price, ok := runHyperliquidCheck(&sc, nil, posCtx, regimePayload, cfg.Regime, notifier, logger)
	if !ok {
		return 0, 0, RegimePayload{}, false
	}
	return result.CloseFraction, price, cycleRegimePayload(regimePayload, result.Regime), true
}
