...
func (sdb *StateDB) migrateSchema() error {
	...
	migrations := []string{
		...
		// #1159 phase 1: per-strategy correlated hedge legs. is_hedge/hedge_for_symbol/
		// hedge_for_position_id are set on the hedge leg's own Position row;
		// hedge_symbol is set on the PRIMARY leg's row. hedge_for_position_id (not
		// coin inference) is the sole restart/reconcile pairing key.
		...
		// Hedge-leg trade rows are tagged so LifetimeTradeStats can exclude them
		// from #T/W-L (not independent alpha positions) while tradeLedgerDeltaSQL
		// still sums them into the owning strategy's ledger.
...
func (sdb *StateDB) LifetimeTradeStatsAll() (map[string]LifetimeTradeStats, error) {
	...
	// #1159: hedge legs are not independent alpha positions — exclude them
	// from #T/W-L the same way scale-in adds are excluded above.
