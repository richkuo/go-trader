...
CREATE TABLE IF NOT EXISTS positions (
    ...
    -- #1159: correlated hedge leg metadata. hedge_for names the PRIMARY coin
    -- this row hedges ('' = ordinary position) and is the sole ownership
    -- record — hedge legs are never re-derived from coin->config inference.
...
func (sdb *StateDB) LifetimeTradeStatsAll() (map[string]LifetimeTradeStats, error) {
	...
	// distinct round-trips opened. W/L below is unaffected by scale-ins (it
	// groups close legs, which a scale-in never is).
	// #1159: correlated hedge legs are excluded from BOTH counts — a hedge is
	// one half of a single risk decision, not a second round-trip, and its
	// close leg loses by construction whenever the primary wins. Their cash
	// effect still lands in the ledger (tradeLedgerDeltaSQL ignores
	// trade_type).
