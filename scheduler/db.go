...
CREATE TABLE IF NOT EXISTS positions (
    ...
    hedge_ratio_at_open REAL NOT NULL DEFAULT 0,
    hedge_margin_mode_at_open TEXT NOT NULL DEFAULT '',
...
func (sdb *StateDB) migrateSchema() error {
	...
	migrations := []string{
		...
		// #1159: correlated hedge leg ownership + sizing watermark. hedge_for
		// marks a persisted position as an auto-managed inverse hedge; the sync
		// recovers ownership from this column on boot (never from coin->symbol
		// inference). hedge_primary_qty_basis is the primary qty the hedge was
		// last sized against.
		...
		// #1159 review round 1: freeze the resolved hedge sizing config at open
		// so validateHedgeStateConsistency can detect ratio/margin drift from a
		// config-edit-plus-restart (the SIGHUP guard's blind spot).
		"ALTER TABLE positions ADD COLUMN hedge_ratio_at_open REAL NOT NULL DEFAULT 0",
		"ALTER TABLE positions ADD COLUMN hedge_margin_mode_at_open TEXT NOT NULL DEFAULT ''",
	}
