...
func (sdb *StateDB) migrateSchema() error {
	...
	migrations := []string{
		...
		// #1159 correlated hedge legs: hedge ownership metadata persisted on
		// the hedge Position row so restart/reconcile recover the pairing from
		// state, never from coin→configured-symbol inference (constraint 5).
