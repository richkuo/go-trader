...
func (sdb *StateDB) migrateSchema() error {
	...
	migrations := []string{
		...
		// #1159 (phase 1) correlated hedge legs: HedgeFor is the persisted
		// ownership stamp (primary symbol this leg hedges; "" = not a hedge —
		// ownership is never inferred from coin→symbol mapping), and
		// HedgePrimaryQtyBasis is the primary-qty watermark the hedge-sync
		// reconciler diffs to detect qty events.
...
func (sdb *StateDB) LifetimeTradeStatsAll() (map[string]LifetimeTradeStats, error) {
	...
	// #1159: hedge legs open on their own (hedge-coin) position id and are not
	// independent alpha — exclude them from #T the same way. Their close legs
	// are filtered out of the W/L aggregation below.
...
func (sdb *StateDB) LifetimeTradeStatsForStrategy(strategyID string) (LifetimeTradeStats, error) {
	...
	// #1159: hedge open/close legs are excluded the same way (coupled
	// risk-management legs, not independent alpha).
