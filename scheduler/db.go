...
func (sdb *StateDB) migrateSchema() error {
	...
	migrations := []string{
		...
		// #1159: correlated hedge legs. hedge_for names the PRIMARY symbol a
		// hedge position is coupled to ('' = ordinary position) and is the ONLY
		// ownership record — startup recovery reads it rather than inferring
		// hedge membership from the live config, so a config edit can never
		// retroactively reclassify an on-chain leg. hedge_primary_qty_basis is
		// the primary quantity the hedge was last sized against.
...
func (sdb *StateDB) LifetimeTradeStatsAll() (map[string]LifetimeTradeStats, error) {
	...
	closeRows, err := sdb.db.Query(`SELECT
		...
		FROM (
			...
			FROM trades
			-- #1159: a hedge leg's round-trip is not independent alpha (it
			-- loses by construction when the primary wins), so it must not
			-- inflate W/L. Its PnL/fees still reach the strategy's ledger —
			-- tradeLedgerDeltaSQL/tradeNetPnLSQL ignore trade_type by design.
	...
	if err := sdb.db.QueryRow(`SELECT
		...
		FROM (
			SELECT
				...
				SUM`+tradeNetPnLSQL+` AS net_pnl
			FROM trades
			-- #1159: hedge round-trips excluded (mirrors LifetimeTradeStatsAll).
