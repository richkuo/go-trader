...
func (sdb *StateDB) migrateSchema() error {
	...
	migrations := []string{
		...
		// #1159: hedge-leg ownership metadata. HedgeFor names the primary
		// symbol this position hedges ("" = not a hedge leg — every existing
		// row migrates as non-hedge); HedgePrimaryQtyBasis is the primary qty
		// watermark runHedgeSync diffs against. Persisted so a restart never
		// loses hedge ownership (never inferred from coin->symbol mapping).
...
func (sdb *StateDB) LifetimeTradeStatsAll() (map[string]LifetimeTradeStats, error) {
	...
	// distinct round-trips opened. #1159: a hedge leg's open is a coupled
	// risk-management leg, not an independent round-trip — exclude it too.
	// W/L below is unaffected (it groups close legs, which neither of these
	// ever is).
...
func (sdb *StateDB) LifetimeTradeStatsForStrategy(strategyID string) (LifetimeTradeStats, error) {
	...
	var opens sql.NullInt64
	// #873/#1159: exclude scale-in add legs and hedge-leg opens — both are
	// open-side legs that are not new round-trips (mirrors LifetimeTradeStatsAll).
