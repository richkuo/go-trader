...
CREATE TABLE IF NOT EXISTS positions (
    ...
    hedge_qty_ratio REAL NOT NULL DEFAULT 0,
...
func (sdb *StateDB) migrateSchema() error {
	...
	migrations := []string{
		...
		// #1159: correlated hedge leg persistence. is_hedge marks a
		// scheduler-managed hedge position; hedge_for names the primary
		// symbol it mirrors (set only on the hedge row). hedge_symbol is the
		// mirror-image field on the PRIMARY row: the hedge coin opened
		// alongside it, "" when no hedge is open. Restart/reconcile recovers
		// primary<->hedge ownership from these columns alone — never from
		...
		// #1159 review round 1: hedge_qty_ratio (PRIMARY row only) is the
		// fixed, price-independent basis syncHedgeCoherence compares live
		// quantities against — hedge qty per unit of primary qty, as of the
		// last confirmed open/add event. 0 = no basis yet (a position that
		// predates this column, or brand-new this cycle); syncHedgeCoherence
		// treats <=0 as "bootstrap from current quantities, take no action",
		// so a restart never mis-fires a reduction from the zero default.
		"ALTER TABLE positions ADD COLUMN hedge_qty_ratio REAL NOT NULL DEFAULT 0",
	}
