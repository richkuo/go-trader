...
func (sdb *StateDB) migrateSchema() error {
	...
	migrations := []string{
		...
		// #1159 phase 1: correlated hedge leg pairing. hedge_for names the
		// primary symbol this position is the auto-managed hedge for (""=not
		// a hedge leg); hedge_primary_qty_basis is the primary quantity
		// watermark the state-derived hedge sync diffs against. Persisted
		// metadata is the sole restart/reconcile ownership source — never
		// inferred from coin→configured-symbol mapping.
...
func (sdb *StateDB) LifetimeTradeStatsAll() (map[string]LifetimeTradeStats, error) {
	...
	// legs, which a scale-in never is). #1159: hedge legs are the same
	// exclusion shape — a hedge open is not a new alpha round-trip.
...
func (sdb *StateDB) LifetimeTradeStatsForStrategy(strategyID string) (LifetimeTradeStats, error) {
	...
	// position, not new round-trips (mirrors LifetimeTradeStatsAll). #1159:
	// hedge legs get the same exclusion.
