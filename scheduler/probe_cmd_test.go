...
func TestRunProbeHappyPath(t *testing.T) {
	...
	// HL signal-check (adx+composite), HL --fetch-atr (#689), HL --execute (PR #769),
	// spot signal-check (adx+composite), dashboard helpers, regime bundle (#879),
	// and #883 limit-order shapes.
	if len(probed) != 13 {
		t.Fatalf("expected 13 probe invocations, got %d: %v", len(probed), probed)
	}
	var hlSignal, hlFetchATR, hlExecute, hlLimitOpen, hlLimitStatus, hlCancelOrder, spotSignal, regimeBundle, candleHelper, schemaHelper, simulateHelper int
	for _, p := range probed {
		...
		case p.script == "shared_scripts/check_strategy.py" && p.mode == "signal":
			spotSignal++
		case p.script == regimeBundleScript && p.mode == "signal":
			regimeBundle++
	...
	if hlSignal != 2 || hlFetchATR != 1 || hlExecute != 1 || hlLimitOpen != 1 || hlLimitStatus != 1 || hlCancelOrder != 1 || spotSignal != 2 || regimeBundle != 1 || candleHelper != 1 || schemaHelper != 1 || simulateHelper != 1 {
		t.Fatalf("expected hl-signal=2, hl-fetch-atr=1, hl-execute=1, hl-limit-open=1, hl-limit-status=1, hl-cancel-order=1, spot-signal=2, regime-bundle=1, candle-helper=1, schema=1, simulate=1; got %d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d (probed=%v)",
			hlSignal, hlFetchATR, hlExecute, hlLimitOpen, hlLimitStatus, hlCancelOrder, spotSignal, regimeBundle, candleHelper, schemaHelper, simulateHelper, probed)
	}
}
