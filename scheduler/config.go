...
type StrategyConfig struct {
	...
	sharedWalletExitDeferred    bool                     `json:"-"`                         // startup-only safety latch: pool→capital_pct could not resolve a positive balance, so keep the strategy manage-only until a later restart can complete the durable transition (#1408 review)
