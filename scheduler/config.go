...
func validateConfig(cfg *Config, skipLiveCredentialChecks bool) error {
	...
	for i, sc := range cfg.Strategies {
		...
		if sc.AllowScaleIn || sc.MaxScaleInAdds != nil || sc.MaxScaleInNotionalUSD != nil {
			if sc.Platform != "hyperliquid" || (sc.Type != "perps" && sc.Type != "manual") {
				errs = append(errs, fmt.Sprintf("%s: allow_scale_in is only supported for HL perps/manual strategies (got platform=%q type=%q)", prefix, sc.Platform, sc.Type))
			}
			if !sc.AllowScaleIn && (sc.MaxScaleInAdds != nil || sc.MaxScaleInNotionalUSD != nil) {
				errs = append(errs, fmt.Sprintf("%s: max_scale_in_* requires allow_scale_in=true", prefix))
			}
			if sc.MaxScaleInAdds != nil && *sc.MaxScaleInAdds < 0 {
				errs = append(errs, fmt.Sprintf("%s: max_scale_in_adds must be >= 0, got %d", prefix, *sc.MaxScaleInAdds))
			}
