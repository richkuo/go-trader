...
func TestRebaselinePortfolioPeakAfterPrune_PreventsImmediateKillSwitch(t *testing.T) {
	...
	state := &AppState{
		...
		PortfolioRisk: map[RiskPartition]*PortfolioRiskState{livePartition: {PeakValue: 15148.90}},
	}

	state.partitionRisk(livePartition).PeakValue = rebaselinePortfolioPeakAfterPrune(state, cfg, nil)

	prsCfg := &PortfolioRiskConfig{MaxDrawdownPct: 25, WarnThresholdPct: 80}
	allowed, _, _, reason := CheckPortfolioRisk(state.partitionRisk(livePartition), prsCfg, 9034.24, 0, 0, 0)
	if !allowed {
...
func TestRebaselinePortfolioPeakAfterPrune_MatchesRiskPathTotalWithManual(t *testing.T) {
	...
	state.partitionRisk(livePartition).PeakValue = rebaseline
	prsCfg := &PortfolioRiskConfig{MaxDrawdownPct: 20, WarnThresholdPct: 16}
	allowed, _, warning, reason := CheckPortfolioRisk(state.partitionRisk(livePartition), prsCfg, totalPV, 0, 0, 0)
	if !allowed || warning || state.partitionRisk(livePartition).CurrentDrawdownPct != 0 {
		t.Errorf("flat post-prune: allowed=%v warning=%v dd=%.2f reason=%q", allowed, warning, state.partitionRisk(livePartition).CurrentDrawdownPct, reason)
	}
}
