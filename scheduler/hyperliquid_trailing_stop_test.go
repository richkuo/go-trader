...
func TestApplyTrailingStopUpdateResultKeepsStateOnOutcomeUnknown(t *testing.T) {
	...
	// (a) cancel landed, placement outcome unreadable — recorded OID and
	// trigger unchanged, so the position is never an Unprotected re-arm
	// candidate on the next cycle.
	ss := newSS()
	fill, _ := applyTrailingStopUpdateResult(ss, "ETH", "long", 111, 0, true,
		&HyperliquidStopLossUpdateResult{CancelStopLossSucceeded: true, StopLossOutcomeUnknown: true}, "", logger)
	if fill {
		t.Fatalf("immediateFill = true, want false")
	}
	pos := ss.Positions["ETH"]
	if pos.StopLossOID != 111 || pos.StopLossTriggerPx != 1850 {
		t.Fatalf("outcome-unknown cleared recorded state: OID %d trigger %.2f, want 111 / 1850", pos.StopLossOID, pos.StopLossTriggerPx)
	}
	...
	if _, _ = applyTrailingStopUpdateResult(ss, "ETH", "long", 111, 0, true,
		&HyperliquidStopLossUpdateResult{CancelStopLossSucceeded: true}, "", logger); ss.Positions["ETH"].StopLossOID != 0 || ss.Positions["ETH"].StopLossTriggerPx != 0 {
	...
	ss = newSS()
	if _, _ = applyTrailingStopUpdateResult(ss, "ETH", "long", 111, 0, true,
		&HyperliquidStopLossUpdateResult{CancelStopLossSucceeded: true, StopLossOutcomeUnknown: true, StopLossOID: 222, StopLossTriggerPx: 1900}, "", logger); ss.Positions["ETH"].StopLossOID != 222 {
	...
	if fill, fillPx := applyTrailingStopUpdateResult(ss, "ETH", "long", 111, 0, true,
		&HyperliquidStopLossUpdateResult{CancelStopLossSucceeded: true, StopLossOutcomeUnknown: true, StopLossFilledImmediately: true, StopLossTriggerPx: 1850}, "", logger); !fill || fillPx != 1850 {
