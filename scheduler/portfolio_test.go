...
// ── HL sz_decimals helpers (#592) ──────────────────────────────────────────

func TestHyperliquidSzDecimals(t *testing.T) {
	cases := []struct {
		symbol string
		want   int
	}{
		...
		{"ETH", 3},
		{"SOL", 2},
		{"UNKNOWN_COIN", 3}, // fallback
		{"", 3},             // empty fallback
	}
	for _, tc := range cases {
		got := hyperliquidSzDecimals(tc.symbol)
		if got != tc.want {
			t.Errorf("hyperliquidSzDecimals(%q) = %d, want %d", tc.symbol, got, tc.want)
		}
	}
}

func TestHlFloorToSzDecimals(t *testing.T) {
	cases := []struct {
		size     float64
		decimals int
		want     float64
	}{
		// Issue #592 exact reproduction: 0.421 ETH TP1 (50% = 0.2105) floors to 0.210
		{0.2105, 3, 0.210},
		// 0.421 * 0.5 in float is 0.2105 exactly representable? floor to 3 → 0.210
		{0.421 * 0.5, 3, 0.210},
		// Already clean — no change
		{0.210, 3, 0.210},
		{0.001, 3, 0.001},
		// BTC (5 decimals) — 0.000015 has 6 decimal places, floors to 0.00001
		{0.00001, 5, 0.00001},
		{0.000019, 5, 0.00001},
		// SOL (2 decimals)
		{1.234, 2, 1.23},
	...
	for _, tc := range cases {
		got := hlFloorToSzDecimals(tc.size, tc.decimals)
		// Use tolerance to handle float representation noise
		diff := got - tc.want
		if diff < 0 {
			diff = -diff
		}
		if diff > 1e-10 {
			t.Errorf("hlFloorToSzDecimals(%g, %d) = %g, want %g", tc.size, tc.decimals, got, tc.want)
		}
	}
}
