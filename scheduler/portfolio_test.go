...
func TestExecutePerpsSignalDecouplesSizingAndExchangeLeverage(t *testing.T) {
	...
	trades, err := ExecutePerpsSignalWithLeverage(s, 1, "ETH", 2000, 2, 20, 0, 0, "", 0, DirectionLong, 0, ScaleInPolicy{}, logger)
	if err != nil {
		t.Fatal(err)
	}
	if trades != 1 {
...
func TestPerpsOrderSkipReason(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := PerpsOrderSkipReason(tc.signal, tc.posSide, tc.direction, false)
			if (got != "") != tc.wantSet {
...
func TestPerpsOrderSkipReasonScaleInEnabled(t *testing.T) {
	cases := []struct {
		name      string
		signal    int
		posSide   string
		...
		{"scale_in_long_buy", 1, "long", DirectionLong, false},
		{"scale_in_short_sell", -1, "short", DirectionShort, false},
		{"scale_in_both_long", 1, "long", DirectionBoth, false},
		{"scale_in_both_short", -1, "short", DirectionBoth, false},
		{"scale_in_flat_still_skips", 1, "", DirectionLong, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := PerpsOrderSkipReason(tc.signal, tc.posSide, tc.direction, true)
			if (got != "") != tc.wantSet {
				t.Errorf("PerpsOrderSkipReason(scale-in, %d, %q, direction=%q) = %q, wantSet=%v",
					tc.signal, tc.posSide, tc.direction, got, tc.wantSet)
			}
		})
	}
...
func TestExecutePerpsSignal_DirectionShort_OpenShortFromFlat(t *testing.T) {
	...
	trades, err := ExecutePerpsSignalWithLeverage(s, -1, "ETH", 2000, 1, 1, 0, 0, "", 0, DirectionShort, 0, ScaleInPolicy{}, logger)
	if err != nil {
...
func TestPerpsLiveOrderSize_FlipIncludesCloseLeg(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			size, ok, reason := perpsLiveOrderSize(tc.signal, 2000, 1000, tc.posQty, tc.avgCost, 1.0, 1.0, 0, tc.posSide, tc.direction, 0, ScaleInPolicy{})
			if ok != tc.wantOK {
...
func TestPerpsLiveOrderSize_FlipLongToShortExceedsCloseOnly(t *testing.T) {
	posQty := 0.5
	size, ok, _ := perpsLiveOrderSize(-1, 2000, 1000, posQty, 2000, 1.0, 1.0, 0, "long", DirectionBoth, 0, ScaleInPolicy{})
	if !ok {
		t.Fatal("expected ok")
	}
	if size <= posQty {
...
func TestPerpsLiveOrderSize_FlipSizesAgainstPostCloseMargin(t *testing.T) {
	...
	size, ok, reason := perpsLiveOrderSize(-1, 1900, 1000, 0.5, 2000, 5.0, 5.0, 0, "long", DirectionBoth, 0, ScaleInPolicy{})
	if !ok {
...
func TestPerpsLiveOrderSize_CatastrophicFlipDegradesToCloseOnly(t *testing.T) {
	...
	size, ok, reason := perpsLiveOrderSize(-1, 500, 100, 1.0, 2000, 1.0, 1.0, 0, "long", DirectionBoth, 0, ScaleInPolicy{})
	if !ok {
...
func TestPerpsLiveOrderSize_FlipProfitableFlipUsesRealizedGain(t *testing.T) {
	...
	size, ok, _ := perpsLiveOrderSize(1, 1900, 1000, 0.5, 2000, 5.0, 5.0, 0, "short", DirectionBoth, 0, ScaleInPolicy{})
	if !ok {
		t.Fatal("expected ok")
	}
	wantSize := 0.5 + float64(1000+50)*5/1900
...
func TestExecutePerpsSignalMarginPerTradeUSDOverridesSizingLeverage(t *testing.T) {
	...
	trades, err := ExecutePerpsSignalWithLeverage(s, 1, "ETH", 2257, 0.1, 20, 56, 0, "", 0, DirectionLong, 0, ScaleInPolicy{}, logger)
	if err != nil {
		t.Fatal(err)
	}
	if trades != 1 {
...
func TestPerpsLiveOrderSize_PartialCloseScalesPosQty(t *testing.T) {
	...
	size, ok, reason := perpsLiveOrderSize(-1, 2100, 1000, 0.4, 2000, 1.0, 1.0, 0, "long", DirectionLong, 0.5, ScaleInPolicy{})
	if !ok {
		t.Fatalf("expected ok, got reason=%q", reason)
	}
...
func TestPerpsLiveOrderSize_FullCloseFractionIsFullPosQty(t *testing.T) {
	for _, frac := range []float64{0, 1.0} {
		size, ok, _ := perpsLiveOrderSize(-1, 2100, 1000, 0.4, 2000, 1.0, 1.0, 0, "long", DirectionLong, frac, ScaleInPolicy{})
...
func TestExecutePerpsSignal_PartialCloseLongPaperPreservesRemainder(t *testing.T) {
	...
	trades, err := ExecutePerpsSignalWithLeverage(s, -1, "ETH", 2100, 1, 1, 0, 0, "", 0, DirectionLong, 0.5, ScaleInPolicy{}, logger)
	if err != nil {
		t.Fatal(err)
	}
	if trades != 1 {
...
func TestExecutePerpsSignal_PartialCloseDoesNotFlipShortWithAllowShorts(t *testing.T) {
	...
	trades, err := ExecutePerpsSignalWithLeverage(s, -1, "ETH", 2100, 1, 1, 0, 0, "", 0, DirectionBoth, 0.5, ScaleInPolicy{}, logger)
	if err != nil {
		t.Fatal(err)
	}
	if trades != 1 {
...
func TestExecutePerpsSignal_FullCloseFromRegistryDoesNotFlip(t *testing.T) {
	...
	trades, err := ExecutePerpsSignalWithLeverage(s, -1, "ETH", 2100, 1, 1, 0, 0, "", 0, DirectionBoth, 1.0, ScaleInPolicy{}, logger)
	if err != nil {
		t.Fatal(err)
	}
	if trades != 1 {
...
func TestExecutePerpsSignal_PartialCloseLongLiveUsesFillQty(t *testing.T) {
	...
	_, err := ExecutePerpsSignalWithLeverage(s, -1, "ETH", 2100, 1, 1, 0, 0.18, "live-oid", 0.05, DirectionLong, 0.5, ScaleInPolicy{}, logger)
	if err != nil {
		t.Fatal(err)
	}
	got := s.Positions["ETH"]
