...
- **#844 trailing_tp_ratchet / trailing_tp_ratchet_regime:** a trailing ATR stop where each cleared TP tier tightens the trail and optionally scales out. The strategy declares a positive strategy-level `trailing_stop_atr_mult` (the initial loose trail — and the SL owner). The close ref's `tp_tiers` is a list (plain) or `{regime: [tiers]}` (regime form, frozen at open via `Position.Regime`). Each tier is `{atr_multiple, close_fraction?, trailing_mult_after | tp_atr_fraction}`: `close_fraction` (default `0`, cumulative target) scales out, `0` = trail-only rung; the trail tightens to `trailing_mult_after` (absolute ATR mult) **or** `tp_atr_fraction × atr_multiple` (relative) — mutually exclusive — monotonically (never loosens). Places **no on-chain TP**: partial closes ride the close evaluator, the on-chain SL rides the trailing-stop walker. Tier triggers use **entry ATR** (frozen at open). HL perps only (v1; manual rejected at load). Backtestable. Example: `{"trailing_stop_atr_mult": 3.0, "close_strategy": {"name": "trailing_tp_ratchet", "params": {"tp_tiers": [{"atr_multiple": 1.5, "close_fraction": 0.0, "trailing_mult_after": 2.0}, {"atr_multiple": 3.0, "close_fraction": 0.3, "tp_atr_fraction": 0.33}]}}}`.
