...
- **#1408** shared-wallet pool trade budgets — 2+ live HL/OKX perps may omit `capital`/`capital_pct`/`initial_capital` when every member sets positive `margin_per_trade_usd`; mixed pooled/allocated rejected. Allocated↔pool is flat-only + restart. See Adjustable Settings.md.
