...
- Manual: `type: manual` + `manual-open`/`manual-close` CLI; top-level `manual_defaults`; shares coin with HL perps (#619)
- Alerts: `discord.trade_alert_channels`, `notify_ratchet_triggers`, `circuit_breaker`
- Open strategies: see registry / `go-trader init --list-json` (incl. hidden deprecated: `amd_ifvg`, `range_scalper`, `session_breakout`, `vol_momentum`)
...
- Default SL multiplier for `type=manual` is **2.0× ATR** (#691/#692, widened #1121), distinct from the fleet-wide `default_stop_loss_atr_mult` (typically 1.0×) used by non-manual HL perps. Explicit `stop_loss_atr_mult`/`stop_loss_pct`/`stop_loss_margin_pct`/`trailing_stop_pct`/`trailing_stop_atr_mult` on the strategy still wins; fleet `default_stop_loss_atr_mult: 0` opts manual out too. Ratchet regime-read fallback uses the same resolver but ignores `manual_defaults.stop_loss_atr_mult: 0` (#1121).
...
| User close defaults | `user_close_defaults` (top-level block) | Optional: `{"tiered_tp_atr": {"tp_tiers": [...]}, "tiered_tp_atr_live": {...}, ...}` injects tiers into any matching close ref omitting `tp_tiers`. Three-layer resolution: system → user → strategy (explicit `tp_tiers` wins). Supported evaluators: `tiered_tp_pct`, `tiered_tp_atr`, `tiered_tp_atr_live`, `trailing_tp_ratchet`, and `_regime` variants. The `trailing_tp_ratchet_regime` entry may also carry a `trailing_stop_atr_regime` sub-block that supplies its coupled SL/trail owner (#1133). A reserved `regime_atr` section — `{"regime_atr": {"stop_loss_atr_regime": {<trend_regime map>}, "trailing_stop_atr_regime": {<trend_regime map>}}}` — overrides the system baseline for **standalone** `stop_loss_atr_regime`/`trailing_stop_atr_regime` `use_defaults:true` owners that are NOT a close-evaluator/ratchet SL (#1134); per-strategy explicit `trend_regime` maps still win, and a `use_defaults:true` user sub-block is a documented no-op (re-expands the system table). The two homes (`regime_atr` standalone vs `trailing_tp_ratchet_regime` ratchet-coupled) are kept disjoint by an explicit close-strategy guard on each injection path; unified later in #1135. SIGHUP-hot-reloadable. Additive: no `config_version` bump. `tp_tiers: []` rejected at validation — omit the key to inherit system defaults (#866/#870). Backtest: `--defaults system\|user` selector. |
