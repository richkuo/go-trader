...
- A full `manual-close` that fills short of the book (a capped plan or a partial IOC fill) books the fill, queues the close, and in the same command restores the stop it asked the venue to cancel through the verify-first stop-loss restore below (`restoreManualStopLoss`), at the recorded trigger and sized at the remainder (the book minus the fill, capped at a fresh on-chain read). The take-profit tiers return at the scheduler's protection sync for that strategy after the queued close applies.
