...
- **Regime-aware ATR multipliers across stop/TP surfaces (#733/#735)** — four new HL-perps-only call sites resolve their ATR multiplier from the active trend regime instead of a scalar: `stop_loss_atr_regime` and `trailing_stop_atr_regime` (strategy-level), and `tiered_tp_atr_regime` / `tiered_tp_atr_live_regime` (close-strategy refs). Shape: `{"trend_regime": {"trending_up": {"atr": …}, "trending_down": {"atr": …}, "ranging": {"atr": …}}}` (close-ref tiers add `close_fraction`), or `{"use_defaults": true}` for a baseline (trending: tighter SL / wider TP; ranging: tighter TP / looser SL). The `trend_regime` wrapper is required from day one so a future `vol_regime` lands as a sibling without renaming. Mutually exclusive with the five scalar SL fields; counts as one of the now-seven mutually-exclusive owners — `LoadConfig` correctly skips the `default_stop_loss_atr_mult` auto-default when a regime block is configured (review-fixed). Regime is frozen at open via `pos.Regime` (stamped parallel to `EntryATR`); the `_live_regime` close ref re-resolves the label each tick. Requires `regime.enabled=true`; validation rejects partial regime sets, bare labels (no wrapper), mixed tier shapes, scalar↔regime mutex collisions, and any regime-aware surface with `regime.enabled=false`. SIGHUP blocks scalar↔regime mode flips AND `RegimeATRBlock` shape changes while a position is open. **Out of scope at v0.61:** backtester parity, `sl_after` regime extension, `go-trader inspect` provenance for the new surfaces — follow-ups will land separately. No default behavior change; operators opt in by switching scalar → `_regime` sibling.
- **Post-TP stop-loss adjustment `sl_after` (#708/#710/#712)** — strategy-level default and/or per-tier rule on a `tiered_tp_atr` / `tiered_tp_atr_live` close ref that cancel+replaces the on-chain SL when a TP tier fills. Modes: `"breakeven"` (SL → AvgCost), `{atr_mult: N}` (SL → AvgCost ± N·EntryATR, signed; +N = toward profit), `{trail_from_here: {atr_mult: M}}` (perps only — converts the remaining SL into a trailing stop at M·EntryATR and the existing trailing walker takes over). Per-tier overrides shadow the strategy-level default for that tier. Scope: HL perps + `type=manual` (manual rejects `trail_from_here` in v1). Idempotent — already-applied tiers can't fire twice. Highest cleared tier wins on multi-tier same-cycle fills. Requires a fixed SL to adjust (rejected with trailing-only SL or no SL); rejected when placed under non-tiered_tp_atr* close refs. Owner DM gated on `notify_tp_sl_fills`. SIGHUP rejects rule shape changes while the strategy has an open position. Backtester parity (#712) — `Backtester(stop_loss_atr_mult=..., trailing_stop_*=..., strategy_type=...)` mirrors the live behavior including the bump-bar SL-hit suppression (#715/#718). Add via `close_strategies[i].params.sl_after` (strategy-level default) and/or `close_strategies[i].params.tiers[j].sl_after` (per-tier override). Example:
  ```json
...
| Post-TP SL adjustment | `close_strategies[i].params.sl_after` (strategy-level) and/or `close_strategies[i].params.tiers[j].sl_after` (per-tier override) — modes: `"breakeven"`, `{atr_mult: N}` (signed), `{trail_from_here: {atr_mult: M}}` (perps only) | HL perps + manual on `tiered_tp_atr*` refs. Requires a fixed SL; rejects combining with strategy-level trailing or placing under non-tiered_tp_atr* refs; manual rejects `trail_from_here`. SIGHUP blocked while position open. Owner DM gated on `notify_tp_sl_fills`. Backtester parity since #712 (#708/#710). |
| Regime-aware ATR stop/trailing | `stop_loss_atr_regime`, `trailing_stop_atr_regime` | HL perps. Resolves ATR multiplier per regime label (`trending_up`/`trending_down`/`ranging`) — frozen at open via `pos.Regime`. Shape: `{"trend_regime": {"<label>": {"atr": N}}}` (all three required) or `{"use_defaults": true}` (trending: tighter SL / wider TP; ranging: opposite). Mutually exclusive with all five scalar SL fields. Requires `regime.enabled=true`. Backtester does **not** consume — HL-live-only at v0.61. SIGHUP rejects scalar↔regime mode flips AND shape changes while open (#733/#735). |
| Regime-aware tiered TP | `close_strategies[i].params` with ref `tiered_tp_atr_regime` or `tiered_tp_atr_live_regime` | HL perps on-chain reduce-only TPs with per-regime tiers. Each per-regime entry under `trend_regime` carries `{atr, close_fraction}`; `_live_regime` re-resolves the regime label each tick (reason string records source: live classifier vs. frozen `pos.Regime`). Backtester parity TBD (#733/#735). |
