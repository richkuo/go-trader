...
5. **Apply.** The same command with `--apply`. This is the **recovery boundary**: before `config done` in the journal nothing outside the staging area has changed and there is nothing to roll back; after it the live config and some drop-ins are installed and only `--rollback` (or a re-`--apply`, which restores the retained files first) returns the deployment to its previous state. An interrupted apply restores what it installed and exits 4; if that restore also fails it prints CRITICAL with the retained paths and stops, and the deployment needs a hand repair before any further run.
