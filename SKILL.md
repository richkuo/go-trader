...
- **New open strategies (#957–#973)** — five new entries are registered and selectable (`go-trader init` / `/add-strategy`); existing configs are unaffected until an operator adds one. `mtf_confluence` (HTF trend gate over LTF pullback entries, #957), `vol_momentum` (volatility-targeted ATR-normalized time-series momentum, #959), `funding_skew` (funding-rate z-score crowding extremes — needs HL funding history; backtester attaches a per-bar `funding_rate` column, #960), `regime_adaptive` (composite-metric breakout/mean-reversion switch, #958), and `regime_adaptive_htf` (HTF-classified composite-regime fades, #973). The first four register **bidirectional** futures variants (run with `direction: "short"` or `"both"` on HL perps); `regime_adaptive_htf` ships **long/flat only**. All are spot+futures registry entries with optimizer param ranges; add with the wizard or `/add-strategy` and validate with a backtest before going live.
