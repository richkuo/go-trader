...
- **HL perps scale-in / pyramiding (#873)** — opt in with `allow_scale_in: true` plus required `scale_in_max_position_notional_usd`. Same-side entry signals add to the open position up to the cap instead of being skipped; add legs blend `AvgCost`, increase `Quantity` and `InitialQuantity`, share the original `position_id`, and keep first-entry `EntryPrice`/`EntryATR`/regime/TP-SL geometry/tier watermarks frozen. SIGHUP changes are allowed only while flat. Manual equivalent: `manual-add`.
...
- **`type: "manual"` strategy + `manual-open` / `manual-add` / `manual-close` CLI (#569/#873)** — operator-driven HL perps with auto-defaults SL@1.5×ATR (#691/#692; was 1×ATR) + `tiered_tp_atr_live` (TP1@2× / TP2@3×); can now share a coin with HL perps or another `type=manual` (#619/#620 — blanket ban lifted; owner guards + `shouldCloseFullPosition` + `extraCancelOIDs` prevent cross-strategy mutation; peers must agree on `leverage` and `margin_mode`). SL + TP[n] orders now placed inline on `manual-open` (#633) so the position is never naked between fill and the next scheduler cycle. `manual-add` scales into the existing side and keeps first-entry ATR/regime/protection geometry frozen while the next scheduler cycle resizes protection. **`--atr` is optional and now auto-fetched (#689/#690):** when omitted, the binary calls `check_hyperliquid.py --fetch-atr` to compute ATR(14) from the strategy's symbol+timeframe (same baseline strategy opens see via `ensure_atr_indicator`); falls back to `0.1*fillPrice/leverage` only if the fetch fails (network error, insufficient candles). **`--side` defaults to `long` (#691/#692);** when no sizing flag is passed (and not `--record-only`), defaults to `--margin 50`
...
- **Post-TP stop-loss adjustment `sl_after` (#708/#710/#712/#835/#836/#873)** — strategy-level default and/or per-tier rule on a `tiered_tp_atr*` close ref that cancel+replaces the on-chain SL when a TP tier fills. Scalar modes: `"breakeven"` (SL → first-entry price), `{atr_mult: N}` (SL → first-entry price ± N·EntryATR, signed; 0=breakeven, negative=behind entry), `{trail_from_here: {atr_mult: M}}` (perps only — converts to trailing at M·EntryATR), and `{trail_from_here: {tp_atr_fraction: F}}` (trail distance = F × firing tier's `atr_multiple`). **Regime-aware shapes (#736/#742/#835):** `{kind:"atr_offset","trend_regime":{...}}`, `{kind:"trail_from_here","trail_from_here":{"trend_regime":{...}}}`, and `{trail_from_here:{tp_atr_fraction:{trend_regime:{label:F}}}}`; resolves from the ATR regime label at fire time, supports composite labels via `regime_atr_window`, and defers when label missing. Scalar keys rejected when regime block present. Backtester supports scalar modes including scalar `tp_atr_fraction`; regime-aware `sl_after` remains HL-live-only ("HL-live-only" error). Per-tier overrides shadow strategy-level default. Scope: HL perps + `type=manual` (manual rejects `trail_from_here`). Idempotent; highest cleared tier wins. Requires fixed SL (`stop_loss_atr_mult`, `stop_loss_atr_regime`, `stop_loss_pct`, or `stop_loss_margin_pct`). SIGHUP rejects scalar↔regime or shape changes while open. Add via `close_strategies[i].params.sl_after` and/or `tiers[j].sl_after`. Example:
  ...
  ```
- **`trailing_tp_ratchet` / `trailing_tp_ratchet_regime` close (#844)** — a trailing-ATR-stop close where each cleared TP tier monotonically tightens the trail and optionally scales out `close_fraction`. Strategy declares a positive `trailing_stop_atr_mult` (the initial loose trail + the SL owner — no other stop fields allowed); the close ref's `tp_tiers` carries the ratchet rungs (plain list or `{regime: [tiers]}`, regime frozen at open). Places **no on-chain TP**: scale-outs ride the Python close evaluator, the on-chain SL rides the trailing-stop walker. **Scope: HL perps + `type=manual`.** Backtestable. Opt in by setting a `trailing_tp_ratchet*` close ref + strategy-level `trailing_stop_atr_mult`; see the config-shape bullet above for the tier schema (`trailing_mult_after` XOR `tp_atr_fraction`). No default behavior change for existing strategies. **The #844 review fix unified the trailing-stop result handling** across perps and manual dispatches (`applyTrailingStopUpdateResult`) — manual trailing closes now book an immediate fill as `trailing_stop_loss_immediate` instead of surfacing it as a delayed `hl_sync_external` reconcile; no operator action.
...
3. ADJUSTABLE SETTINGS
   ...
   Per-strategy: capital, max_drawdown_pct, interval_seconds, htf_filter,
     params, leverage, sizing_leverage, margin_per_trade_usd, allow_scale_in,
     scale_in_max_position_notional_usd, stop_loss_pct,
...
# Add — increases the existing same-side manual position
./go-trader manual-add hl-manual-btc --size 0.005
...
./go-trader manual-add   hl-manual-btc --size 0.005 --record-only --fill-price 68100
...
- `manual-add` scales into the existing same-side position (#873). It blends `AvgCost`, grows `Quantity` and `InitialQuantity`, and keeps first-entry `EntryPrice`, `EntryATR`, regime, TP/SL OIDs, and tier watermarks unchanged; the next scheduler cycle resizes HL protection from that frozen first-entry geometry. Record-only adds require `--size` and `--fill-price`.
...
Hot reload (`SIGHUP`) re-applies a safe subset: capital, drawdown, intervals, params, stop-loss (incl. `%`/ATR-mult trailing), sizing leverage, scale-in settings when flat, theta-harvest, portfolio risk knobs, summary cadence, correlation thresholds, `allowed_regimes` per-strategy, auto-update mode, Discord/Telegram channels and tokens; per-strategy `regime_*_window` selectors when flat. Refuses if strategy roster, script/args/type/platform, HTF filter, kill-switch identity, or DB path changed; refuses per-strategy exchange `leverage` / HL `margin_mode` / scale-in settings while positions open; refuses `regime_*_window` while open. Global `regime` block (enabled/period/adx_threshold/windows) requires full restart (mirrors `correlation`). Re-runs HL peer-on-same-coin check (`margin_mode`/exchange `leverage` agreement; at most one trailing-stop owner). On rejection, fall back to restart. Status server reflects new port immediately.
...
| Fixed ATR stop | `stop_loss_atr_mult` | HL perps — trigger `entry_price ± mult × entry_atr`, armed once after open; `entry_price` is the first-entry price and stays frozen across scale-ins (#873). Top-level `default_stop_loss_atr_mult` defaults to `1.0` and applies to every HL perps with all five stop fields omitted (incl. shared-coin peers since #601) (#562/#601/#605); per-strategy `0` or top-level `0` restores `max_drawdown_pct` fallback. |
...
| Trailing stop (ATR×mult) | `trailing_stop_atr_mult` | HL perps — `mult × entry_atr / entry_price` frozen at open; `entry_price` is the first-entry price and stays frozen across scale-ins (#873). Mutually exclusive when positive. Live + paper (#532). Arms cycle after open once ATR exists. |
...
| Scale-in / pyramiding | `allow_scale_in`, `scale_in_max_position_notional_usd` | HL perps only (#873). `allow_scale_in: true` lets same-side entry signals add to an existing position instead of being deduped; the cap is required and limits total position notional, with each add clipped to remaining room. Adds blend `AvgCost` and grow `InitialQuantity`; first-entry `EntryPrice`, `EntryATR`, regime, TP/SL geometry, and tier watermarks stay frozen. SIGHUP-aware when flat; blocked while open. |
...
| Trailing-ratchet close | `close_strategy.params.tp_tiers` with ref `trailing_tp_ratchet` or `trailing_tp_ratchet_regime` (#844) | HL perps + `type=manual`. Each tier `{atr_multiple, close_fraction?, trailing_mult_after \| tp_atr_fraction}` tightens the trail (monotonic, never loosens) and optionally scales out (cumulative `close_fraction`, `0`=trail-only). Requires strategy-level `trailing_stop_atr_mult > 0` as the SL owner + initial trail; rejects all other stop fields and `trailing_stop_pct`/`trailing_stop_atr_regime`. First rung must be ≤ initial trail. Regime form keys `tp_tiers` as `{label: [tiers]}`, frozen at open. Places **no on-chain TP**. `trailing_tp_ratchet_regime` needs `regime.enabled=true`. Backtestable. SIGHUP blocks tier-table change while open. |
