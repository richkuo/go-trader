...
| Fixed ATR stop loss | `stop_loss_atr_mult` | HL perps only — trigger `avg_cost ± mult × entry_atr`, armed once after open, never updated. Top-level `default_stop_loss_atr_mult` defaults to `1.0` when all five stop fields are omitted; first configured shared-coin peer owns that default (#562/#605); `0` restores `max_drawdown_pct` fallback. |
...
- Multiple HL perps strategies on the same coin share an on-chain position; peers must agree on `margin_mode` and exchange `leverage` (`sizing_leverage` may differ); at most one peer may own HL protection across `stop_loss_pct`, `stop_loss_margin_pct`, `trailing_stop_pct`, `trailing_stop_atr_mult`, `stop_loss_atr_mult` (#491/#501/#507/#562/#605). If no peer explicitly owns a stop, the first configured peer with omitted stop fields receives `default_stop_loss_atr_mult`; later omitted peers are coerced to `0` by `LoadConfig` (#494/#605). **Per-strategy CB (#515):** CB drain skips on-chain close when peers share the coin — exchange leg stays open until another path flattens it. Sub-account isolation is the only path for full per-strategy independence.
