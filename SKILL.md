...
- **#1275** M5-deprecated-edge roster (32 strategies) now hidden from discovery + tagged `edge=deprecated_m5` with a one-time owner DM on startup/reload; `allow_deprecated: true` on a strategy silences the DM (tag stays `(ack)`). Existing configs on an affected strategy keep trading unchanged — this is a warning surface, not a block.
