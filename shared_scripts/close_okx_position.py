...
def main():
    ...
    parser.add_argument('--sz', type=float, default=None, help='partial close size in contract units (omit for full position)')
    args = parser.parse_args()
    if args.mode != 'live':
        print(json.dumps({'close': None, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': '--mode=live required for emergency close'}))
        sys.exit(1)
    try:
...
def _emit_success(symbol, fill, already_flat=False):
    ...
    print(json.dumps({'close': close, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat()}))

def _emit_error(symbol, message):
    print(json.dumps({'close': {'symbol': symbol, 'fill': {}}, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': message}))
    sys.exit(1)
