...
def run_signal_check(strategy_name, symbol, timeframe, mode, htf_filter_enabled=False, strategy_params=None, open_strategy=None, close_strategies=None, position_side='', position_ctx=None, regime_enabled=False, regime_windows_spec=None, ohlcv_limit=200, regime_atr_window='', regime_payload_json=None, close_params_by_name=None, atr_method='simple'):
    try:
        ...
        if not market_open:
            print(json.dumps({'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': 0, 'price': 0, 'contract_spec': adapter.get_contract_spec(symbol), 'market_open': False, 'indicators': {}, 'mode': mode, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat()}))
            return
        ...
        if not candles or len(candles) < 30:
            print(json.dumps({'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': 0, 'price': 0, 'contract_spec': adapter.get_contract_spec(symbol), 'market_open': market_open, 'indicators': {}, 'mode': mode, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': f'Insufficient data: {(len(candles) if candles else 0)} candles'}))
            sys.exit(1)
        ...
        if htf_info:
            for k, v in htf_info.items():
                if isinstance(v, (int, float)):
                    indicators[k] = v
        output = {'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': signal, 'price': round(price, 2), 'contract_spec': adapter.get_contract_spec(symbol), 'market_open': market_open, 'indicators': indicators, 'regime': stdout_regime, 'mode': mode, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat()}
        if decision:
            output.update(decision)
        print(json.dumps(output))
    except Exception as e:
        traceback.print_exc(file=sys.stderr)
        print(json.dumps({'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': 0, 'price': 0, 'contract_spec': {}, 'market_open': False, 'indicators': {}, 'regime': None, 'mode': mode, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': str(e)}))
        sys.exit(1)
...
def run_execute(symbol, side, contracts, mode):
    ...
    try:
        ...
        fill = {}
        try:
            fill = {'avg_px': float(result.get('avgPrice', 0) or 0), 'total_contracts': int(result.get('filledQuantity', contracts) or contracts)}
        ...
        print(json.dumps({'execution': {'action': 'buy' if is_buy else 'sell', 'symbol': symbol, 'contracts': contracts, 'fill': fill}, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat()}))
    except Exception as e:
        traceback.print_exc(file=sys.stderr)
        print(json.dumps({'execution': None, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': str(e)}))
        sys.exit(1)
