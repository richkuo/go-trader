...
def _parse_args(argv=None):
    ...
    parser.add_argument('--since-ms', type=int, default=0, help='Only return bills settled at or after this epoch-millisecond cursor.')
...
def main(argv=None):
    ...
    try:
        ...
        _emit_error(str(e))
        return
    print(json.dumps({'bills': bills or [], 'capped': bool(capped), 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat()}))

def _emit_error(message):
    print(json.dumps({'bills': [], 'capped': False, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': message}))
    sys.exit(1)
