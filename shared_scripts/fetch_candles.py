...
def _row_to_candle(row):
    ts = int(row[0])
    return {'time': ts // 1000, 'open': _clean_float(row[1]), 'high': _clean_float(row[2]), 'low': _clean_float(row[3]), 'close': _clean_float(row[4]), 'volume': _clean_float(row[5]) if len(row) > 5 else 0.0}
...
def _fetch(args):
    ...
    df = fetch_ohlcv(symbol=symbol, timeframe=args.timeframe, limit=args.limit, exchange_id=exchange_id, store=False)
