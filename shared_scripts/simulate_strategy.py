...
def _candles_to_df(candles: List[dict]) -> pd.DataFrame:
    ...
    for candle in candles:
        ...
        rows.append({'open': float(candle.get('open') or 0), 'high': float(candle.get('high') or 0), 'low': float(candle.get('low') or 0), 'close': float(candle.get('close') or 0), 'volume': float(candle.get('volume') or 0)})
        index.append(dt)
    if not rows:
        return pd.DataFrame()
...
def _marker(time: int, kind: str, is_close: bool, price: float, pnl: float=0.0) -> dict:
    if kind == 'buy':
        return {'time': time, 'position': 'belowBar', 'color': '#059669', 'shape': 'arrowUp', 'text': 'BUY', 'side': 'buy', 'is_close': False, 'price': price}
    if kind == 'sell':
        return {'time': time, 'position': 'aboveBar', 'color': '#dc2626', 'shape': 'arrowDown', 'text': 'SELL', 'side': 'sell', 'is_close': False, 'price': price}
    return {'time': time, 'position': 'aboveBar', 'color': '#2563eb', 'shape': 'circle', 'text': 'CLOSE', 'side': 'sell' if is_close else 'buy', 'is_close': True, 'price': price, 'realized_pnl': pnl}
...
def _simulate_one(cfg: dict, candles: List[dict]) -> List[dict]:
    ...
    if isinstance(single_close, dict) and single_close.get('name'):
        ...
        if len(legacy) > 1:
            raise ValueError(f'{len(legacy)} close_strategies supplied; the array model was collapsed to a single close_strategy (#842) — keep one profit-taking close and move risk backstops to strategy-level stop fields')
        close_refs = [dict(r) for r in legacy]
    if close_refs:
        df_signals = ensure_atr_indicator(df_signals, method=str(cfg.get('atr_method') or 'simple'))
    ...
    bt = Backtester(initial_capital=float(cfg.get('initial_capital') or 1000), platform=_fee_platform(platform, strategy_type), open_strategy={'name': open_name, 'params': merged_params}, close_strategies=close_refs, regime_enabled=regime_enabled, regime_period=int(regime_cfg.get('period') or 14), regime_adx_threshold=float(regime_cfg.get('adx_threshold') or 20), allowed_regimes=allowed, regime_gate_on_failure=gate_on_failure, stop_loss_atr_mult=cfg.get('stop_loss_atr_mult'), stop_loss_pct=cfg.get('stop_loss_pct'), stop_loss_margin_pct=cfg.get('stop_loss_margin_pct'), trailing_stop_atr_mult=cfg.get('trailing_stop_atr_mult'), trailing_stop_pct=cfg.get('trailing_stop_pct'), stop_loss_atr_regime=cfg.get('stop_loss_atr_regime'), trailing_stop_atr_regime=cfg.get('trailing_stop_atr_regime'), strategy_type=strategy_type)
    results = bt.run(df_signals, strategy_name=open_name, symbol=symbol, timeframe=timeframe, params=merged_params, save=False)
    markers: List[dict] = []
