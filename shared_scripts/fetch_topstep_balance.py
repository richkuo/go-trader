...
def main():
    try:
        ...
        _emit_error(str(e))
        return
    print(json.dumps({'balance': balance, 'unrealized_pnl': unrealized_pnl, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat()}))

def _emit_error(message):
    print(json.dumps({'balance': 0.0, 'unrealized_pnl': 0.0, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': message}))
    sys.exit(1)
