...
STYLE_RULES = 'Write for a smart 18-year-old with no trading background: plain language, no jargon, no indicator names without a one-word gloss. HARD CAP: {cap} words. Never exceed it.'
...
def summarize_ohlcv(candles):
    ...
    hi = max(highs)
    lo = min(lows)
    return {'last_close': last, 'bars': len(closes), 'change_pct_5': pct_change(5), 'change_pct_20': pct_change(20), 'change_pct_50': pct_change(50), 'pct_below_high': round((hi - last) / hi * 100, 2) if hi else None, 'pct_above_low': round((last - lo) / lo * 100, 2) if lo else None}
...
def build_llm_call(model, api_url=ANTHROPIC_API_URL, timeout=PER_CALL_TIMEOUT_S):
    ...
    def llm_call(system, user):
        payload = json.dumps({'model': model, 'max_tokens': MAX_TOKENS_PER_CALL, 'system': system, 'messages': [{'role': 'user', 'content': user}]}).encode('utf-8')
        req = urllib.request.Request(api_url, data=payload, method='POST', headers={'content-type': 'application/json', 'x-api-key': api_key, 'anthropic-version': ANTHROPIC_VERSION})
...
def entry_summary(ctx):
    ...
    return f"Strategy {ctx.get('strategy_id')} just opened a {ctx.get('side')} position on {ctx.get('symbol')} ({ctx.get('platform')}, {ctx.get('type')}, {('live' if ctx.get('is_live') else 'paper')}) at {ctx.get('entry_price')}{lev_txt}, timeframe {ctx.get('timeframe')}" + (f", market regime '{ctx['regime']}'" if ctx.get('regime') else '') + '.'
...
def run_pipeline(ctx, market, llm_call, max_debate_rounds=DEFAULT_MAX_DEBATE_ROUNDS, word_cap=DEFAULT_WORD_CAP):
    style = STYLE_RULES.format(cap=word_cap)
    entry = entry_summary(ctx)
    per_analyst = {}
    tech_data = {'indicators': ctx.get('indicators') or {}, 'entry_atr': ctx.get('entry_atr'), 'ohlcv_summary': market.get('ohlcv_summary')}
    per_analyst['technical'] = truncate_to_word_cap(llm_call('You are the technical analyst on a trading desk. ' + style, f"{entry}\nTechnical data (the strategy's own entry indicators plus recent price stats; null means unavailable):\n{json.dumps(tech_data, default=str)}\nGive your read on this entry's price/momentum picture."), word_cap)
    funding = market.get('funding')
    if funding is not None:
        per_analyst['derivatives'] = truncate_to_word_cap(llm_call('You are the derivatives analyst on a trading desk. ' + style, f'{entry}\nFunding-rate data (positive = longs pay shorts):\n{json.dumps(funding, default=str)}\nGive your read on what positioning/funding implies for this entry.'), word_cap)
    ...
    for _ in range(max(0, max_debate_rounds)):
        ...
        bull = truncate_to_word_cap(llm_call('You are the bull researcher: argue the strongest honest case FOR this entry. ' + style, f'{entry}\nAnalyst notes:\n{notes}\nDebate so far:\n{prior}'), word_cap)
        transcript.append(f'bull: {bull}')
        bear = truncate_to_word_cap(llm_call('You are the bear researcher: argue the strongest honest case AGAINST this entry. ' + style, f'{entry}\nAnalyst notes:\n{notes}\nDebate so far:\n' + '\n'.join(transcript)), word_cap)
        transcript.append(f'bear: {bear}')
    judge_raw = llm_call("You are the risk manager issuing the desk's final read on an already-open position. This is commentary only — the trade stands regardless. " + style + ' Reply with ONLY a JSON object: {"verdict": "bullish"|"bearish"|"mixed", "rationale": "<plain-language reasoning>"}.', f'{entry}\nAnalyst notes:\n{notes}\nDebate:\n' + ('\n'.join(transcript) if transcript else '(debate skipped)'))
    verdict, rationale = parse_judge_output(judge_raw, word_cap)
    return {'verdict': verdict, 'rationale': rationale, 'per_analyst': per_analyst, 'model': ctx.get('model')}
