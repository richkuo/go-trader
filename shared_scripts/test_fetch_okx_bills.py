...
def _run_script(bills_or_exc, capped=False, is_live=True, argv=None):
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fetch_okx_bills.py')
    ...
    with patch('builtins.__import__', side_effect=mock_import), patch('sys.stdout', captured), patch('sys.argv', argv or ['fetch_okx_bills.py', '--since-ms=123']), patch.object(mod.sys, 'exit', side_effect=mock_exit):
        try:
            mod.main()
        except SystemExit:
            pass
...
class TestSuccess:
    ...
    def test_relays_bills(self):
        bills = [{'bill_id': 'b1', 'ts_ms': 100, 'ccy': 'USDT', 'type': '2', 'bal_chg': 19.7}, {'bill_id': 'b2', 'ts_ms': 200, 'ccy': 'USDT', 'type': '8', 'bal_chg': -1.0}]
