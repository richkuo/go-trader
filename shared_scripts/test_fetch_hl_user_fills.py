...
def _run_script(pages_or_exc, argv, account_address='0xabc'):
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fetch_hl_user_fills.py')
    ...
    with patch('builtins.__import__', side_effect=mock_import), patch('sys.stdout', captured), patch('sys.argv', ['fetch_hl_user_fills.py'] + argv), patch.object(mod.sys, 'exit', side_effect=mock_exit):
        try:
            mod.main()
        except SystemExit:
            pass
...
class TestSinglePage:
    ...
    def test_aggregates_per_oid(self):
        page = [{'oid': 111, 'coin': 'ETH', 'time': 1000, 'fee': '0.40', 'closedPnl': '0', 'tid': 1}, {'oid': 222, 'coin': 'BTC', 'time': 1100, 'fee': '0.30', 'closedPnl': '9.7', 'tid': 2}]
        out, code = _run_script([page, []], ['--since-ms=500'])
        assert code == 0, out
        assert out['error'] == ''
        assert out['fill_count'] == 2
        assert out['by_oid']['111']['fee'] == pytest.approx(0.4)
        assert out['by_oid']['222']['fee'] == pytest.approx(0.3)
    ...
    def test_partial_fills_same_oid_summed(self):
        page = [{'oid': 111, 'coin': 'eth', 'time': 1000, 'fee': '0.20', 'closedPnl': '5.0', 'tid': 1}, {'oid': 111, 'coin': 'ETH', 'time': 1200, 'fee': '0.20', 'closedPnl': '4.7', 'tid': 2}]
        out, code = _run_script([page, []], ['--since-ms=500'])
        assert code == 0, out
        entry = out['by_oid']['111']
        assert entry['fee'] == pytest.approx(0.4)
    ...
    def test_conflicting_coin_metadata_fails_closed(self):
        page = [{'oid': 111, 'coin': 'ETH', 'time': 1000, 'fee': '0.20', 'closedPnl': '5.0', 'tid': 1}, {'oid': 111, 'coin': 'BTC', 'time': 1200, 'fee': '0.20', 'closedPnl': '4.7', 'tid': 2}]
        out, code = _run_script([page, []], ['--since-ms=500'])
        assert code == 0, out
...
class TestMultiPage:
    ...
    def test_advances_cursor_to_last_fill_time(self):
        page1 = [{'oid': 111, 'time': 1000, 'fee': '0.10', 'closedPnl': '0', 'tid': 1}, {'oid': 222, 'time': 2000, 'fee': '0.20', 'closedPnl': '0', 'tid': 2}]
        page2 = [{'oid': 333, 'time': 2000, 'fee': '0.30', 'closedPnl': '0', 'tid': 3}, {'oid': 444, 'time': 3000, 'fee': '0.40', 'closedPnl': '0', 'tid': 4}]
    ...
    def test_dedups_boundary_row_seen_in_prior_page(self):
        page1 = [{'oid': 111, 'time': 1000, 'fee': '0.10', 'closedPnl': '0', 'tid': 1}, {'oid': 222, 'time': 2000, 'fee': '0.20', 'closedPnl': '0', 'tid': 2}]
        page2 = [{'oid': 222, 'time': 2000, 'fee': '0.20', 'closedPnl': '0', 'tid': 2}, {'oid': 333, 'time': 3000, 'fee': '0.30', 'closedPnl': '0', 'tid': 3}]
        out, code = _run_script([page1, page2, []], ['--since-ms=500'])
        assert code == 0, out
        assert out['by_oid']['222']['count'] == 1
        assert out['by_oid']['222']['fee'] == pytest.approx(0.2)
...
class TestErrorPaths:
    ...
    def test_user_fills_exception_surfaces(self):
        out, code = _run_script(RuntimeError('indexer down'), ['--since-ms=500'], account_address='0xabc')
        assert code == 1
