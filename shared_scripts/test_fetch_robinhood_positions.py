...
def _run_script(positions_or_exc, is_live=True):
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fetch_robinhood_positions.py')
    ...
    with patch('builtins.__import__', side_effect=mock_import), patch('sys.stdout', captured), patch('sys.argv', ['fetch_robinhood_positions.py']), patch.object(mod.sys, 'exit', side_effect=mock_exit):
        try:
            mod.main()
        except SystemExit:
            pass
...
class TestSuccess:
    ...
    def test_single_position(self):
        out, code = _run_script([{'symbol': 'BTC', 'quantity': 0.01, 'avg_price': 42000.0}])
        assert code == 0
    ...
    def test_zero_quantity_filtered(self):
        out, code = _run_script([{'symbol': 'BTC', 'quantity': 0, 'avg_price': 0}])
        assert code == 0
...
class TestFailurePaths:
    ...
    def test_not_logged_in_raises(self):
        out, code = _run_script(RuntimeError('Robinhood adapter not logged in — cannot fetch crypto positions'))
        assert code == 1
