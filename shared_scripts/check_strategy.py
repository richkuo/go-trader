...
def _position_ctx(position_side):
    ...
    for flag, key in (('--position-avg-cost', 'avg_cost'), ('--position-qty', 'current_quantity'), ('--position-initial-qty', 'initial_quantity'), ('--position-entry-atr', 'entry_atr')):
...
def main():
    ...
    if atr_method not in ('simple', 'wilder'):
        print(json.dumps({'error': f"--atr-method must be 'simple' or 'wilder', got {atr_method!r}"}))
        sys.exit(1)
    ...
    for a in sys.argv[1:]:
        if skip_next:
            skip_next = False
            continue
        if a in ('--params', '--open-strategy', '--close-strategies', '--strategy-refs', '--position-side', '--position-avg-cost', '--position-qty', '--position-initial-qty', '--position-entry-atr', '--position-regime', '--regime-windows-spec-json', '--ohlcv-limit', '--regime-atr-window', '--regime-directional-window', '--regime-payload-json', '--atr-method'):
            skip_next = True
            continue
        if a.startswith('--'):
            continue
    ...
    if len(positional_args) < 3:
        print(json.dumps({'error': f'Usage: {sys.argv[0]} <strategy> <symbol> <timeframe> [symbol_b] [--htf-filter]'}))
        sys.exit(1)
    ...
    try:
        ...
        validate_close_strategy_names(parse_close_strategies(close_strategies_raw), get_strategy, get_close_strategy, list_strategies, list_close_strategies)
        needs_pair = 'pairs_spread' in configured_names
        if needs_pair and (not symbol_b):
            print('Warning: pairs_spread requires a secondary symbol (symbol_b); degrading to self-mean-reversion. Pass a 4th argument to enable proper stat-arb (e.g. ETH/USDT for a BTC/USDT primary).', file=sys.stderr)
        ...
        if needs_pair and symbol_b:
            ...
            if df_b.empty:
                print(json.dumps({'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': 0, 'price': 0, 'indicators': {}, 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': f'No data returned for secondary symbol {symbol_b}'}))
                sys.exit(1)
        ...
        if df.empty or len(df) < 30:
            print(json.dumps({'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': 0, 'price': 0, 'indicators': {}, 'regime': None, 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': f'Insufficient data: {len(df)} candles'}))
            return
        ...
        if open_close_enabled:
            ...
            evaluation = evaluate_open_close(apply_strategy, get_strategy, df, strategy_name, open_strategy, parse_close_strategies(close_strategies_raw), position_side, strategy_params, position_ctx, close_evaluate=close_evaluate, market_ctx=market_ctx, close_params_by_name=close_params_by_name)
        ...
        indicators = {}
        indicator_cols = [c for c in result_df.columns if c not in ('open', 'high', 'low', 'close', 'close_b', 'volume', 'timestamp', 'signal', 'position', 'datetime')]
        ...
        if htf_info:
            for k, v in htf_info.items():
                if isinstance(v, (int, float)):
                    indicators[k] = v
        output = {'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': signal, 'price': round(price, 2), 'indicators': indicators, 'regime': stdout_regime, 'timestamp': datetime.now(timezone.utc).isoformat()}
        if decision:
            output.update(decision)
        print(json.dumps(output))
    except Exception as e:
        traceback.print_exc(file=sys.stderr)
        print(json.dumps({'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': 0, 'price': 0, 'indicators': {}, 'regime': None, 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': str(e)}))
        sys.exit(1)
