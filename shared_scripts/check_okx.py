...
def run_signal_check(strategy_name, symbol, timeframe, mode, htf_filter_enabled=False, inst_type='swap', strategy_params_override=None, open_strategy=None, close_strategies=None, position_side='', position_ctx=None, regime_enabled=False, regime_windows_spec=None, ohlcv_limit=200, regime_atr_window='', regime_payload_json=None, close_params_by_name=None, atr_method='simple'):
    try:
        ...
        if strategy_name == 'delta_neutral_funding' and inst_type == 'swap':
            try:
                ...
                strategy_params = {'current_funding_rate': current_rate, 'avg_funding_rate_7d': avg_rate}
        ...
        if not candles or len(candles) < 30:
            print(json.dumps({'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': 0, 'price': 0, 'indicators': {}, 'mode': mode, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': f'Insufficient data: {(len(candles) if candles else 0)} candles'}))
            sys.exit(1)
        ...
        if open_close_enabled:
            ...
            evaluation = evaluate_open_close(apply_strategy, get_strategy, df, strategy_name, open_strategy, parse_close_strategies(close_strategies), position_side, strategy_params or None, position_ctx, close_evaluate=close_evaluate, market_ctx=market_ctx, close_params_by_name=close_params_by_name)
        ...
        if htf_info:
            for k, v in htf_info.items():
                if isinstance(v, (int, float)):
                    indicators[k] = v
        output = {'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': signal, 'price': round(price, 2), 'indicators': indicators, 'regime': stdout_regime, 'mode': mode, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat()}
        if decision:
            output.update(decision)
        print(json.dumps(output))
    except Exception as e:
        traceback.print_exc(file=sys.stderr)
        print(json.dumps({'strategy': strategy_name, 'symbol': symbol, 'timeframe': timeframe, 'signal': 0, 'price': 0, 'indicators': {}, 'regime': None, 'mode': mode, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': str(e)}))
        sys.exit(1)
...
def run_execute(symbol, side, size, mode, inst_type='swap'):
    ...
    try:
        ...
        fill = {}
        try:
            fill = {'avg_px': float(result.get('average', 0) or 0), 'total_sz': float(result.get('filled', 0) or 0)}
            ...
            pass
        print(json.dumps({'execution': {'action': 'buy' if is_buy else 'sell', 'symbol': symbol, 'size': size, 'fill': fill}, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat()}))
    except Exception as e:
        traceback.print_exc(file=sys.stderr)
        print(json.dumps({'execution': None, 'platform': 'okx', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': str(e)}))
        sys.exit(1)
...
def main():
    if '--execute' in sys.argv:
        ...
        run_signal_check(args.strategy, args.symbol, args.timeframe, args.mode, args.htf_filter, args.inst_type, params_override, open_strategy_name, close_strategies_arg, args.position_side, position_ctx, regime_enabled=args.regime_enabled, regime_windows_spec=regime_windows_spec, ohlcv_limit=args.ohlcv_limit, regime_atr_window=args.regime_atr_window, regime_payload_json=args.regime_payload_json, close_params_by_name=close_params_by_name, atr_method=args.atr_method)
