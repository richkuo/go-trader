...
def main():
    ...
    if args.mode != 'live':
        print(json.dumps({'close': None, 'platform': 'robinhood', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': '--mode=live required for emergency close'}))
        sys.exit(1)
    try:
...
def _emit_success(symbol, fill, already_flat=False):
    ...
    print(json.dumps({'close': close, 'platform': 'robinhood', 'timestamp': datetime.now(timezone.utc).isoformat()}))

def _emit_error(symbol, message):
    print(json.dumps({'close': {'symbol': symbol, 'fill': {}}, 'platform': 'robinhood', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': message}))
    sys.exit(1)
