...
def mod():
    return _load()
CTX = {'strategy_id': 'hl-btc', 'symbol': 'BTC', 'platform': 'hyperliquid', 'type': 'perps', 'side': 'long', 'entry_price': 50000.0, 'quantity': 0.1, 'leverage': 3, 'entry_atr': 400.0, 'timeframe': '4h', 'regime': 'trending_up', 'is_live': True, 'indicators': {'atr': 400.0, 'rsi': 61.2}, 'model': 'claude-sonnet-5'}
...
class TestSubprocessContract:
    ...
    def test_missing_api_key_errors_json(self):
        ...
        r = subprocess.run([sys.executable, SCRIPT], input=json.dumps({**CTX, 'platform': 'nonexistent'}), capture_output=True, text=True, timeout=60, env=env)
        assert r.returncode == 1
        out = json.loads(r.stdout)
        assert 'ANTHROPIC_API_KEY' in out['error']

    def test_garbage_stdin_errors_json(self):
        r = subprocess.run([sys.executable, SCRIPT], input='not json', capture_output=True, text=True, timeout=30)
        assert r.returncode == 1
