...
def _parse_args(argv=None):
    ...
    parser.add_argument('--since-ms', type=int, default=0, help='Only return fills settled at or after this epoch-millisecond cursor.')
...
def main(argv=None):
    ...
    try:
        ...
        _emit_error(str(e))
        return
    print(json.dumps({'fills': fills or [], 'capped': bool(capped), 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat()}))

def _emit_error(message):
    print(json.dumps({'fills': [], 'capped': False, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': message}))
    sys.exit(1)
