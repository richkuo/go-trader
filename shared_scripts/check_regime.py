...
from regime import CLASSIFIER_ADX, CLASSIFIER_COMPOSITE, _DEFAULT_COMPOSITE_THRESHOLDS, compute_multi_regime, latest_regime, latest_regime_composite, parse_regime_windows_spec_json

def _emit_error(args, message: str) -> None:
    print(json.dumps({'platform': getattr(args, 'platform', ''), 'symbol': getattr(args, 'symbol', ''), 'timeframe': getattr(args, 'timeframe', ''), 'regime': None, 'error': message, 'timestamp': datetime.now(timezone.utc).isoformat()}))
    sys.exit(1)
...
def _fetch_dataframe(args):
    ...
    if platform == 'binanceus':
        ...
        return fetch_ohlcv(symbol=args.symbol, timeframe=args.timeframe, limit=args.ohlcv_limit, store=False)
    rows = None
    adapter_err = None
    try:
...
def compute_regime_bundle(df, windows_spec: dict) -> dict:
    ...
    for name in sorted(windows_spec.keys()):
        ...
        if spec.get('classifier') == CLASSIFIER_COMPOSITE:
            ...
            adx3 = str(latest_regime(df, period=period, adx_threshold=_window_adx_threshold(spec)).get('regime') or '')
        else:
            adx3 = str(snap.get('regime') or '')
            composite7 = str(latest_regime_composite(df, period, spec.get('thresholds')).get('regime') or '')
...
def main() -> None:
    ...
    parser.add_argument('--platform', required=True, help='data source: binanceus | hyperliquid | okx | topstep | robinhood | <options platform>')
    parser.add_argument('--symbol', required=True)
    parser.add_argument('--timeframe', required=True)
    parser.add_argument('--regime-windows-spec-json', required=True, help='resolved window specs from Go regimeWindowsSpecJSON')
    parser.add_argument('--ohlcv-limit', type=int, default=200)
    parser.add_argument('--min-bars', type=int, default=30, help="insufficient-data floor; mirrors the check scripts' candle guard")
    parser.add_argument('--allow-spot-fallback', action='store_true', default=False, help='options platforms: fall back to BinanceUS <symbol>/USDT when the adapter has no OHLCV')
    ...
    try:
        ...
        try:
            ...
            bar_time = ''
        print(json.dumps({'platform': args.platform, 'symbol': args.symbol, 'timeframe': args.timeframe, 'bar_time': bar_time, 'regime': bundle['regime'], 'views': bundle['views'], 'timestamp': datetime.now(timezone.utc).isoformat()}))
    except SystemExit:
        raise
    except Exception as e:
        import traceback
