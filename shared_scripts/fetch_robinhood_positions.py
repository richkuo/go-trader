...
def main():
    ...
    for p in raw or []:
        ...
        try:
            ...
            avg_price = 0.0
        positions.append({'coin': symbol, 'size': qty, 'avg_price': avg_price})
    print(json.dumps({'positions': positions, 'platform': 'robinhood', 'timestamp': datetime.now(timezone.utc).isoformat()}))

def _emit_error(message):
    print(json.dumps({'positions': [], 'platform': 'robinhood', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': message}))
    sys.exit(1)
