...
def main():
    ...
    if args.mode != 'live':
        print(json.dumps({'close': None, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': '--mode=live required for emergency close'}))
        sys.exit(1)
    try:
...
def _emit_success(symbol, fill):
    print(json.dumps({'close': {'symbol': symbol, 'fill': fill}, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat()}))

def _emit_error(symbol, message):
    print(json.dumps({'close': {'symbol': symbol, 'fill': {}}, 'platform': 'topstep', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error': message}))
    sys.exit(1)
