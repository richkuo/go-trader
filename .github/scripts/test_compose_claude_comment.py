...
class ComposeTest(unittest.TestCase):
    ...
    def test_empty_effort_reads_unknown(self):
        ...
        self.assertTrue(out.endswith('LLM: Claude Sonnet 5 | unknown | Harness: ' + HARNESS))
    ...
    def test_status_note_before_footer(self):
        ...
        self.assertTrue(out.endswith('LLM: (model not resolved) | unknown | Harness: ' + HARNESS))
    ...
    def test_rewrites_create_pr_link(self):
        ...
        comment = f'[Create PR](https://github.com/o/r/compare/main...b?quick_pull=1&body={encoded})'
...
class ComposeCLITest(unittest.TestCase):
    ...
    def test_cli_matches_function(self):
        ...
        env = {**os.environ, 'BODY_IN': 'body', 'MODEL_ID': 'claude-opus-4-8[1m]', 'EFFORT': 'xhigh', 'CLAUDE_HARNESS': HARNESS, 'STATUS_NOTE': ''}
        result = subprocess.run([sys.executable, script], env=env, capture_output=True, text=True, check=True)
        self.assertEqual(result.stdout, compose('body', 'claude-opus-4-8[1m]', 'xhigh', HARNESS))
