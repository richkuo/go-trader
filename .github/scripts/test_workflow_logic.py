...
def extract_step_run_block(yml_text, step_name):
    ...
    name_pat = re.compile('^(\\s*)- name:\\s*' + re.escape(step_name) + '\\s*$')
    start = None
    step_indent = None
    for idx, ln in enumerate(lines):
        ...
        if m:
            ...
            break
    if start is None:
        raise AssertionError(f"step '{step_name}' not found in workflow — renamed? Update this extractor.")
    run_pat = re.compile('^(\\s*)run:\\s*\\|\\s*$')
    next_step_pat = re.compile('^ {%d}- name:' % step_indent)
    run_idx = None
    run_indent = None
    for idx in range(start + 1, len(lines)):
        ...
        if m:
            ...
            break
    if run_idx is None:
        raise AssertionError(f"no `run: |` block found under step '{step_name}' — structure changed?")
    body = []
...
def _run_block(script, env_overrides, output_key):
    with tempfile.TemporaryDirectory() as d:
        ...
        if value is None:
            raise AssertionError(f'run block wrote no {output_key}= line to GITHUB_OUTPUT; stderr:\n{r.stderr}')
        return value
...
def run_classify_mode(event_name, stripped, pr_url='', pr_author_assoc='', pr_author_login='', flow=''):
    ...
    return _run_block(script, {'EVENT_NAME': event_name, 'PR_URL': pr_url, 'FLOW': flow, 'STRIPPED': stripped, 'PR_AUTHOR_ASSOC': pr_author_assoc, 'PR_AUTHOR_LOGIN': pr_author_login}, 'mode')
...
def run_verify_invocation(event_name, body, trigger_actor='someuser'):
    ...
    return _run_block(script, {'EVENT_NAME': event_name, 'COMMENT_BODY': body, 'REVIEW_BODY': body, 'ISSUE_BODY': body, 'TRIGGER_ACTOR': trigger_actor}, 'invoked')

def test_trusted_member_pr_comment_no_review_word_is_fix_pr():
    assert run_classify_mode('issue_comment', '@claude correct the lint error', pr_url=PR_URL, pr_author_assoc='MEMBER') == 'fix-pr'

def test_trusted_owner_pr_comment_is_fix_pr():
    assert run_classify_mode('issue_comment', '@claude address the feedback', pr_url=PR_URL, pr_author_assoc='OWNER') == 'fix-pr'

def test_claude_bot_authored_pr_is_fix_pr():
    assert run_classify_mode('issue_comment', '@claude address the feedback', pr_url=PR_URL, pr_author_assoc='NONE', pr_author_login='claude[bot]') == 'fix-pr'

def test_external_author_pr_comment_is_review_only():
    assert run_classify_mode('issue_comment', '@claude fix the lint error', pr_url=PR_URL, pr_author_assoc='NONE') == 'review'

def test_contributor_author_pr_comment_is_review_only():
    assert run_classify_mode('issue_comment', '@claude fix this', pr_url=PR_URL, pr_author_assoc='CONTRIBUTOR') == 'review'

def test_review_keyword_forces_review_even_for_trusted_author():
    assert run_classify_mode('issue_comment', '@claude review this carefully', pr_url=PR_URL, pr_author_assoc='MEMBER') == 'review'

def test_review_keyword_after_model_shorthand_is_review():
    assert run_classify_mode('issue_comment', '@claude sonnet review', pr_url=PR_URL, pr_author_assoc='MEMBER') == 'review'

def test_review_and_fix_loses_push_on_purpose():
    assert run_classify_mode('issue_comment', '@claude review and fix it', pr_url=PR_URL, pr_author_assoc='OWNER') == 'review'

def test_review_word_later_in_sentence_no_longer_forces_review():
    assert run_classify_mode('issue_comment', '@claude fix the review comments', pr_url=PR_URL, pr_author_assoc='MEMBER') == 'fix-pr'

def test_fix_keyword_routes_to_fix_pr():
    assert run_classify_mode('issue_comment', '@claude fix', pr_url=PR_URL, pr_author_assoc='MEMBER') == 'fix-pr'

def test_fix_keyword_after_model_shorthand_routes_to_fix_pr():
    assert run_classify_mode('issue_comment', '@claude opus fix and be thorough', pr_url=PR_URL, pr_author_assoc='OWNER') == 'fix-pr'

def test_old_fix_pr_spelling_routes_to_fix_pr():
    assert run_classify_mode('issue_comment', '@claude fix-pr', pr_url=PR_URL, pr_author_assoc='MEMBER') == 'fix-pr'

def test_fix_keyword_untrusted_pr_author_is_review_only():
    assert run_classify_mode('issue_comment', '@claude fix', pr_url=PR_URL, pr_author_assoc='NONE') == 'review'

def test_fix_keyword_on_plain_issue_is_implement():
    assert run_classify_mode('issue_comment', '@claude fix', pr_url='', pr_author_assoc='MEMBER') == 'implement'

def test_fix_keyword_on_inline_review_surface_stays_review():
    assert run_classify_mode('pull_request_review_comment', '@claude fix', pr_url=PR_URL, pr_author_assoc='OWNER') == 'review'

def test_pull_request_review_surface_is_review():
    assert run_classify_mode('pull_request_review', '@claude fix this', pr_url=PR_URL, pr_author_assoc='MEMBER') == 'review'

def test_pull_request_review_comment_surface_is_review():
    assert run_classify_mode('pull_request_review_comment', '@claude fix this', pr_url=PR_URL, pr_author_assoc='OWNER') == 'review'

def test_issues_event_is_implement():
    assert run_classify_mode('issues', '@claude build this feature', pr_author_assoc='MEMBER') == 'implement'

def test_docs_release_flow_routes_to_implement_even_on_pr():
    assert run_classify_mode('issue_comment', '@claude sync-docs', pr_url=PR_URL, pr_author_assoc='MEMBER', flow='sync-docs') == 'implement'

def test_issue_comment_on_issue_is_implement():
    assert run_classify_mode('issue_comment', '@claude implement this', pr_url='', pr_author_assoc='MEMBER') == 'implement'
...
def test_second_nonblank_line_does_not_fire():
    assert run_verify_invocation('issue_comment', '@claude review\nplease also fix the flaky test', 'claude[bot]') == 'false'
