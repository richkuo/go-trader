...
def run(body_in, footer=FOOTER):
    result = subprocess.run([sys.executable, SCRIPT], env={**os.environ, 'BODY_IN': body_in, 'FOOTER_TEXT': footer}, capture_output=True, text=True, check=True)
...
class RewriteCreatePRLinkTest(unittest.TestCase):
    ...
    def test_idempotent(self):
        ...
        once = run(comment)
        twice = run(once)
        self.assertEqual(extract_body_param(once), extract_body_param(twice))
