...
_CREATE_PR_LINK = re.compile('\\((https://github\\.com/[^)\\s]*compare/[^)\\s]*[?&]quick_pull=1[^)\\s]*)\\)')
...
def rewrite_create_pr_link(body: str, footer: str) -> str:
    ...
    def rewrite(match):
        ...
        new_url = urllib.parse.urlunsplit((parts.scheme, parts.netloc, parts.path, new_query, parts.fragment))
        return '(' + new_url + ')'
    return _CREATE_PR_LINK.sub(rewrite, body)
if __name__ == '__main__':
    sys.stdout.write(rewrite_create_pr_link(os.environ['BODY_IN'], os.environ['FOOTER_TEXT']))
    sys.stdout.write('\n')
