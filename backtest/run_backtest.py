...
def load_strategy_config(config_path: str, strategy_id: str,
    ...
    for sc in cfg.get("strategies", []) or []:
        ...
        # #1159: a correlated hedge leg is a SECOND live instrument whose PnL,
        # fees, and funding the single-instrument backtester cannot model. A
        # silent drop would report the primary's standalone edge as if it were
        # the hedged strategy's — the exact misdiagnosis the loud-reject pattern
        # exists to prevent. An explicitly disabled block changes nothing live,
        # so it passes through; only an ENABLED hedge is rejected.
        ...
        if isinstance(hedge_cfg, dict) and hedge_cfg.get("enabled"):
            raise ValueError(
                ...
                f"phase 1 (see #1159) — the backtester models a single "
                f"instrument and would silently drop the hedge's PnL, fees, and "
                f"funding. Remove or disable the hedge block to backtest the "
                f"primary leg alone."
            )
