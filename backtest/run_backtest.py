...
def load_strategy_config(config_path: str, strategy_id: str,
    ...
    for sc in cfg.get("strategies", []) or []:
        ...
        # #1159: a correlated hedge leg is HL-live-only in phase 1. The
        # backtester models a single instrument and would silently drop the
        # hedge's PnL/fees, so a hedge-enabled config must reject loudly rather
        # than backtest a misleadingly hedge-free result. An explicitly disabled
        # block (enabled: false) changes nothing live, so it passes through.
        hedge = sc.get("hedge")
