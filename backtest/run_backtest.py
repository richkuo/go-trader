...
def load_strategy_config(config_path: str, strategy_id: str,
    ...
    for sc in cfg.get("strategies", []) or []:
        ...
        if isinstance(hedge, dict) and hedge.get("enabled", True):
            # Explicit enabled:false is inert and allowed; anything else rejects.
            raise ValueError(
                f"{config_path}: strategy {strategy_id!r} configures a correlated "
                f"hedge leg (hedge block), which is HL-live-only in phase 1 (#1159) — "
                f"the backtester models a single instrument and would silently drop "
                f"the hedge's PnL/fees. Remove the hedge block to backtest the "
                f"primary leg alone."
            )
