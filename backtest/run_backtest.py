...
def _validate_user_close_defaults_regime_atr(user_defaults: Optional[dict]) -> None:
    ...
    # ``regime_atr`` lives under shared_strategies/close, which is NOT on
    # sys.path under script-style invocation (`python backtest/run_backtest.py`,
    # where only the script dir + shared_tools are added). Mirror
    # backtester._ensure_close_strategies_path so the bare import resolves in
    # production, not just under pytest (which inserts the path during collection).
...
def load_strategy_config(config_path: str, strategy_id: str,
    ...
    for sc in cfg.get("strategies", []) or []:
        ...
        # #1159 phase 1: a hedge block places real correlated-hedge orders on a
        # second instrument via a per-cycle scheduler mirror (runHedgeSync) —
        # the bar-level backtester models a single instrument and has no
        # resolver hook for a second leg's fills/fees/slippage, so it would
        # silently drop the hedge's PnL/fees and backtest the primary alone.
        # An explicitly disabled block (`enabled: false`) changes nothing live,
        # so only reject when enabled.
        hedge_cfg = sc.get("hedge")
        if isinstance(hedge_cfg, dict) and hedge_cfg.get("enabled", True):
            raise ValueError(
                f"{config_path}: strategy {strategy_id!r} configures a "
                f"correlated hedge leg (hedge block), which is HL-live-only "
                f"in phase 1 (#1159) — the backtester models a single "
                f"instrument and would silently drop the hedge's PnL/fees. "
                f"Remove the hedge block to backtest the primary leg alone."
            )
