...
def load_registry(platform: str='spot') -> ModuleType:
    ...
    if key not in _PLATFORM_DIRS:
        raise ValueError(f"Unknown platform '{platform}' — expected 'spot' or 'futures'")
    if key in _cached:
        return _cached[key]
    _ensure_import_paths()
    path = os.path.join(_PLATFORM_DIRS[key], 'strategies.py')
    spec = importlib.util.spec_from_file_location(f'_backtest_{key}_strategies', path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    registry = getattr(mod, 'STRATEGY_REGISTRY', None)
    if not registry:
        raise RuntimeError(f'{path} loaded but STRATEGY_REGISTRY is missing or empty')
    _cached[key] = mod
    return mod
