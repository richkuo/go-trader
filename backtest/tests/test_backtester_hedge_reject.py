...
def _hl_strategy(**over):
    sc = {'id': 'hl-test', 'type': 'perps', 'platform': 'hyperliquid', 'script': 'shared_scripts/check_hyperliquid.py', 'args': ['tema_cross', 'ETH', '4h', '--mode', 'paper'], 'capital': 1000, 'max_drawdown_pct': 50, 'open_strategy': {'name': 'tema_cross', 'params': {}}, 'stop_loss_atr_mult': 2.0}
    sc.update(over)
    return sc

def test_rejects_enabled_hedge_block(tmp_path):
    path = _config(tmp_path, _hl_strategy(hedge={'enabled': True, 'symbol': 'BTC', 'ratio': 1.0}))
...
def test_allows_explicitly_disabled_hedge_block(tmp_path):
    path = _config(tmp_path, _hl_strategy(hedge={'enabled': False, 'symbol': 'BTC', 'ratio': 1.0}))
...
def test_only_the_hedge_enabled_strategy_is_rejected(tmp_path):
    cfg = {'config_version': 16, 'strategies': [_hl_strategy(id='hl-hedged', hedge={'enabled': True, 'symbol': 'BTC'}), _hl_strategy(id='hl-plain')]}
    path = tmp_path / 'config.json'
    path.write_text(json.dumps(cfg))
