...
def _make_df(opens, highs, lows, closes, signals):
    ...
    return pd.DataFrame({'open': opens, 'high': highs, 'low': lows, 'close': closes, 'signal': signals}, index=idx)
...
def test_fill_uses_next_bar_open_not_signal_bar_close():
    ...
    assert trade['entry_price'] == pytest.approx(110.0, rel=1e-09), "Entry must fill at the bar after the signal bar's open (110), not the signal bar's close (100) — that would be look-ahead bias."
    assert trade['shares'] == pytest.approx(1000.0 / 110.0, rel=1e-09), 'Share count must divide available cash by the actual fill price (110), not the signal-bar close (100).'
...
def test_exit_uses_next_bar_open_not_signal_bar_close():
    ...
    assert len(results['trades']) == 1
    trade = results['trades'][0]
    assert trade['entry_price'] == pytest.approx(100.0, rel=1e-09)
    assert trade['exit_price'] == pytest.approx(200.0, rel=1e-09), "Exit must fill at the bar after the sell-signal bar's open (200), not the signal bar's close (100)."
    expected_final_cash = 1000.0 * (200.0 / 100.0)
    assert results['final_capital'] == pytest.approx(expected_final_cash, rel=1e-09), 'Final cash should double with the 2x gap-up fill, not stay flat.'
...
def test_falls_back_to_close_when_open_column_missing():
    ...
    assert len(results['trades']) == 1
    assert results['trades'][0]['entry_price'] == pytest.approx(110.0, rel=1e-09)
...
def test_signal_on_bar_zero_fills_on_bar_one():
    ...
    assert len(results['trades']) == 1
    assert results['trades'][0]['entry_price'] == pytest.approx(200.0, rel=1e-09)
...
def test_sell_signal_while_flat_is_ignored():
    ...
    assert results['total_trades'] == 0
    assert results['final_capital'] == pytest.approx(1000.0, rel=1e-09)
