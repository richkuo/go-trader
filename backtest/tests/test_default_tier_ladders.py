...
_HYPERLIQUID_PROTECTION_GO = os.path.join(_REPO_ROOT, 'scheduler', 'hyperliquid_protection.go')
EXPECTED_LADDER = ((1.5, 0.4), (3.0, 0.8), (5.0, 1.0))
...
def _python_tiered_tp_atr_ladder():
    raw = _load_close_module('tiered_tp_atr.py', 'DEFAULT_TIERS', '_ladder_probe_tiered_tp_atr')
    return tuple(((float(t['atr_multiple']), float(t['close_fraction'])) for t in raw))

def _python_scalar_tp_ladder():
    raw = _load_close_module('post_tp_sl.py', '_DEFAULT_SCALAR_TP_TIERS', '_ladder_probe_post_tp_sl')
...
def _go_default_protection_tiers():
    ...
    body = re.search('func defaultHLProtectionTiers\\(\\)\\s*\\[\\]hlProtectionTier\\s*\\{(.*?)\\n\\}', text, re.DOTALL)
    assert body, 'defaultHLProtectionTiers() not found in hyperliquid_protection.go'
    rows = re.findall('\\{\\s*Multiple:\\s*([0-9.]+),\\s*Fraction:\\s*([0-9.]+)\\s*\\}', body.group(1))
...
def test_all_three_default_tier_ladders_agree():
    ...
    assert go == py_atr == py_scalar, f'Default tier ladder desync — retune one of defaultHLProtectionTiers() (Go), DEFAULT_TIERS (tiered_tp_atr.py), _DEFAULT_SCALAR_TP_TIERS (post_tp_sl.py) and you MUST update all three together. Go={go} tiered_tp_atr={py_atr} scalar={py_scalar}'

def test_final_tier_closes_everything_remaining():
    for ladder in (_go_default_protection_tiers(), _python_tiered_tp_atr_ladder(), _python_scalar_tp_ladder()):
