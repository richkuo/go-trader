...
def _alloc(confirm_bars=2):
    return {'profiles': {'up': 'trend', 'down': 'fade'}, 'param_sets': {'trend': {'k': 1}, 'fade': {'k': 0}}, 'confirm_bars': confirm_bars, 'initial_profile': 'fade'}
...
def _flat_df(n=40):
    ...
    return pd.DataFrame({'open': close, 'high': close + 0.01, 'low': close - 0.01, 'close': close, 'volume': 1000.0}, index=idx)
...
def test_engine_selects_active_profile_signal():
    ...
    bt = Backtester(initial_capital=1000.0, commission_pct=0.0, slippage_pct=0.0, profile_allocation=_alloc(confirm_bars=2))
