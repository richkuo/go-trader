...
def test_bootstrap_p_le_zero_all_positive_is_zero():
    assert gen.bootstrap_p_mean_le_zero([1.0, 2.0, 3.0], n_resamples=500, seed=3) == 0.0
...
def test_dedupe_drops_same_dataset_same_entry():
    samples = [{'dataset': 'BTC/USDT 1h', 'entry_date': '2025-06-15', 'pnl_pct': 1.0}, {'dataset': 'BTC/USDT 1h', 'entry_date': '2025-06-15', 'pnl_pct': 1.0}, {'dataset': 'ETH/USDT 1h', 'entry_date': '2025-06-15', 'pnl_pct': 2.0}]
...
def test_dedupe_keeps_distinct_entries():
    samples = [{'dataset': 'BTC/USDT 1h', 'entry_date': '2025-06-15', 'pnl_pct': 1.0}, {'dataset': 'BTC/USDT 1h', 'entry_date': '2025-06-16', 'pnl_pct': -1.0}]
    out, dropped = gen.dedupe_samples(samples)
...
def test_analyze_sample_verdict_matches_primary_test():
    ...
    noisy = gen.analyze_sample([1.0, -1.0, 0.5, -0.5, 0.2], n_resamples=1000, seed=5)
    assert noisy['verdict'] in (gen.VERDICT_INDISTINGUISHABLE, gen.VERDICT_NO_EDGE)
    for key in ('summary', 'permutation', 'bootstrap', 'bootstrap_p_mean_le_zero', 'sign_test', 'wilcoxon', 'alpha', 'verdict'):
...
def test_pool_exact_duplicate_still_dropped():
    legs = [{'dataset': 'BTC/USDT 1h', 'window': 'is', 'trade_samples': [{'entry_date': '2025-08-15 04:00:00', 'pnl_pct': 1.0}]}, {'dataset': 'BTC/USDT 1h', 'window': 'is', 'trade_samples': [{'entry_date': '2025-08-15 04:00:00', 'pnl_pct': 1.0}]}, {'dataset': 'ETH/USDT 1h', 'window': 'is', 'trade_samples': []}]
...
def test_pool_overlap_nonidentical_entries_dropped():
    legs = [{'dataset': 'BTC/USDT 1h', 'window': 'is', 'trade_samples': [{'entry_date': '2025-06-17 19:00:00', 'pnl_pct': 1.37}]}, {'dataset': 'BTC/USDT 1h', 'window': '2025H1', 'trade_samples': [{'entry_date': '2025-06-18 21:00:00', 'pnl_pct': 0.22}, {'entry_date': '2025-03-01 00:00:00', 'pnl_pct': -0.5}]}]
    samples, n_exact, n_overlap = gen.pool_trade_samples(legs)
    assert n_exact == 0 and n_overlap == 1
    assert [s['entry_date'] for s in samples] == ['2025-06-17 19:00:00', '2025-03-01 00:00:00']

def test_pool_shared_start_windows_keep_distinct_entries():
    windows = {'a': ('2023-01-01', '2024-01-01'), 'b': ('2023-01-01', '2025-01-01')}
    legs = [{'dataset': 'BTC/USDT 1h', 'window': 'a', 'trade_samples': [{'entry_date': '2023-05-01 00:00:00', 'pnl_pct': 1.0}]}, {'dataset': 'BTC/USDT 1h', 'window': 'b', 'trade_samples': [{'entry_date': '2023-06-01 00:00:00', 'pnl_pct': 2.0}, {'entry_date': '2024-06-01 00:00:00', 'pnl_pct': 3.0}]}]
...
def test_pool_zero_trade_leg_still_claims_coverage():
    windows = {'a': ('2023-01-01', '2024-01-01'), 'b': ('2023-06-01', '2024-06-01')}
    legs = [{'dataset': 'BTC/USDT 1h', 'window': 'a', 'trade_samples': []}, {'dataset': 'BTC/USDT 1h', 'window': 'b', 'trade_samples': [{'entry_date': '2023-08-01 00:00:00', 'pnl_pct': 1.0}, {'entry_date': '2024-03-01 00:00:00', 'pnl_pct': 2.0}]}]
    samples, n_exact, n_overlap = gen.pool_trade_samples(legs, windows=windows)
    assert n_overlap == 1
    assert [s['pnl_pct'] for s in samples] == [2.0]

def test_pool_coverage_is_per_dataset():
    windows = {'a': ('2023-01-01', '2024-01-01'), 'b': ('2023-01-01', '2024-01-01')}
    legs = [{'dataset': 'BTC/USDT 1h', 'window': 'a', 'trade_samples': [{'entry_date': '2023-05-01 00:00:00', 'pnl_pct': 1.0}]}, {'dataset': 'ETH/USDT 1h', 'window': 'b', 'trade_samples': [{'entry_date': '2023-05-01 00:00:00', 'pnl_pct': 2.0}]}]
    samples, n_exact, n_overlap = gen.pool_trade_samples(legs, windows=windows)
    assert n_exact == 0 and n_overlap == 0
    assert len(samples) == 2

def test_pool_open_ended_window_claims_to_infinity():
    windows = {'oos': ('2026-01-01', None), 'late': ('2026-06-01', '2027-01-01')}
    legs = [{'dataset': 'BTC/USDT 1h', 'window': 'oos', 'trade_samples': [{'entry_date': '2026-02-01 00:00:00', 'pnl_pct': 1.0}]}, {'dataset': 'BTC/USDT 1h', 'window': 'late', 'trade_samples': [{'entry_date': '2026-08-01 00:00:00', 'pnl_pct': 2.0}]}]
    samples, n_exact, n_overlap = gen.pool_trade_samples(legs, windows=windows)
    assert n_overlap == 1
...
def test_trade_samples_from_results_extracts_entry_and_pnl():
    results = {'trades': [{'entry_date': '2025-06-15 04:00:00', 'pnl_pct': 1.25, 'pnl': 12.5}, {'entry_date': '2025-06-20 09:00:00', 'pnl_pct': -0.4, 'pnl': -4.0}]}
    out = ew.trade_samples_from_results(results)
    assert out == [{'entry_date': '2025-06-15 04:00:00', 'pnl_pct': 1.25, 'pnl_pct_net': 1.25}, {'entry_date': '2025-06-20 09:00:00', 'pnl_pct': -0.4, 'pnl_pct_net': -0.4}]
...
def test_run_leg_keep_trades_attaches_samples(monkeypatch):
    ...
    leg = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None), keep_trades=True)
...
def test_collect_gross_legs_zeroes_friction(monkeypatch):
    ...
    import data_fetcher
    monkeypatch.setattr(data_fetcher, 'load_cached_data', lambda *a, **k: df, raising=True)
    legs = gen.collect_gross_legs(_FakeRegistry(), 'alternator', None, [('BTC/USDT', '1h')], ['oos'])
    assert len(legs) == 1
    gross = legs[0]
    net = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ew.WINDOWS['oos'])
