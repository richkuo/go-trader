...
_STUDY_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'candidates', 'squeeze_momentum_1198')

def _base_spec(**over):
    spec = {'study': 't', 'registry': 'spot', 'harnesses': ['m1_noise', 'm1'], 'windows': ['is', 'oos'], 'correction': {'method': 'benjamini_hochberg', 'alpha': 0.05}, 'candidates': [{'key': 'c1', 'candidate': {'name': 'squeeze_momentum', 'direction': 'long'}}]}
    spec.update(over)
    return spec
...
def test_load_spec_accepts_committed_1198_shape():
    ...
    assert [c['key'] for c in spec['candidates']] == ['baseline', 'adx_not_down', 'comp_up_family']
...
def test_load_spec_runs_candidate_validator():
    bad = _base_spec(candidates=[{'key': 'c', 'candidate': {'name': 'squeeze_momentum', 'allowed_regimes': 'trending_up'}}])
...
def test_expand_sweep_and_gate_variants_cartesian_deterministic():
    spec = asug.load_spec(_base_spec(candidates=[], base={'name': 'squeeze_momentum', 'direction': 'long', 'params': {'kc_mult': 1.5}}, sweep={'kc_mult': [1.3, 1.5]}, gate_variants=[{'label': 'up', 'allowed_regimes': ['trending_up_clean'], 'regime_windows_spec': {'medium': {'classifier': 'composite', 'period': 21}}}, {'label': 'rng', 'allowed_regimes': ['ranging_quiet']}]), _STUDY_DIR)
    entries = asug.expand_candidates(spec)
    keys = [e['key'] for e in entries]
    assert keys == ['squeeze_momentum.kc_mult1.3.up', 'squeeze_momentum.kc_mult1.3.rng', 'squeeze_momentum.kc_mult1.5.up', 'squeeze_momentum.kc_mult1.5.rng']
...
def test_expand_rejects_duplicate_keys():
    spec = asug.load_spec(_base_spec(candidates=[{'key': 'dup', 'candidate': {'name': 'squeeze_momentum'}}, {'key': 'dup', 'candidate': {'name': 'squeeze_momentum'}}]), _STUDY_DIR)
...
def test_close_stack_specs_round_trip_through_optimizer_grid():
    ...
    spec = asug.load_spec(_base_spec(candidates=[], m6={'baseline_config': 'cfg.json', 'strategy_id': 's', 'close_stack_specs': stack_specs}), _STUDY_DIR)
    entries = asug.expand_candidates(spec)
    ab = [e for e in entries if e['kind'] == 'exit_ab']
    assert len(ab) == expected

def test_non_replayable_m6_close_excluded():
    spec = asug.load_spec(_base_spec(candidates=[], base=None, m6={'baseline_config': 'cfg.json', 'strategy_id': 's', 'candidate_close_variants': [{'key': 'bad', 'candidate_close': [{'name': 'tiered_tp_atr_live_regime_dynamic'}]}, {'key': 'good', 'candidate_close': [{'name': 'atr_stop', 'params': {'atr_mult': 2}}]}]}), _STUDY_DIR)
...
def test_m6_requires_exactly_one_incumbent_source():
    ...
    both = _base_spec(**base, m6={'strategy_id': 's', 'baseline_config': 'cfg.json', 'incumbent_close': [{'name': 'tiered_tp_atr'}], 'candidate_close_variants': []})
    with pytest.raises(ValueError, match='EXACTLY one'):
        asug.load_spec(both, _STUDY_DIR)
    neither = _base_spec(**base, m6={'strategy_id': 's', 'candidate_close_variants': []})
    with pytest.raises(ValueError, match='EXACTLY one'):
        asug.load_spec(neither, _STUDY_DIR)

def test_m6_incumbent_close_only_spec_loads():
    spec = asug.load_spec(_base_spec(candidates=[], m6={'strategy_id': 'squeeze_momentum', 'incumbent_close': [{'name': 'tiered_tp_atr', 'params': {}}], 'candidate_close_variants': [{'key': 'v', 'candidate_close': [{'name': 'atr_stop', 'params': {'atr_mult': 2}}]}]}), _STUDY_DIR)
    entries = asug.expand_candidates(spec)
    ab = [e for e in entries if e['kind'] == 'exit_ab']
...
def test_m6_missing_strategy_id_everywhere_fails_at_load():
    bad = _base_spec(candidates=[], m6={'incumbent_close': [{'name': 'tiered_tp_atr', 'params': {}}], 'candidate_close_variants': [{'key': 'v', 'candidate_close': [{'name': 'atr_stop', 'params': {'atr_mult': 2}}]}]})
    with pytest.raises(ValueError, match='strategy_id'):
        asug.load_spec(bad, _STUDY_DIR)

def test_m6_per_variant_strategy_id_override_loads_without_m6_default():
    spec = asug.load_spec(_base_spec(candidates=[], m6={'incumbent_close': [{'name': 'tiered_tp_atr', 'params': {}}], 'candidate_close_variants': [{'key': 'v', 'strategy_id': 'squeeze_momentum', 'candidate_close': [{'name': 'atr_stop', 'params': {'atr_mult': 2}}]}]}), _STUDY_DIR)
...
def test_m6_close_stack_specs_require_m6_level_strategy_id():
    bad = _base_spec(candidates=[], m6={'incumbent_close': [{'name': 'tiered_tp_atr', 'params': {}}], 'candidate_close_variants': [], 'close_stack_specs': [{'close': {'name': 'atr_stop', 'params': {'atr_mult': [2.0]}}}]})
    with pytest.raises(ValueError, match='close_stack_specs'):
        asug.load_spec(bad, _STUDY_DIR)

def test_m6_baseline_config_path_also_requires_strategy_id():
    bad = _base_spec(candidates=[], m6={'baseline_config': 'cfg.json', 'candidate_close_variants': [{'key': 'v', 'candidate_close': [{'name': 'atr_stop', 'params': {'atr_mult': 2}}]}]})
    with pytest.raises(ValueError, match='strategy_id'):
        asug.load_spec(bad, _STUDY_DIR)
...
def _load_template_json():
    import re
    return json.loads(re.sub('//.*', '', open(_TEMPLATE).read()))
...
def test_template_documents_every_variant_and_candidate_option():
    ...
    assert any(('strategy_id' in v for v in m6['candidate_close_variants'])), 'template omits per-variant strategy_id override'
...
def test_m5_params_limitation_flagged():
    spec = asug.load_spec(_base_spec(harnesses=['m5'], candidates=[{'key': 'c', 'candidate': {'name': 'squeeze_momentum', 'params': {'kc_mult': 2.0}}}]), _STUDY_DIR)
    entry = asug.expand_candidates(spec)[0]
    assert 'm5_params_unaudited' in entry['limitations']

def test_m1_argv_tail():
    assert asug.m1_argv_tail('/t/c.json', 'spot', ['is', 'oos'], ['BTC/USDT:1h'], '/t/o.json') == ['--candidate-json', '/t/c.json', '--registry', 'spot', '--windows', 'is,oos', '--json', '/t/o.json', '--datasets', 'BTC/USDT:1h']
...
def test_noise_argv_tail_threads_seed_and_direction():
    tail = asug.noise_argv_tail('sq', '{"kc_mult": 2}', 'futures', 'short', ['is', 'oos'], None, 500, 1066, 0.05, '/t/n.json')
    assert tail[:8] == ['--strategy', 'sq', '--registry', 'futures', '--windows', 'is,oos', '--resamples', '500']
...
def test_m6_argv_tail_repeats_allowed_regimes():
    m6c = {'baseline_config': '/cfg', 'strategy_id': 's', 'candidate_close': [{'name': 'atr_stop'}], 'candidate_stops': 'inherit', 'allowed_regimes': ['ranging_quiet', 'ranging_volatile']}
...
def test_m6_argv_tail_baseline_config_path():
    m6c = {'baseline_config': '/cfg.json', 'strategy_id': 's', 'candidate_close': [{'name': 'atr_stop'}]}
...
def test_m6_argv_tail_incumbent_close_path_omits_baseline():
    m6c = {'strategy_id': 'squeeze_momentum', 'incumbent_close': [{'name': 'tiered_tp_atr', 'params': {}}], 'candidate_close': [{'name': 'atr_stop', 'params': {'atr_mult': 2}}]}
    tail = asug.m6_argv_tail(m6c, 'spot', ['oos'], None, 100, 1066, '/t/m6.json')
...
def test_m5_argv_tail():
    ...
    assert tail == ['--strategies', 'sq', '--registry', 'spot', '--windows', 'oos', '--json', '/t/m5.json']

def _m6_payload(is_rows, oos_rows):

    def _mk(rows):
        return [{'dataset': ds, 'per_regime': {'all': {'n': n, 'paired_delta': {'mean': mean, 'signed_rank': {'p_value': p}}}}} for ds, mean, n, p in rows]
    return {'results': {'is': _mk(is_rows), 'oos': _mk(oos_rows)}}

def test_m6_window_rollup_matches_paired_n_weighting():
    payload = _m6_payload(is_rows=[('BTC 1h', 0.1, 100, 0.01), ('ETH 1h', -0.2, 50, 0.3)], oos_rows=[('BTC 1h', 0.05, 40, 0.2)])
    ...
    assert roll['is']['per_dataset'][0] == {'dataset': 'BTC 1h', 'mean': 0.1, 'n': 100, 'p': 0.01}

def test_m6_rollup_skips_none_mean_datasets():
    payload = {'results': {'oos': [{'dataset': 'A', 'per_regime': {'all': {'n': 10, 'paired_delta': {'mean': None, 'signed_rank': {'p_value': None}}}}}, {'dataset': 'B', 'per_regime': {'all': {'n': 20, 'paired_delta': {'mean': 0.3, 'signed_rank': {'p_value': 0.04}}}}}]}}
    roll = asug.m6_window_rollup(payload)
...
def test_extract_noise():
    payload = {'trade_level': {'verdict': 'distinguishable_positive', 'permutation': {'p_value': 0.012, 'mean': 0.4}, 'summary': {'n': 88}}}
    assert asug.extract_noise(payload) == {'verdict': 'distinguishable_positive', 'permutation_p': 0.012, 'mean': 0.4, 'n': 88}

def test_extract_m1():
    payload = {'window_scores': [{'window': 'is', 'verdict': 'pass', 'mean_sharpe': 1.2, 'mean_ddadj': 0.8}, {'window': 'oos', 'verdict': 'fail', 'mean_sharpe': -0.1, 'mean_ddadj': 0.0}]}
    out = asug.extract_m1(payload)
    assert out['is']['verdict'] == 'pass'
    assert out['oos']['verdict'] == 'fail'

def test_extract_m5_matches_strategy_row():
    payload = {'rows': [{'strategy': 'other', 'verdict': 'healthy'}, {'strategy': 'sq', 'verdict': 'graduate_m1', 'fee_drag_pp': 0.3, 'trades_per_year': 12.0, 'mean_gross_ret': 0.5, 'mean_net_ret': 0.2}]}
...
def test_apply_correction_matches_direct_bh_and_reports_threshold():
    tests = [{'candidate_key': 'a', 'harness': 'm6', 'p': p, 'effect_positive': True} for p in [0.001, 0.02, 0.04, 0.5]]
    ...
    if any(mask):
        assert corr['effective_threshold'] == max((p for p, ok in zip([t['p'] for t in tests], mask) if ok))
...
def test_bh_family_size_widens_denominator_and_is_stricter():
    ...
    assert benjamini_hochberg(pvals, 0.05, family_size=2) == benjamini_hochberg(pvals, 0.05)
...
def test_apply_correction_family_size_reports_m_and_tests_run():
    tests = [{'candidate_key': 'a', 'harness': 'm6', 'p': p, 'effect_positive': True} for p in [0.001, 0.02]]
    corr = asug.apply_family_correction(copy.deepcopy(tests), alpha=0.05, family_size=8)
    assert corr['m'] == 8
    assert corr['tests_run'] == 2
    assert corr['bonferroni_threshold'] == pytest.approx(0.05 / 8)
    strict = asug.apply_family_correction(copy.deepcopy(tests), alpha=0.05, family_size=8)
...
def test_apply_correction_family_size_below_produced_count_raises():
    tests = [{'candidate_key': 'a', 'harness': 'm6', 'p': p, 'effect_positive': True} for p in [0.001, 0.02, 0.04]]
    with pytest.raises(ValueError, match='family_size'):
        asug.apply_family_correction(tests, alpha=0.05, family_size=2)

def test_load_spec_accepts_positive_family_size():
    spec = asug.load_spec(_base_spec(correction={'method': 'benjamini_hochberg', 'alpha': 0.05, 'family_size': 40}), _STUDY_DIR)
...
def test_load_spec_rejects_bad_family_size(bad):
    with pytest.raises(ValueError, match='family_size'):
        asug.load_spec(_base_spec(correction={'method': 'benjamini_hochberg', 'family_size': bad}), _STUDY_DIR)

def test_collect_family_pvalues_dedupes_noise_and_excludes_m3_m5():
    e1 = {'key': 'a', 'kind': 'open', 'noise_family_key': 'K', 'results': {'m1_noise': {'data': {'permutation_p': 0.01, 'mean': 0.2}}, 'm3': {'data': {'x': 1}}, 'm5': {'data': {'salvage_verdict': 'healthy'}}}}
    e2 = {'key': 'b', 'kind': 'open', 'noise_family_key': 'K', 'results': {'m1_noise': {'data': {'permutation_p': 0.01, 'mean': 0.2}}}}
    e3 = {'key': 'c', 'kind': 'exit_ab', 'results': {'m6': {'data': {'oos': {'per_dataset': [{'dataset': 'BTC 1h', 'mean': 0.3, 'p': 0.02}]}}}}}
...
def _open_entry(key, noise=None, m1=None, harnesses=('m1_noise', 'm1'), fam=None):
    ...
    return {'key': key, 'kind': 'open', 'harnesses': list(harnesses), 'precondition_errors': [], 'noise_family_key': fam or f'fam::{key}', 'results': results}

def _noise_test(fam, key, p, bh_pass, positive=True):
    return {'candidate_key': key, 'harness': 'm1_noise', 'noise_family_key': fam, 'p': p, 'effect_positive': positive, 'bh_pass': bh_pass}

def test_verdict_run_failed_never_survivor():
    e = {'key': 'x', 'kind': 'open', 'precondition_errors': [], 'results': {'m1': {'status': 'failed'}}}
    assert asug.candidate_verdict(e, []) == 'run_failed'

def test_verdict_excluded_not_replayable():
    e = {'key': 'x', 'kind': 'exit_ab', 'precondition_errors': ['excluded_not_replayable'], 'results': {}}
    assert asug.candidate_verdict(e, []) == 'excluded_not_replayable'

def test_verdict_noise_gate_blocks_before_selectivity():
    e = _open_entry('x', noise={'verdict': 'no_positive_edge', 'mean': -0.1}, m1={'is': {'verdict': 'pass'}, 'oos': {'verdict': 'pass'}})
    assert asug.candidate_verdict(e, []) == 'noise_gate_blocked'

def test_verdict_open_survivor_requires_bh_survival():
    e = _open_entry('x', fam='F', noise={'verdict': 'distinguishable_positive', 'mean': 0.3}, m1={'is': {'verdict': 'pass'}, 'oos': {'verdict': 'pass'}})
...
def test_verdict_open_m1_fail_is_incumbent_stands():
    e = _open_entry('x', fam='F', noise={'verdict': 'distinguishable_positive', 'mean': 0.3}, m1={'is': {'verdict': 'pass'}, 'oos': {'verdict': 'fail'}})
    tests = [_noise_test('F', 'x', 0.001, True)]
...
def test_gated_siblings_share_noise_bh_verdict():
    ...
    siblings = [_open_entry(k, fam=fam, noise=noise, m1=m1_pass) for k in ('baseline', 'adx_gate', 'comp_gate')]
...
def test_distinct_param_families_keep_independent_noise_tests():
    e_a = {'key': 'a', 'kind': 'open', 'noise_family_key': 'famA', 'results': {'m1_noise': {'data': {'permutation_p': 0.01, 'mean': 0.2}}}}
    e_b = {'key': 'b', 'kind': 'open', 'noise_family_key': 'famB', 'results': {'m1_noise': {'data': {'permutation_p': 0.3, 'mean': 0.1}}}}
...
def _ab_entry(key, is_pooled, oos_pooled):
    return {'key': key, 'kind': 'exit_ab', 'precondition_errors': [], 'results': {'m6': {'status': 'ok', 'data': {'is': {'pooled_delta_net_pct_per_entry': is_pooled}, 'oos': {'pooled_delta_net_pct_per_entry': oos_pooled}}}}}

def test_verdict_m6_survivor_needs_bh_positive_and_no_contradiction():
    e = _ab_entry('x', 0.2, 0.15)
    tests = [{'candidate_key': 'x', 'harness': 'm6', 'p': 0.01, 'effect_positive': True, 'bh_pass': True}]
    assert asug.candidate_verdict(e, tests) == 'survivor'

def test_verdict_m6_significant_contradiction_blocks():
    e = _ab_entry('x', 0.2, 0.15)
    tests = [{'candidate_key': 'x', 'harness': 'm6', 'p': 0.01, 'effect_positive': True, 'bh_pass': True}, {'candidate_key': 'x', 'harness': 'm6', 'p': 0.02, 'effect_positive': False, 'bh_pass': False}]
    assert asug.candidate_verdict(e, tests) == 'incumbent_stands'

def test_verdict_m6_positive_uncorrected_only():
    e = _ab_entry('x', 0.2, 0.15)
    tests = [{'candidate_key': 'x', 'harness': 'm6', 'p': 0.03, 'effect_positive': True, 'bh_pass': False}]
    assert asug.candidate_verdict(e, tests) == 'positive_uncorrected_only'
...
def test_rank_survivors_first_failed_still_present():
    entries = [{'key': 'loser', 'verdict': 'incumbent_stands', 'results': {}}, {'key': 'win', 'verdict': 'survivor', 'results': {}}, {'key': 'broke', 'verdict': 'run_failed', 'results': {}}]
...
def test_format_shortlist_has_correction_line_context_label_and_footer():
    report = {'study': 't', 'exploratory': False, 'correction': {'method': 'benjamini_hochberg', 'alpha': 0.05, 'm': 3, 'effective_threshold': 0.01, 'bonferroni_threshold': 0.0167, 'n_survivors': 1}, 'ranked': [{'key': 'win', 'verdict': 'survivor', 'limitations': [], 'results': {'m5': {'data': {'salvage_verdict': 'graduate_m1'}}}}]}
...
def test_reproduction_command_uses_relative_harness_paths():
    entry = {'key': 'x', 'results': {'m1': {'argv_tail': ['--candidate-json', '/t/c.json', '--registry', 'spot']}}}
...
def _survivor_entry(**results_over):
    results = {'m1_noise': {'status': 'ok', 'data': {'verdict': 'distinguishable_positive', 'permutation_p': 0.001, 'mean': 0.5, 'n': 80}}, 'm1': {'status': 'ok', 'data': {'is': {'verdict': 'pass'}, 'oos': {'verdict': 'pass'}}}}
    results.update(results_over)
    return {'key': 'c1', 'kind': 'open', 'limitations': [], 'precondition_errors': [], 'noise_family_key': 'fam', 'results': results}

def _mc_ok_run():
    return {'status': 'ok', 'data': {'oos': {'per_dataset': {}, 'worst': {'permute': {'p_dd_ge_kill_switch': 0.42, 'p95_max_dd': 61.0, 'p_final_below_start': 0.3}}}}}
...
def test_mc_absent_present_and_failed_all_yield_the_same_verdict():
    ...
    verdicts = {name: asug.candidate_verdict(e, _family_tests(e)) for name, e in (('absent', absent), ('present', present), ('failed', failed))}
    assert verdicts == {'absent': 'survivor', 'present': 'survivor', 'failed': 'survivor'}

def test_gate_relevant_results_drops_only_advisory_harnesses():
    entry = _survivor_entry(mc={'status': 'failed'}, m5={'status': 'ok', 'data': {}})
    gate = asug.gate_relevant_results(entry)
    assert set(gate) == {'m1_noise', 'm1', 'm5'}
    assert 'mc' not in gate
    assert asug.candidate_verdict(_survivor_entry(m5={'status': 'failed'}), []) == 'run_failed'
...
def test_mc_contributes_no_pvalue_and_leaves_bh_family_size_unchanged():
    ...
    assert asug.apply_family_correction(with_mc, 0.05)['m'] == asug.apply_family_correction(without, 0.05)['m']
    assert all((t['harness'] != 'mc' for t in with_mc))

def test_mc_argv_tail_threads_the_candidate_json_not_a_bare_strategy():
    tail = asug.mc_argv_tail('/t/c.json', 'spot', ['is', 'oos'], ['BTC/USDT:1h'], 500, 7, {}, '/t/o.json')
...
def test_mc_argv_tail_threshold_sources_are_exclusive():
    explicit = asug.mc_argv_tail('/c', 'spot', ['is'], None, 10, 1, {'kill_switch_pct': 30}, '/o')
    assert explicit[explicit.index('--kill-switch-pct') + 1] == '30'
    assert '--config' not in explicit
    from_cfg = asug.mc_argv_tail('/c', 'spot', ['is'], None, 10, 1, {'config': '/cfg.json', 'strategy_id': 'hl-x'}, '/o')
...
def test_load_spec_rejects_both_mc_threshold_sources():
    with pytest.raises(ValueError, match='mutually exclusive'):
        asug.load_spec(_base_spec(mc={'kill_switch_pct': 30, 'config': 'c.json', 'strategy_id': 'x'}), _STUDY_DIR)
...
def test_load_spec_resolves_mc_config_against_the_spec_dir():
    spec = asug.load_spec(_base_spec(mc={'config': 'cfg.json', 'strategy_id': 'hl-x'}), _STUDY_DIR)
...
def _mc_payload():
    ...
    def leg(window, ds, p_ks, p95, p_down, status='ok'):
        return {'window': window, 'dataset': ds, 'status': status, 'n_trades': 20, 'schemes': [{'scheme': 'permute', 'p_dd_ge_kill_switch': p_ks, 'max_dd_pct_percentiles': {'p5': 1.0, 'p50': 10.0, 'p95': p95}, 'p_final_below_start': p_down}]}
    return {'legs': [leg('is', 'BTC/USDT 1h', 0.1, 30.0, 0.2), leg('oos', 'BTC/USDT 1h', 0.3, 45.0, 0.25), leg('oos', 'ETH/USDT 4h', 0.55, 70.0, 0.4)]}
...
def test_extract_mc_keys_by_window_and_takes_the_worst_dataset():
    ...
    assert worst == {'p_dd_ge_kill_switch': 0.55, 'p95_max_dd': 70.0, 'p_final_below_start': 0.4}
    assert set(out['oos']['per_dataset']) == {'BTC/USDT 1h', 'ETH/USDT 4h'}

def test_extract_mc_tolerates_no_data_legs_and_missing_percentiles():
    payload = {'legs': [{'window': 'oos', 'dataset': 'BTC/USDT 1h', 'status': 'no_data', 'n_trades': 0, 'schemes': []}, {'window': 'oos', 'dataset': 'ETH/USDT 1h', 'status': 'ok', 'n_trades': 3, 'schemes': [{'scheme': 'permute', 'p_dd_ge_kill_switch': 0.2, 'max_dd_pct_percentiles': {'p50': 5.0}, 'p_final_below_start': 0.1}]}]}
...
def test_format_shortlist_labels_mc_advisory_and_prints_the_oos_worst_case():
    report = {'study': 't', 'exploratory': False, 'correction': {'method': 'benjamini_hochberg', 'alpha': 0.05, 'm': 1, 'effective_threshold': 0.01, 'bonferroni_threshold': 0.05, 'n_survivors': 1}, 'ranked': [{'key': 'win', 'verdict': 'survivor', 'limitations': [], 'results': {'mc': {'data': asug.extract_mc(_mc_payload())}}}]}
    text = asug.format_shortlist(report)
...
def test_format_shortlist_omits_the_mc_segment_when_the_run_failed():
    report = {'study': 't', 'exploratory': False, 'correction': {'method': 'benjamini_hochberg', 'alpha': 0.05, 'm': 1, 'effective_threshold': None, 'bonferroni_threshold': None, 'n_survivors': 0}, 'ranked': [{'key': 'win', 'verdict': 'survivor', 'limitations': ['mc_run_failed'], 'results': {'mc': {'status': 'failed'}}}]}
    text = asug.format_shortlist(report)
...
def test_dry_run_prints_a_command_for_every_enabled_harness():
    spec = asug.load_spec(_base_spec(harnesses=['m1_noise', 'm1', 'm3', 'm5', 'mc']), _STUDY_DIR)
    ...
    for harness in ('m1_noise', 'm1', 'm3', 'm5', 'mc'):
        assert any((asug.HARNESS_REL[harness] in c for c in cmds)), f'dry-run omits {harness} — it would spawn it anyway'
...
def test_dry_run_with_default_harnesses_omits_mc():
    ...
    for harness in ('m1_noise', 'm1', 'm3', 'm5'):
        assert any((asug.HARNESS_REL[harness] in c for c in cmds)), f'dry-run omits {harness} — it would spawn it anyway'
    assert not any(('monte_carlo.py' in c for c in cmds)), "mc must not run when a spec doesn't opt into it (#1316)"
...
def test_candidate_json_is_rewritten_over_a_stale_file_from_a_prior_run(monkeypatch, tmp_path):
    ...
    stale.write_text(json.dumps({'name': 'STALE_STRATEGY', 'allowed_regimes': ['stale_gate']}))
...
def test_mc_also_reads_a_freshly_written_candidate_not_a_stale_one(monkeypatch, tmp_path):
    spec = _open_spec(['mc'])
    entry = asug.expand_candidates(spec)[0]
    (tmp_path / f"{entry['key']}.candidate.json").write_text(json.dumps({'name': 'STALE_STRATEGY'}))
    seen = _capture_candidate(monkeypatch, '--candidate-json')
    asug.run_open_entry(entry, spec, str(tmp_path), {}, {})
...
def test_candidate_json_is_written_once_per_run_and_shared_by_m1_and_mc(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(asug, '_run_harness', lambda h, tail, out: paths.append(tail[tail.index('--candidate-json') + 1]) or {'harness': h, 'argv_tail': tail, 'status': 'failed'})
    asug.run_open_entry(entry, spec, str(tmp_path), {}, {})
...
def _mc_data(**windows):
    ...
    for w, stats in windows.items():
        out[w] = {'per_dataset': {}, 'worst': {} if stats is None else {'permute': stats}}
    return out
_SCORED = {'p_dd_ge_kill_switch': 0.3, 'p95_max_dd': 40.0, 'p_final_below_start': 0.2}
_SCORED_OOS = {'p_dd_ge_kill_switch': 0.55, 'p95_max_dd': 70.0, 'p_final_below_start': 0.4}
_NO_TRADES = {'p_dd_ge_kill_switch': None, 'p95_max_dd': None, 'p_final_below_start': None}
