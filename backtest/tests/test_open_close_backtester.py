...
def test_backtester_open_close_columns_close_before_open():
    ...
    df = pd.DataFrame({'open': [100, 100, 110, 120, 120], 'close': [100, 110, 120, 120, 120], 'open_action': ['none', 'long', 'short', 'none', 'none'], 'close_fraction': [0, 0, 1, 0, 0]}, index=idx)
...
def test_backtester_close_fraction_columns_use_max_wins():
    idx = pd.date_range('2024-01-01', periods=5, freq='D')
    df = pd.DataFrame({'open': [100, 100, 100, 100, 100], 'close': [100, 100, 100, 100, 100], 'open_action': ['none', 'long', 'none', 'none', 'none'], 'close_fraction:slow': [0, 0, 0.25, 0, 0], 'close_fraction:fast': [0, 0, 0.5, 0, 0]}, index=idx)
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0)
    result = bt.run(df, save=False)
    assert result['total_trades'] == 2
...
def test_backtester_open_action_short_round_trip():
    ...
    df = pd.DataFrame({'open': [100, 100, 100, 90], 'close': [100, 100, 90, 80], 'open_action': ['none', 'short', 'none', 'none'], 'close_fraction': [0, 0, 0, 0]}, index=idx)
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0)
    result = bt.run(df, save=False)
...
def test_backtester_short_open_charges_commission_once():
    idx = pd.date_range('2024-01-01', periods=4, freq='D')
    df = pd.DataFrame({'open': [100, 100, 100, 100], 'close': [100, 100, 100, 100], 'open_action': ['none', 'short', 'none', 'none'], 'close_fraction': [0, 0, 1, 0]}, index=idx)
