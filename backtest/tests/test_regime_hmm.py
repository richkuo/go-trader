...
def test_stationary_distribution_sums_to_one():
    ...
    assert abs(pi.sum() - 1.0) < 1e-09 and (pi > 0).all()
    np.testing.assert_allclose(pi @ A, pi, atol=1e-09)
...
def test_fit_no_spurious_transition_across_midseries_nan():
    ...
    assert abs(A[0][1] - 0.25) < 1e-09
    assert abs(A[0][0] - 0.75) < 1e-09
...
def test_forward_filter_recovers_regime():
    ...
    assert (conf >= 0).all() and (conf <= 1.0 + 1e-09).all()
