...
def _leg(sharpe, ddadj=0.0, trades=5, return_pct=-1.0, max_dd_pct=-10.0):
    return {'sharpe': sharpe, 'ddadj': ddadj, 'trades': trades, 'return_pct': return_pct, 'max_dd_pct': max_dd_pct, 'bh_return_pct': -30.0}
...
def test_leg_from_results_collapses_backtester_dict():
    results = {'total_return_pct': -12.5, 'max_drawdown_pct': -25.0, 'sharpe_ratio': -0.9, 'total_trades': 17}
...
def test_incumbent_bars_per_dataset_median():
    legs = {'BTC/USDT 1h': {'a': _leg(-1.0, ddadj=-0.5), 'b': _leg(-2.0, ddadj=-1.0), 'c': _leg(-3.0, ddadj=-1.5)}}
...
def test_incumbent_bars_skips_missing_legs_and_empty_datasets():
    legs = {'BTC/USDT 1h': {'a': _leg(-1.0), 'b': None, 'c': _leg(-3.0)}, 'SOL/USDT 4h': {'a': None, 'b': None}}
    bars = ew.incumbent_bars(legs)
    assert bars['BTC/USDT 1h']['sharpe'] == pytest.approx(-2.0)
...
def test_score_degenerate_zero_trade_majority_rejected():
    legs = {'d1': _leg(-0.3, ddadj=-0.2, trades=4), 'd2': _leg(0.0, ddadj=0.0, trades=0), 'd3': _leg(0.0, ddadj=0.0, trades=0), 'd4': _leg(0.0, ddadj=0.0, trades=0)}
    score = ew.score_candidate(legs, _bars())
    assert score['degenerate']
    assert score['verdict'] == 'degenerate'

def test_score_trading_exactly_half_is_not_degenerate():
    legs = {'d1': _leg(-0.3, ddadj=-0.2, trades=4), 'd2': _leg(-0.3, ddadj=-0.2, trades=1), 'd3': _leg(0.0, ddadj=0.0, trades=0), 'd4': _leg(0.0, ddadj=0.0, trades=0)}
    score = ew.score_candidate(legs, _bars())
    assert not score['degenerate']
    assert score['verdict'] == 'pass'

def test_score_unscored_datasets_excluded_from_means():
    legs = {'d1': _leg(-0.3, ddadj=-0.2), 'd2': None, 'd3': _leg(-0.5, ddadj=-0.3)}
...
class _FakeRegistry:
    STRATEGY_REGISTRY = {'alternator': {'default_params': {'period': 2}, 'description': 'test'}}
...
def test_run_leg_returns_leg_metrics(monkeypatch):
    ...
    assert leg['trades'] > 0
    for key in ('sharpe', 'return_pct', 'max_dd_pct', 'ddadj', 'trades', 'bh_return_pct'):
        assert key in leg
...
def test_run_leg_empty_data_returns_none(monkeypatch):
    ...
    assert ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2023-01-01', '2024-01-01')) is None

def test_run_leg_threads_stop_loss_atr_mult(monkeypatch):
    df = _synthetic_df(n=240)
    import data_fetcher
    monkeypatch.setattr(data_fetcher, 'load_cached_data', lambda *a, **k: df, raising=True)
    plain = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None))
    stopped = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None), stop_loss_atr_mult=0.1)
    assert plain is not None and stopped is not None
    assert (stopped['return_pct'], stopped['max_dd_pct']) != (plain['return_pct'], plain['max_dd_pct']), 'stop_loss_atr_mult=0.1 produced an identical leg — the stop never reached the Backtester'
...
def test_run_leg_threads_allowed_regimes_and_blocks_entries(monkeypatch):
    ...
    monkeypatch.setattr(data_fetcher, 'load_cached_data', lambda *a, **k: df, raising=True)
    plain = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None))
    gated = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None), allowed_regimes=['trending_down'])
    assert plain is not None and plain.get('trades', 0) > 0
    assert gated is not None
    assert gated.get('trades', -1) == 0, 'allowed_regimes gate did not suppress entries when bar regime was never in the allowed set'
...
def test_validate_candidate_allows_short_with_close_refs():
    c = {'name': 'x', 'direction': 'short', 'close_strategies': [{'name': 'tp_at_pct', 'params': {}}]}
    assert ew.validate_candidate(c) is c

def test_validate_candidate_invert_signal_gated_by_type():
    assert ew.validate_candidate({'name': 'x', 'invert_signal': True})
    assert ew.validate_candidate({'name': 'x', 'invert_signal': True, 'type': 'manual'})
    with pytest.raises(ValueError, match='invert_signal'):
        ew.validate_candidate({'name': 'x', 'invert_signal': True, 'type': 'spot'})
...
def test_validate_candidate_accepts_allowed_regimes():
    ...
    assert ew.validate_candidate(c) is c
    c2 = {'name': 'x', 'direction': 'short', 'close_strategies': [{'name': 'atr_stop', 'params': {'atr_mult': 1.5}}], 'allowed_regimes': ['ranging', 'trending_down']}
...
def test_validate_candidate_accepts_legacy_regime_lookback():
    c = {'name': 'x', 'allowed_regimes': ['trending_up'], 'regime_period': 21, 'regime_adx_threshold': 25.0}
    assert ew.validate_candidate(c) is c
...
def test_validate_candidate_rejects_bad_regime_lookback():
    ...
    with pytest.raises(ValueError, match='windows spec owns'):
        ew.validate_candidate({'name': 'x', 'allowed_regimes': ['trending_up'], 'regime_windows_spec': {'medium': {'classifier': 'adx', 'period': 14}}, 'regime_period': 21})
    for bad in (True, 1, '14', 14.0):
        with pytest.raises(ValueError, match='regime_period'):
            ew.validate_candidate({'name': 'x', 'allowed_regimes': ['trending_up'], 'regime_period': bad})
    for bad in (True, 0, -5, '20'):
        with pytest.raises(ValueError, match='regime_adx_threshold'):
            ew.validate_candidate({'name': 'x', 'allowed_regimes': ['trending_up'], 'regime_adx_threshold': bad})
...
def test_run_candidate_leg_threads_regime_lookback(monkeypatch):
    ...
    cand = {'name': 'x', 'allowed_regimes': ['trending_up'], 'regime_period': 21, 'regime_adx_threshold': 25.0}
    ew.run_candidate_leg(_FakeRegistry(), cand, 'BTC/USDT', '1h', ('2026-01-01', None))
    assert seen['regime_period'] == 21
    assert seen['regime_adx_threshold'] == 25.0
    seen.clear()
    ew.run_candidate_leg(_FakeRegistry(), {'name': 'x'}, 'BTC/USDT', '1h', ('2026-01-01', None))
...
def test_run_leg_threads_regime_windows_spec_composite_gate(monkeypatch):
    ...
    plain = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None))
    gated = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None), allowed_regimes=['trending_up'], regime_windows_spec=spec)
    assert plain is not None and plain.get('trades', 0) > 0
    assert gated is not None
    assert gated.get('trades', -1) == 0, 'composite windows spec did not reach the Backtester — the gate classified with the legacy ADX vocabulary and let entries through'
...
def test_validate_candidate_normalizes_regime_windows_spec():
    ...
    c2 = {'name': 'x', 'regime_windows_spec': {'medium': {'classifier': 'composite', 'period': 14}}, 'allowed_regimes': ['trending_up_clean']}
    assert ew.validate_candidate(c2) is c2
...
def test_validate_candidate_rejects_malformed_regime_windows_spec():
    ...
    with pytest.raises(ValueError, match='reserved'):
        ew.validate_candidate({'name': 'x', 'regime_windows_spec': {'regime': 14}})
    with pytest.raises(ValueError, match='regime_windows_spec'):
        ew.validate_candidate({'name': 'x', 'regime_windows_spec': {'medium': 'fast'}})
...
def test_validate_candidate_stop_owners_mutually_exclusive():
    ...
    with pytest.raises(ValueError, match='mutually exclusive'):
        ew.validate_candidate({'name': 'x', 'stop_loss_atr_mult': 2.0, 'trailing_stop_atr_mult': 2.5})

def test_evaluate_window_validates_before_any_work():
    with pytest.raises(ValueError, match='silently dropped'):
        ew.evaluate_window(None, {'name': 'x', 'direction': 'both'}, [('BTC/USDT', '1h')], 'oos', 1000.0, {})

def test_validate_candidate_accepts_regime_directional_policy():
    c = {'name': 'x', 'direction': 'both', 'close_strategies': [{'name': 'atr_stop', 'params': {'atr_mult': 2.0}}], 'regime_directional_policy': {'trend_regime': {'trending_up': {'direction': 'long'}, 'trending_down': {'direction': 'both'}}}}
    assert ew.validate_candidate(c) is c
    assert c['regime_directional_policy']['trend_regime']['trending_up'] == {'direction': 'long', 'invert_signal': False}
...
def test_validate_candidate_rejects_malformed_regime_directional_policy():
    with pytest.raises(ValueError, match='regime_directional_policy'):
        ew.validate_candidate({'name': 'x', 'regime_directional_policy': 'trending_up'})
    with pytest.raises(ValueError, match='trend_regime'):
        ew.validate_candidate({'name': 'x', 'regime_directional_policy': {'trending_up': {'direction': 'long'}}})
    with pytest.raises(ValueError, match='regime_directional_policy'):
        ew.validate_candidate({'name': 'x', 'regime_directional_policy': {'trend_regime': {'trending_up': {'direction': 'flat'}}}})
...
def test_validate_candidate_rejects_both_state_policy_without_close_refs():
    with pytest.raises(ValueError, match='close_strategies'):
        ew.validate_candidate({'name': 'x', 'regime_directional_policy': {'trend_regime': {'trending_down': {'direction': 'both'}}}})
...
def test_run_leg_threads_regime_directional_policy_research_certified(monkeypatch):
    ...
    leg = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None), regime_directional_policy=policy)
    assert leg is not None and calls
    kw = calls[0]
...
def test_run_leg_policy_switches_plain_path_side(monkeypatch):
    ...
    monkeypatch.setattr(data_fetcher, 'load_cached_data', lambda *a, **k: df, raising=True)
    plain = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None))
    gated = ew.run_leg(_FakeRegistry(), 'alternator', None, 'BTC/USDT', '1h', ('2026-01-01', None), regime_directional_policy={'trend_regime': {'trending_up': {'direction': 'short'}}})
    assert plain is not None and plain.get('trades', 0) > 0
    assert gated is not None and gated.get('trades', 0) > 0
    assert (gated['return_pct'], gated['max_dd_pct']) != (plain['return_pct'], plain['max_dd_pct']), 'trending_up→short policy produced a leg identical to the ungated long run — the policy never reached the Backtester'
...
def test_evaluate_window_threads_regime_directional_policy(monkeypatch):
    ...
    def _fake_run_leg(reg, name, params, symbol, timeframe, window, **kwargs):
        seen.update(kwargs)
        return {'sharpe': 1.0, 'return_pct': 1.0, 'max_dd_pct': 1.0, 'ddadj': 1.0, 'trades': 1, 'bh_return_pct': 0.0, 'span_days': 30.0}
    monkeypatch.setattr(ew, 'run_leg', _fake_run_leg, raising=True)
    monkeypatch.setattr(ew, 'incumbent_bars', lambda legs: {}, raising=True)
    monkeypatch.setattr(ew, 'compute_incumbent_legs', lambda *a, **k: {}, raising=True)
    policy = {'trend_regime': {'trending_up': {'direction': 'long'}}}
    cand = {'name': 'x', 'direction': 'both', 'close_strategies': [{'name': 'atr_stop', 'params': {'atr_mult': 2.0}}], 'regime_directional_policy': policy}
    ew.evaluate_window(None, cand, [('BTC/USDT', '1h')], 'oos', 1000.0, {})
    assert seen.get('regime_directional_policy') == {'trend_regime': {'trending_up': {'direction': 'long', 'invert_signal': False}}}
