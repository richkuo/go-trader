...
def test_epsilon_squared_clamps_and_guards_degenerate_shapes():
    assert np.isnan(M.epsilon_squared(5.0, 3, 3))
    assert np.isnan(M.epsilon_squared(5.0, 10, 0))
    assert M.epsilon_squared(1000000000.0, 100, 3) == 1.0
...
def test_inject_effect_unknown_state_defaults_to_unit_ratio():
    labels = np.array(['a', 'z'], dtype=object)
...
def test_block_bootstrap_length_and_membership():
    ...
    assert len(out) == 7
    assert set(out.tolist()).issubset(set(src.tolist()))
...
def _synthetic_cell(n=600, block_len=6, seed=0):
    ...
    labels = np.repeat(rng.integers(0, 3, size=n // block_len + 1).astype(object), block_len)[:n]
    fwd = np.abs(rng.normal(1.0, 0.3, size=n)) + 0.1
    return (labels, fwd)
...
def test_simulate_power_null_is_near_alpha_and_monotone():
    ...
    powers = M.simulate_power(labels, fwd, block_len=6, profile=prof, lam_grid=lam_grid, n_sim=120, n_perm=99, alpha=0.05, seed=7)
    assert powers[0.0] <= 0.2
...
def _cell(symbol, tf, window, p, *, knife_edge=False, seed_stable=True):
    return {'symbol': symbol, 'timeframe': tf, 'window': window, 'p_value': p, 'knife_edge': knife_edge, 'seed_stable': seed_stable}

def test_replicates_requires_primary_breadth_and_two_symbols():
    alpha = 0.01
    cells = [_cell('BTC/USDT', '1h', '2023', 0.001), _cell('BTC/USDT', '1h', '2024', 0.002), _cell('ETH/USDT', '1h', '2023', 0.003), _cell('SOL/USDT', '1h', '2023', 0.5)]
...
def test_primary_only_fails_breadth():
    alpha = 0.01
    cells = [_cell('BTC/USDT', '1h', '2023', 0.001), _cell('ETH/USDT', '1h', '2023', 0.5), _cell('SOL/USDT', '1h', '2023', 0.5), _cell('ETH/USDT', '4h', '2024', 0.5)]
    v = M.replication_verdict(cells, alpha)
    assert v['primary_met'] is True
    assert v['breadth_met'] is False
    assert v['replicates'] is False

def test_breadth_without_primary_fails():
    alpha = 0.01
    cells = [_cell('ETH/USDT', '1h', '2023', 0.001), _cell('SOL/USDT', '1h', '2023', 0.001), _cell('BTC/USDT', '1h', '2023', 0.5)]
    v = M.replication_verdict(cells, alpha)
...
def test_knife_edge_primary_blocks_replication():
    alpha = 0.01
    cells = [_cell('BTC/USDT', '1h', '2023', 0.001, knife_edge=True), _cell('ETH/USDT', '1h', '2023', 0.002)]
    v = M.replication_verdict(cells, alpha)
    assert v['primary_met'] is False
    assert v['replicates'] is False
...
def test_seed_unstable_primary_blocks_replication():
    alpha = 0.01
    cells = [_cell('BTC/USDT', '1h', '2023', 0.001, seed_stable=False), _cell('ETH/USDT', '1h', '2023', 0.002)]
    v = M.replication_verdict(cells, alpha)
    assert v['primary_met'] is False
    assert v['replicates'] is False
...
def test_single_symbol_breadth_fails_symbols():
    alpha = 0.01
    cells = [_cell('BTC/USDT', '1h', '2023', 0.001), _cell('BTC/USDT', '4h', '2023', 0.001), _cell('BTC/USDT', '1h', '2024', 0.001), _cell('ETH/USDT', '1h', '2023', 0.5)]
    v = M.replication_verdict(cells, alpha)
...
def _fake_df(index):
    import pandas as pd
    n = len(index)
    return pd.DataFrame({'open': np.ones(n), 'high': np.ones(n), 'low': np.ones(n), 'close': np.ones(n), 'volume': np.ones(n)}, index=index)
...
def test_load_cell_rejects_duplicate_timestamps(monkeypatch):
    ...
    dup_index = base.insert(10, base[9])
...
def test_load_cell_rejects_non_monotonic_index(monkeypatch):
    ...
    base[20], base[21] = (base[21], base[20])
...
def test_load_cell_on_cached_data_if_available():
    ...
    rep = run_window('BTC/USDT', '1h', 'oos', model=None, horizons=(M.PRIMARY_HORIZON,), target='volatility', n_perm=99, seed=0)
