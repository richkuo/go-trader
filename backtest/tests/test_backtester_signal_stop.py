...
def _df_with_signal(closes, signals, atr=None):
    ...
    df = pd.DataFrame({'open': closes, 'high': closes + 0.5, 'low': closes - 0.5, 'close': closes, 'volume': np.full(n, 1000.0), 'signal': np.asarray(signals, dtype=float)}, index=idx)
    if atr is not None:
        df['atr'] = float(atr)
    return df
...
def test_stop_fills_next_bar_open_not_same_bar():
    ...
    n = len(closes)
    idx = pd.date_range('2024-01-01', periods=n, freq='D')
    df = pd.DataFrame({'open': np.array(opens, float), 'high': np.array(closes, float) + 0.5, 'low': np.minimum(opens, closes) - 0.5, 'close': np.array(closes, float), 'volume': np.full(n, 1000.0), 'signal': np.array([1, 0, 0, 0, 0], float), 'atr': np.full(n, 2.0)}, index=idx)
