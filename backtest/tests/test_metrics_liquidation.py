...
def test_deeper_blowup_never_ranks_above_shallower():
    ...
    for key in ('sharpe_ratio', 'total_return_pct', 'max_drawdown_pct', 'volatility_pct', 'sortino_ratio'):
...
def test_short_leg_blowup_end_to_end():
    ...
    df = pd.DataFrame({'open': closes, 'high': closes + 0.5, 'low': closes - 0.5, 'close': closes, 'volume': np.full(n, 1000.0), 'signal': np.array([-1, 0, 0, 0, 0, 0], dtype=float)}, index=idx)
    bt = Backtester(initial_capital=10000.0, direction='short', commission_pct=0.0, slippage_pct=0.0)
    results = bt.run(df, strategy_name='x', symbol='BTC/USDT', timeframe='1d', save=False)
...
def _results(ret=-100.0, dd=-100.0, sharpe=-2.0, trades=3, liquidated=True):
    return {'total_return_pct': ret, 'max_drawdown_pct': dd, 'sharpe_ratio': sharpe, 'total_trades': trades, 'liquidated': liquidated}
...
def test_score_candidate_counts_liquidated_legs_without_verdict_change():
    ...
    healthy = ew.leg_from_results(_results(ret=20.0, dd=-10.0, sharpe=1.5, liquidated=False))
    bar = {'sharpe': 0.5, 'ddadj': 0.5, 'n': 8}
    score = ew.score_candidate({'A 1h': blown, 'B 1h': healthy}, {'A 1h': bar, 'B 1h': bar})
...
def test_liquidated_legs_counts_every_blowup_including_unscored():
    ...
    healthy = ew.leg_from_results(_results(ret=20.0, dd=-10.0, sharpe=1.5, liquidated=False))
    score = ew.score_candidate({'A 1h': blown_no_bar, 'B 1h': healthy}, {'B 1h': {'sharpe': 0.5, 'ddadj': 0.5, 'n': 8}})
...
def _screen_leg_with(monkeypatch, net_liquidated, gross_liquidated):
    ...
    def fake_run_leg(reg, name, params, sym, tf, window, capital=0.0, direction=None, commission_pct=None, slippage_pct=None, **kw):
        ...
        return {'trades': 3, 'span_days': 30.0, 'return_pct': -100.0 if blown else 1.0, 'sharpe': -100.0 if blown else 0.5, 'liquidated': blown}
...
def _fa_leg(liquidated=False):
    return {'error': None, 'trades': 10, 'span_days': 365.0, 'net_ret': -100.0 if liquidated else 5.0, 'gross_ret': -100.0 if liquidated else 8.0, 'net_sharpe': -2.0 if liquidated else 1.0, 'liquidated': liquidated}

def test_aggregate_counts_liquidated_legs():
    row = fa.aggregate_strategy('blower', 'futures', [_fa_leg(True), _fa_leg(False)])
...
def test_render_markdown_liquidated_section_only_when_present():
    meta = {'command': 'uv run ...', 'registry': 'futures', 'windows_desc': '2023', 'datasets_desc': 'SOL/USDT 4h', 'capital': 1000.0, 'date': '2026-06-12'}
...
def test_leg_from_results_floors_ddadj_when_liquidated():
    ...
    survivor = ew.leg_from_results(_results(ret=-50.0, dd=-25.0, sharpe=-1.0, liquidated=False))
