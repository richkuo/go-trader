...
def _synthetic_df(start='2019-01-01', end='2026-03-01', freq='1D'):
    ...
    return pd.DataFrame({'open': base, 'high': base * 1.02, 'low': base * 0.98, 'close': base + np.cos(np.arange(n) / 5.0), 'volume': np.full(n, 1000.0)}, index=idx)
...
def _sma_config(params=None, sid='spot-sma-btc'):
    return {'config_version': 15, 'strategies': [{'id': sid, 'type': 'spot', 'platform': 'binanceus', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'sma_crossover', 'params': params if params is not None else {'fast_period': 18, 'slow_period': 50}}}]}

def _make_args(**over):
    base = dict(param=None, datasets=None, windows=['is', 'oos'], step_frac=0.25, neighborhood_steps=0, splits=5, capital=1000.0, optimize_metric='sharpe_ratio', alpha=0.05, max_candidates=64, jobs=1, dry_run=False, verbose=False)
    base.update(over)
...
def _canned_stage2(spec_path, out_json, out_dir, jobs, survivor_key='cand_0'):
    ...
    for c in spec['candidates']:
        ...
        ranked.append({'key': c['key'], 'verdict': verdict, 'candidate': c['candidate'], 'evidence': {}, 'limitations': []})
    return {'correction': {'method': 'benjamini_hochberg', 'alpha': 0.05, 'm': spec['correction']['family_size'], 'tests_run': len(spec['candidates']), 'effective_threshold': 0.01, 'n_survivors': 1}, 'ranked': ranked, '_exit_code': 0}

@pytest.mark.parametrize('raw,expected', [('10', 10), ('1.5', 1.5), ('true', True), ('false', False), ('us_open', 'us_open'), ('-3', -3)])
...
def test_build_search_grid_searched_frozen_override():
    ...
    grid = tl.build_search_grid(eff, default_row, override_grids={'slow_period': [45, 55]}, frozen={'fast_period'}, step_frac=0.25, n_steps=0, value_ok=lambda p, v: True)
...
def test_build_search_grid_value_ok_filters_but_keeps_live():
    ...
    grid = tl.build_search_grid(eff, default_row, {}, set(), 0.25, 0, value_ok=lambda p, v: False)
    assert grid['fast_period'] == [18]
...
def test_resolve_overrides_valid_and_freeze():
    ...
    og, fr = tl.resolve_overrides('s', {'fast_period': [10, 15]}, {'s': {'freeze': ['slow_period']}}, {'fast_period', 'slow_period'}, _check_value(reg))
...
def test_resolve_overrides_bad_value_refused_loudly():
    ...
    with pytest.raises(ValueError, match='constraint'):
        tl.resolve_overrides('s', {'fast_period': [-3]}, {}, {'fast_period', 'slow_period'}, _check_value(reg))

def test_resolve_overrides_freeze_and_override_conflict():
    reg = load_registry('spot')
    with pytest.raises(ValueError, match='both overridden and frozen'):
        tl.resolve_overrides('s', {'fast_period': [10]}, {'s': {'freeze': ['fast_period']}}, {'fast_period', 'slow_period'}, _check_value(reg))

def test_resolve_overrides_frozen_unknown_refused():
    reg = load_registry('spot')
    with pytest.raises(ValueError, match='unknown params'):
        tl.resolve_overrides('s', {}, {'s': {'freeze': ['ghost']}}, {'fast_period'}, _check_value(reg))
...
def test_param_changes_and_build_patch():
    changes = tl.param_changes({'fast_period': 10, 'slow_period': 50}, {'fast_period': 18, 'slow_period': 50})
    assert changes == {'fast_period': 10}
    patch = tl.build_patch('s', 'sma_crossover', {'fast_period': 10, 'slow_period': 50}, {'fast_period': 18, 'slow_period': 50})
    assert patch['strategy_id'] == 's'
    assert patch['open_strategy'] == {'name': 'sma_crossover', 'params': {'fast_period': 10, 'slow_period': 50}}
    assert patch['param_changes'] == {'fast_period': 10}

def test_build_candidate_carries_close_direction_stops():
    resolution = {'strategy_type': 'perps', 'direction': 'both', 'close_strategies': [{'name': 'tiered_tp_atr', 'params': {}}], 'stop_loss_atr_mult': 2.0, 'regime_enabled': True, 'allowed_regimes': ['trending_up']}
...
def test_build_candidate_dormant_gate_not_copied():
    resolution = {'strategy_type': 'perps', 'direction': 'long', 'allowed_regimes': ['trending_up'], 'regime_enabled': False, 'regime_windows_spec': {'primary': {'classifier': 'adx', 'period': 14}}}
...
def test_build_candidate_threads_live_adx_lookback():
    resolution = {'regime_enabled': True, 'allowed_regimes': ['trending_up'], 'regime_period': 21, 'regime_adx_threshold': 25.0}
    cand = tl.build_candidate('sma_crossover', {'fast_period': 9}, resolution)
    assert cand['allowed_regimes'] == ['trending_up']
...
def test_build_candidate_composite_gate_owns_lookback():
    ...
    resolution = {'regime_enabled': True, 'allowed_regimes': ['trending_up'], 'regime_period': 21, 'regime_adx_threshold': 25.0, 'regime_windows_spec': spec}
    cand = tl.build_candidate('sma_crossover', {'fast_period': 9}, resolution)
    assert cand['allowed_regimes'] == ['trending_up']
...
def test_stage1_skip_composite_only_when_gate_active():
    ...
    assert tl.stage1_skip_reason(active) == 'composite_regime_gate_unmodelable_in_walk_forward'
    assert tl.stage1_skip_reason(dormant) is None

@pytest.mark.parametrize('resolution,expected', [({'stop_loss_atr_regime': {'x': 1}}, 'unsupported_stop:stop_loss_atr_regime'), ({'stop_loss_pct': 0.05}, 'unsupported_stop:stop_loss_pct'), ({'regime_directional_policy': {'x': 1}}, 'unsupported_regime_directional_policy'), ({'profile_allocation': {'x': 1}}, 'unsupported_profile_allocation'), ({'invert_signal': True}, 'unsupported_invert_signal'), ({'risk_per_trade_pct': 1.0}, 'unsupported_risk_per_trade_pct'), ({'allow_scale_in': True}, 'unsupported_allow_scale_in'), ({'atr_method': 'wilder'}, 'unsupported_atr_method:wilder'), ({'regime_gate_on_failure': 'closed'}, 'unsupported_regime_gate_on_failure_closed'), ({'stop_loss_atr_mult': 2.0}, None), ({'atr_method': 'simple'}, None), ({'regime_gate_on_failure': 'open'}, None), ({}, None)])
def test_unsupported_reason(resolution, expected):
    assert tl.unsupported_reason(resolution) == expected

@pytest.mark.parametrize('resolution,expected', [({'direction': 'short'}, 'short_direction_long_only_seeder'), ({'direction': 'long', 'regime_enabled': True, 'regime_windows_spec': {'d': {'classifier': 'adx', 'period': 14}}}, 'composite_regime_gate_unmodelable_in_walk_forward'), ({'direction': 'long'}, None), ({'direction': 'both'}, None)])
...
def test_baseline_params_byte_match_load_strategy_config(tmp_path):
    ...
    res = tl.tune_strategy(path, 'spot-sma-btc', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(dry_run=True), {}, str(tmp_path))
...
def test_stage2_spec_family_size_is_searched_N_not_survivor_count(tmp_path, monkeypatch):
    ...
    res = tl.tune_strategy(path, 'spot-sma-btc', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(), {}, str(tmp_path))
...
def test_generated_candidates_pass_auto_suggest_load_spec(tmp_path):
    path = _write_config(tmp_path, _sma_config())
    tl.tune_strategy(path, 'spot-sma-btc', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(dry_run=True), {}, str(tmp_path))
...
_PROMOTION_BASELINE_KEYS = ('open_strategy', 'open_strategy_present', 'user_defaults', 'user_defaults_present', 'user_close_defaults', 'user_close_defaults_present')
...
def test_full_main_writes_versioned_artifact_and_progress(tmp_path, monkeypatch):
    ...
    rc = tl.main(['--config', path, '--strategy', 'spot-sma-btc', '--out-dir', str(tmp_path), '--json', out, '--jobs', '1'])
    assert rc == 0
    ...
    assert s['promotion_baseline']['open_strategy'] == {'name': 'sma_crossover', 'params': {'fast_period': 18, 'slow_period': 50}}
...
def test_promotion_baseline_raw_fidelity_with_user_defaults(tmp_path):
    open_block = {'name': 'sma_crossover', 'params': {'fast_period': 18}}
    user_defaults = {'close': {'trailing_tp_ratchet': {'tp_tiers': [{'atr_multiple': 2.0, 'trailing_mult_after': 1.5, 'close_fraction': 0.5}]}}}
    cfg = {'config_version': 15, 'user_defaults': user_defaults, 'strategies': [{'id': 'spot-sma-btc', 'type': 'spot', 'platform': 'binanceus', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': open_block, 'close_strategy': {'name': 'trailing_tp_ratchet', 'params': {'use_defaults': True}}, 'trailing_stop_atr_mult': 3.0}]}
    path = _write_config(tmp_path, cfg)
...
def test_promotion_baseline_presence_matrix(tmp_path):
    ...
    def _load(subdir, cfg, sid, **kw):
        d = tmp_path / subdir
        d.mkdir()
        return load_strategy_config(_write_config(d, cfg), sid, include_promotion_baseline=True, **kw)
    kwargs = _load('args', {'config_version': 15, 'strategies': [{'id': 'spot-args', 'type': 'spot', 'platform': 'binanceus', 'args': ['sma_crossover', 'BTC/USDT', '1d']}]}, 'spot-args')
    ...
    b = _load('empty_ud', {'config_version': 15, 'user_defaults': {}, 'strategies': [{'id': 'spot-empty-ud', 'type': 'spot', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'sma_crossover', 'params': {}}}]}, 'spot-empty-ud')['promotion_baseline']
    assert b['user_defaults'] == {} and b['user_defaults_present'] is True
    b = _load('null_open', {'config_version': 15, 'strategies': [{'id': 'spot-null-open', 'type': 'spot', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': None}]}, 'spot-null-open')['promotion_baseline']
    assert b['open_strategy'] is None and b['open_strategy_present'] is True
    kwargs = _load('empty_name', {'config_version': 15, 'strategies': [{'id': 'spot-empty-name', 'type': 'spot', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': '', 'params': {'fast_period': 18}}}]}, 'spot-empty-name')
    b = kwargs['promotion_baseline']
    assert b['open_strategy'] == {'name': '', 'params': {'fast_period': 18}}
    assert kwargs['open_strategy']['name'] == 'sma_crossover'
    legacy = {'trailing_tp_ratchet': {'tp_tiers': [{'atr_multiple': 2.0, 'trailing_mult_after': 1.5, 'close_fraction': 0.5}]}}
    b = _load('legacy', {'config_version': 15, 'user_close_defaults': legacy, 'strategies': [{'id': 'hl-legacy', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['tema_cross_bd', 'BTC', '1h'], 'open_strategy': {'name': 'tema_cross_bd', 'params': {}}, 'trailing_stop_atr_mult': 3.0, 'close_strategy': {'name': 'trailing_tp_ratchet', 'params': {'use_defaults': True}}}]}, 'hl-legacy', inject_user_defaults=True)['promotion_baseline']
    assert b['user_defaults'] is None and b['user_defaults_present'] is False
...
def test_promotion_baseline_opt_in_spread_contract(tmp_path):
    ...
    kwargs_flag = load_strategy_config(path, 'spot-sma-btc', include_promotion_baseline=True)
...
def test_promotion_baseline_deepcopy_no_alias(tmp_path):
    from run_backtest import load_strategy_config
    path = _write_config(tmp_path, _sma_config(params={'fast_period': 18, 'slow_period': 50}))
    kwargs = load_strategy_config(path, 'spot-sma-btc', include_promotion_baseline=True)
...
def test_dry_run_emits_spec_without_running_stage2(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(tl, 'load_cached_data', lambda *a, **k: (_ for _ in ()).throw(AssertionError('no data fetch in --dry-run')))
    path = _write_config(tmp_path, _sma_config())
    res = tl.tune_strategy(path, 'spot-sma-btc', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(dry_run=True), {}, str(tmp_path))
...
def test_stage1_failed_marks_and_fleet_continues(tmp_path, monkeypatch):
    ...
    def fake_wfo(df, name, grid, **kw):
        ...
        return {'n_valid_folds': 3, 'param_grid_size': tl.grid_size(grid), 'window_results': [{'best_params': {'fast_period': 8, 'slow_period': 21}}]}
    monkeypatch.setattr(tl, 'walk_forward_optimize', fake_wfo)
    cfg = {'config_version': 15, 'strategies': [{'id': 'a-sma', 'type': 'spot', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'sma_crossover', 'params': {'fast_period': 18, 'slow_period': 50}}}, {'id': 'b-ema', 'type': 'spot', 'args': ['ema_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'ema_crossover', 'params': {'fast_period': 12, 'slow_period': 26}}}]}
    path = _write_config(tmp_path, cfg)
    out = str(tmp_path / 'art.json')
...
def test_short_direction_skips_stage1(tmp_path, monkeypatch):
    monkeypatch.setattr(tl, 'run_stage2', _canned_stage2)
    monkeypatch.setattr(tl, 'load_cached_data', lambda *a, **k: (_ for _ in ()).throw(AssertionError('short skips stage 1 — no data fetch')))
    cfg = {'config_version': 15, 'strategies': [{'id': 'hl-sma-short', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'direction': 'short', 'open_strategy': {'name': 'sma_crossover', 'params': {'fast_period': 18, 'slow_period': 50}}}]}
    path = _write_config(tmp_path, cfg)
    res = tl.tune_strategy(path, 'hl-sma-short', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(), {}, str(tmp_path))
    assert res['status'] == 'ranked'
    assert res['stage1']['ran'] is False
...
def test_constraint_invalid_combos_dropped_on_skip_path(tmp_path, monkeypatch):
    ...
    res = tl.tune_strategy(path, 'hl-sma-short', 'BTC/USDT', '1d', 'spot', load_registry('spot'), args, {}, str(tmp_path))
    assert res['status'] == 'ranked'
...
def test_constraint_invalid_combos_dropped_from_auto_neighborhood(tmp_path, monkeypatch):
    monkeypatch.setattr(tl, 'run_stage2', _canned_stage2)
    monkeypatch.setattr(tl, 'load_cached_data', lambda *a, **k: (_ for _ in ()).throw(AssertionError('short skips stage 1 — no data fetch')))
    cfg = {'config_version': 15, 'strategies': [{'id': 'hl-sma-short', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'direction': 'short', 'open_strategy': {'name': 'sma_crossover', 'params': {'fast_period': 40, 'slow_period': 50}}}]}
    path = _write_config(tmp_path, cfg)
    res = tl.tune_strategy(path, 'hl-sma-short', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(neighborhood_steps=1, step_frac=0.25), {}, str(tmp_path))
    assert res['status'] == 'ranked'
...
def test_bidirectional_runs_stage1_with_direction_both(tmp_path, monkeypatch):
    ...
    def fake_wfo(df, name, grid, **kw):
        ...
        return {'n_valid_folds': 3, 'param_grid_size': tl.grid_size(grid), 'window_results': [{'best_params': {'fast_period': 8, 'slow_period': 21}}]}
    monkeypatch.setattr(tl, 'walk_forward_optimize', fake_wfo)
    cfg = {'config_version': 15, 'strategies': [{'id': 'hl-sma-both', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'direction': 'both', 'open_strategy': {'name': 'sma_crossover', 'params': {'fast_period': 18, 'slow_period': 50}}, 'close_strategy': {'name': 'tiered_tp_atr', 'params': {'tp_tiers': [{'atr_multiple': 2.0, 'close_fraction': 1.0}]}}}]}
    path = _write_config(tmp_path, cfg)
    res = tl.tune_strategy(path, 'hl-sma-both', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(), {}, str(tmp_path))
    assert res['status'] == 'ranked'
    assert res['stage1']['ran'] is True
    assert seen['direction'] == 'both'

def test_disjoint_slice_no_data_refuses(monkeypatch):
    small = pd.DataFrame({'open': 100.0, 'high': 101.0, 'low': 99.0, 'close': 100.0, 'volume': 1000.0}, index=pd.date_range('2025-05-20', '2026-01-01', freq='1D'))
    monkeypatch.setattr(tl, 'load_cached_data', lambda *a, **k: small)
    with pytest.raises(ValueError, match='disjoint slice leaves only'):
        tl.run_stage1('sma_crossover', {'fast_period': [10, 20]}, {'direction': 'long'}, 'BTC/USDT', '1d', 'spot', '2025-06-10', 5, 1000.0, 'sharpe_ratio', False)
...
def test_config_strategy_entries_uses_trade_timeframe_not_regime(tmp_path):
    cfg = {'config_version': 15, 'regime': {'enabled': True, 'timeframe': '1d'}, 'strategies': [{'id': 's', 'type': 'spot', 'args': ['sma_crossover', 'BTC/USDT', '4h'], 'open_strategy': {'name': 'sma_crossover', 'params': {}}}]}
    path = _write_config(tmp_path, cfg)
...
def test_config_strategy_entries_accepts_ordered_strategy_subset(tmp_path):
    ...
    assert entries == [('second', 'ETH/USDT', '1h', 'spot'), ('first', 'BTC/USDT', '1d', 'spot')]
...
def test_main_auto_resolves_mixed_registry_per_strategy(tmp_path, monkeypatch):
    ...
    futures.update({'id': 'futures-one', 'type': 'futures', 'platform': 'topstep', 'args': ['breakout', 'ES', '1h'], 'open_strategy': {'name': 'breakout', 'params': {}}})
    ...
    def fake_tune(config_path, sid, symbol, timeframe, registry, reg_mod, args, overrides, out_dir):
    ...
    rc = tl.main(['--config', path, '--out-dir', str(tmp_path), '--json', str(tmp_path / 'mixed.json'), '--dry-run'])
    assert rc == 0
    assert loaded == ['spot', 'futures']
    assert tuned == [('spot-one', 'spot', 'spot'), ('futures-one', 'futures', 'futures')]

@pytest.mark.parametrize('override', ['spot', 'futures'])
def test_main_explicit_registry_overrides_whole_mixed_run(tmp_path, monkeypatch, override):
    cfg = _sma_config(sid='spot-one')
    ...
    loaded = []
    tuned = []
    monkeypatch.setattr(tl, 'load_registry', lambda registry: loaded.append(registry) or types.SimpleNamespace(name=registry))
    monkeypatch.setattr(tl, 'tune_strategy', lambda config_path, sid, symbol, timeframe, registry, reg_mod, args, overrides, out_dir: tuned.append((sid, registry)) or {'strategy_id': sid, 'status': 'dry_run'})
    rc = tl.main(['--config', path, '--registry', override, '--out-dir', str(tmp_path), '--json', str(tmp_path / f'{override}.json'), '--dry-run'])
    assert rc == 0
...
def test_regime_timeframe_mismatch_skipped_unsupported(tmp_path):
    cfg = {'config_version': 15, 'regime': {'enabled': True, 'timeframe': '1d', 'period': 14}, 'strategies': [{'id': 'hl-sma', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['sma_crossover', 'BTC/USDT', '4h'], 'allowed_regimes': ['trending'], 'open_strategy': {'name': 'sma_crossover', 'params': {}}}]}
    path = _write_config(tmp_path, cfg)
    res = tl.tune_strategy(path, 'hl-sma', 'BTC/USDT', '4h', 'spot', load_registry('spot'), _make_args(dry_run=True), {}, str(tmp_path))
...
def test_regime_disabled_stale_timeframe_still_supported(tmp_path):
    cfg = {'config_version': 15, 'regime': {'enabled': False, 'timeframe': '1d'}, 'strategies': [{'id': 's', 'type': 'spot', 'args': ['sma_crossover', 'BTC/USDT', '4h'], 'open_strategy': {'name': 'sma_crossover', 'params': {}}}]}
    path = _write_config(tmp_path, cfg)
    res = tl.tune_strategy(path, 's', 'BTC/USDT', '4h', 'spot', load_registry('spot'), _make_args(dry_run=True), {}, str(tmp_path))
    assert res['status'] == 'dry_run'
...
def test_wilder_atr_method_skipped_unsupported(tmp_path, atr_key, atr_val):
    cfg = {'config_version': 15, atr_key: atr_val, 'strategies': [{'id': 'hl-sma', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'sma_crossover', 'params': {}}}]}
    path = _write_config(tmp_path, cfg)
    res = tl.tune_strategy(path, 'hl-sma', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(dry_run=True), {}, str(tmp_path))
    assert res['status'] == 'unsupported'
    assert res['reason'] == 'unsupported_atr_method:wilder'

def test_all_failed_fleet_returns_nonzero(tmp_path):
    cfg = {'config_version': 13, 'strategies': [{'id': 'a', 'type': 'spot', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'sma_crossover', 'params': {}}}, {'id': 'b', 'type': 'spot', 'args': ['ema_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'ema_crossover', 'params': {}}}]}
    path = _write_config(tmp_path, cfg)
    rc = tl.main(['--config', path, '--out-dir', str(tmp_path), '--json', str(tmp_path / 'art.json'), '--dry-run'])
    assert rc == 1
    art = json.loads((tmp_path / 'art.json').read_text())
...
def test_mixed_fleet_with_one_success_returns_zero(tmp_path, monkeypatch):
    monkeypatch.setattr(tl, 'load_cached_data', lambda *a, **k: _synthetic_df())
    monkeypatch.setattr(tl, 'run_stage2', _canned_stage2)
    cfg = {'config_version': 15, 'strategies': [{'id': 'good', 'type': 'spot', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'sma_crossover', 'params': {'fast_period': 18, 'slow_period': 50}}}, {'id': 'bad-stop', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['sma_crossover', 'ETH/USDT', '1d'], 'stop_loss_pct': 0.05, 'open_strategy': {'name': 'sma_crossover', 'params': {}}}]}
    path = _write_config(tmp_path, cfg)
    rc = tl.main(['--config', path, '--out-dir', str(tmp_path), '--json', str(tmp_path / 'art.json'), '--jobs', '1'])
    assert rc == 0
    art = json.loads((tmp_path / 'art.json').read_text())
...
def test_all_benign_skip_fleet_returns_zero(tmp_path):
    cfg = {'config_version': 15, 'strategies': [{'id': 'hl-sma', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'stop_loss_pct': 0.05, 'open_strategy': {'name': 'sma_crossover', 'params': {}}}]}
    path = _write_config(tmp_path, cfg)
    rc = tl.main(['--config', path, '--out-dir', str(tmp_path), '--json', str(tmp_path / 'art.json'), '--dry-run'])
    assert rc == 0

def test_unsupported_stop_owner_skipped(tmp_path):
    cfg = {'config_version': 15, 'strategies': [{'id': 'hl-sma', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['sma_crossover', 'BTC/USDT', '1d'], 'open_strategy': {'name': 'sma_crossover', 'params': {}}, 'stop_loss_pct': 0.05}]}
    path = _write_config(tmp_path, cfg)
    res = tl.tune_strategy(path, 'hl-sma', 'BTC/USDT', '1d', 'spot', load_registry('spot'), _make_args(dry_run=True), {}, str(tmp_path))
    assert res['status'] == 'unsupported'
...
def test_overrides_unknown_strategy_id_refused(tmp_path):
    ...
    with pytest.raises(SystemExit, match='unknown strategy ids'):
        tl.main(['--config', path, '--overrides', str(ov), '--out-dir', str(tmp_path), '--dry-run'])
...
def test_override_grid_counts_toward_searched_family(tmp_path, monkeypatch):
    ...
    res = tl.tune_strategy(path, 'spot-sma-btc', 'BTC/USDT', '1d', 'spot', load_registry('spot'), args, {}, str(tmp_path))
    assert res['status'] == 'ranked'
...
def test_family_size_covers_baseline_against_real_bh_guard(tmp_path, monkeypatch):
    ...
    def stage2_with_real_guard(spec_path, out_json, out_dir, jobs):
        ...
        auto_suggest.apply_family_correction([{'p': 0.5} for _ in range(n_cand)], 0.05, family_size=fam)
        ranked = [{'key': c['key'], 'verdict': 'incumbent_stands', 'candidate': c['candidate'], 'evidence': {}, 'limitations': []} for c in spec['candidates']]
        return {'correction': {'m': fam, 'tests_run': n_cand, 'n_survivors': 0}, 'ranked': ranked, '_exit_code': 0}
    monkeypatch.setattr(tl, 'run_stage2', stage2_with_real_guard)
    cfg = {'config_version': 15, 'strategies': [{'id': 'hl-rsi-short', 'type': 'perps', 'platform': 'hyperliquid', 'args': ['rsi', 'BTC/USDT', '1d'], 'direction': 'short', 'open_strategy': {'name': 'rsi', 'params': {'period': 14}}}]}
    path = _write_config(tmp_path, cfg)
    args = _make_args(param=['period=10,20,28'])
    res = tl.tune_strategy(path, 'hl-rsi-short', 'BTC/USDT', '1d', 'spot', load_registry('spot'), args, {}, str(tmp_path))
    assert res['status'] == 'ranked'
    assert res['searched_family_size'] >= res['n_candidates']

def test_too_many_candidates_refused(tmp_path):
    path = _write_config(tmp_path, _sma_config())
    args = _make_args(dry_run=True, max_candidates=3, param=['fast_period=10,15,20,25', 'slow_period=40,50,60,80'])
    res = tl.tune_strategy(path, 'spot-sma-btc', 'BTC/USDT', '1d', 'spot', load_registry('spot'), args, {}, str(tmp_path))
