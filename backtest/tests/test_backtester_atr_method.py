...
def _base_config(global_atr=None, strategy_atr=None):
    sc = {'id': 'hl-temacb-btc', 'type': 'perps', 'platform': 'hyperliquid', 'open_strategy': {'name': 'tema_cross_bd'}, 'close_strategy': {'name': 'tiered_tp_atr', 'params': {'tp_tiers': [{'atr_multiple': 2.0, 'close_fraction': 1.0}]}}}
...
@pytest.mark.parametrize('raw,want', [('simple', 'simple'), ('wilder', 'wilder'), (' Wilder ', 'wilder'), ('', 'simple'), (None, 'simple')])
...
def _big_ohlcv(n=80, seed=3):
    ...
    df = pd.DataFrame({'open': close, 'high': high, 'low': low, 'close': close, 'volume': np.full(n, 100.0)}, index=pd.date_range('2024-01-01', periods=n, freq='1h'))
    df['signal'] = 0
...
def test_wilder_changes_stamped_entry_atr():
    ...
    for method in ('simple', 'wilder'):
        ...
        results[method] = res
    assert results['simple']['trades'] != results['wilder']['trades'] or results['simple']['metrics'] != results['wilder']['metrics']

def test_config_default_resolves_simple(tmp_path):
    kwargs = run_backtest.load_strategy_config(_write_config(tmp_path, _base_config()), 'hl-temacb-btc')
    assert kwargs['atr_method'] == 'simple'

def test_config_global_wilder_inherited(tmp_path):
    kwargs = run_backtest.load_strategy_config(_write_config(tmp_path, _base_config(global_atr='wilder')), 'hl-temacb-btc')
    assert kwargs['atr_method'] == 'wilder'

def test_config_per_strategy_wins_over_global(tmp_path):
    kwargs = run_backtest.load_strategy_config(_write_config(tmp_path, _base_config(global_atr='wilder', strategy_atr='simple')), 'hl-temacb-btc')
    assert kwargs['atr_method'] == 'simple'

def test_config_rejects_unknown_global_atr_method(tmp_path):
    with pytest.raises(ValueError, match='atr_method'):
        run_backtest.load_strategy_config(_write_config(tmp_path, _base_config(global_atr='rma')), 'hl-temacb-btc')

def test_config_rejects_unknown_per_strategy_even_with_valid_global(tmp_path):
    with pytest.raises(ValueError, match='atr_method'):
        run_backtest.load_strategy_config(_write_config(tmp_path, _base_config(global_atr='simple', strategy_atr='bogus')), 'hl-temacb-btc')
