...
def _row(symbol='BTC/USDT', timeframe='1h', classifier='composite', state='trending_up', window='oos', p=0.0001, policy_dir=1, sign_aligned=True):
    return {'classifier': classifier, 'symbol': symbol, 'timeframe': timeframe, 'window': window, 'horizon': 4, 'state': state, 'gap': 0.01, 'mean_fwd': 0.01, 'p_value': p, 'fdr_reject': True, 'policy_dir': policy_dir, 'sign_aligned': sign_aligned, 'candidate_edge': True}
...
def test_composite_sublabel_maps_to_canonical():
    art = certify_mod.certify([_row(state='trending_down_strong', policy_dir=-1)], generated_at=GEN_AT)
    assert len(art['certified']) == 1
    assert art['certified'][0]['states'] == {'trending_down': 'short'}
import pytest
import regime_1076_directional_premise as premise_mod
FULL_UNIVERSE = dict(symbols=premise_mod.DEFAULT_SYMBOLS, timeframes=premise_mod.DEFAULT_TIMEFRAMES, windows=premise_mod.DEFAULT_WINDOWS, classifiers=premise_mod.DEFAULT_CLASSIFIERS, horizons=premise_mod.DEFAULT_HORIZONS)

def test_screened_family_size_counts_directional_rows_only():
    rows = [_row(), _row(symbol='ETH/USDT'), _row(state='ranging', policy_dir=0, sign_aligned=False)]
...
def test_provenance_keys_pass_through_and_nest_under_criteria():
    ...
    art = certify_mod.certify([_row()], generated_at=GEN_AT, universe=FULL_UNIVERSE, data_sources=sources, n_perm=30000)
    c = art['criteria']
    ...
    assert set(art) == {'schema_version', 'generated_at', 'generator', 'source_evidence', 'criteria', 'default_ttl_days', 'certified'}
    assert art['schema_version'] == 1
...
def test_family_is_superset_true_when_a_symbol_is_added():
    assert certify_mod.family_is_superset(symbols=premise_mod.parse_symbols_arg('BTC/USDT,ETH/USDT,SOL/USDT,HYPE/USDC:USDC@hyperliquid'), timeframes=premise_mod.DEFAULT_TIMEFRAMES, windows=premise_mod.DEFAULT_WINDOWS, classifiers=premise_mod.DEFAULT_CLASSIFIERS, horizons=premise_mod.DEFAULT_HORIZONS)

def test_family_is_superset_ignores_the_exchange_suffix():
    assert certify_mod.family_is_superset(symbols=('BTC/USDT@kraken', 'ETH/USDT', 'SOL/USDT'), timeframes=premise_mod.DEFAULT_TIMEFRAMES, windows=premise_mod.DEFAULT_WINDOWS, classifiers=premise_mod.DEFAULT_CLASSIFIERS, horizons=premise_mod.DEFAULT_HORIZONS)

@pytest.mark.parametrize('axis', ['symbols', 'timeframes', 'windows', 'classifiers', 'horizons'])
...
def test_narrowed_run_refuses_to_write_the_repo_artifact():
    with pytest.raises(SystemExit) as exc:
        certify_mod.main(['--symbols', 'ETH/USDT', '--timeframes', '1h', '--classifiers', 'composite'])
...
def test_narrowed_run_may_write_a_research_artifact_elsewhere(tmp_path, monkeypatch):
    ...
    rc = certify_mod.main(['--symbols', 'ETH/USDT', '--timeframes', '1h', '--classifiers', 'composite', '--windows', 'oos', '--horizons', '4', '--n-perm', '1', '--out', str(out), '--report-out', str(report)])
    assert rc == 0
    art = json.loads(out.read_text())
    assert art['schema_version'] == 1
    assert art['criteria']['universe']['symbols'] == ['ETH/USDT']
    assert art['criteria']['screened_family_size'] == 1
    assert json.loads(report.read_text())['cell_verdicts'][0]['verdict'] == certify_mod.VERDICT_CERTIFIED
...
def test_repo_artifact_run_over_the_full_family_is_allowed(tmp_path, monkeypatch):
    ...
    rc = certify_mod.main(['--symbols', 'BTC/USDT,ETH/USDT,SOL/USDT,HYPE/USDC:USDC@hyperliquid', '--out', str(tmp_path / 'art.json'), '--report-out', str(tmp_path / 'rep.json')])
    assert rc == 0
...
def test_cell_verdict_no_directional_rows():
    v = certify_mod.cell_verdicts([_row(state='ranging', policy_dir=0, sign_aligned=False)])['BTC', '1h', 'composite']
...
def test_cell_verdicts_cover_every_screened_cell_not_only_certified_ones():
    rows = [_row(symbol='BTC/USDT'), _row(symbol='ETH/USDT', p=0.9), _row(symbol='HYPE/USDC:USDC', p=0.9, window='is')]
    v = certify_mod.cell_verdicts(rows)
    assert set(v) == {('BTC', '1h', 'composite'), ('ETH', '1h', 'composite'), ('HYPE', '1h', 'composite')}
    assert {e['global_bh_family_size'] for e in v.values()} == {3}

def test_cell_verdict_best_row_records_the_data_source():
    rows = [dict(_row(symbol='HYPE/USDC:USDC', window='is'), source='hyperliquid')]
...
def test_baseline_source_violations_is_empty_on_the_baseline_venue():
    sources = {'BTC/USDT': 'binanceus', 'ETH/USDT': 'binanceus', 'SOL/USDT': 'binanceus'}
    assert certify_mod.baseline_source_violations(sources, 'binanceus') == {}

def test_baseline_source_violations_ignores_added_symbols():
    sources = {'BTC/USDT': 'binanceus', 'ETH/USDT': 'binanceus', 'SOL/USDT': 'binanceus', 'HYPE/USDC:USDC': 'hyperliquid'}
    assert certify_mod.baseline_source_violations(sources, 'binanceus') == {}

def test_baseline_source_violations_reports_every_repointed_default():
    sources = {'BTC/USDT': 'kraken', 'ETH/USDT': 'coinbase', 'SOL/USDT': 'binanceus'}
    assert certify_mod.baseline_source_violations(sources, 'binanceus') == {'BTC/USDT': 'kraken', 'ETH/USDT': 'coinbase'}
...
def test_repointed_baseline_may_be_written_elsewhere(tmp_path, monkeypatch):
    ...
    rc = certify_mod.main(['--symbols', 'BTC/USDT@kraken,ETH/USDT,SOL/USDT', '--out', str(out), '--report-out', ''])
    assert rc == 0
...
def test_narrowed_run_refuses_when_only_the_report_targets_the_repo(monkeypatch):
    with pytest.raises(SystemExit) as exc:
        certify_mod.main(['--symbols', 'BTC/USDT', '--timeframes', '1h', '--classifiers', 'composite', '--out', '/tmp/research_cert.json'])
    assert 'narrowed family' in str(exc.value)
...
def test_empty_report_out_skips_cleanly_instead_of_refusing(tmp_path, monkeypatch):
    ...
    rc = certify_mod.main(['--symbols', 'ETH/USDT', '--timeframes', '1h', '--classifiers', 'composite', '--windows', 'oos', '--horizons', '4', '--n-perm', '1', '--out', str(out), '--report-out', ''])
    assert rc == 0
    assert out.exists()
...
def test_zero_coverage_run_refuses_and_leaves_the_artifact_untouched(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(certify_mod, 'DEFAULT_ARTIFACT', str(art))
    with pytest.raises(SystemExit) as exc:
        certify_mod.main(['--symbols', ','.join(premise_mod.DEFAULT_SYMBOLS), '--report-out', ''])
...
def test_under_resolved_run_refuses_the_repo_artifact(tmp_path, monkeypatch):
    monkeypatch.setattr(premise_mod, 'run', lambda *a, **k: [_row(p=0.5) for _ in range(400)])
    monkeypatch.setattr(certify_mod, 'DEFAULT_ARTIFACT', str(tmp_path / 'art.json'))
    with pytest.raises(SystemExit) as exc:
        certify_mod.main(['--symbols', ','.join(premise_mod.DEFAULT_SYMBOLS), '--n-perm', '500', '--report-out', ''])
...
def test_degenerate_research_run_written_elsewhere_is_allowed(tmp_path, monkeypatch):
    monkeypatch.setattr(premise_mod, 'run', lambda *a, **k: [])
    out = tmp_path / 'research.json'
    rc = certify_mod.main(['--symbols', 'BTC/USDT', '--timeframes', '1h', '--classifiers', 'composite', '--windows', 'oos', '--horizons', '4', '--allow-narrowed-family', '--out', str(out), '--report-out', str(tmp_path / 'r.json')])
    assert rc == 0
...
def test_allow_degenerate_run_is_separate_from_allow_narrowed_family(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(certify_mod, 'DEFAULT_ARTIFACT', str(tmp_path / 'art.json'))
    with pytest.raises(SystemExit) as exc:
        certify_mod.main(['--symbols', 'BTC/USDT', '--timeframes', '1h', '--classifiers', 'composite', '--windows', 'oos', '--horizons', '4', '--allow-narrowed-family', '--report-out', ''])
    assert 'NO directional rows' in str(exc.value)
    rc = certify_mod.main(['--symbols', 'BTC/USDT', '--timeframes', '1h', '--classifiers', 'composite', '--windows', 'oos', '--horizons', '4', '--allow-narrowed-family', '--allow-degenerate-run', '--report-out', ''])
    assert rc == 0
    assert json.loads((tmp_path / 'art.json').read_text())['certified'] == []
...
def test_cert_asset_collisions_flags_two_venues_for_one_asset():
    got = certify_mod.cert_asset_collisions(premise_mod.parse_symbols_arg('BTC/USDT,BTC/USDC:USDC@hyperliquid'))
    assert got == {'BTC': ['BTC/USDC:USDC', 'BTC/USDT']}

def test_cert_asset_collisions_accepts_distinct_assets():
    assert certify_mod.cert_asset_collisions(premise_mod.parse_symbols_arg('BTC/USDT,ETH/USDT,SOL/USDT,HYPE/USDC:USDC@hyperliquid')) == {}

def test_colliding_assets_refuse_before_any_data_access():
    with pytest.raises(SystemExit) as exc:
        certify_mod.main(['--symbols', 'BTC/USDT,ETH/USDT,SOL/USDT,BTC/USDC:USDC@hyperliquid'])
...
def test_certified_cell_does_not_display_a_wrong_signed_best_row():
    rows = [_row(p=1e-06, sign_aligned=False), _row(p=1e-05, sign_aligned=True)]
    v = certify_mod.cell_verdicts(rows)['BTC', '1h', 'composite']
    assert v['verdict'] == certify_mod.VERDICT_CERTIFIED
    assert v['best_row']['sign_aligned'] is True
    assert v['best_row']['p_value'] == 1e-05
    assert v['best_row_basis'] == certify_mod.BASIS_CERTIFIED_HELD_OUT
    assert v['min_p_value'] == 1e-06

def test_certified_cell_does_not_display_a_historical_window_best_row():
    rows = [_row(p=1e-06, window='2024'), _row(p=1e-05, window='oos')]
    v = certify_mod.cell_verdicts(rows)['BTC', '1h', 'composite']
    assert v['verdict'] == certify_mod.VERDICT_CERTIFIED
...
def test_not_held_out_cell_displays_an_aligned_row():
    rows = [_row(p=1e-06, window='2024', sign_aligned=False), _row(p=1e-05, window='2024', sign_aligned=True)]
    v = certify_mod.cell_verdicts(rows)['BTC', '1h', 'composite']
    assert v['verdict'] == certify_mod.VERDICT_NOT_HELD_OUT
    assert v['best_row']['sign_aligned'] is True
...
def test_wrong_signed_cell_displays_a_surviving_row():
    rows = [_row(p=1e-06, sign_aligned=False), _row(p=1e-05, sign_aligned=False)]
    v = certify_mod.cell_verdicts(rows)['BTC', '1h', 'composite']
    assert v['verdict'] == certify_mod.VERDICT_WRONG_SIGNED
    assert v['best_row']['p_value'] == 1e-06
    assert v['best_row_basis'] == certify_mod.BASIS_SURVIVED_BH

def test_single_row_cells_display_that_row_unchanged():
    for row, verdict in ((_row(p=1e-06), certify_mod.VERDICT_CERTIFIED), (_row(p=1e-06, window='2024'), certify_mod.VERDICT_NOT_HELD_OUT), (_row(p=1e-06, sign_aligned=False), certify_mod.VERDICT_WRONG_SIGNED), (_row(p=0.9), certify_mod.VERDICT_FAILS_GLOBAL_BH)):
