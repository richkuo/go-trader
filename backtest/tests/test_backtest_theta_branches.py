...
def _bt(**kw) -> ThetaHarvestBacktester:
    base = dict(initial_capital=10000.0, max_positions=2, profit_target_pct=0, stop_loss_pct=0, min_dte_close=0, label='t')
    base.update(kw)
    return ThetaHarvestBacktester(**base)

def test_profit_target_branch_closes_and_logs_theta_harvest():
    bt = _bt(profit_target_pct=60)
    pos = OptionPosition('call', 'sell', 60000.0, expiry_idx=2, premium=0.005, premium_usd=100.0, opened_idx=0)
    closed = bt._check_early_exit(pos, spot=20000.0, current_idx=0, hist_vol=_HIST_VOL, date=_DATE)
...
def test_stop_loss_branch_closes_and_logs_stop_loss():
    ...
    pos = OptionPosition('call', 'sell', 12000.0, expiry_idx=20, premium=0.01, premium_usd=200.0, opened_idx=0)
    closed = bt._check_early_exit(pos, spot=20000.0, current_idx=0, hist_vol=_HIST_VOL, date=_DATE)
    assert closed is True
...
def test_dte_floor_branch_closes_and_logs_dte_close():
    ...
    pos = OptionPosition('call', 'sell', 25000.0, expiry_idx=2, premium=0.005, premium_usd=100.0, opened_idx=0)
    closed = bt._check_early_exit(pos, spot=20000.0, current_idx=0, hist_vol=_HIST_VOL, date=_DATE)
    assert closed is True
...
def test_no_branch_fires_when_within_bounds_leaves_position_open():
    ...
    pos = OptionPosition('call', 'sell', 21000.0, expiry_idx=30, premium=0.02, premium_usd=2000.0, opened_idx=0)
    closed = bt._check_early_exit(pos, spot=20000.0, current_idx=0, hist_vol=_HIST_VOL, date=_DATE)
...
def test_bought_option_is_never_early_closed():
    ...
    pos = OptionPosition('call', 'buy', 60000.0, expiry_idx=2, premium=0.005, premium_usd=100.0, opened_idx=0)
    closed = bt._check_early_exit(pos, spot=20000.0, current_idx=0, hist_vol=_HIST_VOL, date=_DATE)
    assert closed is False
...
def test_zero_premium_position_is_skipped():
    ...
    pos = OptionPosition('call', 'sell', 60000.0, expiry_idx=2, premium=0.0, premium_usd=0.0, opened_idx=0)
    closed = bt._check_early_exit(pos, spot=20000.0, current_idx=0, hist_vol=_HIST_VOL, date=_DATE)
    assert closed is False
    assert bt.total_trades == 0

def test_report_metrics_block_computes_expected_fields():
    bt = _bt(initial_capital=10000.0, profit_target_pct=60, stop_loss_pct=150, min_dte_close=2)
    bt.cash = 11000.0
    bt.total_trades = 4
    bt.winning_trades = 3
    bt.losing_trades = 1
    ...
    n = 366
    bt.equity_curve = [((start + timedelta(days=i)).strftime('%Y-%m-%d'), 10000.0 + 1000.0 * i / (n - 1)) for i in range(n)]
    start_date = bt.equity_curve[0][0]
    end_date = bt.equity_curve[-1][0]
    report = bt._report('BTC', start_date, end_date, 20000.0, 22000.0)
    assert report['label'] == 't'
    assert report['underlying'] == 'BTC'
    assert report['days'] == n
    assert report['initial_capital'] == 10000.0
    assert report['final_value'] == pytest.approx(11000.0)
...
def test_report_zero_trades_yields_zero_win_rate():
    bt = _bt()
    bt.cash = 10000.0
    bt.equity_curve = [('2023-01-01', 10000.0), ('2023-01-02', 10000.0)]
    report = bt._report('BTC', '2023-01-01', '2023-01-02', 20000.0, 20000.0)
