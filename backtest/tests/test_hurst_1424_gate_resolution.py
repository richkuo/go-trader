...
def test_primary_pick_never_reads_the_1422_outcomes(tmp_path):
    ...
    stub.write_text(json.dumps({'configs': [{'config_id': 'mean_reversion/gate/W128/arm0.4/dis0.48', 'p_raw': 0.9}, {'config_id': 'momentum/size/W256/gain5', 'p_raw': 0.002}, {'config_id': 'momentum/gate/W512/arm0.52/dis0.48', 'p_raw': 0.5}, {'config_id': 'momentum/gate/W128/arm0.55/dis0.5', 'p_raw': None}]}))
...
def test_sweep_grid_primary_is_exactly_the_pinned_hypothesis():
    ...
    for family, mode, hw, arm, disarm, gain in grid:
        ids.append(study.gate_config_id(family, hw, arm, disarm) if mode == 'gate' else study.size_config_id(family, hw, gain))
...
def test_mean_reversion_is_exploratory_only():
    ...
    primary_families = {f for f, *_ in study._sweep_grid(study.COHORT_PRIMARY, study.HURST_WINDOWS)}
    assert primary_families == {'momentum'}

def test_bh_denominator_is_per_cohort_and_primary_is_one():
    cfgs = [{'cohort': study.COHORT_PRIMARY, 'p_cluster': 0.04}, {'cohort': study.COHORT_EXPLORATORY, 'p_cluster': 0.04}] + [{'cohort': study.COHORT_EXPLORATORY, 'p_cluster': 0.9} for _ in range(29)]
...
def test_binanceus_symbols_keep_their_plain_identity():
    ...
    assert study.qualified_symbol('coinbaseexchange', 'ETH/USD') == 'ETH/USD@coinbaseexchange'
...
def test_base_asset_ignores_quote_and_venue():
    for symbol in ('BTC/USDT', 'BTC/USD', 'BTC/USD@bitstamp', 'btc/usd@coinbaseexchange'):
...
def test_new_venue_cells_are_primary_and_1410_cells_stay_exploratory():
    assert study.cell_cohort('bitstamp', 'BTC/USD', '1h', '2013') == study.COHORT_PRIMARY
    assert study.cell_cohort('coinbaseexchange', 'ETH/USD', '1h', '2018') == study.COHORT_PRIMARY
    for key, window in study.D_1410:
        symbol, timeframe = key.rsplit(' ', 1)
        assert study.cell_cohort('binanceus', symbol, timeframe, window) == study.COHORT_EXPLORATORY

def test_a_resampled_audit_tape_stays_exploratory():
    assert study.cell_cohort('binanceus', 'BTC/USDT', '2h', 'is') == study.COHORT_EXPLORATORY
    assert study.cell_cohort('binanceus', 'BNB/USDT', '1h', 'is') == study.COHORT_PRIMARY
...
def test_same_base_asset_is_credited_full_correlation():
    ...
    frames = {}
    for ds in (('bitstamp', 'BTC/USD', '1h'), ('binanceus', 'BTC/USDT', '1h'), ('binanceus', 'ETH/USDT', '1h')):
...
def test_full_correlation_survives_into_effective_n():
    ...
    overlapping = [_trade(symbol='BTC/USD@bitstamp', exchange='bitstamp', window='2013', day=0, hold_days=10, cohort=study.COHORT_PRIMARY), _trade(symbol='BTC/USDT', window='2021', day=0, hold_days=10)]
    assert study.effective_n(overlapping, rho) == pytest.approx(1.0, abs=1e-06)

def test_coverage_audit_never_reports_an_unowned_cell_as_dropped():
    idx = pd.date_range('2013-01-01', periods=50, freq='1h')
    frames = {('bitstamp', 'BTC/USD', '1h'): pd.DataFrame({'close': np.arange(50.0)}, index=idx)}
...
def test_each_dataset_carries_its_own_measured_history_floor():
    assert study.history_since_for(('coinbaseexchange', 'ETH/USD', '1h')) == '2016-06-01'
    assert study.history_since_for(('coinbaseexchange', 'LTC/USD', '1h')) == '2016-09-01'
    assert study.history_since_for(('bitstamp', 'BTC/USD', '4h')) == study.HISTORY_SINCE['bitstamp']
    assert study.history_since_for(('binanceus', 'BTC/USDT', '1h')) == study1422.HISTORY_SINCE
...
def test_empty_backfill_is_reported_as_not_ok(monkeypatch, capsys):
    import data_fetcher
    monkeypatch.setattr(data_fetcher, 'fetch_full_history', lambda *a, **kw: pd.DataFrame())
...
def test_efficiency_is_one_on_a_monotone_advance_and_signed_by_direction():
    ...
    assert study.signed_efficiency(closes, 0, 4, 1) == pytest.approx(1.0, abs=1e-09)
    assert study.signed_efficiency(closes, 0, 4, -1) == pytest.approx(-1.0, abs=1e-09)

def test_efficiency_is_zero_on_a_round_trip():
    closes = np.array([100.0, 105.0, 100.0, 105.0, 100.0])
    assert study.signed_efficiency(closes, 0, 4, 1) == pytest.approx(0.0, abs=1e-09)
...
def test_target_filter_runs_before_the_cluster_usability_filter():
    rows = [_trade(symbol='XRP/USDT', day=0, eff=0.1), _trade(symbol='XRP/USDT', day=400, eff=None)]
...
def test_mde_raises_when_the_permutation_count_cannot_resolve_the_bar():
    ...
    with pytest.raises(ValueError) as exc:
        study.min_detectable_effect_on_grid(trades, values, mask, family_size=1, grid_step=0.02, grid_max=0.5, refine_step=0.001, cluster=False, n_perm=5)
...
def test_mde_returns_none_when_the_largest_grid_point_cannot_clear_the_bar():
    ...
    values = [0.0] * 8
    mask = [True] * 4 + [False] * 4
    out = study.min_detectable_effect_on_grid(trades, values, mask, family_size=1, grid_step=0.5, grid_max=0.5, refine_step=0.5, cluster=False, n_perm=200)
...
def test_efficiency_and_pp_wrappers_use_their_own_grids():
    ...
    assert calls == [(study.MDE_EFF_GRID_STEP, study.MDE_EFF_GRID_MAX, study.MDE_EFF_REFINE_STEP), (study.MDE_PP_GRID_STEP, study.MDE_PP_GRID_MAX, study.MDE_PP_REFINE_STEP)]
...
def test_mde_shrinks_as_the_family_denominator_shrinks():
    ...
    one = study.min_detectable_effect_on_grid(trades, values, mask, family_size=1, grid_step=0.05, grid_max=1.0, refine_step=0.05, cluster=False, n_perm=1000)
    thirty = study.min_detectable_effect_on_grid(trades, values, mask, family_size=30, grid_step=0.05, grid_max=1.0, refine_step=0.05, cluster=False, n_perm=1000)
    assert one is not None and thirty is not None
    assert one <= thirty

def _win(dd=-1.0, chop=-1.0, ret_g=10.0, ret_u=10.0, legs=1):
    return {'n_legs': legs, 'dd_delta': dd, 'chop_delta': chop, 'ret_gated': ret_g, 'ret_ungated': ret_u}

def test_protocol_rule_passes_on_three_of_four():
    windows = {'2017': _win(), '2018': _win(), '2021': _win(), '2022': _win(dd=+1.0)}
    ok, holding, with_legs, reasons = study.protocol_verdict(windows, study.PRIMARY_PROTOCOL_WINDOWS, study.PRIMARY_PROTOCOL_MIN_WINDOWS)
    assert ok and holding == 3 and (with_legs == 4) and (reasons == [])

def test_protocol_rule_fails_on_two_of_four():
    windows = {'2017': _win(), '2018': _win(), '2021': _win(dd=+1.0), '2022': _win(chop=+1.0)}
    ok, holding, _with, reasons = study.protocol_verdict(windows, study.PRIMARY_PROTOCOL_WINDOWS, study.PRIMARY_PROTOCOL_MIN_WINDOWS)
...
def test_protocol_rule_fails_closed_when_too_few_windows_carry_legs():
    windows = {'2017': _win(), '2018': _win(legs=0), '2021': _win(legs=0), '2022': _win(legs=0)}
    ok, _holding, with_legs, reasons = study.protocol_verdict(windows, study.PRIMARY_PROTOCOL_WINDOWS, study.PRIMARY_PROTOCOL_MIN_WINDOWS)
...
def test_protocol_rule_enforces_the_return_give_up_tolerance():
    windows = {'2017': _win(), '2018': _win(), '2021': _win(), '2022': _win(ret_g=-100.0, ret_u=100.0)}
    ok, holding, _w, _r = study.protocol_verdict(windows, study.PRIMARY_PROTOCOL_WINDOWS, study.PRIMARY_PROTOCOL_MIN_WINDOWS)
    assert ok and holding == 3

def test_exploratory_protocol_rule_still_requires_every_window():
    assert study.EXPLORATORY_PROTOCOL_MIN_WINDOWS == len(study.EXPLORATORY_PROTOCOL_WINDOWS)
    windows = {'is': _win(), 'oos': _win(dd=+1.0)}
    ok, _h, _w, _r = study.protocol_verdict(windows, study.EXPLORATORY_PROTOCOL_WINDOWS, study.EXPLORATORY_PROTOCOL_MIN_WINDOWS)
    assert not ok

def test_primary_protocol_and_held_out_windows_partition_the_window_set():
    assert set(study.PRIMARY_PROTOCOL_WINDOWS).isdisjoint(study.PRIMARY_HELD_OUT_WINDOWS)
    assert set(study.PRIMARY_PROTOCOL_WINDOWS) | set(study.PRIMARY_HELD_OUT_WINDOWS) == set(study.WINDOW_ORDER)
...
def _cfg(**over):
    cfg = {'config_id': study.PRIMARY_CONFIG_ID, 'cohort': study.COHORT_PRIMARY, 'family': 'momentum', 'mode': 'gate', 'sense': study.SENSE_HIGH, 'hurst_window': 512, 'arm': 0.52, 'disarm': 0.48, 'gain': None, 'p_raw': 0.01, 'p_raw_return': 0.01, 'p_cluster': 0.01, 'p_cluster_return': 0.01, 'bh_reject': True, 'n_pooled_trades': 200, 'n_pooled_effective': 100.0, 'n_missing_target': 2, 'n_suppressed': 80, 'n_kept': 120, 'n_suppressed_effective': 50.0, 'n_kept_effective': 50.0, 'protocol_windows': list(study.PRIMARY_PROTOCOL_WINDOWS), 'protocol_min_windows': study.PRIMARY_PROTOCOL_MIN_WINDOWS, 'held_out_windows': list(study.PRIMARY_HELD_OUT_WINDOWS), 'windows': {w: _win() for w in study.PRIMARY_PROTOCOL_WINDOWS}}
...
def test_untestable_cluster_p_fails_closed():
    ok, reasons = study.config_verdict(_cfg(p_cluster=None, cluster_reason='no draws'))
    assert not ok
    assert any(('untestable' in r for r in reasons))

def test_significance_reads_the_primary_target_not_net_return():
    ok, reasons = study.config_verdict(_cfg(bh_reject=False, p_cluster=0.4, p_cluster_return=0.0001))
    assert not ok
    assert any(('primary target' in r for r in reasons))

def test_effective_volume_floors_bind_not_nominal_counts():
    ok, reasons = study.config_verdict(_cfg(n_suppressed_effective=5.0, n_suppressed=5000))
    assert not ok
...
def _mde(mom_limit=0.05, mom_sep=0.09, mr_limit=0.05, mr_sep=0.02, pooled=None, **extra):
    out = {'by_family_cluster': {study.PRIMARY_FAMILY: mom_limit, _MR: mr_limit}, 'by_family_separation': {study.PRIMARY_FAMILY: mom_sep, _MR: mr_sep}, 'by_family_n': {study.PRIMARY_FAMILY: 100, _MR: 100}, 'pooled_primary_cluster': 0.001 if pooled is None else pooled, 'observed_separation_by_pool': {'primary': {f'{study.PRIMARY_FAMILY}|512': mom_sep, f'{_MR}|512': mr_sep}}}
    out.update(extra)
    return out
...
def test_validity_gate_is_unchanged_when_the_pool_holds_one_family():
    ...
    alone = study.validity_gate({'by_family_cluster': {study.PRIMARY_FAMILY: 0.05}, 'by_family_separation': {study.PRIMARY_FAMILY: 0.09}, 'by_family_n': {study.PRIMARY_FAMILY: 100}, 'pooled_primary_cluster': 0.05, 'observed_separation_by_pool': {'primary': {f'{study.PRIMARY_FAMILY}|512': 0.09}}})
...
def test_validity_gate_never_borrows_the_other_familys_limit():
    gate = study.validity_gate(_mde(mom_limit=None, mom_sep=0.09, mr_limit=0.001, mr_sep=0.5))
    assert gate['passed'] is False
...
def test_validity_gate_refuses_when_every_family_points_the_untested_way():
    gate = study.validity_gate(_mde(mom_limit=0.005, mom_sep=-0.05, mr_limit=0.005, mr_sep=-0.08))
    assert gate['passed'] is False
    assert 'OPPOSITE' in gate['reason']

def test_validity_gate_decides_on_direction_not_the_larger_magnitude():
    gate = study.validity_gate(_mde(mom_limit=0.02, mom_sep=0.05, mr_limit=0.02, mr_sep=-0.4))
    assert gate['passed'] is True
...
def test_a_reversed_separation_never_closes_the_question():
    decision = study.decide_recommendation([], _mde(mom_limit=0.001, mom_sep=-0.4))
...
_FAILING_MDE = _mde(mom_limit=0.2, mom_sep=0.09)
...
def test_a_passing_primary_config_produces_a_recommendation():
    ...
    assert decision['verdict'] == study.VERDICT_CONFIG
    assert decision['families']['momentum']['winner']['config_id'] == study.PRIMARY_CONFIG_ID
...
def _render_payload(decision=None, mde=None, configs=None):
    ...
    mde.setdefault('observed_separation_pp_by_pool', {'primary': {'momentum|512': 0.4}})
    mde.setdefault('by_family_cluster_return', {study.PRIMARY_FAMILY: 0.9, _MR: 0.5})
    mde.setdefault('by_family_separation_return', {study.PRIMARY_FAMILY: 0.4, _MR: -0.2})
    cfgs = list(configs if configs is not None else [_cfg(bh_reject=False, p_cluster=0.4)])
...
def test_largest_signed_prefers_the_tested_direction():
    assert study._largest_signed({'a|512': 0.01, 'b|512': -0.3}) == pytest.approx(0.01)
    assert study._largest_signed({'a|512': -0.05, 'b|512': -0.3}) == pytest.approx(-0.05)
...
def test_the_pool_tables_print_every_family_with_its_sign():
    mde = _mde(mom_limit=0.2, mom_sep=0.009, mr_limit=0.2, mr_sep=-0.024)
...
def test_the_pool_tables_flag_a_reversed_pool_rather_than_resolving_it():
    mde = _mde(mom_limit=0.2, mom_sep=-0.05, mr_limit=0.2, mr_sep=-0.08, pooled=0.001)
    text = study.report_from_payload(_render_payload(mde=mde))
    assert 'n/a (reversed)' in text

def test_the_key_risk_paragraph_matches_the_reason_the_gate_failed():
    reversed_text = study.report_from_payload(_render_payload(mde=_mde(mom_limit=0.01, mom_sep=-0.3)))
    assert 'points the OTHER WAY' in reversed_text
    assert 'would have been caught, and none was' not in reversed_text
    small_text = study.report_from_payload(_render_payload(mde=_mde(mom_limit=0.2, mom_sep=0.09)))
...
def test_render_only_rehydrates_a_winner_stored_as_an_id():
    ...
    round_tripped = json.loads(json.dumps({'verdict': decision['verdict'], 'justification': decision['justification'], 'validity_gate': decision['validity_gate'], 'key_risk_held': decision['key_risk_held'], 'families': {f: {'n_tested': d['n_tested'], 'n_passing': d['n_passing'], 'winner': (d['winner'] or {}).get('config_id')} for f, d in decision['families'].items()}}))
...
def test_1424_owns_the_contract_path():
    ...
    assert os.path.basename(study._DEFAULT_JSON_OUT) == 'hurst_1424_gate_resolution.json'
...
def test_no_predecessor_study_still_defaults_to_the_contract_path():
    ...
    assert os.path.basename(study1422._DEFAULT_REPORT_OUT) == 'hurst_1422_gate_power.md'
...
def test_1422_may_not_write_the_contract_path_even_when_asked(tmp_path):
    ...
    with pytest.raises(SystemExit) as exc:
        study1422.main(['--report-out', contract, '--json-out', str(tmp_path / 'scoped.json')])
...
@pytest.mark.parametrize('kw,needle', [({'n_perm_mde': study.N_PERM_MDE - 1}, '--n-perm-mde'), ({'n_perm_mde': study.N_PERM_MDE + 1}, '--n-perm-mde'), ({'n_perm': 200}, '--n-perm '), ({'seed': study.SEED + 1}, '--seed'), ({'no_mirror_check': True}, '--no-mirror-check')])
...
def test_dataset_arg_defaults_to_the_binanceus_venue():
    ...
    assert study._parse_datasets('coinbaseexchange=ETH/USD:1h') == [('coinbaseexchange', 'ETH/USD', '1h')]
...
def test_the_cluster_null_and_effective_n_come_from_1422_unchanged():
    assert study.cluster_permutation_pvalue_group_diff is study1422.cluster_permutation_pvalue_group_diff
