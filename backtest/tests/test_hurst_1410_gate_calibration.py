...
import hurst_1410_gate_calibration as study
_UNSET = object()

@pytest.mark.parametrize('value,expected', [(0.0, '<0.45'), (0.4499999, '<0.45'), (0.45, '0.45-0.50'), (0.4999999, '0.45-0.50'), (0.5, '0.50-0.55'), (0.5499999, '0.50-0.55'), (0.55, '>=0.55'), (1.2, '>=0.55')])
...
def test_hysteresis_high_sense_arms_and_disarms():
    values = [0.4, 0.52, 0.56, 0.52, 0.49, 0.52]
    mask = study.hysteresis_mask(values, 0.55, 0.5, study.SENSE_HIGH)
    assert mask.tolist() == [False, False, True, True, False, False]

def test_hysteresis_low_sense_is_mirrored():
    values = [0.6, 0.48, 0.44, 0.48, 0.55, 0.48]
    mask = study.hysteresis_mask(values, 0.45, 0.5, study.SENSE_LOW)
    assert mask.tolist() == [False, False, True, True, False, False]

def test_hysteresis_starts_armed():
    assert study.GATE_INITIAL_ARMED is True
    mask = study.hysteresis_mask([float('nan')], 0.55, 0.5, study.SENSE_HIGH)
    assert mask.tolist() == [True]
    mask = study.hysteresis_mask([0.52], 0.55, 0.5, study.SENSE_HIGH)
    assert mask.tolist() == [True]

def test_nan_holds_gate_state_in_both_senses():
    high = study.hysteresis_mask([0.4, float('nan'), float('nan'), 0.56, float('nan')], 0.55, 0.5, study.SENSE_HIGH)
    assert high.tolist() == [False, False, False, True, True]
    low = study.hysteresis_mask([0.6, None, 0.44, float('nan')], 0.45, 0.5, study.SENSE_LOW)
...
@pytest.mark.parametrize('arm,disarm,sense', [(0.5, 0.55, study.SENSE_HIGH), (0.55, 0.5, study.SENSE_LOW), (0.55, 0.55, study.SENSE_HIGH)])
...
def test_unknown_sense_is_rejected():
    with pytest.raises(ValueError):
        study.hysteresis_mask([0.5], 0.55, 0.5, 'sideways')
...
def test_rolling_hurst_value_uses_only_its_own_trailing_window():
    ...
    assert study.rolling_hurst(mutated, window).iloc[target] == pytest.approx(rolling.iloc[target], abs=1e-12)
...
def test_decision_value_at_bar_n_is_invariant_to_bar_n_close():
    ...
    assert study.rolling_hurst(mutated, window).iloc[target] != pytest.approx(study.rolling_hurst(close, window).iloc[target], abs=1e-09)
...
def _entry(window, entry_date, strategy='momentum', symbol='BTC/USDT', timeframe='1h', pnl=1.0):
    return {'window': window, 'entry_date': entry_date, 'strategy': strategy, 'symbol': symbol, 'timeframe': timeframe, 'pnl_pct_net': pnl}
...
def test_distinct_physical_entries_are_never_collapsed():
    ...
    rows = [_entry('is', ts, strategy='momentum'), _entry('is', ts, strategy='vol_momentum'), _entry('is', ts, symbol='ETH/USDT'), _entry('is', ts, timeframe='4h'), _entry('is', '2025-06-21 04:00:00')]
...
def test_compound_equity_matches_a_hand_computed_sequence():
    ...
    assert ret == pytest.approx(-1.0, abs=1e-09)
    assert dd == pytest.approx(-10.0, abs=1e-09)
...
def test_multipliers_scale_each_trade():
    ...
    assert ret_full == pytest.approx(21.0, abs=1e-09)
    assert ret_half == pytest.approx(10.25, abs=1e-09)

def test_a_zero_multiplier_removes_a_trade_entirely():
    ret, dd = study.compound_equity([-50.0, 20.0], [0.0, 1.0])
    assert ret == pytest.approx(20.0, abs=1e-09)
    assert dd == pytest.approx(0.0, abs=1e-09)

def test_equity_is_floored_at_zero_and_stays_there():
    ret, dd = study.compound_equity([-90.0, -90.0, 500.0], [1.5, 1.5, 1.5])
    assert ret == pytest.approx(-100.0, abs=1e-09)
    assert dd == pytest.approx(-100.0, abs=1e-09)
...
def test_size_multiplier_clamps_both_ends():
    assert study.size_multiplier(0.99, study.SENSE_HIGH, 5.0) == pytest.approx(study.SIZING_CLAMP_HI)
    assert study.size_multiplier(0.01, study.SENSE_HIGH, 5.0) == pytest.approx(study.SIZING_CLAMP_LO)
...
def test_bucket_tables_route_every_trade_including_nan():
    trades = [{'pnl_pct_net': 3.0, 'h': {128: 0.6}}, {'pnl_pct_net': -1.0, 'h': {128: 0.6}}, {'pnl_pct_net': 2.0, 'h': {128: 0.4}}, {'pnl_pct_net': -4.0, 'h': {128: None}}]
...
def _window_row(dd=-3.0, chop=-2.0, ret_gated=10.0, ret_ungated=9.0, n_legs=6):
    return {'n_legs': n_legs, 'dd_delta': dd, 'chop_delta': chop, 'ret_gated': ret_gated, 'ret_ungated': ret_ungated, 'trades_gated': 50, 'trades_ungated': 90}

def _cfg(family=study.FAMILY_MOMENTUM, mode='gate', **over):
    cfg = {'config_id': over.pop('config_id', f'{family}/{mode}/W256/arm0.55/dis0.5'), 'family': family, 'mode': mode, 'sense': study.FAMILY_SENSE[family], 'hurst_window': 256, 'arm': 0.55, 'disarm': 0.5, 'gain': None, 'n_pooled_trades': 400, 'n_suppressed': 120, 'n_kept': 280, 'p_raw': 0.0005, 'bh_reject': True, 'windows': {w: _window_row() for w in study.WINDOWS}}
    cfg.update(over)
    return cfg
...
def test_return_give_up_is_tolerated_up_to_the_pre_registered_band():
    windows = {w: _window_row() for w in study.WINDOWS}
    windows['is'] = _window_row(ret_gated=99.1, ret_ungated=100.0)
    assert study.config_verdict(_cfg(windows=windows))[0]
    windows['is'] = _window_row(ret_gated=88.0, ret_ungated=100.0)
    ok, reasons = study.config_verdict(_cfg(windows=windows))
...
def test_the_absolute_tolerance_floor_applies_to_a_tiny_baseline():
    windows = {w: _window_row() for w in study.WINDOWS}
    windows['oos'] = _window_row(ret_gated=-0.5, ret_ungated=0.2)
    assert study.config_verdict(_cfg(windows=windows))[0]
    windows['oos'] = _window_row(ret_gated=-2.0, ret_ungated=0.2)
...
def test_no_winner_anywhere_yields_the_inconclusive_verdict():
    configs = [_cfg(family=f, bh_reject=False, config_id=f'{f}/gate/W256/arm0.55/dis0.5') for f in study.FAMILIES]
...
def test_the_inconclusive_justification_counts_the_failure_modes():
    economics_ok = _cfg(family=study.FAMILY_MOMENTUM, bh_reject=False, config_id='momentum/gate/W256/arm0.55/dis0.5')
    economics_bad = _cfg(family=study.FAMILY_MEAN_REVERSION, bh_reject=False, config_id='mean_reversion/gate/W256/arm0.45/dis0.5', windows={w: _window_row(dd=+3.0) for w in study.WINDOWS})
...
def test_a_winner_in_one_family_yields_a_config_verdict():
    configs = [_cfg(family=study.FAMILY_MOMENTUM, config_id='momentum/gate/W256/arm0.55/dis0.5'), _cfg(family=study.FAMILY_MEAN_REVERSION, bh_reject=False, config_id='mean_reversion/gate/W256/arm0.45/dis0.5')]
    decision = study.decide_recommendation(configs)
...
def test_the_tie_break_picks_the_largest_protocol_drawdown_reduction():
    small = _cfg(config_id='momentum/gate/W128/arm0.52/dis0.48', windows={w: _window_row(dd=-1.0) for w in study.WINDOWS})
    big = _cfg(config_id='momentum/gate/W512/arm0.6/dis0.52', windows={w: _window_row(dd=-9.0) for w in study.WINDOWS})
...
def test_the_tie_break_is_deterministic_for_equal_reductions():
    ...
    assert first['families'][study.FAMILY_MOMENTUM]['winner']['config_id'] == second['families'][study.FAMILY_MOMENTUM]['winner']['config_id']

def test_inconclusive_recommendation_prints_the_bare_token():
    configs = [_cfg(family=f, bh_reject=False, config_id=f'{f}/gate/W256/a/d') for f in study.FAMILIES]
    text = study.render_recommendation(study.decide_recommendation(configs), configs)
...
def test_a_winning_sizing_recommendation_states_its_gain():
    cfg = _cfg(family=study.FAMILY_MOMENTUM, mode='size', arm=None, disarm=None, gain=2.5, config_id='momentum/size/W256/gain2.5')
    text = study.render_recommendation(study.decide_recommendation([cfg]), [cfg])
...
def _report_payload(verdict_configs=None, warmup=_UNSET):
    configs = verdict_configs or [_cfg(family=f, bh_reject=False, config_id=f'{f}/gate/W256/arm0.55/dis0.5') for f in study.FAMILIES]
    for cfg in configs:
        cfg.setdefault('bh_reject', False)
    decision = study.decide_recommendation(configs)
    pooled = {f: [] for f in study.FAMILIES}
    return {'schema_version': study.SCHEMA_VERSION, 'pre_registered': {'families': {f: list(study.FAMILY_EXEMPLARS[f]) for f in study.FAMILIES}, 'family_sense': dict(study.FAMILY_SENSE), 'exemplar_close_overrides': study.EXEMPLAR_CLOSE_OVERRIDES, 'buckets': list(study.BUCKETS), 'hurst_windows': [256], 'gate_pairs': {f: [list(p) for p in study.GATE_PAIRS[f]] for f in study.FAMILIES}, 'gate_initial_armed': study.GATE_INITIAL_ARMED, 'sizing': {'gains': list(study.SIZING_GAINS), 'clamp_lo': study.SIZING_CLAMP_LO, 'clamp_hi': study.SIZING_CLAMP_HI, 'nan_multiplier': study.SIZING_NAN_MULTIPLIER}, 'min_suppressed': study.MIN_SUPPRESSED_TRADES, 'min_kept': study.MIN_KEPT_TRADES, 'return_tolerance_pp': study.RETURN_TOLERANCE_PP, 'return_tolerance_frac': study.RETURN_TOLERANCE_FRAC, 'held_out_min_non_degrading': study.HELD_OUT_MIN_NON_DEGRADING, 'alpha': study.ALPHA, 'n_perm': study.N_PERM, 'seed': study.SEED, 'windows': {k: list(v) for k, v in study.WINDOWS.items()}, 'protocol_windows': list(study.PROTOCOL_WINDOWS), 'held_out_windows': list(study.HELD_OUT_WINDOWS), 'datasets': ['BTC/USDT 1h'], 'platform': study.PLATFORM, 'fee_platform': study.FEE_PLATFORM, 'capital': study.DEFAULT_CAPITAL}, 'run_summary': {'legs': 1, 'gated_arms': 3, 'mirror_verified_legs': 1, 'raw_trades': {f: 0 for f in study.FAMILIES}, 'pooled_trades': {f: 0 for f in study.FAMILIES}, 'warmup': study.warmup_audit({'BTC/USDT 1h': 5000}, [256]) if warmup is _UNSET else warmup, 'elapsed_sec': 1.0}, 'buckets': {f: {'256': study.bucket_tables(pooled[f], 256)} for f in study.FAMILIES}, 'configs': configs, 'legs': [], 'decision': decision}
...
def test_report_recommendation_section_survives_a_winner():
    winner = _cfg(family=study.FAMILY_MOMENTUM, config_id='momentum/gate/W256/arm0.55/dis0.5')
    loser = _cfg(family=study.FAMILY_MEAN_REVERSION, bh_reject=False, config_id='mean_reversion/gate/W256/arm0.45/dis0.5')
...
def test_report_is_a_pure_function_of_the_committed_payload():
    payload = _report_payload()
    stored = dict(payload)
    stored['decision'] = {'verdict': payload['decision']['verdict'], 'justification': payload['decision']['justification'], 'families': {f: {'n_tested': d['n_tested'], 'n_passing': d['n_passing'], 'winner': (d['winner'] or {}).get('config_id')} for f, d in payload['decision']['families'].items()}}
...
def test_render_only_rebuilds_a_winning_recommendation_too():
    ...
    payload['decision'] = {'verdict': 'config', 'justification': '', 'families': {}}
...
def test_cache_miss_when_the_series_shifts_without_changing_length():
    ...
    assert study.cache_entry_is_usable(meta, _idx(3000, '2023-06-01'), '2023-01-01') is False

def test_cache_miss_on_a_legacy_entry_with_no_metadata():
    idx = _idx()
    assert study.cache_entry_is_usable(None, idx, '2023-01-01') is False
    assert study.cache_entry_is_usable(np.array([1, 2], dtype=np.int64), idx, '2023-01-01') is False
...
def test_cache_meta_round_trips_through_an_int64_npz_array():
    ...
    assert study.cache_entry_is_usable(np.asarray(meta.tolist(), dtype=np.int64), idx, '2023-01-01') is True
...
def test_bh_marks_a_clearly_significant_config():
    configs = [_cfg(config_id='c0', p_raw=1e-06)]
...
def test_the_five_named_exemplars_are_the_declared_families():
    assert study.FAMILY_EXEMPLARS[study.FAMILY_MOMENTUM] == ('momentum', 'vol_momentum', 'squeeze_momentum')
    assert study.FAMILY_EXEMPLARS[study.FAMILY_MEAN_REVERSION] == ('mean_reversion', 'atr_band_revert')

def test_hurst_windows_clear_the_estimator_minimum():
    assert min(study.HURST_WINDOWS) > 100
