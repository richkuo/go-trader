...
def test_rejects_live_only_dynamic_close():
    arm = {'close_strategies': [{'name': 'tiered_tp_atr_live_regime_dynamic', 'params': {}}]}
    with pytest.raises(ValueError, match='HL-live-only'):
        m.validate_arm_config(arm)

def test_rejects_regime_aware_sl_after_surface():
    arm = {'close_strategies': [{'name': 'tiered_tp_atr_regime', 'params': {'tp_tiers': [{'trend_regime': {'ranging': {'atr_multiple': 1.0, 'close_fraction': 1.0, 'sl_after': {'trend_regime': {'ranging': {'atr_offset': 0.5}}}}}}]}}]}
...
def test_validate_label_stream_rejects_degenerate_and_unknown_labels():
    with pytest.raises(ValueError, match='degenerate'):
        m.validate_label_stream(['ranging_quiet', 'ranging_quiet', 'ranging_quiet'], [True, True, True], source='probe')
    with pytest.raises(ValueError, match='unknown'):
        m.validate_label_stream(['ranging_quiet', 'bogus', 'trending_up_clean'], [True, True, True], source='probe')

def test_validate_label_stream_counts_valid_rows_only():
    stats = m.validate_label_stream(['ranging_quiet', 'bogus_warmup', 'trending_up_clean', 'ranging_quiet'], [True, False, True, True], source='probe')
...
def _trade(entry_date, reason, mae, mfe=2.0, pnl=1.0):
    return {'entry_date': entry_date, 'exit_reason': reason, 'mae_pct': mae, 'mfe_pct': mfe, 'pnl_pct': pnl, 'shares': 1.0, 'entry_price': 100.0, 'exit_price': 101.0, 'entry_fee': 0.0, 'exit_fee': 0.0}

def _raw_result(*, sharpe=1.0, ret=10.0, dd=-5.0, mae=-1.0, reason='tiered_tp_atr:1'):
    return {'total_return_pct': ret, 'max_drawdown_pct': dd, 'sharpe_ratio': sharpe, 'total_trades': 1, 'trades': [_trade('d1', reason, mae)]}

def test_summarize_results_stop_out_rate_and_mae_by_entry():
    results = {'total_return_pct': 10.0, 'max_drawdown_pct': -5.0, 'sharpe_ratio': 1.25, 'total_trades': 3, 'trades': [_trade('d1', 'tiered_tp_atr:1.5', -0.5), _trade('d1', 'sl', -1.5), _trade('d2', 'tiered_tp_atr:3', -0.25)]}
...
def test_compare_summaries_requires_all_gate_axes():
    control = {'entries': 3, 'sharpe': 1.0, 'ddadj': 0.5, 'stop_out_rate': 0.25, 'median_mae_pct': -2.0, 'total_return_pct': 5.0, 'max_drawdown_pct': -10.0}
    candidate = {'entries': 3, 'sharpe': 1.2, 'ddadj': 0.75, 'stop_out_rate': 0.1, 'median_mae_pct': -1.0, 'total_return_pct': 6.0, 'max_drawdown_pct': -8.0}
...
def test_compare_summaries_equal_risk_adjusted_metrics_do_not_pass():
    control = {'entries': 2, 'sharpe': 1.0, 'ddadj': 0.5, 'stop_out_rate': 0.0, 'median_mae_pct': -1.0, 'total_return_pct': 5.0, 'max_drawdown_pct': -10.0}
...
def test_run_cell_gates_against_best_control_not_width_extreme(monkeypatch):
    ...
    got = m.run_cell(df, labels, valid=[True, True], surface='fixed_sl', registry='futures', strategy='probe', params={}, capital=10000.0, platform='hyperliquid', direction='long', symbol='BTC/USDT', timeframe='1h', thresholds={}, control_grids={'flat_atr': [1.0, 5.0]})
...
def test_run_cell_single_arm_grid_winner_passes(monkeypatch):
    ...
    monkeypatch.setattr(m, '_strategy_frame', fake_strategy_frame)
    monkeypatch.setattr(m, 'run_arm', fake_run_arm)
    got = m.run_cell(df, labels, valid=[True, True], surface='fixed_sl', registry='futures', strategy='probe', params={}, capital=10000.0, platform='hyperliquid', direction='long', symbol='BTC/USDT', timeframe='1h', thresholds={}, control_grids={'flat_atr': [1.0]})
    assert got['verdict']['pass'] is True
...
def test_run_cell_threads_valid_mask_to_injection(monkeypatch):
    ...
    monkeypatch.setattr(m, '_strategy_frame', fake_strategy_frame)
    monkeypatch.setattr(m, 'run_arm', fake_run_arm)
    m.run_cell(df, labels, valid=[False, False, True], surface='fixed_sl', registry='futures', strategy='probe', params={}, capital=10000.0, platform='hyperliquid', direction='long', symbol='BTC/USDT', timeframe='1h', thresholds={}, control_grids={'flat_atr': [1.0]})
    assert seen
...
def test_injected_raw_regime_labels_are_shifted_by_backtester_no_lookahead():
    ...
    df = pd.DataFrame({'open': [100.0, 100.0, 100.0, 100.26, 100.26, 100.26], 'close': [100.0, 100.0, 100.0, 100.26, 100.26, 100.26], 'atr': [1.0] * 6, 'open_action': ['none', 'long', 'none', 'none', 'none', 'none']}, index=idx)
    labels = ['ranging', 'ranging', 'trending_up', 'trending_up', 'trending_up', 'trending_up']
    frame = m.inject_regime_labels(df, labels)
    close_ref = {'name': 'tiered_tp_atr_regime', 'params': {'tp_tiers': [{'trend_regime': {'ranging': {'atr_multiple': 0.25, 'close_fraction': 1.0}, 'trending_up': {'atr_multiple': 10.0, 'close_fraction': 1.0}, 'trending_down': {'atr_multiple': 10.0, 'close_fraction': 1.0}}}]}}
    bt = Backtester(initial_capital=10000.0, commission_pct=0.0, slippage_pct=0.0, close_strategies=[close_ref], regime_enabled=True)
