...
@pytest.mark.parametrize('strategy_type,expected', [('spot', 'spot'), ('options', 'spot'), ('perps', 'futures'), ('futures', 'futures'), ('manual', 'futures'), (' PERPS ', 'futures')])
...
def test_param_ranges_cover_every_registered_strategy():
    ...
    assert not missing, f'Strategies without DEFAULT_PARAM_RANGES — walk-forward will fall back to a single-point grid: {sorted(missing)}'
