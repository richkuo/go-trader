...
def test_sign_test_all_positive_is_significant():
    ...
    assert r['n'] == 5
    assert r['p_value'] == pytest.approx(0.0625, abs=1e-06)
...
def test_binom_p_log_space_matches_small_n_exact():
    ...
    def direct(k, n, pr=0.5):
        ...
        def cdf(u):
            return sum((math.comb(n, i) * pr ** i * (1.0 - pr) ** (n - i) for i in range(0, u + 1)))
        lo = cdf(k)
        hi = 1.0 - cdf(k - 1) if k > 0 else 1.0
        return min(1.0, 2.0 * min(lo, hi))
    for k, n in [(0, 5), (3, 10), (7, 8), (25, 60)]:
        assert m._binom_two_sided_p(k, n) == pytest.approx(direct(k, n), rel=1e-09)
...
def _leg(entry_date='2025-01-01', side='long', pnl_pct=2.0, shares=1.0, entry_price=100.0, exit_price=102.0, entry_fee=0.1, exit_fee=0.1, mfe_pct=3.0, mae_pct=-1.0, bars_held=5, exit_reason='tp'):
    return {'entry_date': entry_date, 'side': side, 'pnl_pct': pnl_pct, 'shares': shares, 'entry_price': entry_price, 'exit_price': exit_price, 'entry_fee': entry_fee, 'exit_fee': exit_fee, 'mfe_pct': mfe_pct, 'mae_pct': mae_pct, 'bars_held': bars_held, 'exit_reason': exit_reason}
...
def test_collapse_multi_leg_notional_weighted():
    leg_a = _leg(shares=3.0, pnl_pct=1.0, mfe_pct=2.0, mae_pct=-0.5, bars_held=4, entry_fee=0.0, exit_fee=0.0, exit_price=101.0)
    leg_b = _leg(shares=1.0, pnl_pct=5.0, mfe_pct=6.0, mae_pct=-2.0, bars_held=9, entry_fee=0.0, exit_fee=0.0, exit_price=105.0)
    rec = m.collapse_entry([leg_a, leg_b])
    assert rec['net_pct'] == pytest.approx(2.0, abs=1e-06)
...
def test_group_and_free_arm_entries_orders_by_entry():
    trades = [_leg(entry_date='2025-01-01'), _leg(entry_date='2025-01-05'), _leg(entry_date='2025-01-01')]
...
def test_build_paired_rows_pairs_and_counts_unmatched():
    control = [{'entry_date': 'd1', 'side': 'long', 'net_pct': 1.0, 'mfe_pct': 2.0, 'mae_pct': -1.0, 'bars_held': 3}, {'entry_date': 'd2', 'side': 'long', 'net_pct': -1.0, 'mfe_pct': 0.5, 'mae_pct': -2.0, 'bars_held': 4}]
    candidate_by_date = {'d1': {'net_pct': 2.5, 'mfe_pct': 3.0, 'mae_pct': -0.5, 'bars_held': 6}, 'd2': None}
...
def test_build_paired_rows_unknown_regime_label():
    control = [{'entry_date': 'd1', 'side': 'long', 'net_pct': 1.0, 'mfe_pct': 1.0, 'mae_pct': -1.0, 'bars_held': 2}]
    rows, _ = m.build_paired_rows(control, {'d1': {'net_pct': 1.0, 'mfe_pct': 1.0, 'mae_pct': -1.0, 'bars_held': 2}}, {})
    assert rows[0]['regime'] == m.UNKNOWN_REGIME

def _row(regime, ctrl, cand, mfe=2.0, mae=-1.0):
    return {'entry_date': 'd', 'regime': regime, 'side': 'long', 'control_net_pct': ctrl, 'candidate_net_pct': cand, 'delta_net_pct': cand - ctrl, 'control_mfe_pct': mfe, 'candidate_mfe_pct': mfe, 'control_mae_pct': mae, 'candidate_mae_pct': mae, 'control_bars_held': 3, 'candidate_bars_held': 3}

def test_per_regime_table_buckets_and_all_and_sorted():
    rows = [_row('trending', 1.0, 2.0), _row('ranging', 0.0, -1.0), _row('trending', 2.0, 4.0)]
    ...
    assert table['all']['paired_delta']['mean'] == pytest.approx(2.0 / 3.0, abs=1e-06)
...
def test_arm_summary_passes_max_dd_and_computes_winrate():
    results = {'total_trades': 2, 'total_return_pct': 5.0, 'max_drawdown_pct': -3.0, 'sharpe_ratio': 1.2, 'liquidated': False, 'trades': [_leg(entry_date='d1', pnl_pct=2.0), _leg(entry_date='d2', pnl_pct=-1.0, exit_price=99.0)]}
...
def test_replayable_true_for_rule_based_exits():
    ...
    assert m.candidate_is_replayable([{'name': 'tiered_tp_atr', 'params': {}}, {'name': 'trailing_stop_atr_mult', 'params': {'atr_mult': 3}}])
...
def test_replayable_true_for_ratchet_and_frozen_regime_tp():
    ...
    assert m.candidate_is_replayable([{'name': 'trailing_tp_ratchet_regime', 'params': {'use_defaults': True}}])
...
def test_replayable_false_for_per_tick_regime_variants():
    ...
    assert not m.candidate_is_replayable([{'name': 'tiered_tp_atr_live_regime_dynamic'}])
...
def _regime_args(classifier=None, windows_json=None, period=14, adx=20.0, gate_window=None):
    import types
    return types.SimpleNamespace(regime_classifier=classifier, regime_windows_json=windows_json, regime_period=period, regime_adx_threshold=adx, gate_window=gate_window)
...
def test_resolve_regime_explicit_adx_overrides_config_windows():
    cfg = m.resolve_regime_cfg(_regime_args(classifier='adx'), {'windows': {'medium': {'classifier': 'composite', 'period': 14}}})
    assert cfg['classifier'] == 'adx' and cfg['windows_spec'] is None

def test_stops_from_kwargs_collects_all_present_and_drops_none():
    kwargs = {'open_strategy': {'name': 'x', 'params': {}}, 'close_strategies': [{'name': 'tiered_tp_atr', 'params': {}}], 'stop_loss_atr_mult': 1.5, 'stop_loss_pct': None, 'stop_loss_margin_pct': None, 'trailing_stop_atr_mult': None, 'trailing_stop_pct': None, 'stop_loss_atr_regime': None, 'trailing_stop_atr_regime': {'trending_up': 2.0, 'ranging_quiet': 3.0}}
    stops = m._stops_from_kwargs(kwargs)
    assert stops == {'stop_loss_atr_mult': 1.5, 'trailing_stop_atr_regime': {'trending_up': 2.0, 'ranging_quiet': 3.0}}
...
def test_backtester_kwargs_threads_present_stops_only():
    kw = m._backtester_kwargs('sqz', {}, [{'name': 'tiered_tp_atr', 'params': {}}], 'long', 10000.0, {'allowed_regimes': None}, stops={'stop_loss_atr_mult': 1.5})
...
def test_backtester_kwargs_no_stops_means_no_stop_keys():
    kw = m._backtester_kwargs('sqz', {}, None, 'long', 10000.0, {'allowed_regimes': None}, stops=None)
...
def test_backtester_kwargs_threads_regime_trailing_stop_with_open_as_close():
    ...
    kw = m._backtester_kwargs('sqz', {}, None, 'long', 10000.0, {'allowed_regimes': None}, stops={'trailing_stop_atr_regime': regime_trail})
...
def test_control_keeps_stop_regardless_of_candidate_mode():
    ...
    control_kw = m._backtester_kwargs('sqz', {}, [{'name': 'tiered_tp_atr', 'params': {}}], 'long', 10000.0, {'allowed_regimes': None}, stops=incumbent_stops)
    assert control_kw['stop_loss_atr_mult'] == 1.5
    inherit_kw = m._backtester_kwargs('sqz', {}, [{'name': 'atr_stop', 'params': {}}], 'long', 10000.0, {'allowed_regimes': None}, stops=m._candidate_stops('inherit', incumbent_stops))
    drop_kw = m._backtester_kwargs('sqz', {}, [{'name': 'atr_stop', 'params': {}}], 'long', 10000.0, {'allowed_regimes': None}, stops=m._candidate_stops('drop', incumbent_stops))
    assert inherit_kw['stop_loss_atr_mult'] == 1.5
    assert 'stop_loss_atr_mult' not in drop_kw

def _spec_args(extra=None):
    base = ['--strategy', 'squeeze_momentum', '--incumbent-close', 'none', '--candidate-close', '[{"name":"atr_stop","params":{}}]']
...
_MULTI_WINDOW = '{"fast":{"classifier":"composite","period":7},"slow":{"classifier":"composite","period":21}}'
_SINGLE_WINDOW = '{"slow":{"classifier":"composite","period":21}}'

def test_gate_window_on_multi_window_spec_rejected():
    with pytest.raises(SystemExit):
        m._resolve_spec(_spec_args(['--regime-windows-json', _MULTI_WINDOW, '--gate-window', 'slow']))
    with pytest.raises(SystemExit):
        m._resolve_spec(_spec_args(['--regime-windows-json', _MULTI_WINDOW, '--gate-window', 'fast']))
...
def test_gate_window_naming_absent_window_rejected():
    with pytest.raises(SystemExit):
        m._resolve_spec(_spec_args(['--regime-windows-json', _SINGLE_WINDOW, '--gate-window', 'nope']))

def test_gate_window_on_single_window_spec_naming_that_window_ok():
    spec = m._resolve_spec(_spec_args(['--regime-windows-json', _SINGLE_WINDOW, '--gate-window', 'slow']))
...
def test_reject_regime_directional_policy_incumbent():
    with pytest.raises(SystemExit):
        m._reject_unreplayable_entry_shapers({'regime_directional_policy': {'trending_up': 'long'}})

def test_reject_profile_allocation_incumbent():
    with pytest.raises(SystemExit):
        m._reject_unreplayable_entry_shapers({'profile_allocation': {'window': 'long', 'param_sets': {}}})

def test_reject_names_all_offenders():
    with pytest.raises(SystemExit) as ei:
        m._reject_unreplayable_entry_shapers({'invert_signal': True, 'regime_directional_policy': {'x': 'long'}, 'profile_allocation': {'y': 1}})
    msg = str(ei.value)
    assert 'invert_signal' in msg and 'regime_directional_policy' in msg and ('profile_allocation' in msg)

def test_no_reject_when_entry_shapers_absent_or_falsy():
    m._reject_unreplayable_entry_shapers({'open_strategy': {'name': 'x'}})
    m._reject_unreplayable_entry_shapers({'invert_signal': False, 'regime_directional_policy': None, 'profile_allocation': {}})

def test_stop_candidate_stacks_under_inherited_stop():
    assert m._candidate_stacks_on_inherited_stop([{'name': 'atr_stop', 'params': {}}], 'inherit', {'stop_loss_atr_mult': 1.5})
    assert m._candidate_stacks_on_inherited_stop([{'name': 'trailing_stop_atr_regime', 'params': {}}], 'inherit', {'trailing_stop_atr_mult': 2.0})

def test_tp_ladder_candidate_does_not_stack():
    assert not m._candidate_stacks_on_inherited_stop([{'name': 'tiered_tp_atr', 'params': {}}], 'inherit', {'stop_loss_atr_mult': 1.5})
    assert not m._candidate_stacks_on_inherited_stop([{'name': 'zscore_target', 'params': {}}], 'inherit', {'stop_loss_atr_mult': 1.5})

def test_no_stack_warning_when_dropped_or_no_inherited_stop():
    assert not m._candidate_stacks_on_inherited_stop([{'name': 'atr_stop', 'params': {}}], 'drop', {'stop_loss_atr_mult': 1.5})
    assert not m._candidate_stacks_on_inherited_stop([{'name': 'atr_stop', 'params': {}}], 'inherit', {})
...
def test_replay_positions_anchored_on_df_signals_not_df(monkeypatch):
    ...
    def fake_run_free_arm(reg, open_name, params, sig, close_refs, direction, capital, gate, symbol, timeframe, stops=None):
        return {'total_trades': 1, 'total_return_pct': 0.0, 'max_drawdown_pct': 0.0, 'sharpe_ratio': 0.0, 'liquidated': False, 'trades': [_leg(entry_date=entry_ts, side='long')]}

    def fake_replay(reg, open_name, params, sig, sig_pos, side_sign, candidate_close, direction, capital, gate, symbol, timeframe, stops=None):
    ...
    spec = {'open_name': 'test_open', 'params': None, 'direction': 'long', 'incumbent_close': [{'name': 'tiered_tp_atr', 'params': {}}], 'candidate_close': [{'name': 'atr_stop', 'params': {}}], 'control_stops': {}, 'candidate_stops': {}, 'replayable': True, 'gate': {'allowed_regimes': None}, 'regime_cfg': {'classifier': 'adx'}, 'capital': 10000.0, 'n_resamples': 50, 'ci': 0.95, 'seed': 1}
    res = m.evaluate_dataset_window(object(), spec, 'BTC/USDT', '1h', ('2025-01-01', None))
...
def test_parser_accepts_and_rejects_intrabar_modes():
    ...
    assert p.parse_args(base + ['--intrabar-resolution', 'bar_close']).intrabar_resolution == 'bar_close'
    with pytest.raises(SystemExit):
