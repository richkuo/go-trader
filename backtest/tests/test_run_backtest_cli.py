...
def test_build_parser_accepts_platform_and_registry():
    ...
    args = parser.parse_args(['--registry', 'futures', '--platform', 'hyperliquid', '--strategy', 'sma_crossover', '--mode', 'single'])
...
def test_run_single_backtest_threads_platform_to_backtester(monkeypatch):
    ...
    class SpyBacktester:
        ...
        def run(self, df, **kwargs):
            return {'strategy_name': kwargs.get('strategy_name'), 'symbol': 'BTC/USDT', 'timeframe': '1d', 'start_date': str(df.index[0]), 'end_date': str(df.index[-1]), 'initial_capital': 1000.0, 'final_capital': 1000.0, 'total_return_pct': 0.0, 'annual_return_pct': 0.0, 'sharpe_ratio': 0.0, 'sortino_ratio': 0.0, 'max_drawdown_pct': 0.0, 'calmar_ratio': 0.0, 'volatility_pct': 0.0, 'win_rate': 0.0, 'profit_factor': 0.0, 'total_trades': 0, 'avg_win_pct': 0.0, 'avg_loss_pct': 0.0, 'trades': [], 'params': {}}
    df = pd.DataFrame({'open': [100] * 60, 'high': [101] * 60, 'low': [99] * 60, 'close': [100] * 60, 'volume': [1000] * 60}, index=pd.date_range('2024-01-01', periods=60, freq='D'))
    ...
    result = run_backtest.run_single_backtest(strategy_name='sma_crossover', symbol='BTC/USDT', timeframe='1d', since='2024-01-01', capital=777.0, platform='robinhood', registry='spot')
    assert result is not None
    assert seen['platform'] == 'robinhood', f'platform did not thread through to Backtester — got {seen}'
...
def test_backtester_imports_under_script_style_sys_path(tmp_path):
    ...
    snippet.write_text(f"import sys\nsys.path.insert(0, {backtest_dir!r})\nimport backtester\nbacktester.Backtester()\nprint('OK')\n")
    proc = subprocess.run([sys.executable, str(snippet)], cwd=tmp_path, capture_output=True, text=True, timeout=120)
    assert proc.returncode == 0, proc.stderr
    assert 'OK' in proc.stdout

def test_build_parser_accepts_close_stack_flags():
    parser = run_backtest._build_parser()
    args = parser.parse_args(['--mode', 'optimize', '--strategy', 'sma_crossover', '--sweep-close', '--optimize-metric', 'dd_adjusted_return', '--direction', 'long'])
...
def test_single_mode_threads_direction(monkeypatch):
    ...
    monkeypatch.setattr('sys.argv', ['run_backtest.py', '--mode', 'single', '--strategy', 'sma_crossover', '--direction', 'short'])
    run_backtest.main()
    assert seen['calls'][0]['direction'] == 'short'

def test_compare_mode_threads_direction(monkeypatch):
    seen = _spy_single(monkeypatch)
    monkeypatch.setattr('sys.argv', ['run_backtest.py', '--mode', 'compare', '--strategy', 'sma_crossover', '--direction', 'short'])
    run_backtest.main()
    assert seen['calls'][0]['direction'] == 'short'

def test_multi_mode_threads_direction(monkeypatch):
    seen = _spy_single(monkeypatch)
    monkeypatch.setattr('sys.argv', ['run_backtest.py', '--mode', 'multi', '--strategy', 'sma_crossover', '--symbols', 'BTC/USDT', '--direction', 'short'])
    run_backtest.main()
    assert seen['calls'][0]['direction'] == 'short'

def test_direction_both_without_close_rejected_before_running(monkeypatch):
    seen = _spy_single(monkeypatch)
    monkeypatch.setattr('sys.argv', ['run_backtest.py', '--mode', 'single', '--strategy', 'sma_crossover', '--direction', 'both'])
    with pytest.raises(SystemExit):
        run_backtest.main()
...
def test_direction_both_with_close_strategy_threads_through(monkeypatch):
    seen = _spy_single(monkeypatch)
    monkeypatch.setattr('sys.argv', ['run_backtest.py', '--mode', 'single', '--strategy', 'sma_crossover', '--direction', 'both', '--close-strategy', 'tiered_tp_atr'])
    run_backtest.main()
    assert seen['calls'][0]['direction'] == 'both'
    assert seen['calls'][0]['close_strategies'] == [{'name': 'tiered_tp_atr', 'params': {}}]

def test_direction_short_rejected_in_optimize_mode(monkeypatch):
    seen = {}
    monkeypatch.setattr(run_backtest, 'run_walk_forward', lambda *a, **kw: seen.setdefault('hit', True))
    for extra in ([], ['--sweep-close'], ['--close-strategy', 'tiered_tp_atr']):
        monkeypatch.setattr('sys.argv', ['run_backtest.py', '--mode', 'optimize', '--strategy', 'sma_crossover', '--direction', 'short', *extra])
        with pytest.raises(SystemExit):
            run_backtest.main()
...
def test_direction_both_with_default_sweep_grid_rejected(monkeypatch):
    seen = {}
    monkeypatch.setattr(run_backtest, 'run_walk_forward', lambda *a, **kw: seen.setdefault('hit', True))
    monkeypatch.setattr('sys.argv', ['run_backtest.py', '--mode', 'optimize', '--strategy', 'sma_crossover', '--sweep-close', '--direction', 'both'])
    with pytest.raises(SystemExit):
        run_backtest.main()
    assert 'hit' not in seen

def test_direction_both_with_close_only_stacks_json_reaches_walk_forward(monkeypatch, tmp_path):
    import json
    specs = tmp_path / 'stacks.json'
    specs.write_text(json.dumps([{'close': {'name': 'tiered_tp_atr', 'params': {}}}]))
    seen = {}

    def spy_wf(*args, **kwargs):
        seen.update(kwargs)
    monkeypatch.setattr(run_backtest, 'run_walk_forward', spy_wf)
    monkeypatch.setattr('sys.argv', ['run_backtest.py', '--mode', 'optimize', '--strategy', 'sma_crossover', '--close-stacks-json', str(specs), '--direction', 'both'])
    run_backtest.main()
...
def test_run_walk_forward_threads_close_stack_grid(monkeypatch):
    ...
    monkeypatch.setattr(run_backtest, 'load_cached_data', lambda *a, **k: pd.DataFrame({'open': [1.0] * 300, 'high': [1.0] * 300, 'low': [1.0] * 300, 'close': [1.0] * 300, 'volume': [1.0] * 300}, index=pd.date_range('2024-01-01', periods=300, freq='D')))
    grid = [{'label': 'baseline', 'close_strategies': [], 'stop_loss_atr_mult': None, 'trailing_stop_atr_mult': None}]
    run_backtest.run_walk_forward('sma_crossover', close_stack_grid=grid, optimize_metric='dd_adjusted_return', direction='long')
