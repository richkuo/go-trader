...
import pytest
from backtest_options import ADAPTER_STRIKE_STEP, DEFAULT_STRIKE_STEP, adapter_strike, black_scholes_price
...
def test_strike_grid_matches_adapter_source():
    ...
    match = re.search('def get_real_strike\\([^)]*\\)[^:]*:.*?return round\\(target_strike / (\\d+)\\) \\* (\\d+)', text, re.DOTALL)
...
@pytest.mark.parametrize('spot,strike,dte,vol,option_type', [(67000, 68000, 30, 0.8, 'call'), (67000, 66000, 30, 0.8, 'put'), (67000, 67000, 7, 0.6, 'call'), (3400, 3500, 14, 0.7, 'call'), (3400, 3300, 45, 0.7, 'put')])
def test_backtest_bs_matches_shared_pricing(spot, strike, dte, vol, option_type):
    ...
    assert got == pytest.approx(expected, rel=1e-12), f'Drift between backtest BS and shared_tools BS on ({spot}, {strike}, {dte}d, {vol}, {option_type})'

def test_greeks_populated_on_bs_call():
    price, greeks = bs_price_and_greeks(67000, 67000, 30, 0.8, option_type='call')
    assert price > 0
...
def test_greeks_populated_on_bs_put():
    price, greeks = bs_price_and_greeks(67000, 67000, 30, 0.8, option_type='put')
    assert price > 0
    assert -0.6 < greeks['delta'] < -0.3, greeks
    assert greeks['gamma'] > 0
    assert greeks['theta'] < 0

def _deterministic_candles(underlying: str='BTC', start_price: float=50000.0, n: int=200) -> list:
    candles = []
    price = start_price
    for i in range(n):
...
def test_trade_log_entries_carry_delta_and_on_grid_strikes():
    ...
    bt = OptionsBacktester(initial_capital=10000, max_positions=2, check_interval=1)
    bt.run_vol_mean_reversion(candles, 'BTC')
    opens = [t for t in bt.trade_log if t['event'] == 'open']
    assert opens, 'Deterministic high-vol tail must produce at least one entry; if this fails, the iv_rank thresholds or vol-window sizes changed'
    for t in opens:
        assert 'delta' in t, f'trade log entry missing delta: {t}'
        assert t['strike'] % ADAPTER_STRIKE_STEP['BTC'] == 0, f"BTC strike {t['strike']} is off the $1000 grid"
