...
def _synthetic_ohlcv(n=600, seed=0):
    ...
    return pd.DataFrame({'open': openp, 'high': np.maximum(high, np.maximum(openp, close)), 'low': np.minimum(low, np.minimum(openp, close)), 'close': close, 'volume': volume}, index=idx)

def _synthetic_funding(idx, seed=1):
    rng = np.random.default_rng(seed)
    ts = (idx.as_unit('ns').asi8 // 1000000).astype('int64')
    rate = rng.normal(0.0001, 5e-05, size=len(idx))
...
def test_subset_selection_preserves_canonical_first_order():
    df = _synthetic_ohlcv()
    fund = _synthetic_funding(df.index)
    mat = ref.enriched_feature_matrix(df, period=48, funding=fund, columns=['htf_range_eff', 'adx', 'return_eff', 'funding_rate', 'range_eff', 'efficiency'])
    assert list(mat.columns) == ['return_eff', 'range_eff', 'efficiency', 'adx', 'funding_rate', 'htf_range_eff']
...
def test_htf_feature_does_not_leak_in_progress_bar():
    df = _synthetic_ohlcv(n=600)
    mat = ref.enriched_feature_matrix(df, period=20, columns=ref.CANONICAL_COLUMNS + ['htf_range_eff'], htf_multiple=4)
    htf = mat['htf_range_eff'].to_numpy()
    mutated = df.copy()
    mutated.iloc[-1, mutated.columns.get_loc('high')] *= 5.0
    htf2 = ref.enriched_feature_matrix(mutated, period=20, columns=ref.CANONICAL_COLUMNS + ['htf_range_eff'], htf_multiple=4)['htf_range_eff'].to_numpy()
...
def test_volume_z_is_trailing_and_warmup_nan():
    ...
    mat = ref.enriched_feature_matrix(df, period=48, columns=ref.CANONICAL_COLUMNS + ['volume_z'], vol_window=24)
    ...
    assert vz[i] == pytest.approx(expected, rel=1e-09)

def test_hurst_column_is_trailing_and_warmup_nan():
    df = _synthetic_ohlcv(n=300)
    mat = ref.enriched_feature_matrix(df, period=48, columns=ref.CANONICAL_COLUMNS + ['hurst'], hurst_window=100)
...
def test_hurst_column_uses_own_window_independent_of_period():
    df = _synthetic_ohlcv(n=300)
    mat_a = ref.enriched_feature_matrix(df, period=20, columns=ref.CANONICAL_COLUMNS + ['hurst'], hurst_window=100)
    mat_b = ref.enriched_feature_matrix(df, period=48, columns=ref.CANONICAL_COLUMNS + ['hurst'], hurst_window=100)
...
def test_fit_unsupervised_enriched_schema_and_decode():
    ...
    model = rvm.fit_unsupervised(mat.to_numpy(dtype=float), family='hmm', k=4, filter_window=32, thresholds=dict(TH), seed=0, feature_names=cols, canonical_indices=ref.canonical_indices_for(cols))
...
def test_fit_unsupervised_rejects_feature_name_count_mismatch():
    ...
    with pytest.raises(ValueError, match='columns but feature_names lists'):
        rvm.fit_unsupervised(feats, family='kmeans', k=3, filter_window=16, thresholds=dict(TH), feature_names=['a', 'b', 'c'])
...
def test_decode_contract_rejects_column_reorder():
    ...
    cols = ref.ENRICHED_COLUMNS
    mat = ref.enriched_feature_matrix(df, period=48, funding=fund, columns=cols)
    model = rvm.fit_unsupervised(mat.to_numpy(dtype=float), family='kmeans', k=4, filter_window=32, thresholds=dict(TH), seed=0, feature_names=cols, canonical_indices=(0, 1, 2, 3))
...
def test_column_order_is_load_bearing_for_labels():
    ...
    cols = ref.ENRICHED_COLUMNS
    mat = ref.enriched_feature_matrix(df, period=48, funding=fund, columns=cols)
    model = rvm.fit_unsupervised(mat.to_numpy(dtype=float), family='hmm', k=5, filter_window=32, thresholds=dict(TH), seed=0, feature_names=cols, canonical_indices=(0, 1, 2, 3))
    correct, _ = forward_filter_labels(mat.to_numpy(dtype=float), model)
    swapped = mat[['adx', 'range_eff', 'efficiency', 'return_eff', 'funding_rate', 'volume_z', 'htf_range_eff', 'hurst']]
...
def test_naming_ignores_extra_dims_given_same_canonical_centroids():
    ...
    mean = np.zeros(6)
    std = np.ones(6)
    em_a = np.array([[0.0, 0.02, 0.1, 8.0, 0.0, 0.0], [0.4, 0.5, 0.9, 40.0, 0.0, 0.0]], dtype=float)
    em_b = em_a.copy()
    em_b[:, 4:] = 99.0
...
def test_naming_with_canonical_columns_not_first():
    ...
    mean = np.zeros(5)
    std = np.ones(5)
    em = np.array([[0.0, 0.0, 0.02, 0.1, 8.0], [0.0, 0.4, 0.5, 0.9, 40.0]], dtype=float)
...
def test_combined_family_plan_spans_all_subsets():
    ...
    th = rvm.NonDegeneracyThresholds(min_active_labels=4, max_occupancy=0.9, min_transition_rate=0.0)
    subsets = ['canonical', 'volume', 'htf']
    plan, reasons, denom = m95.combined_family_plan(subsets, ('kmeans', 'hmm'), range(3, 6), th, ineligible_reason_fn=m80.structurally_ineligible_reason)
...
def test_enriched_bakeoff_smoke_if_data_available():
    try:
        ...
        report = mod.run_bakeoff('BTC/USDT', '1h', families=('kmeans',), k_range=range(3, 4), subsets={'canonical': list(ref.CANONICAL_COLUMNS), 'volume': ref.CANONICAL_COLUMNS + ['volume_z']}, eval_windows=('is', 'oos'), n_perm=200)
    ...
    assert report['bonferroni_denominator'] == sum((1 for c in report['candidates'] if not c['structurally_ineligible']))
