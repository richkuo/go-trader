...
from backtest_pairs import PairsBacktester, SIDE_LONG_A, SIDE_SHORT_A, _liquidation_loss

def _flat_df(prices: list[float], start: str='2024-01-01') -> pd.DataFrame:
    idx = pd.date_range(start, periods=len(prices), freq='1h')
    return pd.DataFrame({'open': prices, 'high': prices, 'low': prices, 'close': prices, 'volume': [1.0] * len(prices)}, index=idx)
...
def _make_spread_series(n: int, base_a: float, base_b: float, spike_at: int, spike_pct: float, direction: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    prices_a = [base_a] * n
    prices_b = [base_b] * n
...
def test_short_a_signal_fires_when_spread_high() -> None:
    df_a, df_b = _make_spread_series(n=300, base_a=2000.0, base_b=60000.0, spike_at=200, spike_pct=0.05, direction=+1)
    bt = PairsBacktester(base_notional=1000.0, beta=1.0, leverage=20.0, lookback=50, entry_z=2.0, exit_z=0.5, taker_fee_pct=0.0, maker_fee_pct=0.0)
    results = bt.run(df_a, df_b)
    assert any((t.side_a == SIDE_SHORT_A for t in results.trades)), 'spike up in A should trigger short-A trade'

def test_long_a_signal_fires_when_spread_low() -> None:
    df_a, df_b = _make_spread_series(n=300, base_a=2000.0, base_b=60000.0, spike_at=200, spike_pct=0.05, direction=-1)
    bt = PairsBacktester(base_notional=1000.0, beta=1.0, leverage=20.0, lookback=50, entry_z=2.0, exit_z=0.5, taker_fee_pct=0.0, maker_fee_pct=0.0)
    results = bt.run(df_a, df_b)
    assert any((t.side_a == SIDE_LONG_A for t in results.trades)), 'spike down in A should trigger long-A trade'

def test_beta_hedge_scales_b_notional() -> None:
    df_a, df_b = _make_spread_series(n=300, base_a=2000.0, base_b=60000.0, spike_at=200, spike_pct=0.05, direction=+1)
    bt = PairsBacktester(base_notional=1000.0, beta=1.2, leverage=20.0, lookback=50, entry_z=2.0, exit_z=0.5, taker_fee_pct=0.0, maker_fee_pct=0.0)
    results = bt.run(df_a, df_b)
...
def test_fees_accumulate_round_trip() -> None:
    df_a, df_b = _make_spread_series(n=300, base_a=2000.0, base_b=60000.0, spike_at=200, spike_pct=0.05, direction=+1)
    bt = PairsBacktester(base_notional=1000.0, beta=1.0, leverage=20.0, lookback=50, entry_z=2.0, exit_z=0.5, taker_fee_pct=0.000432, maker_fee_pct=0.000144)
    results = bt.run(df_a, df_b)
    closed = [t for t in results.trades if t.exit_bar is not None]
    assert closed, 'expected at least one closed trade'
    t = closed[0]
    expected_fee = (1000.0 + 1000.0) * 0.000432 * 2
    assert t.fees == pytest.approx(expected_fee, rel=1e-06)

def test_liquidation_on_large_adverse_move() -> None:
    n = 300
    prices_a = [2000.0] * 200 + [2100.0] * 5 + [2180.0] * (n - 205)
    prices_b = [60000.0] * n
    df_a = _flat_df(prices_a)
    df_b = _flat_df(prices_b)
    bt = PairsBacktester(base_notional=1000.0, beta=1.0, leverage=20.0, maintenance_margin=0.02, lookback=50, entry_z=2.0, exit_z=0.5, taker_fee_pct=0.0, maker_fee_pct=0.0)
    results = bt.run(df_a, df_b)
...
def test_liquidation_loss_capped_at_margin_posted() -> None:
    ...
    t = liquidated[0]
    assert -t.pnl_a <= t.margin_a + 1e-09, f'leg A loss {-t.pnl_a:.2f} exceeds posted margin {t.margin_a:.2f}'
    max_loss = t.margin_a + t.margin_b + t.fees
    assert -t.net_pnl <= max_loss + 1e-09, f'net loss {-t.net_pnl:.2f} exceeds margin+fees cap {max_loss:.2f}'

def test_funding_accrues_correctly_per_leg() -> None:
    df_a, df_b = _make_spread_series(n=300, base_a=2000.0, base_b=60000.0, spike_at=200, spike_pct=0.05, direction=+1)
    bt = PairsBacktester(base_notional=1000.0, beta=0.0, leverage=20.0, lookback=50, entry_z=2.0, exit_z=0.5, taker_fee_pct=0.0, maker_fee_pct=0.0, funding_a_per_hour=0.0001, funding_b_per_hour=0.0, bar_hours=1.0)
    results = bt.run(df_a, df_b)
    ...
    assert expected_min <= t.funding <= expected_max, f'funding {t.funding} not in [{expected_min}, {expected_max}] for {bars_held} bars'
...
def test_no_lookahead_fills_at_next_bar_open() -> None:
    ...
    assert t.entry_price_a == pytest.approx(2050.0), f'entry price should be bar 201 open (2050), got {t.entry_price_a}'
