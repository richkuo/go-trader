...
def test_doubled_tail_formula_is_the_pre_registered_one():
    ...
    assert study.doubled_tail_p(20, 80, 100) == pytest.approx(2.0 * 21.0 / 101.0,
                                                              abs=1e-6)
...
def test_both_tails_are_counted_over_surviving_draws_not_the_request():
    ...
    out = study.two_sided_cluster_permutation_pvalue_group_diff(
        trades, values, mask, n_perm=300, seed=1426)
...
def test_a_reversed_effect_is_detected_by_the_two_sided_cluster_null():
    ...
    one = study1422.cluster_permutation_pvalue_group_diff(
...
def test_a_reversed_effect_is_detected_by_the_two_sided_free_shuffle():
    ...
    assert study1410.permutation_pvalue_group_diff(
        values, mask, n_perm=2000, seed=1426) > 0.9
    assert study.two_sided_permutation_pvalue_group_diff(
        values, mask, n_perm=2000, seed=1426) <= study.ALPHA
...
def test_a_forward_effect_is_still_detected():
    ...
    assert study.two_sided_cluster_permutation_pvalue_group_diff(
        trades, values, mask, n_perm=2000, seed=1426)["p"] <= study.ALPHA
    assert study.two_sided_permutation_pvalue_group_diff(
        values, mask, n_perm=2000, seed=1426) <= study.ALPHA
...
def test_a_reversed_sizing_pairing_is_detected():
    ...
    assert study.two_sided_permutation_pvalue_weighted(
        rets, mults, n_perm=2000, seed=1426) <= study.ALPHA
    assert study.two_sided_cluster_permutation_pvalue_weighted(
        trades, rets, mults, n_perm=2000, seed=1426)["p"] <= study.ALPHA
...
def test_the_two_sided_functions_keep_the_untestable_semantics():
    ...
    assert study.two_sided_permutation_pvalue_group_diff(
        [1.0, 2.0], [True, True], n_perm=10) is None
    assert study.two_sided_permutation_pvalue_weighted(
        [1.0, 2.0], [1.0, 1.0], n_perm=10) is None
    out = study.two_sided_cluster_permutation_pvalue_group_diff(
        [], [], [], n_perm=10)
...
def _single_direction_limit(values, mask, sign, *, grid_step=0.05, grid_max=1.0,
    ...
    for i in range(int(round(grid_max / grid_step)) + 1):
        ...
        p = study.two_sided_permutation_pvalue_group_diff(
            shifted, m, n_perm=n_perm, seed=study.SEED)
...
def test_the_two_sided_limit_is_the_max_over_the_two_directions():
    ...
    both = study.two_sided_min_detectable_effect_on_grid(
        trades, values, mask, 1, grid_step=0.05, grid_max=1.0, refine_step=0.05,
        cluster=False, n_perm=1000, seed=study.SEED)
...
def test_an_injection_of_the_published_limit_is_detected_pointing_either_way():
    trades, values, mask = _mde_inputs()
    limit = study.two_sided_min_detectable_effect_on_grid(
        trades, values, mask, 1, grid_step=0.05, grid_max=1.0, refine_step=0.05,
        cluster=False, n_perm=1000, seed=study.SEED)
    ...
    for sign in (+1.0, -1.0):
        ...
        p = study.two_sided_permutation_pvalue_group_diff(
            shifted, m, n_perm=1000, seed=study.SEED)
...
def test_the_resolvability_floor_is_two_over_n_plus_one():
    ...
    study.two_sided_min_detectable_effect_on_grid(
        trades, values, mask, 1, grid_step=0.5, grid_max=0.5, refine_step=0.5,
        cluster=False, n_perm=39)


def test_the_mde_returns_none_when_no_grid_point_clears_the_bar():
    trades, values, mask = _mde_inputs(n=60)
    out = study.two_sided_min_detectable_effect_on_grid(
        trades, values, mask, 1, grid_step=0.0001, grid_max=0.0001,
        refine_step=0.0001, cluster=False, n_perm=200)
...
def test_the_eff_and_pp_wrappers_use_1424s_grids_verbatim():
    ...
    assert calls == [
        (study1424.MDE_EFF_GRID_STEP, study1424.MDE_EFF_GRID_MAX,
         study1424.MDE_EFF_REFINE_STEP),
        (study1424.MDE_PP_GRID_STEP, study1424.MDE_PP_GRID_MAX,
         study1424.MDE_PP_REFINE_STEP),
    ]
...
def test_detection_limits_report_a_row_matched_zero_injection_p():
    mde = study.measure_detection_limits(_pooled_for_sweep(), [512],
                                         n_perm=_SWEEP_N_PERM, seed=study.SEED)
...
def _mde(mom_limit=0.05, mom_sep=0.09, mr_limit=0.05, mr_sep=0.02, p0=0.4,
    ...
    out = {
        ...
        "by_family_cluster_p0": {study.PRIMARY_FAMILY: p0, _MR: 0.6},
...
def test_the_gate_fails_closed_on_an_unreachable_limit():
    gate = study.validity_gate(_mde(mom_limit=None, mom_sep=-0.30))
...
def test_the_gate_ignores_the_other_familys_larger_separation():
    gate = study.validity_gate(_mde(mom_limit=0.20, mom_sep=0.05,
                                    mr_limit=0.001, mr_sep=-0.90))
    assert gate["passed"] is False
...
def test_there_is_no_reversed_mode_in_a_symmetric_study():
    ...
    for sep in (-0.30, 0.30):
        assert study.validity_gate(_mde(mom_limit=0.01, mom_sep=sep))["mode"] \
            == study.MODE_OK
...
def _passing_cfg(**over):
    cfg = {
        ...
        "family": study.PRIMARY_FAMILY, "mode": "gate", "hurst_window": 512,
        "n_pooled_trades": 400, "n_pooled_effective": 60.0,
        "n_suppressed_effective": 100.0, "n_kept_effective": 100.0,
        "n_suppressed": 200, "n_kept": 200, "n_missing_target": 0,
        "separation": -0.005, "separation_return": -0.12,
        "p_raw": 0.001, "p_cluster": 0.001, "p_cluster_return": 0.01,
        "bh_reject": True, "cluster_excluded_datasets": [],
...
def test_a_config_that_passes_1424s_rule_still_wins_nothing():
    ...
    decision = study.decide_recommendation([cfg], _mde(p0=0.001, mom_limit=0.01,
                                                       mom_sep=-0.30))
...
def test_no_verdict_can_name_a_winner_whatever_the_inputs():
    for p0, limit, sep in ((0.001, 0.01, -0.30), (0.9, 0.01, -0.30),
                           (0.9, 0.5, -0.005), (0.001, 0.5, 0.9)):
        decision = study.decide_recommendation(
            [_passing_cfg()], _mde(p0=p0, mom_limit=limit, mom_sep=sep))
        assert all(v["winner"] is None
                   for v in decision["families"].values()), (p0, limit, sep)
...
def test_every_verdict_carries_the_exploratory_only_sentence():
    ...
    assert seen == {study.VERDICT_SORT_DETECTED, study.VERDICT_RESOLVED_NULL,
                    study.VERDICT_INCONCLUSIVE}


def test_sort_detected_names_the_direction_from_the_sign():
    down = study.decide_recommendation(
        [], _mde(p0=0.001, mom_limit=0.01, mom_sep=-0.30))
    ...
    up = study.decide_recommendation(
        [], _mde(p0=0.001, mom_limit=0.01, mom_sep=0.30))
...
def test_an_underpowered_null_claims_no_bound_and_stays_inconclusive():
    decision = study.decide_recommendation(
        [], _mde(p0=0.9, mom_limit=0.5, mom_sep=-0.005))
...
def test_an_untestable_confirmatory_p_is_not_significance():
    decision = study.decide_recommendation(
        [], _mde(p0=None, mom_limit=0.5, mom_sep=-0.005))
...
_FAILING_MDE = _mde(mom_limit=0.02, mom_sep=-0.005, p0=0.71,
                    ...
                    pooled_primary_cluster_return=1.2, pooled_primary_n=28998,
                    pooled_primary_cluster_p0=0.9, pooled_primary_free_p0=0.9,
                    pooled_primary_cluster_return_p0=1.0)
...
def test_report_prints_the_validity_gate_outcome_both_ways():
    ...
    passing = study.report_from_payload(
        _render_payload(mde=_mde(mom_limit=0.001, mom_sep=-0.30, p0=0.9)))
...
def test_largest_magnitude_signed_prefers_size_and_keeps_the_sign():
    assert study._largest_magnitude_signed({"a|512": 0.10, "b|512": -0.40}) == \
        pytest.approx(-0.40)
    assert study._largest_magnitude_signed({"a|512": 0.40, "b|512": -0.10}) == \
        pytest.approx(0.40)
...
def test_the_pre_registered_draw_count_resolves_the_binding_bar():
    ...
    assert 2.0 / (study.N_PERM_MDE + 1.0) <= binding, (
        "the pre-registered detection-limit draw count must resolve the "
        "tightest bar in the run")
...
def test_only_a_passing_gate_ever_claims_a_bound():
    ...
    no_bound = study.decide_recommendation(
        [], _mde(p0=0.9, mom_limit=0.5, mom_sep=-0.005))
    ...
    assert "bounds any sorting effect" in no_bound["justification"] or \
        "no bound" in no_bound["justification"]
...
def test_1426_does_not_default_to_the_contract_path():
    ...
    assert os.path.basename(study._DEFAULT_JSON_OUT) == \
        "hurst_1426_two_sided_sort.json"
