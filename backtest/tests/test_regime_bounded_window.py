...
from regime import _DEFAULT_COMPOSITE_THRESHOLDS, VALID_LABELS_COMPOSITE, composite_feature_matrix, compute_regime_composite
from regime_hmm import fit_label_anchored_hmm, forward_filter_labels
from regime_bounded_window_validate import DEFAULT_AGREEMENT_THRESHOLD, _align_eval_start, _gate_primary_horizon, _provenance_status, _sweep_blocked, adx_drift_stats, bounded_window_adx, bounded_window_views, full_window_views, go_no_go, label_drift_stats, validate_frames
...
def _ohlcv(n, seed=0, regime_mix=True):
    ...
    if regime_mix:
        a = max(1, n // 3)
        seg = np.concatenate([np.linspace(0, 60, a), np.full(n - 2 * a, 60.0), np.linspace(60, 10, a)])[:n]
    else:
        seg = np.zeros(n)
...
def _fit_model(df, filter_window=24):
    ...
    model = fit_label_anchored_hmm(feats, labels, sorted(VALID_LABELS_COMPOSITE), filter_window=filter_window)
    model['period'] = PERIOD
    return model
...
def test_bounded_adx_equals_full_when_lookback_spans_window():
    ...
    m = ~np.isnan(full)
    assert m.any()
    np.testing.assert_allclose(bounded[m], full[m], rtol=0, atol=1e-09)
...
def test_label_drift_stats_counts_disagreements_on_valid_bars_only():
    ...
    assert abs(s['agreement'] - 2 / 3) < 1e-09
    assert s['transitions'] == {'a->x': 1}

def _scored(kw_h, transition_rate, p_value=0.005):
    return {'stability': {'transition_rate': transition_rate}, 'h4': {'separation': {'kruskal_h': kw_h}, 'significance': {'p_value': p_value}}}

def _drift(agreement, n=500):
    return {'n': n, 'agreement': agreement, 'disagreements': int(round((1 - agreement) * n)), 'transitions': {}}
...
def test_go_no_go_zero_comparable_bars_vacuous_agreement_blocks():
    empty = label_drift_stats(np.array([], dtype=object), np.array([], dtype=object), np.array([], dtype=bool))
...
def test_validate_frames_end_to_end_small():
    ...
    rep = validate_frames(df_window, df_ext, eval_start, model, period=PERIOD, lookback=120, target='volatility', seed=0, horizons=(4,))
...
def test_validate_frames_handrule_only_without_model():
    ...
    eval_start = 140
    df_window = df_ext.iloc[eval_start:].reset_index(drop=True)
    rep = validate_frames(df_window, df_ext, eval_start, None, period=PERIOD, lookback=90, target='volatility', seed=0, horizons=(4,))
...
def test_validate_frames_rejects_horizons_missing_gate_primary():
    ...
    with pytest.raises(ValueError, match='h4'):
        validate_frames(df_window, df_ext, eval_start, model, period=PERIOD, lookback=90, horizons=(1, 12))

def test_validate_frames_handrule_only_allows_horizons_without_gate_primary():
    df_ext = _ohlcv(220, seed=32)
    eval_start = 140
    df_window = df_ext.iloc[eval_start:].reset_index(drop=True)
    rep = validate_frames(df_window, df_ext, eval_start, None, period=PERIOD, lookback=90, horizons=(1, 12))
...
def test_provenance_status_flags_in_sample_and_out_of_sample():
    ...
    assert diff_symbol['in_sample'] is False
...
def test_provenance_status_detects_date_overlap_across_window_names():
    WIN = {'is': ('2025-06-10', '2026-01-01'), 'oos': ('2026-01-01', None), '2023': ('2023-01-01', '2024-01-01'), '2025H1': ('2025-01-01', '2025-07-01')}
    ...
    assert boundary['in_sample'] is False
    cross_instrument = _provenance_status(model, 'ETH/USDT', '1h', '2025H1', WIN)
    assert cross_instrument['in_sample'] is False
...
def test_provenance_status_unresolvable_fit_window_is_unverifiable():
    ...
    assert p['overlap_resolvable'] is False
    assert p['in_sample'] is False
...
def test_go_no_go_unverified_provenance_blocks_when_required():
    hr = _scored(10.0, 0.4)
    md = _scored(9.7, 0.25)
    v = go_no_go(md, hr, md, hr, _drift(0.99), provenance_verified=False, require_provenance=True)
    assert v['promote'] is False
...
def test_validate_frames_in_sample_blocks_end_to_end():
    ...
    df_window = df_ext.iloc[eval_start:].reset_index(drop=True)
    model = _fit_model(df_ext, filter_window=24)
    rep = validate_frames(df_window, df_ext, eval_start, model, period=PERIOD, lookback=120, target='volatility', seed=0, horizons=(4,), in_sample=True)
...
def test_validate_frames_scores_incumbent_at_its_own_period():
    ...
    model['period'] = 30
    rep = validate_frames(df_window, df_ext, eval_start, model, incumbent_period=48, lookback=110, target='volatility', seed=0, horizons=(4,))
    assert rep['model_period'] == 30
    assert rep['incumbent_period'] == 48
    assert rep['model']['period'] == 30
    assert rep['handrule']['period'] == 48
import json
import regime_bounded_window_validate as rbwv
...
def _argv(model_json, window, lookback=250, extra=None):
    a = ['--symbol', 'BTC/USDT', '--timeframe', '1h', '--window', window, '--model-json', model_json, '--lookback', str(lookback), '--horizons', '4', '--agreement-threshold', '0', '--min-agreement-bars', '1', '--seed', '0']
    return a + (extra or [])
...
def test_validate_blocks_confirmed_in_sample_model(monkeypatch):
    ...
    model = _fit_model(df_ext, filter_window=24)
    model['fitted_on'] = {'symbol': 'BTC/USDT', 'timeframe': '1h', 'window': 'is'}
    rep = rbwv.validate('BTC/USDT', '1h', 'is', model, lookback=250, agreement_threshold=0.0, min_agreement_bars=1)
...
def test_validate_promotes_disjoint_oos_model(monkeypatch):
    ...
    model = _fit_model(df_ext, filter_window=24)
    model['fitted_on'] = {'symbol': 'BTC/USDT', 'timeframe': '1h', 'window': 'is'}
    rep = rbwv.validate('BTC/USDT', '1h', 'oos', model, lookback=250, agreement_threshold=0.0, min_agreement_bars=1)
...
def test_main_exit_codes_block_in_sample_promote_oos(monkeypatch, tmp_path, capsys):
    ...
    assert rbwv.main(_argv(mj, 'is')) == 1
    capsys.readouterr()
    assert rbwv.main(_argv(mj, 'oos')) == 0

def test_main_require_provenance_blocks_unstamped(monkeypatch, tmp_path, capsys):
    df_ext, _ = _wire_loader(monkeypatch)
    _stub_gate_ship(monkeypatch)
    mj = _write_model(tmp_path, df_ext, fitted_on=None)
    assert rbwv.main(_argv(mj, 'oos')) == 0
    capsys.readouterr()
    assert rbwv.main(_argv(mj, 'oos', extra=['--require-provenance'])) == 1

def test_main_rejects_horizons_missing_gate_primary(monkeypatch, tmp_path):
    import pytest
    _wire_loader(monkeypatch)
    df = _ohlcv(200, seed=41)
    mj = _write_model(tmp_path, df, fitted_on={'symbol': 'BTC/USDT', 'timeframe': '1h', 'window': 'is'})
...
def test_main_sweep_worst_case_exit_on_in_sample(monkeypatch, tmp_path):
    ...
    mj = _write_model(tmp_path, df_ext, fitted_on={'symbol': 'BTC/USDT', 'timeframe': '1h', 'window': 'is'})
    argv = ['--symbol', 'BTC/USDT', '--timeframe', '1h', '--window', 'is', '--model-json', mj, '--lookback-sweep', '120,250', '--horizons', '4', '--agreement-threshold', '0', '--min-agreement-bars', '1']
    assert rbwv.main(argv) == 1
