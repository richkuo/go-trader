...
def test_composite_label_allows_entry_when_gate_matches(label):
    df = _gated_df(label)
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0, regime_enabled=True, allowed_regimes=[label])
    result = bt.run(df, save=False)
    assert result['total_trades'] >= 1, f"Composite label '{label}' in allowed_regimes should permit the entry"
...
def test_composite_label_blocks_entry_when_gate_mismatches(label):
    ...
    df = _gated_df(label)
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0, regime_enabled=True, allowed_regimes=[other])
    result = bt.run(df, save=False)
    assert result['total_trades'] == 0, f"Bar regime '{label}' must be blocked when only '{other}' is allowed"

@pytest.mark.parametrize('label', COMPOSITE_LABELS)
def test_composite_label_allowed_within_multi_label_gate(label):
    allow = [label] + [l for l in COMPOSITE_LABELS if l != label][:2]
    df = _gated_df(label)
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0, regime_enabled=True, allowed_regimes=allow)
    result = bt.run(df, save=False)
    assert result['total_trades'] >= 1

def test_clean_and_choppy_variants_are_distinct_gates():
    df_choppy = _gated_df('trending_up_choppy')
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0, regime_enabled=True, allowed_regimes=['trending_up_clean'])
    assert bt.run(df_choppy, save=False)['total_trades'] == 0
    df_clean = _gated_df('trending_up_clean')
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0, regime_enabled=True, allowed_regimes=['trending_up_choppy'])
...
def test_ranging_directional_blocked_by_trending_gate():
    ...
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0, regime_enabled=True, allowed_regimes=['trending_up_clean', 'trending_down_clean'])
...
def test_bare_ranging_directional_covers_sub_label_entry(sub):
    df = _gated_df(sub)
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0, regime_enabled=True, allowed_regimes=['ranging_directional'])
...
def test_explicit_sub_label_does_not_cover_bare_or_sibling_entry():
    for bar in ['ranging_directional', 'ranging_directional_down']:
        df = _gated_df(bar)
        bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0, regime_enabled=True, allowed_regimes=['ranging_directional_up'])
        assert bt.run(df, save=False)['total_trades'] == 0, f"explicit _up must NOT cover bar '{bar}'"
