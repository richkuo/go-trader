...
from backtester import Backtester, periods_per_year, TIMEFRAME_PERIODS_PER_YEAR
...
def _df_with_signals(signals):
    ...
    return pd.DataFrame({'open': closes, 'close': closes, 'signal': signals}, index=idx)
...
def test_signal_out_of_domain_raises():
    ...
    with pytest.raises(ValueError, match='signal column must be in'):
...
def test_sharpe_scales_with_timeframe():
    ...
    assert ratio == pytest.approx(math.sqrt(6), rel=0.02), f'4h Sharpe / 1d Sharpe should be sqrt(6) ≈ 2.449; got {ratio:.4f}'
...
def test_volatility_scales_with_timeframe():
    ...
    assert ratio == pytest.approx(math.sqrt(24), rel=0.02), f'1h vol / 1d vol should be sqrt(24); got {ratio:.4f}'
...
def test_annualized_return_uses_calendar_days_not_curve_length():
    bt = OptionsBacktester(initial_capital=1000.0, max_positions=2, check_interval=7)
    bt.cash = 1100.0
    start = datetime(2023, 1, 1)
    bt.equity_curve = [((start + timedelta(days=i * 7)).strftime('%Y-%m-%d'), 1000.0) for i in range(53)]
    bt.equity_curve[-1] = (bt.equity_curve[-1][0], 1100.0)
    report = bt._generate_report('BTC', bt.equity_curve[0][0], bt.equity_curve[-1][0], 20000.0, 22000.0)
    assert report['annualized_return_pct'] == pytest.approx(10.0, abs=1.0), f"Annualized return should be ~10% over a 1-year span, got {report['annualized_return_pct']}"

def test_elapsed_days_returns_calendar_difference():
    bt = OptionsBacktester(initial_capital=1000.0, max_positions=2, check_interval=7)
    bt.equity_curve = [('2023-01-01', 1000.0), ('2023-04-01', 1050.0), ('2023-12-31', 1100.0)]
...
def test_theta_force_close_emits_trade_log_entries():
    ...
    bt = ThetaHarvestBacktester(initial_capital=10000.0, max_positions=2, profit_target_pct=0, stop_loss_pct=0, min_dte_close=0, label='test')
    bt.run(candles, 'BTC')
...
def _htf_ohlcv_df(n=120, start_price=100.0, drift=0.5, seed=3):
    ...
    return pd.DataFrame({'open': closes, 'high': closes, 'low': closes, 'close': closes, 'volume': [1.0] * n}, index=idx)

def test_htf_trend_series_aligns_on_datetime_indexed_frames(monkeypatch):
    import run_backtest
    htf_df = _htf_ohlcv_df(n=120)
    monkeypatch.setattr(run_backtest, 'load_cached_data', lambda *a, **kw: htf_df)
    ltf_index = pd.date_range('2023-02-01', periods=40, freq='6h', name='datetime')
...
def test_htf_trend_series_uses_last_closed_htf_bar_no_lookahead(monkeypatch):
    ...
    htf_df = pd.DataFrame({'open': closes, 'high': closes, 'low': closes, 'close': closes, 'volume': [1.0] * n}, index=idx)
    monkeypatch.setattr(run_backtest, 'load_cached_data', lambda *a, **kw: htf_df)
    ema = htf_df['close'].ewm(span=50, adjust=False).mean()
    assert closes[-2] > ema.iloc[-2]
    assert closes[-1] < ema.iloc[-1]
    ltf_index = pd.DatetimeIndex([idx[-1] + pd.Timedelta(hours=h) for h in (0, 6, 12)], name='datetime')
    trend = run_backtest._htf_trend_series('BTC/USDT', '1h', ltf_index)
    assert list(trend) == [1, 1, 1], "LTF bars inside the forming HTF candle must see the last CLOSED candle's trend (1), not the forming candle's final close (-1)"
    early = run_backtest._htf_trend_series('BTC/USDT', '1h', pd.DatetimeIndex([idx[0] + pd.Timedelta(hours=6)], name='datetime'))
    assert list(early) == [0]
...
def test_run_single_backtest_with_htf_filter(monkeypatch):
    ...
    result = run_backtest.run_single_backtest(strategy_name='sma_crossover', symbol='BTC/USDT', timeframe='1d', since='2023-01-01', capital=1000.0, htf_filter=True)
