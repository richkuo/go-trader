...
from regime_diagnostics import forward_returns, forward_realized_vol, coverage, separation, stability

def test_forward_returns_horizon():
    close = np.array([100.0, 110, 121])
    fr = forward_returns(close, 1)
    np.testing.assert_allclose(fr[:2], [0.1, 0.1])
...
def test_coverage_counts():
    ...
    cov = coverage(labels)
    assert cov['a']['count'] == 3 and abs(cov['a']['pct'] - 0.75) < 1e-09
...
def test_stability_transition_rate():
    ...
    st = stability(labels)
    assert abs(st['transition_rate'] - 0.5) < 1e-09
    assert st['flips'] == 2
...
def test_score_labels_masks_warmup():
    ...
    base_labels = np.array(['up' if r >= 0 else 'down' for r in np.diff(close, prepend=close[0])], dtype=object)
