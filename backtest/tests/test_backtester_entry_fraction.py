...
def _df(closes, signals, entry_fraction=None, opens=None):
    ...
    df = pd.DataFrame({'open': opens, 'high': np.maximum(opens, closes) + 0.5, 'low': np.minimum(opens, closes) - 0.5, 'close': closes, 'volume': np.full(n, 1000.0), 'signal': np.asarray(signals, dtype=float)}, index=idx)
...
def test_short_fractional_entry_commits_fraction_and_keeps_reserve():
    ...
    signals = [-1, 0, 1, 0, 0]
    res = _run(_df(closes, signals, entry_fraction=[0.5] * 5), direction='short')
    assert res['total_trades'] == 1
...
def test_fraction_one_column_matches_no_column_exactly():
    ...
    base = _run(_df(closes, signals), commission_pct=0.001, slippage_pct=0.0005)
    scaled = _run(_df(closes, signals, entry_fraction=[1.0] * 8), commission_pct=0.001, slippage_pct=0.0005)
...
def test_nan_entry_fraction_means_full_notional():
    ...
    assert nan_col['trades'][0]['shares'] == pytest.approx(base['trades'][0]['shares'])
...
def test_standalone_stop_fill_preserves_reserve():
    ...
    signals = [1, 0, 0, 0, 0, 0]
    res = _run(_df(closes, signals, entry_fraction=[0.5] * 6), stop_loss_pct=0.1)
    assert res['total_trades'] == 1
    assert res['final_capital'] == pytest.approx(9250.0)

def test_engine_path_long_fractional_entry():
    idx = pd.date_range('2024-01-01', periods=5, freq='D')
    df = pd.DataFrame({'open': [100.0] * 5, 'high': [101.0] * 5, 'low': [99.0] * 5, 'close': [100.0] * 5, 'volume': [1000.0] * 5, 'open_action': ['none', 'long', 'none', 'none', 'none'], 'close_fraction': [0, 0, 1, 0, 0], 'entry_fraction': [np.nan, 0.5, np.nan, np.nan, np.nan]}, index=idx)
...
def test_engine_path_short_fractional_entry():
    idx = pd.date_range('2024-01-01', periods=5, freq='D')
    df = pd.DataFrame({'open': [100.0, 100.0, 100.0, 80.0, 80.0], 'high': [101.0] * 5, 'low': [79.0] * 5, 'close': [100.0, 100.0, 80.0, 80.0, 80.0], 'volume': [1000.0] * 5, 'open_action': ['none', 'short', 'none', 'none', 'none'], 'close_fraction': [0, 0, 1, 0, 0], 'entry_fraction': [np.nan, 0.5, np.nan, np.nan, np.nan]}, index=idx)
    bt = Backtester(initial_capital=1000, commission_pct=0, slippage_pct=0)
    res = bt.run(df, save=False)
    assert res['total_trades'] == 1
    assert res['trades'][0]['side'] == 'short'
...
def test_zero_entry_fraction_rejected():
    closes = [100, 100, 100]
    signals = [1, 0, 0]
    with pytest.raises(ValueError, match='entry_fraction .* \\(0, 1\\]'):
