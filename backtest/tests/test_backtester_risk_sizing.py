...
def _df(closes, signals, atr=None, entry_fraction=None):
    ...
    df = pd.DataFrame({'open': closes, 'high': closes + 0.4, 'low': closes - 0.4, 'close': closes, 'volume': np.full(n, 1000.0), 'signal': np.asarray(signals, dtype=float)}, index=idx)
    if atr is not None:
...
def test_equal_dollar_risk_across_differing_atr_stops():
    ...
    signals = [1, 0, 0, -1, 0]
    atr = [2.0] * 5
    tight = _run(_df(closes, signals, atr=atr), risk_per_trade_pct=1.0, stop_loss_atr_mult=1.0)
    wide = _run(_df(closes, signals, atr=atr), risk_per_trade_pct=1.0, stop_loss_atr_mult=2.0)
...
def test_stop_out_realizes_the_risked_dollars():
    ...
    signals = [1, 0, 0, 0, 0]
    res = _run(_df(closes, signals, atr=[2.0] * 5), risk_per_trade_pct=1.0, stop_loss_atr_mult=1.0)
...
def test_short_side_sizes_constant_dollar_risk():
    closes = [100, 100, 100, 100, 100]
    signals = [-1, 0, 0, 1, 0]
    res = _run(_df(closes, signals, atr=[2.0] * 5), risk_per_trade_pct=1.0, stop_loss_atr_mult=1.0, direction='short')
    assert res['total_trades'] == 1
...
def test_pct_stop_owner_sizes_from_price_fraction():
    closes = [100, 100, 100, 100, 100]
    signals = [1, 0, 0, -1, 0]
    res = _run(_df(closes, signals), risk_per_trade_pct=1.0, stop_loss_pct=0.02)
    assert res['trades'][0]['shares'] == pytest.approx(50.0)

def test_risk_fraction_capped_at_full_cash():
    closes = [100, 100, 100, 100, 100]
    signals = [1, 0, 0, -1, 0]
    res = _run(_df(closes, signals, atr=[0.5] * 5), risk_per_trade_pct=10.0, stop_loss_atr_mult=1.0)
...
def test_no_usable_atr_skips_entry_fail_closed():
    closes = [100, 100, 100, 100, 100]
    signals = [1, 0, 1, 0, 0]
    res = _run(_df(closes, signals, atr=[np.nan] * 5), risk_per_trade_pct=1.0, stop_loss_atr_mult=1.0)
...
def test_entry_taken_once_atr_becomes_available():
    ...
    res = _run(_df(closes, signals, atr=atr), risk_per_trade_pct=1.0, stop_loss_atr_mult=1.0)
    assert res['total_trades'] == 1
    assert res['trades'][0]['shares'] == pytest.approx(50.0)
...
def test_rejects_entry_fraction_column_combo():
    ...
    with pytest.raises(ValueError, match='entry_fraction'):
        _run(_df(closes, [1, 0, 0], atr=[2.0] * 3, entry_fraction=[0.5] * 3), risk_per_trade_pct=1.0, stop_loss_atr_mult=1.0)
...
def test_rejects_regime_resolved_stop_owner():
    with pytest.raises(ValueError, match='regime-resolved'):
        Backtester(risk_per_trade_pct=1.0, stop_loss_atr_regime={'use_defaults': True})
...
def _risk_strategy(**overrides):
    sc = {'id': 'hl-r-btc', 'type': 'perps', 'platform': 'hyperliquid', 'open_strategy': {'name': 'tema_cross_bd'}, 'risk_per_trade_pct': 1.0, 'stop_loss_atr_mult': 1.5}
    sc.update(overrides)
    return sc
...
def test_config_explicit_zero_stop_owner_rejects(tmp_path):
    for owner in ('stop_loss_pct', 'trailing_stop_pct', 'stop_loss_margin_pct', 'stop_loss_atr_mult', 'trailing_stop_atr_mult'):
        ...
        with pytest.raises(ValueError, match='stop'):
            Backtester(risk_per_trade_pct=kwargs['risk_per_trade_pct'], stop_loss_atr_mult=kwargs['stop_loss_atr_mult'], stop_loss_pct=kwargs['stop_loss_pct'], stop_loss_margin_pct=kwargs['stop_loss_margin_pct'], trailing_stop_atr_mult=kwargs['trailing_stop_atr_mult'], trailing_stop_pct=kwargs['trailing_stop_pct'])
