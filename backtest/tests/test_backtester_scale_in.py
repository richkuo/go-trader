...
def _df(closes, signals, atr=None, highs=None, lows=None, opens=None):
    ...
    df = pd.DataFrame({'open': np.asarray(opens, dtype=float) if opens is not None else closes, 'high': np.asarray(highs, dtype=float) if highs is not None else closes + 0.4, 'low': np.asarray(lows, dtype=float) if lows is not None else closes - 0.4, 'close': closes, 'volume': np.full(n, 1000.0), 'signal': np.asarray(signals, dtype=float)}, index=idx)
    if atr is not None:
...
def _decide(cfg=None, side='long', qty=1.0, avg_cost=100.0, entry_atr=2.0, count=0, added=0.0, last_add=0.0, signal=1, price=100.0, default_notional=1000.0):
    return _scale_in_decision(_normalize_scale_in_cfg(cfg), side, qty, avg_cost, entry_atr, count, added, last_add, signal, price, default_notional)
...
def test_gate_max_added_notional_boundary():
    ...
    assert ok
    _, ok, reason = _decide(cfg, added=1000.0 + 1e-06, default_notional=1000.0)
...
def test_gate_add_notional_override():
    qty, ok, _ = _decide({'add_notional_usd': 500.0}, price=100.0, default_notional=1000.0)
...
def test_init_rejects_risk_sizing_combo():
    with pytest.raises(ValueError, match='risk_per_trade_pct'):
        Backtester(allow_scale_in=True, risk_per_trade_pct=1.0, stop_loss_atr_mult=1.0)
...
def test_two_adds_accumulate_applyscalein_math():
    ...
    res = _run(df, allow_scale_in=True, scale_in={'add_notional_usd': 1050.0})
    ...
    assert res['scale_in_added_notional_usd'] == pytest.approx(q1 * 105.0 + q2 * 110.0)
...
def test_fixed_sl_trigger_frozen_across_add():
    ...
    df = _df(closes, [1, 0, 1, 0, 0, 0, -1, 0], atr=[2.0] * 8, highs=highs, lows=lows)
    res = _run(df, allow_scale_in=True, stop_loss_atr_mult=1.0, scale_in={'add_spacing_atr': 1.0})
    assert res['scale_in_adds'] == 1
    trade, = res['trades']
...
def test_frozen_sl_exits_full_grown_quantity():
    ...
    highs = [c + 0.4 for c in closes]
    df = _df(closes, [1, 0, 1, 0, 0, 0], atr=[2.0] * 6, highs=highs, lows=lows)
    res = _run(df, allow_scale_in=True, stop_loss_atr_mult=1.0, scale_in={'add_spacing_atr': 1.0})
    assert res['scale_in_adds'] == 1
    trade, = res['trades']
...
def test_tiered_tp_threshold_reads_anchor_not_blend():
    ...
    tiers = [{'atr_multiple': 1.0, 'close_fraction': 0.5}, {'atr_multiple': 10.0, 'close_fraction': 1.0}]
    res = _run(df, allow_scale_in=True, close_strategies=[{'name': 'tiered_tp_atr', 'params': {'tp_tiers': tiers, 'atr_source': 'entry'}}], scale_in={'add_spacing_atr': -1.0})
    assert res['scale_in_adds'] == 1
    legs = res['trades']
...
def test_add_pays_commission_and_joins_entry_fee_pool():
    ...
    assert trade['entry_fee'] == pytest.approx(open_comm + add_comm, rel=1e-06)
...
def test_entry_fee_conserves_partial_close_then_add():
    ...
    tiers = [{'atr_multiple': 1.0, 'close_fraction': 0.5}, {'atr_multiple': 10.0, 'close_fraction': 1.0}]
    res = _run(df, allow_scale_in=True, commission_pct=fee, close_strategies=[{'name': 'tiered_tp_atr', 'params': {'tp_tiers': tiers, 'atr_source': 'entry'}}])
    assert res['scale_in_adds'] == 1
    ...
    legs = res['trades']
    assert len(legs) >= 2
    assert sum((t['entry_fee'] for t in legs)) == pytest.approx(open_comm + add_comm, abs=0.0001)
...
def test_entry_fee_conserves_partial_add_partial_full():
    ...
    tiers = [{'atr_multiple': 1.0, 'close_fraction': 0.25}, {'atr_multiple': 2.0, 'close_fraction': 0.25}, {'atr_multiple': 10.0, 'close_fraction': 1.0}]
    res = _run(df, allow_scale_in=True, commission_pct=fee, close_strategies=[{'name': 'tiered_tp_atr', 'params': {'tp_tiers': tiers, 'atr_source': 'entry'}}])
    assert res['scale_in_adds'] == 1
    legs = res['trades']
    assert len(legs) >= 3
...
def test_two_adds_straddling_partial_close_pnl_reconciles():
    ...
    assert total_pnl == pytest.approx(res['final_capital'] - 10000.0, abs=0.05)
...
def test_second_position_starts_with_clean_scale_state():
    ...
    res = _run(_df(closes, sig), allow_scale_in=True, scale_in={'max_adds': 1})
    assert res['scale_in_adds'] == 2
...
def _hl_strategy(**over):
    sc = {'id': 'hl-test', 'type': 'perps', 'platform': 'hyperliquid', 'script': 'shared_scripts/check_hyperliquid.py', 'args': ['tema_cross', 'BTC', '4h', '--mode', 'paper'], 'capital': 1000, 'max_drawdown_pct': 50, 'open_strategy': {'name': 'tema_cross', 'params': {}}, 'stop_loss_atr_mult': 2.0}
    sc.update(over)
    return sc

def test_loader_threads_scale_in_fields(tmp_path):
    path = _config(tmp_path, _hl_strategy(allow_scale_in=True, scale_in={'max_adds': 3, 'add_spacing_atr': -0.5}))
...
def test_loader_rejects_live_static_scalar_sl(tmp_path):
    path = _config(tmp_path, _hl_strategy(allow_scale_in=True, args=['tema_cross', 'BTC', '4h', '--mode', 'live'], stop_loss_atr_mult=None, stop_loss_pct=5.0))
    with pytest.raises(ValueError, match='static scalar'):
        run_backtest.load_strategy_config(path, 'hl-test')

def test_loader_accepts_live_trailing_sl(tmp_path):
    path = _config(tmp_path, _hl_strategy(allow_scale_in=True, args=['tema_cross', 'BTC', '4h', '--mode', 'live'], stop_loss_atr_mult=None, trailing_stop_atr_mult=2.0))
    kwargs = run_backtest.load_strategy_config(path, 'hl-test')
    assert kwargs['allow_scale_in'] is True

def test_loader_preserves_1268_combo_reject(tmp_path):
    path = _config(tmp_path, _hl_strategy(allow_scale_in=True, risk_per_trade_pct=1.0))
    with pytest.raises(ValueError, match='allow_scale_in'):
        run_backtest.load_strategy_config(path, 'hl-test')
