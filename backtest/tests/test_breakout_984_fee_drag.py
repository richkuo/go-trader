...
import os
import pytest
_FEE_DRAG_PATH = os.path.join(os.path.dirname(__file__), '..', 'candidates', 'breakout_984', 'fee_drag.py')
_spec = importlib.util.spec_from_file_location('breakout_984_fee_drag', _FEE_DRAG_PATH)
...
def test_summarize_zero_span_yields_no_annualization():
    s = fee_drag.summarize_fee_drag([_leg(1.0, span_days=0.0)], [_leg(0.5, span_days=0.0)])
