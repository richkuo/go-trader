...
import hurst_1422_gate_power as study
import hurst_1410_gate_calibration as study1410
DAY_NS = 86400000000000

def _trade(symbol='BTC/USDT', timeframe='1h', window='2021', day=0, pnl=1.0, hold_days=1, h=None, adx=None, cohort=None):
    entry = pd.Timestamp('2021-01-01') + pd.Timedelta(days=day)
    return {'strategy': 'momentum', 'symbol': symbol, 'timeframe': timeframe, 'window': window, 'cohort': cohort or study.cell_cohort(symbol, timeframe, window), 'entry_date': str(entry), 'entry_ns': int(entry.value), 'exit_ns': int((entry + pd.Timedelta(days=hold_days)).value), 'pnl_pct_net': float(pnl), 'adx': adx, 'h': {512: h, 128: h}, 'armed': {}}
...
def test_new_timeframe_on_audit_symbol_over_1410_window_is_exploratory():
    for window in study1410.WINDOWS:
        for tf in ('2h', '15m', '30m'):
            assert study.cell_cohort('BTC/USDT', tf, window) == study.COHORT_EXPLORATORY
...
def test_primary_grid_is_exactly_the_pinned_hypotheses():
    ...
    for family, mode, hw, arm, disarm, gain in grid:
        ids.add(study.gate_config_id(family, hw, arm, disarm) if mode == 'gate' else study.size_config_id(family, hw, gain))
...
def test_primary_ids_match_the_committed_1410_argmin():
    path = os.path.join(os.path.dirname(study.__file__), 'hurst_1410_gate_calibration.json')
    assert study.resolve_primary_config_ids(path) == tuple(sorted(study.PRIMARY_CONFIG_IDS))
...
def test_overlap_counter_matches_brute_force():
    ...
    brute = sum((1 for i in range(len(a_s)) for j in range(len(b_s)) if a_s[i] < b_e[j] and b_s[j] < a_e[i]))
...
def test_effective_n_collapses_for_correlated_concurrent_symbols():
    ...
    trades = [_trade(symbol='BTC/USDT', day=0, hold_days=30) for _ in range(4)] + [_trade(symbol='ETH/USDT', day=0, hold_days=30) for _ in range(4)]
...
def test_effective_n_is_bounded_by_n():
    ...
    trades = [_trade(day=int(d), hold_days=int(h)) for d, h in zip(rng.integers(0, 200, 25), rng.integers(1, 40, 25))]
...
def test_symbol_return_correlations_uses_daily_last_from_finest_timeframe():
    ...
    frames = {('AAA/USDT', '1d'): _close_frame(-returns), ('AAA/USDT', '1h'): _close_frame(returns, intraday=True), ('BBB/USDT', '4h'): _close_frame(returns), ('CCC/USDT', '1h'): _close_frame(np.zeros_like(returns)), ('DDD/USDT', '2h'): _close_frame(-returns)}
...
def test_symbol_return_correlations_omits_short_overlap():
    ...
    frames = {('AAA/USDT', '1h'): _close_frame(returns), ('BBB/USDT', '1h'): _close_frame(returns)}
    assert study.symbol_return_correlations(frames) == {}

def _spread_trades(symbol, n, pnl_fn, label_fn, start_day=0, step_days=3):
    out = []
    for i in range(n):
        t = _trade(symbol=symbol, day=start_day + i * step_days, pnl=pnl_fn(i), hold_days=1)
        t['_lab'] = label_fn(i)
        out.append(t)
    return out
...
def test_cluster_p_is_seeded_and_reproducible():
    trades = _spread_trades('BTC/USDT', 120, lambda i: -2.0 if i < 60 else 2.0, lambda i: i < 60)
    vals = [t['pnl_pct_net'] for t in trades]
    sup = [t['_lab'] for t in trades]
    a = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=200, seed=7)
    b = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=200, seed=7)
    assert a['p'] == b['p']
...
def test_cluster_p_detects_a_real_time_aligned_separation():
    trades = _spread_trades('BTC/USDT', 300, lambda i: -5.0 if _blocky(i) else 5.0, _blocky)
    vals = [t['pnl_pct_net'] for t in trades]
    sup = [t['_lab'] for t in trades]
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=500, seed=1)
    assert res['p'] is not None and res['p'] < 0.05

def test_cluster_p_has_little_power_against_a_periodic_pattern():
    trades = _spread_trades('BTC/USDT', 300, lambda i: -5.0 if i % 2 else 5.0, lambda i: bool(i % 2))
    vals = [t['pnl_pct_net'] for t in trades]
    sup = [t['_lab'] for t in trades]
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=500, seed=1)
    assert res['p'] is not None and res['p'] > 0.05
...
def test_cluster_p_is_unimpressed_by_noise():
    ...
    sup = [t['_lab'] for t in trades]
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=500, seed=2)
    assert res['p'] is not None and res['p'] > 0.05

def test_cluster_p_moves_concurrent_datasets_together():
    one = _spread_trades('BTC/USDT', 150, lambda i: -3.0 if i % 2 else 3.0, lambda i: bool(i % 2))
...
def test_cluster_p_excludes_short_span_datasets():
    short = _spread_trades('DOGE/USDT', 10, lambda i: 1.0, lambda i: bool(i % 2), step_days=1)
    long = _spread_trades('BTC/USDT', 150, lambda i: 1.0, lambda i: bool(i % 2))
    trades = short + long
    vals = [t['pnl_pct_net'] for t in trades]
    sup = [t['_lab'] for t in trades]
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=50, seed=3)
...
def test_excluded_dataset_leaves_the_observed_statistic():
    long = _spread_trades('BTC/USDT', 150, lambda i: 1.0, lambda i: bool(i % 2))
    short = _spread_trades('DOGE/USDT', 40, lambda i: -50.0, lambda i: True, step_days=1)
    with_short = long + short
    vals = [t['pnl_pct_net'] for t in with_short]
    sup = [t['_lab'] for t in with_short]
    res_all = study.cluster_permutation_pvalue_group_diff(with_short, vals, sup, n_perm=200, seed=3)
    res_long = study.cluster_permutation_pvalue_group_diff(long, [t['pnl_pct_net'] for t in long], [t['_lab'] for t in long], n_perm=200, seed=3)
...
def test_excluded_rows_leave_the_config_counts_and_effective_n():
    long = _spread_trades('BTC/USDT', 150, lambda i: 1.0, lambda i: bool(i % 2))
    short = _spread_trades('DOGE/USDT', 40, lambda i: 1.0, lambda i: bool(i % 2), step_days=1)
...
def test_cluster_p_is_none_when_no_dataset_can_rotate():
    trades = _spread_trades('BTC/USDT', 5, lambda i: 1.0, lambda i: bool(i % 2), step_days=1)
    vals = [t['pnl_pct_net'] for t in trades]
    sup = [t['_lab'] for t in trades]
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=50, seed=3)
...
def test_cluster_weighted_p_is_none_when_every_dataset_is_short():
    trades = _spread_trades('BTC/USDT', 6, lambda i: 1.0, lambda i: False, step_days=1)
    rets = [t['pnl_pct_net'] for t in trades]
    res = study.cluster_permutation_pvalue_weighted(trades, rets, [1.5 if i % 2 else 0.5 for i in range(len(trades))], n_perm=50, seed=3)
...
def test_short_span_dataset_still_rotates_under_a_long_offset():
    short = _spread_trades('ETH/USDT', 300, lambda i: 0.0, lambda i: False, step_days=3)
    long = _spread_trades('BTC/USDT', 300, lambda i: 0.0, lambda i: False, step_days=7)
...
def test_rotation_shift_is_never_the_identity_for_any_offset():
    short = _spread_trades('ETH/USDT', 120, lambda i: 0.0, lambda i: False, step_days=3)
    long = _spread_trades('BTC/USDT', 400, lambda i: 0.0, lambda i: False, step_days=7)
    clusters = study.cluster_rotation_offsets(short + long)
...
def test_offset_range_is_capped_by_the_shortest_span():
    short = _spread_trades('ETH/USDT', 200, lambda i: 0.0, lambda i: False, step_days=3)
    long = _spread_trades('BTC/USDT', 200, lambda i: 0.0, lambda i: False, step_days=9)
    clusters = study.cluster_rotation_offsets(short + long)
...
def test_ragged_pool_null_no_longer_inherits_the_observed_alignment():
    short = _spread_trades('ETH/USDT', 200, lambda i: -5.0 if _blocky(i) else 5.0, _blocky, step_days=3)
    long = _spread_trades('BTC/USDT', 200, lambda i: -5.0 if _blocky(i) else 5.0, _blocky, step_days=9)
    trades = short + long
    vals = [t['pnl_pct_net'] for t in trades]
    sup = [t['_lab'] for t in trades]
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=500, seed=1)
...
def test_equal_span_pool_is_unaffected_by_the_fold():
    a = _spread_trades('BTC/USDT', 200, lambda i: -4.0 if _blocky(i) else 4.0, _blocky, step_days=3)
    ...
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=300, seed=1)
...
def test_cluster_p_is_none_without_a_contrast():
    ...
    vals = [t['pnl_pct_net'] for t in trades]
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, [False] * len(trades), n_perm=50, seed=3)
    assert res['p'] is None

def test_cluster_p_uses_the_add_one_convention():
    trades = _spread_trades('BTC/USDT', 200, lambda i: -9.0 if i % 2 else 9.0, lambda i: bool(i % 2))
    vals = [t['pnl_pct_net'] for t in trades]
    sup = [t['_lab'] for t in trades]
    res = study.cluster_permutation_pvalue_group_diff(trades, vals, sup, n_perm=100, seed=4)
    assert res['p'] > 0.0

def test_cluster_weighted_p_needs_multiplier_variation():
    trades = _spread_trades('BTC/USDT', 100, lambda i: 1.0, lambda i: False)
    rets = [t['pnl_pct_net'] for t in trades]
    res = study.cluster_permutation_pvalue_weighted(trades, rets, [1.0] * len(trades), n_perm=50, seed=1)
    assert res['p'] is None

def test_cluster_weighted_p_is_seeded():
    trades = _spread_trades('BTC/USDT', 150, lambda i: i % 5 - 2.0, lambda i: False)
    rets = [t['pnl_pct_net'] for t in trades]
    mults = [1.5 if r > 0 else 0.5 for r in rets]
    a = study.cluster_permutation_pvalue_weighted(trades, rets, mults, n_perm=200, seed=9)
    b = study.cluster_permutation_pvalue_weighted(trades, rets, mults, n_perm=200, seed=9)
    assert a['p'] == b['p']
...
def test_mde_returns_none_when_the_design_cannot_detect_anything():
    ...
    vals = [0.0] * 4
    sup = [t['_lab'] for t in trades]
    assert study.min_detectable_effect(trades, vals, sup, 30, cluster=False, n_perm=study.N_PERM_MDE, seed=1) is None
...
def test_mde_injection_of_the_returned_effect_reaches_the_bar():
    ...
    sup = [t['_lab'] for t in trades]
    d = study.min_detectable_effect(trades, vals, sup, 4, cluster=False, n_perm=400, seed=1)
...
def test_mde_is_deterministic_under_the_seed():
    ...
    assert study.min_detectable_effect(trades, vals, sup, 4, **kw) == study.min_detectable_effect(trades, vals, sup, 4, **kw)
...
def test_mde_is_harder_under_a_bigger_family():
    ...
    vals = list(rng.normal(0, 1.0, len(trades)))
    sup = [t['_lab'] for t in trades]
    small = study.min_detectable_effect(trades, vals, sup, 4, cluster=False, n_perm=4000, seed=1)
    big = study.min_detectable_effect(trades, vals, sup, 30, cluster=False, n_perm=4000, seed=1)
...
def test_mde_refuses_a_permutation_count_that_cannot_resolve_the_bar():
    ...
    with pytest.raises(ValueError, match='cannot resolve'):
        study.min_detectable_effect(trades, vals, sup, 30, cluster=False, n_perm=400, seed=1)
...
def test_mde_of_a_degenerate_split_is_none():
    ...
    assert study.min_detectable_effect(trades, [1.0] * 20, [False] * 20, 4, cluster=False, n_perm=study.N_PERM_MDE, seed=1) is None
...
@pytest.mark.parametrize('value,expected', [(0.2, '<0.45'), (0.4499, '<0.45'), (0.45, '0.45-0.55'), (0.5, '0.45-0.55'), (0.55, '0.45-0.55'), (0.5501, '>0.55'), (2.0, '>0.55')])
def test_joint_h_bucket_boundaries(value, expected):
    assert study.joint_h_bucket(value) == expected

@pytest.mark.parametrize('value,expected', [(0.0, '<25'), (24.999, '<25'), (25.0, '>=25'), (80.0, '>=25'), (None, study.BUCKET_NAN), (float('nan'), study.BUCKET_NAN)])
...
def test_joint_table_covers_every_cell_and_counts_correctly():
    trades = [_trade(h=0.3, adx=30.0), _trade(h=0.7, adx=30.0), _trade(h=0.5, adx=10.0), _trade(h=None, adx=None)]
...
def test_joint_verdict_says_no_separation_on_a_null_pool():
    trades = []
    for i in range(200):
        t = _trade(day=i * 3, pnl=1.0 if i % 2 else -1.0, h=0.3 if i % 2 else 0.7, adx=40.0)
        trades.append(t)
...
def test_joint_verdict_measures_its_limit_on_its_own_rows():
    small = [_trade(day=i * 3, pnl=-5.0 if _blocky(i) else 5.0, h=0.3 if _blocky(i) else 0.7, adx=40.0) for i in range(80)]
    large = [_trade(day=i * 3, pnl=-5.0 if _blocky(i) else 5.0, h=0.3 if _blocky(i) else 0.7, adx=40.0) for i in range(400)]
...
def test_joint_verdict_separates_on_a_real_significant_effect():
    strong = [_trade(day=i * 3, pnl=-5.0 if _blocky(i) else 5.0, h=0.3 if _blocky(i) else 0.7, adx=40.0) for i in range(300)]
...
def test_joint_verdict_fails_on_a_noisy_pool_before_it_reaches_the_limit():
    rng = np.random.default_rng(3)
    noisy = [_trade(day=i * 3, pnl=float(rng.normal(0, 5)), h=0.3 if _blocky(i) else 0.7, adx=40.0) for i in range(300)]
...
def test_joint_verdict_uses_one_permutation_resolution_at_the_boundary(monkeypatch):
    trades = [_trade(day=i * 3, pnl=-1.0 if i % 2 else 1.0, h=0.3 if i % 2 else 0.7, adx=40.0) for i in range(100)]
    seen = {}

    def fake_cluster(*args, n_perm, seed):
        seen['p'] = (n_perm, seed)
        return {'p': study.JOINT_ALPHA - 1e-06}

    def fake_mde(*args, n_perm, seed, **kwargs):
        seen['mde'] = (n_perm, seed)
        return 0.0
    monkeypatch.setattr(study, 'cluster_permutation_pvalue_group_diff', fake_cluster)
...
def test_joint_verdict_never_separates_on_an_unrotatable_pool():
    trades = [_trade(day=i, pnl=-5.0 if i % 2 else 5.0, h=0.3 if i % 2 else 0.7, adx=40.0) for i in range(40)]
...
def _frame(start, end, timeframe='1h'):
    idx = pd.date_range(start, end, freq=pd.Timedelta(minutes=study.timeframe_minutes(timeframe)), inclusive='left')
    return pd.DataFrame({'open': 1.0, 'high': 1.0, 'low': 1.0, 'close': 1.0, 'volume': 1.0}, index=idx)

@pytest.mark.parametrize('tf,minutes', [('15m', 15), ('30m', 30), ('1h', 60), ('2h', 120), ('4h', 240), ('1d', 1440)])
...
def test_open_window_denominator_is_shared_across_datasets():
    frames = {('BTC/USDT', '4h'): _frame('2024-01-01', '2026-06-01', '4h'), ('STOP/USDT', '4h'): _frame('2024-01-01', '2026-01-20', '4h')}
...
def test_open_window_keeps_every_cell_when_all_datasets_end_together():
    frames = {('BTC/USDT', '4h'): _frame('2024-01-01', '2026-06-01', '4h'), ('ETH/USDT', '4h'): _frame('2024-01-01', '2026-06-01', '4h')}
    cov = study.coverage_audit(frames, ['oos'], [512])
    assert cov['cells']['BTC/USDT 4h|oos'] is True
...
def test_open_window_catches_a_mid_window_gap_plus_an_early_end():
    ...
    frames = {('BTC/USDT', '4h'): _frame('2024-01-01', '2026-06-01', '4h'), ('GAPPY/USDT', '4h'): pd.concat([head, gapped])}
    cov = study.coverage_audit(frames, ['oos'], [512])
...
def _ohlc(n=400, seed=5):
    ...
    return pd.DataFrame({'open': close, 'high': close + 1.0, 'low': close - 1.0, 'close': close, 'volume': 1.0}, index=idx)
...
def _cfg(**kw):
    base = {'config_id': 'momentum/gate/W512/arm0.52/dis0.48', 'cohort': study.COHORT_PRIMARY, 'family': 'momentum', 'mode': 'gate', 'sense': study.FAMILY_SENSE['momentum'], 'hurst_window': 512, 'arm': 0.52, 'disarm': 0.48, 'gain': None, 'protocol_windows': list(study.PRIMARY_PROTOCOL_WINDOWS), 'held_out_windows': list(study.PRIMARY_HELD_OUT_WINDOWS), 'p_raw': 0.001, 'p_cluster': 0.001, 'bh_reject': True, 'cluster_reason': None, 'n_pooled_trades': 400, 'n_suppressed': 200, 'n_kept': 200, 'n_pooled_effective': 200.0, 'n_suppressed_effective': 60.0, 'n_kept_effective': 90.0, 'windows': {}}
    good = {'n_legs': 3, 'dd_delta': -2.0, 'chop_delta': -3.0, 'ret_gated': 5.0, 'ret_ungated': 5.0, 'trades_gated': 10, 'trades_ungated': 20}
...
def test_verdict_reads_the_cluster_p_not_the_free_p():
    cfg = _cfg(p_raw=0.0001, p_cluster=None, bh_reject=False, cluster_reason='no draws')
...
def test_effective_volume_floors_block_a_correlated_pool():
    cfg = _cfg(n_suppressed=5000, n_kept=5000, n_suppressed_effective=3.0, n_kept_effective=4.0)
    ok, reasons = study.config_verdict(cfg)
    assert not ok
...
def test_verdict_enforces_the_return_give_up_tolerance():
    cfg = _cfg()
    cfg['windows']['2022'] = dict(cfg['windows']['2022'], ret_gated=-50.0, ret_ungated=5.0)
    ok, reasons = study.config_verdict(cfg)
    assert not ok and any(('return give-up' in r for r in reasons))

def test_held_out_rule_matches_1410_on_three_windows():
    windows = {'a': {'n_legs': 1, 'dd_delta': -1.0}, 'b': {'n_legs': 1, 'dd_delta': -1.0}, 'c': {'n_legs': 1, 'dd_delta': 1.0}}
...
def test_held_out_rule_fails_closed_with_too_few_windows():
    windows = {'a': {'n_legs': 1, 'dd_delta': -1.0}, 'b': {'n_legs': 1, 'dd_delta': -1.0}}
...
def test_inconclusive_justification_carries_the_measured_power():
    decision = study.decide_recommendation([_cfg(bh_reject=False)], {'pooled_1410_cluster': 3.25, 'pooled_primary_cluster': 1.5, 'observed_separation_pp_by_pool': {'primary': {'momentum|512': 0.2}}})
    assert decision['verdict'] == 'inconclusive'
...
def test_inconclusive_justification_states_an_unreachable_limit():
    decision = study.decide_recommendation([_cfg(bh_reject=False)], {'pooled_1410_cluster': None, 'pooled_primary_cluster': None, 'observed_separation_pp_by_pool': {'primary': {}}})
    assert 'nothing below' in decision['justification']

def test_a_sub_limit_separation_is_reported_as_unresolvable_not_as_absent():
    decision = study.decide_recommendation([_cfg(bh_reject=False)], {'pooled_1410_cluster': 3.0, 'pooled_primary_cluster': 2.0, 'observed_separation_pp_by_pool': {'primary': {'momentum|512': 0.5}}})
...
def test_an_unreachable_limit_draws_no_conclusion_about_an_edge():
    decision = study.decide_recommendation([_cfg(bh_reject=False)], {'pooled_1410_cluster': 3.0, 'pooled_primary_cluster': None, 'observed_separation_pp_by_pool': {'primary': {'momentum|512': 0.5}}})
    text = decision['justification']
...
def test_the_limit_is_compared_against_its_own_pool_s_separation():
    mde = {'pooled_1410_cluster': 3.0, 'pooled_primary_cluster': 2.0, 'observed_separation_pp': {'momentum|512': 9.0}, 'observed_separation_pp_by_pool': {'primary': {'momentum|512': 0.5}, 'exploratory': {'momentum|512': 9.0}}}
    text = study.decide_recommendation([_cfg(bh_reject=False)], mde)['justification']
    assert '0.50' in text
...
def test_justification_says_the_rule_failed_when_separation_exceeds_the_limit():
    decision = study.decide_recommendation([_cfg(bh_reject=False)], {'pooled_1410_cluster': 0.2, 'pooled_primary_cluster': 0.1, 'pooled_primary_cluster_p0': 0.03, 'observed_separation_pp_by_pool': {'primary': {'momentum|512': 0.45}}})
    text = decision['justification']
...
def test_justification_reports_the_strongest_primary_hypothesis():
    decision = study.decide_recommendation([_cfg(bh_reject=False, p_cluster=0.061), _cfg(config_id='other', bh_reject=False, p_cluster=0.4)], {'observed_separation_pp_by_pool': {'primary': {}}})
...
def test_warmup_leads_use_each_dataset_s_own_earliest_scored_window():
    ...
    coverage = {'cells': {'SOL/USDT 4h|2020H2': False, 'SOL/USDT 4h|2021': True}}
...
def test_bh_families_never_share_a_denominator():
    primary = [_cfg(config_id='p0', cohort=study.COHORT_PRIMARY, p_cluster=0.008)] + [_cfg(config_id=f'p{i}', cohort=study.COHORT_PRIMARY, p_cluster=0.9) for i in range(1, 4)]
    expl = [_cfg(config_id='e0', cohort=study.COHORT_EXPLORATORY, p_cluster=0.008)] + [_cfg(config_id=f'e{i}', cohort=study.COHORT_EXPLORATORY, p_cluster=0.9) for i in range(1, 30)]
...
def test_bh_counts_untestable_configs_in_the_denominator():
    ...
    untestable = [_cfg(config_id=f'u{i}', p_cluster=None, bh_reject=False) for i in range(20)]
...
def test_dedup_is_independent_of_scheduling_order():
    rows = [{'strategy': 'm', 'symbol': 'BTC/USDT', 'timeframe': '1h', 'window': '2021', 'entry_date': '2021-05-01', 'tag': 'new'}, {'strategy': 'm', 'symbol': 'BTC/USDT', 'timeframe': '1h', 'window': 'is', 'entry_date': '2021-05-01', 'tag': 'old'}]
    order = study.WINDOW_ORDER
    a = study.dedup_entries(rows, order)
    b = study.dedup_entries(list(reversed(rows)), order)
    assert len(a) == len(b) == 1
    assert a[0]['tag'] == b[0]['tag'] == 'new'

def test_dedup_keeps_distinct_datasets_that_share_a_timestamp():
    rows = [{'strategy': 'm', 'symbol': 'BTC/USDT', 'timeframe': '1h', 'window': '2021', 'entry_date': '2021-05-01'}, {'strategy': 'm', 'symbol': 'ETH/USDT', 'timeframe': '1h', 'window': '2021', 'entry_date': '2021-05-01'}]
...
def test_inconclusive_report_blames_the_rule_when_the_edge_is_resolvable():
    payload = _render_payload()
    payload['mde'] = dict(payload['mde'], pooled_primary_cluster=0.08, pooled_primary_cluster_p0=0.021, observed_separation_pp_by_pool={'primary': {'momentum|512': 0.45}})
    payload['decision'] = study.decide_recommendation(payload['configs'], payload['mde'])
...
def test_report_reads_a_resolvable_pool_s_uninjected_p_as_exploratory():
    payload = _render_payload()
    payload['mde'] = dict(payload['mde'], pooled_exploratory_cluster=0.4, pooled_exploratory_cluster_p0=0.49, observed_separation_pp_by_pool={'primary': {'momentum|512': 0.1}, 'exploratory': {'momentum|512': 0.99}})
    payload['decision'] = study.decide_recommendation(payload['configs'], payload['mde'])
...
def test_report_omits_the_exploratory_reading_when_no_pool_resolves():
    payload = _render_payload()
    payload['mde'] = dict(payload['mde'], pooled_1410_cluster=9.0, pooled_primary_cluster=9.0, pooled_exploratory_cluster=9.0, observed_separation_pp_by_pool={'1410': {'momentum|512': 0.1}, 'primary': {'momentum|512': 0.1}, 'exploratory': {'momentum|512': 0.1}})
    payload['decision'] = study.decide_recommendation(payload['configs'], payload['mde'])
    text = study.report_from_payload(payload)
...
def test_report_states_whether_any_rows_left_the_cluster_contrast():
    ...
    assert 'no rows were dropped from any cluster contrast' in study.report_from_payload(clean)
...
def test_no_superseded_study_defaults_to_the_contract_path():
    assert os.path.basename(study1410._DEFAULT_REPORT_OUT) == 'hurst_1410_gate_calibration.md'
    assert os.path.basename(study._DEFAULT_REPORT_OUT) == 'hurst_1422_gate_power.md'
    assert os.path.basename(study1410._DEFAULT_REPORT_OUT) != os.path.basename(study._DEFAULT_REPORT_OUT)
    for module in (study1410, study):
        assert os.path.basename(module._DEFAULT_REPORT_OUT) != 'hurst_gate_calibration.md'
...
@pytest.mark.parametrize('flag,value', [('--only', 'momentum'), ('--datasets', 'BTC/USDT:1h'), ('--windows', '2021'), ('--hurst-windows', '128')])
...
def test_scoped_run_is_allowed_on_explicit_paths(tmp_path, monkeypatch):
    ...
    with pytest.raises(_Reached):
        study.main(['--only', 'momentum', '--json-out', str(tmp_path / 'scoped.json'), '--report-out', str(tmp_path / 'scoped.md')])
...
def test_render_only_writes_a_non_contract_path_freely(tmp_path):
    ...
    assert study.main(['--render-only', '--json-out', str(src), '--report-out', str(out)]) == 0
...
def test_every_pool_reports_the_separation_measured_on_its_own_rows():
    ...
    for i in range(240):
        bad = _blocky(i)
        pooled['mean_reversion'].append(_trade(day=i * 3, pnl=-4.0 if bad else 4.0, h=0.7 if bad else 0.3, adx=40.0))
...
def test_detection_limit_pool_drops_short_rows_from_every_compared_number(monkeypatch):
    long = [_trade(symbol='BTC/USDT', day=i * 3, pnl=-2.0 if i % 2 else 2.0, h=0.3 if i % 2 else 0.7, cohort=study.COHORT_PRIMARY) for i in range(120)]
    short = [_trade(symbol='DOGE/USDT', day=i, pnl=-50.0, h=0.3, cohort=study.COHORT_PRIMARY) for i in range(30)]
    calls = []

    def fake_mde(trades, values, suppressed, family_size, *, cluster, n_perm, seed, **kwargs):
        calls.append(('cluster' if cluster else 'free', list(trades), list(values), list(suppressed)))
    ...
    def fake_cluster(trades, values, suppressed, *, n_perm, seed):
        calls.append(('cluster_p0', list(trades), list(values), list(suppressed)))
        return {'p': 0.2}
    ...
    with_short = study._measure_detection_limits({'momentum': long + short, 'mean_reversion': []}, [512], 'x.json', 800, 1)
    without_short = study._measure_detection_limits({'momentum': long, 'mean_reversion': []}, [512], 'x.json', 800, 1)
    assert with_short['observed_separation_pp_by_pool']['primary'] == without_short['observed_separation_pp_by_pool']['primary'] == {'momentum|512': 4.0}
    for key in ('pooled_primary_cluster', 'pooled_primary_free', 'pooled_primary_cluster_p0', 'pooled_primary_free_p0'):
        assert with_short[key] == without_short[key]
    assert calls
    assert all((t['symbol'] != 'DOGE/USDT' for _, trades, _, _ in calls for t in trades))
    assert all((-50.0 not in values for _, _, values, _ in calls if values))

def test_detection_limit_separation_is_absent_when_exclusion_removes_a_side(monkeypatch):
    kept = [_trade(symbol='BTC/USDT', day=i * 3, pnl=2.0, h=0.7, cohort=study.COHORT_PRIMARY) for i in range(120)]
    dropped_suppressed = [_trade(symbol='DOGE/USDT', day=i, pnl=-50.0, h=0.3, cohort=study.COHORT_PRIMARY) for i in range(30)]
    monkeypatch.setattr(study, 'min_detectable_effect', lambda *args, **kwargs: None)
    out = study._measure_detection_limits({'momentum': kept + dropped_suppressed, 'mean_reversion': []}, [512], 'x.json', 800, 1)
    assert 'momentum|512' not in out['observed_separation_pp_by_pool']['primary']

def test_report_marks_a_sub_limit_pool_as_unresolvable():
    payload = _render_payload()
    payload['mde'] = dict(payload['mde'], pooled_primary_cluster=2.0, observed_separation_pp_by_pool={'1410': {'momentum|512': 0.1}, 'primary': {'momentum|512': 0.1}, 'exploratory': {'momentum|512': 0.1}})
    text = study.report_from_payload(payload)
...
def test_report_marks_a_resolvable_pool_as_resolvable():
    payload = _render_payload()
    payload['mde'] = dict(payload['mde'], pooled_primary_cluster=0.05, observed_separation_pp_by_pool={'1410': {'momentum|512': 9.0}, 'primary': {'momentum|512': 9.0}, 'exploratory': {'momentum|512': 9.0}})
    text = study.report_from_payload(payload)
    assert '| yes |' in text

def test_report_states_the_joint_p_and_limit_share_one_resolution():
    text = study.report_from_payload(_render_payload())
    assert 'significance p and contrast-local detection limit both use the inference resolution of 100 draws and seed 1422' in text
...
def _render_payload() -> dict:
    ...
    cfg_e = _cfg(config_id='momentum/gate/W128/arm0.55/dis0.5', cohort=study.COHORT_EXPLORATORY, bh_reject=False, p_cluster=0.5, protocol_windows=list(study.EXPLORATORY_PROTOCOL_WINDOWS), held_out_windows=list(study.EXPLORATORY_HELD_OUT_WINDOWS))
    decision = study.decide_recommendation([cfg_p, cfg_e], {'pooled_1410_cluster': 3.0, 'pooled_primary_cluster': 2.0, 'observed_separation_pp_by_pool': {'primary': {'momentum|512': 0.1}}})
    empty_bucket = {b: {'trades': 0, 'win_rate_pct': None, 'mean_pnl_pct_net': None, 'median_pnl_pct_net': None, 'compounded_return_pct': 0.0, 'trade_seq_max_dd_pct': 0.0, 'chop_loss_pct': 0.0} for b in study.BUCKETS}
    return {'schema_version': study.SCHEMA_VERSION, 'issue': study.ISSUE, 'pre_registered': {'hurst_windows': [512], 'history_since': study.HISTORY_SINCE, 'platform': study.PLATFORM, 'fee_platform': study.FEE_PLATFORM, 'datasets': ['BTC/USDT 1h'], 'windows': {'2021': ['2021-01-01', '2022-01-01']}, 'n_perm': 100, 'n_perm_mde': 50, 'seed': study.SEED}, 'run_summary': {'legs': 2, 'gated_arms': 18, 'mirror_verified_legs': 2, 'raw_trades': {f: 0 for f in study.FAMILIES}, 'pooled_trades': {f: 0 for f in study.FAMILIES}, 'pooled_primary': {f: 0 for f in study.FAMILIES}, 'pooled_exploratory': {f: 0 for f in study.FAMILIES}, 'n_primary_configs': 1, 'n_exploratory_configs': 1, 'n_primary_significant': 0, 'n_exploratory_significant': 0, 'warmup': {'required_bars': 514, 'min_lead_bars': 5000, 'sufficient': True, 'insufficient_datasets': [], 'lead_bars': {}}, 'coverage': {'n_cells': 1, 'n_kept': 1, 'n_dropped': 0, 'required_lead_bars': 514, 'min_window_bar_fraction': study.MIN_WINDOW_BAR_FRACTION, 'cells': {}, 'dropped': []}, 'symbol_correlations': {'BTC/USDT|ETH/USDT': 0.8}, 'elapsed_sec': 1.0}, 'mde': {'pooled_1410_cluster': 3.0, 'pooled_1410_free': 2.5, 'pooled_1410_cluster_p0': 0.3, 'pooled_1410_free_p0': 0.2, 'pooled_primary_cluster': 2.0, 'pooled_primary_free': 1.8, 'pooled_primary_cluster_p0': 0.4, 'pooled_primary_free_p0': 0.3, 'pooled_exploratory_cluster': None, 'pooled_exploratory_free': None, 'pooled_exploratory_cluster_p0': None, 'pooled_exploratory_free_p0': None, 'by_family_cluster': {f: 2.0 for f in study.FAMILIES}, 'observed_separation_pp': {'momentum|512': 0.1}, 'observed_separation_pp_by_pool': {'1410': {'momentum|512': 0.1}, 'primary': {'momentum|512': 0.1}, 'exploratory': {'momentum|512': 0.1}}}, 'buckets': {f: {'512': dict(empty_bucket)} for f in study.FAMILIES}, 'joint': {f: {'table': study.joint_adx_hurst_table([], 512), 'verdict': {'separated': False, 'reason': 'no contrast', 'p_cluster': None, 'delta_mean_pp': None, 'mde_pp': 2.0}} for f in study.FAMILIES}, 'configs': [cfg_p, cfg_e], 'legs': [], 'decision': decision}
