...
_COMMON_PATH = os.path.join(os.path.dirname(__file__), '..', 'candidates', 'squeeze_momentum_1198', 'driver_common.py')
_spec = importlib.util.spec_from_file_location('squeeze_momentum_1198_driver_common', _COMMON_PATH)
dc = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(dc)

def test_candidate_leg_kwargs_threads_all_regime_state():
    candidate = {'name': 'squeeze_momentum', 'direction': 'long', 'allowed_regimes': ['trending_up'], 'regime_windows_spec': {'medium': {'classifier': 'adx', 'period': 14, 'adx_threshold': 25.0}}, 'profile_allocation': {'window_spec': {'classifier': 'adx', 'period': 14}}}
...
def test_candidate_leg_kwargs_defaults_direction_long():
    assert dc.candidate_leg_kwargs({'name': 'squeeze_momentum'})['direction'] == 'long'
...
def test_summarize_fee_drag_pairs_and_annualizes():
    gross = [{'return_pct': 10.0, 'trades': 5, 'span_days': 365.25}, {'return_pct': 20.0, 'trades': 5, 'span_days': 365.25}, None]
    net = [{'return_pct': 6.0, 'trades': 5, 'span_days': 365.25}, {'return_pct': 12.0, 'trades': 5, 'span_days': 365.25}, {'return_pct': 0.0, 'trades': 9, 'span_days': 365.25}]
