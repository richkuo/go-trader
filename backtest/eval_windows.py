...
def run_candidate_leg(reg, candidate: dict, symbol: str, timeframe: str,
                      ...
                      keep_trades: bool = False,
                      intrabar_resolution: str = "ohlc_walk") -> Optional[dict]:
    """Single source of truth for candidate-dict -> ``run_leg`` kwargs (#1295).

    Every field ``validate_candidate`` accepts and the backtester can model is
    threaded here. Callers that resample or re-score a candidate (this
    module's ``evaluate_window``, monte_carlo's per-leg trade series) MUST go
    through this rather than hand-picking a subset: a caller that quietly
    drops ``close_strategies`` / ``allowed_regimes`` / the stop owners reports
    numbers for a DIFFERENT strategy than the one under test, while looking
    correct.

    Returns None when the (symbol, timeframe, window) triple has no cached
    bars, propagating ``run_leg``'s contract unchanged.
    """
    return run_leg(
