...
def standardize(features):
    ...
    std = np.where(std < 1e-08, 1.0, std)
...
def fit_kmeans(z, k, *, seed=0, iters=100, var_floor=0.001):
...
def fit_gmm(z, k, *, seed=0, iters=100, var_floor=0.001, tol=0.0001, min_soft_mass=1.0):
    z = np.asarray(z, dtype=float)
    n = z.shape[0]
...
def fit_hmm(z, k, *, seed=0, iters=50, var_floor=0.001, tol=0.0001, min_soft_mass=1.0):
    z = np.asarray(z, dtype=float)
    n = len(z)
    ...
    for _ in range(iters):
        ...
        xi_sum = np.zeros((k, k))
        for t in range(n - 1):
            log_xi = log_alpha[t][:, None] + logA + (logB[t + 1] + log_beta[t + 1])[None, :]
...
def map_latent_to_names(em_mean_z, feature_means, feature_stds, thresholds, *, canonical_indices=(0, 1, 2, 3)):
    ...
    if mean.shape != (d,) or std.shape != (d,):
        raise ValueError(f'feature_means/stds must be shape ({d},); got {mean.shape} and {std.shape}')
    ...
    for i in range(len(raw)):
        ...
        names.append(name)
        mapping[str(i)] = {'name': name, 'centroid_raw': raw[i].tolist(), 'volatility_rank': int(rank[i])}
    return (names, mapping)

def fit_unsupervised(features, *, family, k, filter_window, period=48, thresholds=None, seed=0, fitted_on=None, feature_names=None, canonical_indices=(0, 1, 2, 3)):
    ...
    if features.ndim != 2 or features.shape[1] != len(names_out):
        raise ValueError(f"feature matrix has {(features.shape[1] if features.ndim == 2 else '?')} columns but feature_names lists {len(names_out)}")
    ...
    names, mapping = map_latent_to_names(em_mean, mean, std, thresholds, canonical_indices=canonical_indices)
    emissions = [{'mean': em_mean[i].tolist(), 'var': em_var[i].tolist(), 'n': int(counts[i])} for i in range(k)]
    return {'type': MODEL_TYPE, 'version': MODEL_VERSION, 'fit_method': family, 'features': names_out, 'canonical_indices': [int(x) for x in canonical_indices], 'feature_means': mean.tolist(), 'feature_stds': std.tolist(), 'states': names, 'latent_count': int(k), 'emissions': emissions, 'transition': transition.tolist(), 'init': init.tolist(), 'filter_window': int(filter_window), 'period': int(period), 'fitted_on': dict(fitted_on or {}), 'mapping': mapping}
...
def non_degeneracy(labels, thresholds):
    ...
    return {'active_labels': active, 'max_occupancy': max_occ, 'transition_rate': tr, 'ok': not reasons, 'reasons': reasons}

def derive_thresholds(handrule_streams, *, active_margin=1, occupancy_margin=0.05, rate_margin=0.5):
    ...
    return NonDegeneracyThresholds(min_active_labels=max(1, worst_active - active_margin), max_occupancy=min(1.0, worst_occ + occupancy_margin), min_transition_rate=max(0.0, worst_tr * rate_margin))
...
def build_parser():
    ...
    p.add_argument('--window', default='is', choices=list(WINDOWS), help=f"known: {', '.join(WINDOWS)}")
...
def main(argv=None):
    ...
    df = load_cached_data(args.symbol, args.timeframe, exchange_id=PLATFORM, start_date=start, end_date=end)
    th = dict(_DEFAULT_COMPOSITE_THRESHOLDS)
    feats = composite_feature_matrix(df, args.period, th).to_numpy()
    model = fit_unsupervised(feats, family=args.family, k=args.k, filter_window=args.filter_window, period=args.period, thresholds=th, seed=args.seed, fitted_on={'symbol': args.symbol, 'timeframe': args.timeframe, 'window': args.window})
