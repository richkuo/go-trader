...
from consolidation_research import Episode, detect_range_containment, detect_regression_flatness, detect_volatility_contraction, measure_box, measure_shape, measure_escape_candle, build_episode_table, benchmark_detectors, classify_pattern, measure_volume_profile, atr

def _candles(highs, lows, closes, opens=None):
    n = len(closes)
    idx = pd.date_range('2024-01-01', periods=n, freq='1h')
    opens = opens if opens is not None else closes
    return pd.DataFrame({'open': opens, 'high': highs, 'low': lows, 'close': closes, 'volume': [1.0] * n}, index=idx)
...
def test_flat_box_detected_and_measured():
    ...
    assert abs(shape['top_edge_slope']) < 0.001
    assert abs(shape['bottom_edge_slope']) < 0.001
...
def test_escape_candle_flagged_upward():
    ...
    quiet_l = [99.6] * 12
    quiet_c = [100.0] * 12
    df = _candles(quiet_h + [104.0], quiet_l + [99.9], quiet_c + [103.5])
...
def test_escape_candle_downward_direction():
    ...
    quiet_l = [99.6] * 12
    quiet_c = [100.0] * 12
    df = _candles(quiet_h + [100.1], quiet_l + [96.0], quiet_c + [96.5])
    ep = Episode(start_idx=0, end_idx=12, method='t')
...
def test_detector_cache_matches_uncached():
    ...
    for bwp in (0.02, 0.03, 0.02):
        params = {'min_bars': 8, 'box_width_pct': bwp, 'bandwidth_threshold': 0.7, 'flatness_slope': 0.0006, 'flatness_residual': 0.02, 'escape_k': 1.5, 'atr_period': 14}
        res_uncached, bench_uncached = benchmark_detectors(df, params)
        res_cached, bench_cached = benchmark_detectors(df, params, detector_cache=cache)
...
def test_classify_pattern_named_shapes():
    assert classify_pattern({'top_edge_travel': 0.05, 'bottom_edge_travel': -0.05}) == 'rectangle'
    assert classify_pattern({'top_edge_travel': 0.1, 'bottom_edge_travel': 0.6}) == 'ascending_triangle'
    assert classify_pattern({'top_edge_travel': -0.6, 'bottom_edge_travel': 0.05}) == 'descending_triangle'
    assert classify_pattern({'top_edge_travel': -0.6, 'bottom_edge_travel': 0.6}) == 'symmetrical_triangle'
    assert classify_pattern({'top_edge_travel': 0.6, 'bottom_edge_travel': -0.6}) == 'broadening'
    assert classify_pattern({'top_edge_travel': 0.6, 'bottom_edge_travel': 0.6}) == 'rising_wedge'
    assert classify_pattern({'top_edge_travel': -0.6, 'bottom_edge_travel': -0.6}) == 'falling_wedge'
...
def test_build_episode_table_columns():
    ...
    for col in ['top', 'bottom', 'mid', 'width_pct', 'top_edge_slope', 'width_contraction', 'escape_tr', 'breakout_direction', 'pattern', 'poc', 'vah', 'val', 'poc_position']:
