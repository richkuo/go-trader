...
def main(argv=None):
    ...
    out = {'window': {'start': args.start, 'end': args.end}, 'effective_range': {}, 'candidates': {}}
    ...
    for fn in [c.strip() for c in args.candidates.split(',') if c.strip()]:
        ...
        legs = {}
        for symbol, tf in DATASETS:
            leg = run_leg(reg, candidate['name'], candidate.get('params'), symbol, tf, window, **candidate_leg_kwargs(candidate))
            legs[dataset_key(symbol, tf)] = leg
            print(f"{label:<18} {symbol} {tf}: sharpe {leg['sharpe']:+.2f}  ret {leg['return_pct']:+8.2f}%  DD {leg['max_dd_pct']:8.2f}%  B&H {leg['bh_return_pct']:+8.2f}%  #T {leg['trades']}")
        ls = list(legs.values())
        print(f"{label:<18} MEAN sharpe {statistics.mean((l['sharpe'] for l in ls)):+.3f}  ret {statistics.mean((l['return_pct'] for l in ls)):+7.2f}%  vsB&H {statistics.mean((l['return_pct'] - l['bh_return_pct'] for l in ls)):+7.2f}  worstDD {min((l['max_dd_pct'] for l in ls)):7.2f}%  #T {sum((l['trades'] for l in ls))}\n")
