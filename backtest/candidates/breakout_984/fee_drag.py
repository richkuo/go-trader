...
def main(argv=None):
    ...
    for fn in [c.strip() for c in args.candidates.split(',') if c.strip()]:
        ...
        common = dict(close_strategies=candidate.get('close_strategies'), direction=candidate.get('direction') or 'long', invert_signal=bool(candidate.get('invert_signal')), stop_loss_atr_mult=candidate.get('stop_loss_atr_mult'), trailing_stop_atr_mult=candidate.get('trailing_stop_atr_mult'))
