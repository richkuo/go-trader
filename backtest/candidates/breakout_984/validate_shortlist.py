#!/usr/bin/env python3
"""Score the #984 shortlist through the M1 harness, sharing incumbent bars.
...
candidate. Use the per-candidate eval_windows.py command from the README
(with --registry futures) to reproduce any single table independently.

Run from repo root:
  uv run --no-sync python backtest/candidates/breakout_984/validate_shortlist.py \
      [--candidates baseline.json,...] \
      [--json backtest/candidates/breakout_984/validation.json]
"""
