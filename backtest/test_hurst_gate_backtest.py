...
def _frame(n: int=400, seed: int=1411) -> pd.DataFrame:
    ...
    return pd.DataFrame({'open': close, 'high': close * 1.005, 'low': close * 0.995, 'close': close, 'volume': 1000.0}, index=idx)

def test_min_only_hysteresis_sequence():
    gate = hg.HurstGate({'enabled': True, 'min': 0.55, 'disarm_min': 0.5})
    assert gate.advance(0.4) == hg.HURST_STATE_DISARMED
...
def test_max_only_hysteresis_sequence():
    gate = hg.HurstGate({'enabled': True, 'max': 0.45, 'disarm_max': 0.5})
    assert gate.advance(0.4) == hg.HURST_STATE_ARMED
...
def test_known_disarmed_holds_regardless_of_position():
    gate = hg.HurstGate({'enabled': True, 'min': 0.55})
    blocked, _ = gate.step(0.2, flat=False)
    assert blocked

@pytest.mark.parametrize('h,floor,want', [(0.5, 0.25, 0.25), (0.575, 0.25, 0.5), (0.65, 0.25, 1.0), (0.8, 0.25, 1.0), (0.35, 0.25, 1.0), (2.0033, 0.25, 1.0), (0.5, 0.9, 0.9)])
...
def test_size_mode_never_holds_on_a_known_reading():
    ...
    blocked, mult = gate.step(0.5, flat=True)
    assert not blocked
...
def test_decision_series_lags_one_bar():
    ...
    for i in range(201, len(df)):
        assert decision.iloc[i] == raw.iloc[i - 1] or (pd.isna(decision.iloc[i]) and pd.isna(raw.iloc[i - 1]))
...
def _parity_frame_hurst(windows_spec, *, period: int=150, n: int=420):
    ...
    df = _frame(n=n)
    cfg = pdiff.ParityConfig(strategy_name='momentum', params={}, registry='spot', hurst_gate={'enabled': True, 'min': 0.55}, regime_windows_spec=windows_spec)
...
def test_parity_config_carries_the_resolved_windows_spec_and_hurst_block():
    pdiff = _load('bt_parity_diff_under_test', 'backtest/parity_diff.py')
    cfg = pdiff.ParityConfig(strategy_name='momentum', regime_windows_spec={'medium': {'period': 20, 'classifier': 'composite'}, 'long': {'period': 120, 'classifier': 'composite'}})
...
def test_validation_enforces_bound_ordering_and_range():
    ...
    for bad in ({'min': 0.0}, {'min': 1.0}, {'max': 0.0}, {'max': 1.0}):
        with pytest.raises(ValueError, match='\\(0, 1\\) exclusive'):
...
def _write_cfg(tmp_path, strategy_extra: dict, regime: dict) -> str:
    cfg = {'config_version': 17, 'regime': regime, 'strategies': [dict({'id': 's1', 'type': 'perps', 'platform': 'hyperliquid', 'script': 'check.py', 'args': ['momentum', 'BTC/USDT', '1h'], 'open_strategy': {'name': 'momentum', 'params': {}}}, **strategy_extra)]}
...
_COMPOSITE_REGIME = {'enabled': True, 'windows': {'medium': {'classifier': 'composite', 'period': 20}}}

def test_backtest_accepts_a_primary_composite_window(tmp_path, load_strategy_config):
    path = _write_cfg(tmp_path, {'hurst_gate': {'enabled': True, 'min': 0.55, 'disarm_min': 0.5}}, _COMPOSITE_REGIME)
...
def test_backtest_resolves_global_on_failure_default(tmp_path, load_strategy_config):
    ...
    path = _write_cfg(tmp_path, {'hurst_gate': {'enabled': True, 'min': 0.55, 'on_failure': 'open'}}, regime)
...
def test_backtest_rejects_non_primary_window(tmp_path, load_strategy_config):
    regime = {'enabled': True, 'windows': {'medium': {'classifier': 'composite', 'period': 20}, 'long': {'classifier': 'composite', 'period': 60}}}
    path = _write_cfg(tmp_path, {'hurst_gate': {'enabled': True, 'min': 0.55, 'window_key': 'long'}}, regime)
...
def test_backtest_rejects_unknown_window(tmp_path, load_strategy_config):
    path = _write_cfg(tmp_path, {'hurst_gate': {'enabled': True, 'min': 0.55, 'window_key': 'nope'}}, _COMPOSITE_REGIME)
...
def test_backtest_rejects_invalid_bounds(tmp_path, load_strategy_config):
    path = _write_cfg(tmp_path, {'hurst_gate': {'enabled': True, 'min': 0.7, 'max': 0.3}}, _COMPOSITE_REGIME)
    with pytest.raises(ValueError, match='must be <'):
        load_strategy_config(path, 's1')

def test_backtest_passes_through_a_disabled_block(tmp_path, load_strategy_config):
    path = _write_cfg(tmp_path, {'hurst_gate': {'enabled': False, 'min': 0.55}}, {'enabled': True, 'windows': {'medium': {'classifier': 'adx', 'period': 20}}})
...
def _run(Backtester, df, **kwargs):
    bt = Backtester(initial_capital=1000.0, platform='binanceus', open_strategy={'name': 'momentum', 'params': {}}, **kwargs)
    work = df.copy()
...
def test_size_mode_shrinks_exposure_relative_to_the_ungated_run(Backtester):
    ...
    assert scaled['total_trades'] == base['total_trades'], 'size mode must never change WHICH trades are taken — only their size'
    ...
    for b, s in zip(base_trades, scaled_trades):
        assert abs(float(s['shares'])) <= abs(float(b['shares'])) + 1e-09
    assert any((abs(float(s['shares'])) < abs(float(b['shares'])) - 1e-09 for b, s in zip(base_trades, scaled_trades))), 'at least one entry should actually be shrunk'

def test_gate_mode_can_only_remove_entries_never_add_them(Backtester):
    df = _frame(n=400)
    base = _run(Backtester, df)
    gated = _run(Backtester, df, hurst_gate={'enabled': True, 'mode': 'gate', 'min': 0.55, 'disarm_min': 0.5})
    assert gated['total_trades'] <= base['total_trades']

def test_gate_mode_fail_closed_before_warmup_blocks_early_entries(Backtester):
    df = _frame(n=400)
    closed = _run(Backtester, df, hurst_gate={'enabled': True, 'min': 0.55, 'on_failure': 'closed'})
    opened = _run(Backtester, df, hurst_gate={'enabled': True, 'min': 0.55, 'on_failure': 'open'})
...
def test_hurst_gate_is_in_the_config_cli_allowlist():
    ...
    assert '"hurst_gate"' in stop_keys_block, "hurst_gate must be in run_backtest's stop_keys allowlist, or the --config CLI path silently drops the entry gate (#1411)"
...
def _run_scale_in(Backtester, df, signal, scale_in=None, **kwargs):
    bt = Backtester(initial_capital=1000.0, platform='binanceus', open_strategy={'name': 'momentum', 'params': {}}, allow_scale_in=True, scale_in=dict(scale_in or {'add_notional_usd': 50.0, 'max_adds': 500}), **kwargs)
    work = df.copy()
    work['signal'] = signal
    return bt.run(work, strategy_name='momentum', symbol='BTC/USDT', timeframe='1h', save=False)
...
def test_gate_mode_holds_scale_in_adds_once_disarmed(Backtester, hurst_module, monkeypatch):
    df = _frame(n=400)
    disarm_at = 200
    _pin_hurst(monkeypatch, hurst_module, lambda n: [0.8] * disarm_at + [0.2] * (n - disarm_at))
    sig = _hold_then_add_signal(len(df), open_bar=100)
    gate = {'enabled': True, 'mode': 'gate', 'min': 0.55, 'disarm_min': 0.5}
    ...
    assert gated['scale_in_adds'] < ungated['scale_in_adds'], 'a disarmed gate must hold scale-in adds, exactly as pausedBlocksSignal holds a same-side signal on an open position live (#1411)'
    open_fill_bar = 100 + 1
...
def test_size_mode_scales_scale_in_adds_by_the_multiplier(Backtester, hurst_module, monkeypatch):
    ...
    expected_mult = 0.2
    sig = _hold_then_add_signal(len(df), open_bar=100)
    gated = _run_scale_in(Backtester, df, sig, hurst_gate={'enabled': True, 'mode': 'size', 'size_floor': 0.05})
    ungated = _run_scale_in(Backtester, df, sig)
    assert ungated['scale_in_adds'] > 0
    assert gated['scale_in_adds'] == ungated['scale_in_adds'], 'size mode must never change WHICH adds fire — only their size'
    assert gated['scale_in_added_notional_usd'] == pytest.approx(expected_mult * ungated['scale_in_added_notional_usd'], rel=1e-07)
...
def test_open_bar_multiplier_never_leaks_into_later_adds(Backtester, hurst_module, monkeypatch):
    ...
    _pin_hurst(monkeypatch, hurst_module, lambda n: [0.53] * recover_at + [0.8] * (n - recover_at))
    low_open = _run_scale_in(Backtester, df, sig, scale_in=scale_cfg, hurst_gate=gate)
    _pin_hurst(monkeypatch, hurst_module, lambda n: [0.8] * n)
    ...
    assert low_open['scale_in_added_notional_usd'] == pytest.approx(full_open['scale_in_added_notional_usd'], rel=1e-07), "the open bar's multiplier must not survive into the per-add default notional — live's defOpenNotional is recomputed ungated every cycle"

def test_armed_gate_leaves_scale_in_adds_byte_identical(Backtester, hurst_module, monkeypatch):
    df = _frame(n=400)
    _pin_hurst(monkeypatch, hurst_module, lambda n: [0.8] * n)
    sig = _hold_then_add_signal(len(df), open_bar=100)
    gated = _run_scale_in(Backtester, df, sig, hurst_gate={'enabled': True, 'mode': 'gate', 'min': 0.55, 'disarm_min': 0.5})
    ungated = _run_scale_in(Backtester, df, sig)
...
def test_absent_hurst_gate_leaves_the_scale_in_baseline_byte_identical(Backtester):
    ...
    disabled = _run_scale_in(Backtester, df, sig, hurst_gate={'enabled': False, 'min': 0.55})
...
def test_scale_in_hurst_arms_live_inside_the_add_helper():
    ...
    assert 'if hurst_blocked:' in body, "the hold arm must be the add helper's own guard (#1411)"
    assert 'add_qty *= hurst_size_mult' in body, 'the size arm must scale the DECIDED add quantity (#1411)'
    call_lines = [ln for ln in source.splitlines() if '_try_scale_in_add(i, ' in ln]
    assert len(call_lines) == 4, call_lines
    for line in call_lines:
        assert 'hurst' not in line, 'call sites must stay gate-free so no site can be missed: ' + line
