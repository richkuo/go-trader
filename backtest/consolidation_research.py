...
_INDICATORS_CORE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'shared_strategies', 'open', 'indicators_core.py')
_spec = importlib.util.spec_from_file_location('_go_trader_indicators_core_research', _INDICATORS_CORE_PATH)
...
def detect_range_containment(df: pd.DataFrame, min_bars: int=8, box_width_pct: float=0.04, atr_period: int=14, **_: object) -> List[Episode]:
...
def detect_volatility_contraction(df: pd.DataFrame, min_bars: int=8, bandwidth_threshold: float=0.7, bb_period: int=20, bb_std: float=2.0, **_: object) -> List[Episode]:
    close = df['close']
...
def detect_regression_flatness(df: pd.DataFrame, min_bars: int=8, flatness_slope: float=0.0006, flatness_residual: float=0.02, **_: object) -> List[Episode]:
...
DETECTORS: Dict[str, Callable[..., List[Episode]]] = {'range_containment': detect_range_containment, 'volatility_contraction': detect_volatility_contraction, 'regression_flatness': detect_regression_flatness}
DETECTOR_PARAM_KEYS: Dict[str, Tuple[str, ...]] = {'range_containment': ('min_bars', 'box_width_pct'), 'volatility_contraction': ('min_bars', 'bandwidth_threshold'), 'regression_flatness': ('min_bars', 'flatness_slope', 'flatness_residual')}
...
def measure_box(df: pd.DataFrame, ep: Episode) -> Dict[str, float]:
    ...
    mid = (top + bottom) / 2.0
    return {'top': top, 'bottom': bottom, 'mid': mid, 'mean_close': float(seg['close'].mean()), 'range_max': top, 'range_min': bottom, 'range_avg': float((seg['high'] + seg['low']).mean() / 2.0), 'width_pct': (top - bottom) / mid if mid else float('nan')}
...
def measure_shape(df: pd.DataFrame, ep: Episode) -> Dict[str, float]:
    ...
    return {'top_edge_slope': top_slope, 'bottom_edge_slope': bottom_slope, 'top_edge_travel': top_travel, 'bottom_edge_travel': bottom_travel, 'width_contraction': contraction, 'time_top_frac': top_frac, 'time_mid_frac': mid_frac, 'time_bottom_frac': bottom_frac}
...
def measure_volume_profile(df: pd.DataFrame, ep: Episode, bins: int=24, value_area_frac: float=0.7) -> Dict[str, float]:
    seg = df.iloc[ep.start_idx:ep.end_idx]
    out = {'poc': float('nan'), 'vah': float('nan'), 'val': float('nan'), 'poc_position': float('nan'), 'value_area_width_frac': float('nan')}
    top = float(seg['high'].max())
    bottom = float(seg['low'].min())
...
def measure_escape_candle(df: pd.DataFrame, ep: Episode, atr_series: pd.Series, escape_k: float=1.5, edge_margin_pct: float=0.002) -> Dict[str, float]:
    seg = df.iloc[ep.start_idx:ep.end_idx]
    n = len(df)
    out: Dict[str, float] = {'escape_idx': float('nan'), 'escape_tr': float('nan'), 'escape_k_vs_median_tr': float('nan'), 'escape_k_vs_atr': float('nan'), 'escape_by_median_tr': 0.0, 'escape_by_atr': 0.0, 'escape_by_edge': 0.0, 'breakout_direction': 0.0}
    if ep.end_idx >= n:
        return out
...
def benchmark_detectors(df: pd.DataFrame, params: dict, detector_cache: Optional[dict]=None) -> Tuple[Dict[str, List[Episode]], pd.DataFrame]:
    ...
    for name, fn in DETECTORS.items():
        ...
        results[name] = eps
        if not eps:
            rows.append({'method': name, 'n_episodes': 0, 'avg_bars': 0.0, 'avg_width_pct': float('nan'), 'false_break_rate': float('nan'), 'coverage_pct': 0.0})
            continue
        ...
        for ep in eps:
            ...
            if esc['escape_by_edge'] == 0.0 and esc['escape_by_atr'] == 0.0:
                false_breaks += 1
        rows.append({'method': name, 'n_episodes': len(eps), 'avg_bars': float(np.mean(durations)), 'avg_width_pct': float(np.nanmean(widths)), 'false_break_rate': false_breaks / len(eps), 'coverage_pct': sum(durations) / len(df)})
    bench = pd.DataFrame(rows)
    if not bench.empty:
        bench['score'] = (1 - bench['false_break_rate'].fillna(1.0)) * bench['n_episodes'] / (1 + bench['avg_width_pct'].fillna(1.0))
    return (results, bench)

def correlate_shape_breakout(episodes_df: pd.DataFrame) -> Dict[str, object]:
    if episodes_df.empty:
        return {}
    metrics = ['top_edge_slope', 'bottom_edge_slope', 'width_contraction', 'width_pct', 'n_bars', 'poc_position', 'value_area_width_frac']
    targets = {'breakout_direction': episodes_df['breakout_direction'], 'escape_k_vs_atr': episodes_df['escape_k_vs_atr']}
    ...
    for tname, tvals in targets.items():
        ...
        for m in metrics:
            ...
            out['spearman'][tname][m] = float(valid[m].rank().corr(valid['t'].rank()))
    wc = episodes_df.dropna(subset=['width_contraction', 'escape_k_vs_atr'])
    if len(wc) >= 6:
        try:
            wc = wc.assign(bucket=pd.qcut(wc['width_contraction'], 3, labels=['tight', 'mid', 'wide']))
            grp = wc.groupby('bucket', observed=True)['escape_k_vs_atr'].median()
            out['grouped']['escape_by_contraction_tercile'] = {str(k): float(v) for k, v in grp.items()}
    ...
    if 'pattern' in episodes_df:
        ...
        for name, g in episodes_df.groupby('pattern'):
            ...
            pats[str(name)] = {'count': int(len(g)), 'breakout_up_rate': float(up), 'escape_xatr_median': float(g['escape_k_vs_atr'].median())}
        out['patterns'] = pats
    return out

def build_episode_table(df: pd.DataFrame, episodes: List[Episode], params: dict) -> pd.DataFrame:
    atr_series = atr(df, params.get('atr_period', 14))
    escape_k = params.get('escape_k', 1.5)
    rows = []
    for ep in episodes:
        seg = df.iloc[ep.start_idx:ep.end_idx]
        row: Dict[str, object] = {'method': ep.method, 'start': seg.index[0], 'end': seg.index[-1], 'n_bars': ep.n_bars}
...
def render_report(df: pd.DataFrame, bench: pd.DataFrame, primary_method: str, episodes: List[Episode], episodes_df: pd.DataFrame, correlations: Dict[str, object], out_dir: str, symbol: str, timeframe: str) -> Dict[str, str]:
    ...
    summary = {'symbol': symbol, 'timeframe': timeframe, 'n_bars_analyzed': int(len(df)), 'primary_method': primary_method, 'benchmark': bench.to_dict(orient='records'), 'correlations': correlations}
    if not episodes_df.empty:
        summary['distributions'] = {'duration_bars': {'median': float(episodes_df['n_bars'].median()), 'p25': float(episodes_df['n_bars'].quantile(0.25)), 'p75': float(episodes_df['n_bars'].quantile(0.75))}, 'width_pct': {'median': float(episodes_df['width_pct'].median()), 'p25': float(episodes_df['width_pct'].quantile(0.25)), 'p75': float(episodes_df['width_pct'].quantile(0.75))}, 'escape_k_vs_atr': {'median': float(episodes_df['escape_k_vs_atr'].median()), 'p25': float(episodes_df['escape_k_vs_atr'].quantile(0.25)), 'p75': float(episodes_df['escape_k_vs_atr'].quantile(0.75))}}
    summary_path = os.path.join(out_dir, 'summary.json')
    with open(summary_path, 'w') as f:
        json.dump(summary, f, indent=2, default=str)
    paths['summary_json'] = summary_path
    paths.update(_render_charts(df, episodes, episodes_df, out_dir, symbol, timeframe))
    return paths
...
def _render_charts(df, episodes, episodes_df, out_dir, symbol, timeframe):
    ...
    for ep in episodes:
        ...
        ax.add_patch(Rectangle((seg.index[0], bottom), seg.index[-1] - seg.index[0], top - bottom, facecolor='#4c9aff', alpha=0.25, edgecolor='#1f6feb', lw=0.5))
        if ep.end_idx < len(df):
            esc_t = df.index[ep.end_idx]
            ax.scatter([esc_t], [df['close'].iloc[ep.end_idx]], color='red', s=14, zorder=5)
    ...
    for ax, col, title in zip(axes, ['n_bars', 'width_pct', 'escape_k_vs_atr'], ['duration (bars)', 'width (%)', 'escape size (x ATR)']):
        vals = episodes_df[col].dropna()
        if len(vals):
            ax.hist(vals, bins=min(20, max(5, len(vals))), color='#4c9aff', edgecolor='#1f6feb')
        ax.set_title(title)
    ...
    if len(sub):
        colors = ['#2da44e' if d > 0 else '#cf222e' for d in sub['breakout_direction']]
        ax.scatter(sub['width_contraction'], sub['escape_k_vs_atr'], c=colors, alpha=0.7)
        ax.set_xlabel('width contraction (end/start)')
        ax.set_ylabel('escape size (x ATR)')
        ax.set_title(f'{symbol} {timeframe} — shape vs breakout (green=up, red=down)')
    p = os.path.join(out_dir, 'shape_vs_breakout.png')
    fig.savefig(p, dpi=120, bbox_inches='tight')
    plt.close(fig)
    paths['chart_shape_vs_breakout'] = p
    return paths
RUNS_CSV_COLUMNS = ['run_id', 'date', 'symbol', 'timeframe', 'since', 'bars', 'method', 'is_primary', 'n_episodes', 'avg_bars', 'avg_width_pct', 'false_break_rate', 'coverage_pct', 'score', 'dur_median_bars', 'width_pct_median', 'escape_xatr_median', 'corr_contraction_vs_escape', 'params', 'out_dir', 'takeaway']
_METHOD_PARAM_KEYS = {'range_containment': ['min_bars', 'box_width_pct', 'escape_k', 'atr_period'], 'volatility_contraction': ['bandwidth_threshold', 'escape_k'], 'regression_flatness': ['flatness_slope', 'flatness_residual', 'escape_k']}
...
def _fmt(v: object, nd: int=4) -> object:
    return round(v, nd) if isinstance(v, float) and (not np.isnan(v)) else '' if isinstance(v, float) and np.isnan(v) else v

def append_runs_csv(runs_csv: str, run_id: str, run_date: str, args: argparse.Namespace, bars: int, res: Dict[str, object], only_methods: Optional[List[str]]=None) -> None:
    bench = res['benchmark']
    ...
    for method in methods:
        ...
        params = ';'.join((f'{k}={getattr(args, k)}' for k in _METHOD_PARAM_KEYS.get(method, []) if hasattr(args, k)))
        rows.append({'run_id': run_id, 'date': run_date, 'symbol': args.symbol, 'timeframe': args.timeframe, 'since': args.since, 'bars': bars, 'method': method, 'is_primary': int(is_primary), 'n_episodes': b.get('n_episodes', ''), 'avg_bars': _fmt(b.get('avg_bars', '')), 'avg_width_pct': _fmt(b.get('avg_width_pct', '')), 'false_break_rate': _fmt(b.get('false_break_rate', '')), 'coverage_pct': _fmt(b.get('coverage_pct', '')), 'score': _fmt(b.get('score', '')), 'dur_median_bars': dur_med if is_primary else '', 'width_pct_median': width_med if is_primary else '', 'escape_xatr_median': esc_med if is_primary else '', 'corr_contraction_vs_escape': _fmt(corr.get('width_contraction', '')) if is_primary else '', 'params': params, 'out_dir': args.out_dir, 'takeaway': ''})
...
def run(df: pd.DataFrame, params: dict, out_dir: str, symbol: str, timeframe: str, primary_method: Optional[str]=None, write_report: bool=True, detector_cache: Optional[dict]=None) -> Dict[str, object]:
    results, bench = benchmark_detectors(df, params, detector_cache=detector_cache)
    if primary_method is None:
        primary_method = str(bench.sort_values('score', ascending=False).iloc[0]['method']) if not bench.empty and 'score' in bench and bench['score'].notna().any() else next(iter(DETECTORS))
    ...
    paths = render_report(df, bench, primary_method, episodes, episodes_df, correlations, out_dir, symbol, timeframe) if write_report else {}
    return {'primary_method': primary_method, 'n_episodes': len(episodes), 'benchmark': bench, 'paths': paths, 'correlations': correlations, 'episodes_df': episodes_df}
...
def main(argv: Optional[List[str]]=None) -> int:
    ...
    p.add_argument('--out-dir', default=None, help='output dir; defaults to a per-run dir under backtest/consolidation_out/')
    ...
    p.add_argument('--runs-csv', default='docs/research/consolidation_runs.csv', help="append a per-method row to this CSV after the run; '' disables")
    p.add_argument('--run-id', default=None, help='run id for the CSV; auto-increments if omitted')
    args = p.parse_args(argv)
    run_id = args.run_id or (next_run_id(args.runs_csv) if args.runs_csv else '000')
    if args.out_dir is None:
        sym = args.symbol.replace('/', '').lower()
        args.out_dir = os.path.join('backtest', 'consolidation_out', f'run{run_id}_{sym}_{args.timeframe}')
    from data_fetcher import fetch_full_history
    df = fetch_full_history(symbol=args.symbol, timeframe=args.timeframe, since=args.since, exchange_id=args.exchange_id)
    if df.empty:
        print(json.dumps({'error': 'no data'}))
        return 1
    params = {'min_bars': args.min_bars, 'box_width_pct': args.box_width_pct, 'bandwidth_threshold': args.bandwidth_threshold, 'flatness_slope': args.flatness_slope, 'flatness_residual': args.flatness_residual, 'escape_k': args.escape_k, 'atr_period': args.atr_period}
    ...
    print(f"\nPrimary method: {res['primary_method']}  ({res['n_episodes']} episodes)")
    edf = res['episodes_df']
    if not edf.empty:
        print(f"\nDuration (bars): median={edf['n_bars'].median():.0f}  width%: median={edf['width_pct'].median():.3f}  escape(xATR): median={edf['escape_k_vs_atr'].median():.2f}")
    print('\nCorrelations (Pearson, escape size vs shape):')
    print(json.dumps(res['correlations'].get('pearson', {}).get('escape_k_vs_atr', {}), indent=2))
    print('\nOutputs:')
