...
def benjamini_hochberg(pvals: list[float], alpha: float=0.05, family_size: int | None=None) -> list[bool]:
    ...
    if denom < m:
        raise ValueError(f'family_size={denom} is smaller than the number of p-values ({m}); the BH denominator must cover every tested hypothesis')
    order = np.argsort(p)
    ranked = p[order]
