...
def _liquidation_loss(notional: float, leverage: float, maintenance_margin: float) -> float:
    return notional * (1.0 / leverage - maintenance_margin)

class PairsBacktester:

    def __init__(self, initial_capital: float=1000.0, base_notional: float=1000.0, beta: float=1.0, leverage: float=20.0, maintenance_margin: float=0.02, lookback: int=30, entry_z: float=2.0, exit_z: float=0.5, taker_fee_pct: float=0.000432, maker_fee_pct: float=0.000144, use_maker: bool=False, funding_a_per_hour: float=0.0, funding_b_per_hour: float=0.0, bar_hours: float=1.0, bars_per_year: Optional[int]=None):
        ...
        if total_margin > self.initial_capital:
            print(f'warning: total margin ${total_margin:.2f} exceeds initial capital ${self.initial_capital:.2f} — runs as if margin were free', file=sys.stderr)
    ...
    def _open_position(self, fill_bar: int, df_a: pd.DataFrame, df_b: pd.DataFrame, side_a: int) -> PairsTrade:
        ...
        trade = PairsTrade(entry_bar=fill_bar, entry_time=df_a.index[fill_bar], side_a=side_a, entry_price_a=entry_a, entry_price_b=entry_b, qty_a=qty_a, qty_b=qty_b, notional_a=notional_a, notional_b=notional_b, margin_a=margin_a, margin_b=margin_b)
        trade.fees = (notional_a + notional_b) * self.fee_pct
        return trade

    def _close_position(self, trade: PairsTrade, fill_bar: int, df_a: pd.DataFrame, df_b: pd.DataFrame, reason: str) -> None:
    ...
    def _summarize(self, trades: list[PairsTrade], equity_curve: np.ndarray, index: pd.Index, liquidations: int) -> PairsResults:
        ...
        return PairsResults(trades=trades, equity_curve=eq_series, initial_capital=self.initial_capital, final_equity=final_equity, total_return_pct=(final_equity / self.initial_capital - 1.0) * 100.0, sharpe=sharpe, max_drawdown_pct=max_dd * 100.0, win_rate=win_rate, avg_hold_bars=avg_hold, total_fees=total_fees, total_funding=total_funding, liquidations=liquidations)

def format_report(results: PairsResults, symbol_a: str, symbol_b: str, timeframe: str) -> str:
    lines = [f'Pairs Backtest: {symbol_a} vs {symbol_b} ({timeframe})', '-' * 60, f'  Initial capital:   ${results.initial_capital:,.2f}', f'  Final equity:      ${results.final_equity:,.2f}', f'  Total return:      {results.total_return_pct:+.2f}%', f'  Sharpe (ann.):     {results.sharpe:.2f}', f'  Max drawdown:      {results.max_drawdown_pct:.2f}%', f'  Trades:            {len(results.trades)}', f'  Win rate:          {results.win_rate * 100:.1f}%', f'  Avg hold (bars):   {results.avg_hold_bars:.1f}', f'  Total fees:        ${results.total_fees:,.2f}', f'  Total funding:     ${results.total_funding:+,.2f}', f'  Liquidations:      {results.liquidations}']
    return '\n'.join(lines)
...
def main(argv: Optional[list[str]]=None) -> int:
    ...
    p.add_argument('--beta', type=float, default=1.2, help='Hedge ratio: notional_B = base_notional × beta')
    ...
    p.add_argument('--bars-per-year', type=int, default=None, help='override Sharpe annualization (default: 24*365/bar_hours)')
    ...
    if df_a.empty:
        ...
        return 1
    bt = PairsBacktester(initial_capital=args.initial_capital, base_notional=args.base_notional, beta=args.beta, leverage=args.leverage, maintenance_margin=args.maintenance_margin, lookback=args.lookback, entry_z=args.entry_z, exit_z=args.exit_z, taker_fee_pct=args.taker_fee, maker_fee_pct=args.maker_fee, use_maker=args.use_maker, funding_a_per_hour=args.funding_a_per_hour, funding_b_per_hour=args.funding_b_per_hour, bar_hours=args.bar_hours, bars_per_year=args.bars_per_year)
