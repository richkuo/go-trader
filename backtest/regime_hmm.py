...
def fit_label_anchored_hmm(features, labels, states, *, filter_window, var_floor=0.001, laplace=1.0, fitted_on=None) -> dict:
    ...
    return {'type': MODEL_TYPE, 'version': MODEL_VERSION, 'features': list(FEATURES), 'feature_means': mean.tolist(), 'feature_stds': std.tolist(), 'states': list(states), 'emissions': emissions, 'transition': A.tolist(), 'init': stationary_distribution(A).tolist(), 'filter_window': int(filter_window), 'fitted_on': fitted_on or {}}
