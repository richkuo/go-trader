...
def simulate(df, params, regime=None):
    ...
    while i < n - 1:
        ...
        realized_r -= cost_r
        trades.append({'entry_idx': i + 1, 'exit_idx': exit_idx, 'direction': direction, 'r': realized_r, 'hit_tp1': filled_tp1, 'bars_held': exit_idx - (i + 1)})
        i = exit_idx + 1
...
def stats(trades):
    ...
    return {'trades': int(len(r)), 'win_rate': round(float((r > 0).mean()), 3), 'expectancy_R': round(float(r.mean()), 3), 'total_R': round(float(r.sum()), 1), 'avg_win_R': round(float(wins.mean()), 2) if len(wins) else 0.0, 'avg_loss_R': round(float(losses.mean()), 2) if len(losses) else 0.0, 'profit_factor': round(float(wins.sum() / -losses.sum()), 2) if len(losses) and losses.sum() != 0 else float('inf'), 'max_dd_R': round(max_dd, 1), 'tp1_rate': round(float(trades['hit_tp1'].mean()), 3), 'avg_bars_held': round(float(trades['bars_held'].mean()), 1)}
...
def main(argv=None):
    ...
    p.add_argument('--edge-entry-frac', type=float, default=0.2)
    ...
    p.add_argument('--cost-bps', type=float, default=5.0, help='per-side cost (fee+slippage) in basis points; 0 disables')
    ...
    regime = compute_regime(df, period=args.atr_period, adx_threshold=args.adx_threshold)['regime'].to_numpy()
    ranging_frac = float((regime == 'ranging').mean())
    print(f'{len(df)} bars; ranging {ranging_frac:.0%} of the time (ADX<{args.adx_threshold}).\n')
    base = {'min_bars': args.min_bars, 'box_width_pct': args.box_width_pct, 'edge_entry_frac': args.edge_entry_frac, 'stop_atr_mult': args.stop_atr_mult, 'drift_threshold': args.drift_threshold, 'atr_period': args.atr_period, 'max_hold': args.max_hold or args.min_bars * 3}
    rows = []
    matrix = [('geometry: baseline', False, False, 'geometry'), ('geometry: drift only', True, False, 'geometry'), ('geometry: regime only', False, True, 'geometry'), ('geometry: regime+drift', True, True, 'geometry'), ('hybrid:   baseline', False, False, 'hybrid'), ('hybrid:   drift only', True, False, 'hybrid'), ('hybrid:   regime+drift', True, True, 'hybrid')]
    full_trades = None
    for label, drift, reg, mode in matrix:
        t = simulate(df, {**base, 'drift_filter': drift, 'regime_filter': reg, 'exit_mode': mode, 'trail_atr_mult': args.trail_atr_mult, 'cost_bps': args.cost_bps}, regime)
        s = stats(t)
        s['variant'] = label
        rows.append(s)
    ...
    if full_trades is not None and (not full_trades.empty):
        for d, dl in [(1, 'longs'), (-1, 'shorts')]:
            ...
            rows.append(sd)
    cols = ['variant', 'trades', 'win_rate', 'expectancy_R', 'total_R', 'profit_factor', 'avg_win_R', 'avg_loss_R', 'max_dd_R', 'tp1_rate', 'avg_bars_held']
    out = pd.DataFrame(rows)
    out = out[[c for c in cols if c in out.columns]]
    print(f'=== {args.symbol} {args.timeframe} box {args.box_width_pct}/{args.min_bars} stop {args.stop_atr_mult}xATR ===')
