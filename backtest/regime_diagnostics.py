...
def coverage(labels: np.ndarray) -> dict:
    ...
    return {str(s): {'count': int(c), 'pct': float(c / n) if n else 0.0} for s, c in zip(states, counts)}
...
def separation(labels: np.ndarray, fwd: np.ndarray) -> dict:
    ...
    for s in sorted(set(labels.tolist())):
        ...
        if len(r) == 0:
            continue
        groups.append(r)
        per_state[str(s)] = {'n': int(len(r)), 'mean': float(r.mean()), 'std': float(r.std()), 'hit_rate': float((r > 0).mean())}
...
def stability(labels: np.ndarray) -> dict:
    ...
    for x in labels[1:]:
        if x == cur:
            ...
            cur, length = (x, 1)
    runs.setdefault(str(cur), []).append(length)
    return {'transition_rate': float(flips / (n - 1)), 'flips': flips, 'mean_dwell': {s: float(np.mean(v)) for s, v in sorted(runs.items())}}
...
def block_shuffle_pvalue(labels, fwd, block_len, n_perm=200, seed=0) -> dict:
    ...
    for _ in range(n_perm):
        ...
        if separation(shuffled, fwd)['kruskal_h'] >= h_obs:
            ge += 1
    return {'kruskal_h': float(h_obs), 'p_value': float((ge + 1) / (n_perm + 1)), 'block_len': block_len, 'n_perm': int(n_perm)}
...
def per_state_significance(labels, fwd, block_len, n_perm=200, seed=0) -> dict:
    ...
    return {s: {'gap': gap_obs[s], 'p_value': p_vals[s], 'fdr_reject': bool(reject)} for s, reject in zip(active_states, bh_results)}
...
def score_labels(close, labels, features, horizons=(1, 4, 12), block_mult=3, seed=0, target='returns', n_perm=200) -> dict:
    labels = np.asarray(labels, dtype=object)
    features = np.asarray(features, dtype=float)
    try:
    ...
    for h in horizons:
        ...
        out['horizons'][f'h{h}'] = {'separation': separation(vlabels, fwd), 'significance': block_shuffle_pvalue(vlabels, fwd, block_len, n_perm=n_perm, seed=seed), 'yardstick': kmeans_yardstick(features, fwd_full, seed=seed), 'per_state_fdr': per_state_significance(vlabels, fwd, block_len, n_perm=n_perm, seed=seed)}
    if 'h4' in out['horizons']:
        out['h4'] = out['horizons']['h4']
    return out

def run_window(symbol, timeframe, window, *, model=None, horizons=(1, 4, 12), seed=0, target='returns', n_perm=200) -> dict:
    ...
    return score_labels(df['close'].to_numpy(), labels, features, horizons=horizons, seed=seed, target=target, n_perm=n_perm)
...
def build_parser() -> 'argparse.ArgumentParser':
    ...
    p.add_argument('--target', default='returns', choices=('returns', 'volatility'), help='forward variable the separation is scored on (default: returns)')
...
def main(argv=None) -> int:
    ...
    for w in args.windows.split(','):
        ...
        report[w] = run_window(args.symbol, args.timeframe, w, model=model, horizons=horizons, seed=args.seed, target=args.target)
    payload = {'symbol': args.symbol, 'timeframe': args.timeframe, 'target': args.target, 'source': 'model' if model else 'hand_rule', 'windows': report}
