...
from regime_diagnostics import DEFAULT_PERIOD, forward_realized_vol, separation, stability, block_shuffle_pvalue
...
def simulate_power(labels, fwd, block_len, profile, lam_grid, *, n_sim, n_perm, alpha, seed):
    ...
    for lam in lam_grid:
        ...
        for _ in range(n_sim):
            ...
            p = block_shuffle_pvalue(labels_v, planted, block_len, n_perm=n_perm, seed=perm_seed)['p_value']
            if p <= alpha:
                hits += 1
...
def replication_verdict(cells: list, alpha_family: float, *, primary_symbol=PRIMARY_SYMBOL, primary_timeframe=PRIMARY_TIMEFRAME) -> dict:

    def sig(c):
        return float(c['p_value']) <= float(alpha_family)
    n_avail = len(cells)
    sig_cells = [c for c in cells if sig(c)]
    primary_met = any((c['symbol'] == primary_symbol and c['timeframe'] == primary_timeframe and sig(c) and (c.get('knife_edge') is False) and (c.get('seed_stable') is True) for c in cells))
    breadth_met = n_avail > 0 and len(sig_cells) * 2 >= n_avail
    symbols_met = len({c['symbol'] for c in sig_cells}) >= 2
    return {'replicates': bool(primary_met and breadth_met and symbols_met), 'primary_met': bool(primary_met), 'breadth_met': bool(breadth_met), 'symbols_met': bool(symbols_met), 'n_available_cells': int(n_avail), 'n_significant_cells': int(len(sig_cells)), 'significant_symbols': sorted({c['symbol'] for c in sig_cells}), 'alpha_family': float(alpha_family)}
...
def load_cell(symbol, timeframe, window, horizon=PRIMARY_HORIZON, period=DEFAULT_PERIOD):
    ...
    if df is None or len(df) < MIN_VALID_BARS:
        cell.update({'status': 'unavailable', 'reason': f'too_few_bars: {(0 if df is None else len(df))} < {MIN_VALID_BARS}'})
        return cell
    dup_ts = int(df.index.duplicated().sum())
    if dup_ts or not df.index.is_monotonic_increasing:
        cell.update({'status': 'unavailable', 'reason': f'cache_unstable: dup_timestamps={dup_ts} monotonic={bool(df.index.is_monotonic_increasing)} (re-run on a quiescent cache)'})
        return cell
    ...
    if len(vlabels) < MIN_VALID_BARS:
        cell.update({'status': 'unavailable', 'reason': f'too_few_valid_bars: {int(valid.sum())} < {MIN_VALID_BARS}'})
        return cell
    ...
    idx = df.index
    cell.update({'status': 'available', 'labels': vlabels, 'fwd': fwd, 'block_len': int(block_len), 'n_valid': int(len(vlabels)), 'n_eff': n_eff, 'k_states': k, 'kruskal_h': float(sep['kruskal_h']), 'epsilon_squared': epsilon_squared(sep['kruskal_h'], n_eff, k), 'transition_rate': float(st['transition_rate']), 'resolved_start': str(idx[0]) if len(idx) else None, 'resolved_end': str(idx[-1]) if len(idx) else None, 'n_bars_raw': int(len(df)), 'dup_timestamps': dup_ts})
    return cell
...
def run_power(power_cells, *, n_sim, n_perm, alpha, lam_grid, seed, log=None):
    ...
    for symbol, timeframe, window in power_cells:
        ...
        if cell['status'] != 'available':
            ...
            results.append({'symbol': symbol, 'timeframe': timeframe, 'window': window, 'status': cell['status'], 'reason': cell.get('reason')})
            continue
        log(f"[power] {symbol} {timeframe} {window}: n_valid={cell['n_valid']} block_len={cell['block_len']} sims={n_sim} n_perm={n_perm}")
        powers = simulate_power(cell['labels'], cell['fwd'], cell['block_len'], profile, lam_grid, n_sim=n_sim, n_perm=n_perm, alpha=alpha, seed=seed)
        mde = min_detectable_effect(lam_grid, powers)
        entry = _public(cell)
        entry.update({'power_curve': {str(k): v for k, v in powers.items()}, 'power_at_is_effect': powers.get(1.0), 'mde_lambda': mde, 'powered_for_is_effect': bool(powers.get(1.0, 0.0) >= POWER_TARGET), 'alpha': float(alpha), 'n_sim': int(n_sim), 'power_n_perm': int(n_perm)})
        results.append(entry)
    return {'status': 'ok', 'effect_profile': profile, 'profile_window': f'{PRIMARY_SYMBOL} {PRIMARY_TIMEFRAME} is', 'cells': results}

def run_remeasure(datasets, windows, *, requested_n_perm, seed, seed_stability_seeds=(0, 1, 2), log=None):
    log = log or (lambda *_: None)
    m1080 = _load_1080()
    loaded = {}
    ...
    if n_avail == 0:
        return {'status': 'no_cells', 'cells': [], 'unavailable': [{'symbol': s, 'timeframe': t, 'window': w, 'reason': loaded[s, t, w].get('reason')} for s, t, w in loaded if loaded[s, t, w]['status'] != 'available']}
    ...
    for key in available:
        ...
        cell = loaded[key]
        pv = block_shuffle_pvalue(cell['labels'], cell['fwd'], cell['block_len'], n_perm=n_perm, seed=seed)
        ...
        seed_p = None
        if is_primary:
            seed_p = [block_shuffle_pvalue(cell['labels'], cell['fwd'], cell['block_len'], n_perm=n_perm, seed=s)['p_value'] for s in seed_stability_seeds]
            sig_flags = [p <= alpha_family for p in seed_p]
            seed_stable = bool(all(sig_flags) or not any(sig_flags))
        entry = _public(cell)
        entry.update({'p_value': float(pv['p_value']), 'n_perm': int(n_perm), 'alpha_family': float(alpha_family), 'significant': bool(pv['p_value'] <= alpha_family), 'permutation_steps_to_alpha': int(steps), 'knife_edge': bool(m1080.verdict_knife_edge(steps)), 'is_primary': bool(is_primary), 'seed_stability_p': seed_p, 'seed_stable': seed_stable})
        scored.append(entry)
        log(f"[remeasure] {symbol} {timeframe} {window}: p={pv['p_value']:.5f} h={cell['kruskal_h']:.2f} eps2={cell['epsilon_squared']:.4f} sig={entry['significant']} knife_edge={entry['knife_edge']}")
    verdict = replication_verdict(scored, alpha_family)
    return {'status': 'ok', 'n_available_cells': n_avail, 'alpha_family': float(alpha_family), 'n_perm': int(n_perm), 'cells': scored, 'unavailable': [{'symbol': s, 'timeframe': t, 'window': w, 'reason': loaded[s, t, w].get('reason')} for s, t, w in loaded if loaded[s, t, w]['status'] != 'available'], 'replication_verdict': verdict}
...
def build_parser():
    ...
    p.add_argument('--windows', default=','.join(FAMILY_WINDOWS), help=f"held-out family windows; known: {', '.join(WINDOWS)}")
    p.add_argument('--remeasure-n-perm', type=int, default=DEFAULT_REMEASURE_N_PERM, help='requested permutation count for the confirmatory family (floored by #1160)')
    p.add_argument('--power-cells', default='BTC/USDT:1h:oos', help='comma SYMBOL:TIMEFRAME:WINDOW for the power analysis (default: the exact #1177 OOS window; add BTC/USDT:1h:2023 to contrast a full-year window)')
...
def main(argv=None) -> int:
    ...
    payload = {'issue': 1211, 'significance_alpha': float(SIGNIFICANCE_ALPHA), 'primary': {'symbol': PRIMARY_SYMBOL, 'timeframe': PRIMARY_TIMEFRAME, 'horizon': PRIMARY_HORIZON}, 'seed': int(args.seed)}
    if not args.skip_remeasure:
        log('== 1b re-measurement (one Bonferroni family) ==')
        payload['remeasurement'] = run_remeasure(datasets, windows, requested_n_perm=args.remeasure_n_perm, seed=args.seed, log=log)
    ...
    if not args.skip_power:
        ...
        payload['power_analysis'] = run_power(_parse_power_cells(args.power_cells), n_sim=args.n_sim, n_perm=args.power_n_perm, alpha=SIGNIFICANCE_ALPHA, lam_grid=lam_grid, seed=args.seed, log=log)
