...
ADX_PERIOD = 14
ADX_THRESHOLD = 20.0
BARS_PER_YEAR = {'15m': 4 * 24 * 365, '30m': 2 * 24 * 365, '1h': 24 * 365, '2h': 12 * 365, '4h': 6 * 365, '1d': 365}
...
def _labels(df, classifier, th):
    if classifier == 'composite':
        labels = compute_regime_composite(df, period=COMPOSITE_PERIOD, thresholds=th)['regime'].to_numpy()
        feats = composite_feature_matrix(df, COMPOSITE_PERIOD, th).to_numpy()
        valid = ~np.isnan(feats).any(axis=1)
    else:
        labels = compute_regime(df, period=ADX_PERIOD, adx_threshold=ADX_THRESHOLD)['regime'].to_numpy()
...
def _book(close, decision_side, fee_rate):
    ...
    return {'total_return_pct': total_ret, 'max_drawdown_pct': max_dd, 'sharpe': 0.0, '_mu': mu, '_sd': sd, 'ddadj': round(dd_adjusted_return(total_ret, max_dd), 3), 'exposure': float(np.mean(pos != 0)), 'n_flips': int(np.sum(turnover > 0))}
...
def run(symbols, timeframes, windows, classifiers, th, ranging_mode, fee_rate, placebo_perm=0, seed=0):
    rows = []
        ...
        for symbol in symbols:
            for timeframe in timeframes:
                for window in windows:
                    ...
                    books = {}
                    for name, side in (('policy', pol_s), ('long_only', lon_s), ('buyhold', buy_s)):
                    ...
                    placebo_p = None
                    if placebo_perm > 0:
                        placebo_p, _ = _placebo_pvalue(close, pol_s, timeframe, fee_rate, placebo_perm, seed)
                    rows.append({'classifier': classifier, 'symbol': symbol, 'timeframe': timeframe, 'window': window, 'policy': p, 'long_only': lo, 'buyhold': bh, 'beats_buyhold': bool(p['sharpe'] > bh['sharpe'] and p['ddadj'] > bh['ddadj']), 'beats_long_only': bool(lo is not None and p['sharpe'] > lo['sharpe'] and (p['ddadj'] > lo['ddadj'])), 'placebo_p': placebo_p})
    return rows

def report(rows, ranging_mode, fee_rate):
    print('=' * 96)
    print(f'REGIME-TIMING ECONOMIC ISOLATION (#1076 scope 2) | ranging={ranging_mode} fee={fee_rate * 10000.0:.0f}bps/side')
    print('=' * 96)
    hdr = f"{'clf':9s} {'sym':9s} {'tf':4s} {'win':7s} | {'pol_shrp':>8s} {'pol_ret':>8s} {'pol_dda':>8s} | {'bh_shrp':>8s} {'bh_ret':>8s} | {'>BH':>4s} {'>LO':>4s}"
    print(hdr)
    print('-' * len(hdr))
    for r in rows:
        p, bh = (r['policy'], r['buyhold'])
        print(f"{r['classifier']:9s} {r['symbol']:9s} {r['timeframe']:4s} {r['window']:7s} | {p['sharpe']:8.2f} {p['total_return_pct']:8.1f} {p['ddadj']:8.2f} | {bh['sharpe']:8.2f} {bh['total_return_pct']:8.1f} | {('Y' if r['beats_buyhold'] else '.'):>4s} {('Y' if r['beats_long_only'] else '.'):>4s}")
    ...
    print(f"  all cells:           beats buyhold {sum((r['beats_buyhold'] for r in rows)):3d}/{len(rows):<3d}   beats long_only {sum((r['beats_long_only'] for r in rows)):3d}/{len(rows)}")
    print(f"  held-out (is/oos):   beats buyhold {sum((r['beats_buyhold'] for r in held)):3d}/{len(held):<3d}   beats long_only {sum((r['beats_long_only'] for r in held)):3d}/{len(held)}")
    print(f"  oos (2026-) only:    beats buyhold {sum((r['beats_buyhold'] for r in oos)):3d}/{len(oos):<3d}   beats long_only {sum((r['beats_long_only'] for r in oos)):3d}/{len(oos)}")
    print()
    ...
    if placebo_rows:
        ...
        print(f'  cells where regime TIMING beats shuffled null (raw p<=0.05):     {n_raw:3d}/{len(placebo_rows)}')
        print(f'  ... surviving Benjamini-Hochberg FDR q=0.05 across cells:        {n_bh:3d}/{len(placebo_rows)}')
        survivors = [(r, p) for r, p, b in zip(placebo_rows, pvals, rej) if b]
        for r, p in sorted(survivors, key=lambda x: x[1]):
            print(f"    {r['classifier']:9s} {r['symbol']:9s} {r['timeframe']:4s} {r['window']:7s}  placebo_p={p:.3f}  pol_sharpe={r['policy']['sharpe']:.2f}")
        if n_bh == 0:
...
def build_parser():
    ...
    p.add_argument('--ranging-mode', default='flat', choices=('flat', 'long'), help='side in ranging regimes: flat (default) or long')
    p.add_argument('--fee-bps', type=float, default=10.0, help='taker fee bps per unit turnover')
    p.add_argument('--placebo-perm', type=int, default=0, help='block-shuffle placebo permutations per cell (0=off; 300 for rigor)')
...
def main(argv=None) -> int:
    ...
    print(f'# universe: {list(symbols)} x {list(timeframes)} x {list(windows)} classifiers={list(classifiers)} platform={PLATFORM}\n')
    rows = run(symbols, timeframes, windows, classifiers, th, args.ranging_mode, fee_rate, placebo_perm=args.placebo_perm, seed=args.seed)
    report(rows, args.ranging_mode, fee_rate)
    if args.out:
        import json
        with open(args.out, 'w') as fh:
            json.dump({'ranging_mode': args.ranging_mode, 'fee_bps': args.fee_bps, 'placebo_perm': args.placebo_perm, 'rows': rows}, fh, indent=2)
