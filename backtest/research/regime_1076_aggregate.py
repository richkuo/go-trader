...
def main(argv=None) -> int:
    ...
    for p in paths:
        ...
        rows.extend(d['rows'])
        u = d.get('universe', {})
        universe.append(f"{os.path.basename(p)}: {u.get('symbols')} x {u.get('timeframes')} (n_perm={u.get('n_perm')})")
    ...
    print(f'  within-cell candidate edges (uncorrected family-wide): {len(within)} (held-out is/oos {len(within_held)}, oos {len(within_oos)})')
    ...
    if survivors:
        ...
        for r in sorted(survivors, key=lambda x: x['p_value']):
            print(f"  {r['classifier']:10s} {r['symbol']:9s} {r['timeframe']:4s} {r['window']:7s} h{r['horizon']:<3d} {r['state']:22s} p={r['p_value']:.4f} gap={r['gap']:+.5f} aligned={r['sign_aligned']}")
    else:
