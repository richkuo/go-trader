...
def resolve_bakeoff_n_perm(n_candidates, requested=None):
    ...
    if requested < floor:
        raise ValueError(f'n_perm={requested} cannot satisfy the Bonferroni-corrected alpha {alpha:.6f} over {n_candidates} candidates: the minimum achievable permutation p-value is 1/{requested + 1} ~ {1.0 / (requested + 1):.6f} > alpha, so no candidate could ever pass. Use n_perm >= {floor}.')
    return requested

def permutation_steps_to_alpha(p_value, n_perm, alpha=SIGNIFICANCE_ALPHA):
    scale = int(n_perm) + 1
    count = int(round(float(p_value) * scale))
    limit = int(math.floor(float(alpha) * scale + 1e-09))
    return limit - count
...
def structurally_ineligible_reason(k, thresholds):
    if int(k) < int(thresholds.min_active_labels):
        return f'k={int(k)} can emit at most {int(k)} distinct labels < min_active_labels={int(thresholds.min_active_labels)}: non-degeneracy is unsatisfiable'
    return None
...
def select_winner(candidates):
    ...
    eligible = [c for c in candidates if not c.get('structurally_ineligible') and c.get('verdict', {}).get('ship') and c.get('non_degenerate_all') and (c.get('model_p_value') is not None) and (c['model_p_value'] <= alpha)]
    if not eligible:
        return None
...
def run_bakeoff(symbol='BTC/USDT', timeframe='1h', *, in_sample='is', held_out='oos', eval_windows=DEFAULT_WINDOWS, families=('hmm', 'gmm', 'kmeans'), k_range=range(2, 8), period=48, filter_window=64, seed=0, n_perm=None):
    ...
    ineligible_reasons = {cell: structurally_ineligible_reason(cell[1], thresholds) for cell in plan}
    ...
    ineligible_report = [{'family': f, 'k': k, 'reason': ineligible_reasons[f, k]} for f, k in plan if ineligible_reasons[f, k]]
    for entry in ineligible_report:
        print(f"NOTE: candidate {entry['family']}:k={entry['k']} is structurally ineligible and excluded from the Bonferroni denominator — {entry['reason']}", file=sys.stderr)
    print(f'NOTE: n_perm={n_perm} (min achievable p {1.0 / (n_perm + 1):.6f}) vs Bonferroni-corrected alpha {alpha:.6f} over {denominator} structurally eligible of {len(plan)} swept candidates', file=sys.stderr)
    hr_held = run_window(symbol, timeframe, held_out, model=None, seed=seed, target='volatility', n_perm=n_perm)
    ...
    for family, k in plan:
        model = rvm.fit_unsupervised(fit_feats, family=family, k=k, filter_window=filter_window, period=period, thresholds=th, seed=seed, fitted_on={'symbol': symbol, 'timeframe': timeframe, 'window': in_sample})
        md = run_window(symbol, timeframe, held_out, model=model, seed=seed, target='volatility', n_perm=n_perm)
        verdict = gate_verdict(hr_held, md)
        nd = {w: rvm.non_degeneracy(_model_label_stream(symbol, timeframe, w, model, period, th), thresholds) for w in eval_windows}
        candidates.append({'family': family, 'k': k, 'verdict': verdict, 'model_kruskal_h': md['h4']['separation']['kruskal_h'], 'model_p_value': md['h4']['significance']['p_value'], 'stability_gain': float(hr_tr - md['stability']['transition_rate']), 'coverage': md['coverage'], 'non_degeneracy': {w: nd[w] for w in eval_windows}, 'non_degenerate_all': all((nd[w]['ok'] for w in eval_windows)), 'structurally_ineligible': bool(ineligible_reasons[family, k]), 'structural_ineligibility_reason': ineligible_reasons[family, k], 'states': model['states'], 'mapping': model['mapping']})
    for c in candidates:
    ...
    return {'symbol': symbol, 'timeframe': timeframe, 'in_sample': in_sample, 'held_out': held_out, 'target': 'volatility', 'candidate_count': len(candidates), 'significance_alpha': SIGNIFICANCE_ALPHA, 'bonferroni_alpha': alpha, 'bonferroni_denominator': denominator, 'bonferroni_denominator_policy': 'structurally ineligible candidates (k < incumbent-derived min_active_labels) are scored for evidence but excluded from the family-wise denominator', 'structurally_ineligible': ineligible_report, 'n_perm': int(n_perm), 'min_achievable_p_value': 1.0 / (n_perm + 1), 'non_degeneracy_thresholds': vars(thresholds), 'handrule_held_out': {'kruskal_h': hr_held['h4']['separation']['kruskal_h'], 'p_value': hr_p, 'transition_rate': hr_tr, 'abstained': bool(hr_p > SIGNIFICANCE_ALPHA), 'permutation_steps_to_alpha': int(incumbent_steps), 'knife_edge': bool(verdict_knife_edge(incumbent_steps))}, 'candidates': candidates, 'winner': {'family': winner['family'], 'k': winner['k']} if winner else None}
...
def build_parser():
    ...
    p.add_argument('--n-perm', type=int, default=None, help='block-shuffle permutation count for the significance arm (default: auto-resolved so the Bonferroni-corrected alpha is achievable with headroom; explicit values below the achievability floor are rejected)')
...
def main(argv=None):
    ...
    report = run_bakeoff(args.symbol, args.timeframe, in_sample=args.in_sample, held_out=args.held_out, period=args.period, filter_window=args.filter_window, seed=args.seed, n_perm=args.n_perm)
    ...
    print(text)
    w = report['winner']
    print(f'\nWINNER: {w}' if w else '\nWINNER: none eligible (no gate-passing, non-degenerate candidate on this window)', file=sys.stderr)
    return 0
