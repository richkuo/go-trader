...
from regime_diagnostics import forward_returns, forward_realized_vol, separation, stability, block_shuffle_pvalue, kmeans_yardstick, _kmeans
...
def _load(symbol, timeframe, window, thresholds):
    ...
    labels = compute_regime_composite(df, period=PERIOD, thresholds=thresholds)['regime'].to_numpy()
    ...
    return {'close': df['close'].to_numpy(), 'features': features, 'valid': valid, 'vlabels': labels[valid], 'mean_dwell': mean_dwell, 'transition_rate': st['transition_rate']}

def diag_windows(symbol, timeframe, windows, th, horizon=4):
    print(f'=== per-window hand-rule forward-RETURN separation (h{horizon}) + k-means feature ceiling ===')
    print(f"{'window':8s} {'n':>6s} {'HR_KWH':>8s} {'HR_p':>6s} {'sig?':>5s} | {'km_best_k':>9s} {'km_KWH':>8s} | {'HR_tr':>6s}")
    print('-' * 72)
    for w in windows:
        ...
        print(f"{w:8s} {len(d['vlabels']):6d} {sig['kruskal_h']:8.2f} {sig['p_value']:6.3f} {flag:>5s} | {best_k:9d} {yd[best_k]['kruskal_h']:8.2f} | {d['transition_rate']:6.3f}")
    print("(k-means yardstick is IN-SAMPLE, no significance test -> overfit ceiling; see the 'vol' diagnostic Test B for its real significance.)\n")

def diag_horizons(symbol, timeframe, windows, horizons, th):
    print('=== hand-rule forward-RETURN significance swept across horizons (* = block-shuffle p<=0.05) ===')
    ...
    for w in windows:
        ...
        print(row)
    print(f'\n{n_sig}/{len(windows) * len(horizons)} (window x horizon) tests block-shuffle significant.\n')
...
def diag_vol(symbol, timeframe, windows, th):
    ...
    for w in windows:
        ...
        print(row)
    print("\n=== TEST B: is the k-means feature 'ceiling' real? (in-sample fit, block-shuffle significance on forward RETURNS, h4) ===")
    ...
    for w in windows:
        ...
        print(f"{w:8s} | {k:2d} {sig['kruskal_h']:8.1f} {sig['p_value']:10.3f}  {verdict}")
    print()
...
def build_parser():
    ...
    p.add_argument('--diagnostics', default='windows,horizons,vol', help='comma-separated subset of: windows, horizons, vol')
    return p
