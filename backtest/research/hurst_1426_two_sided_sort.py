...
TWO_SIDED = True
TWO_SIDED_P_DEFINITION = (
    "p2 = min(1, 2 * min(p_ge, p_le)), each tail carrying the add-one "
    "convention over the SAME draws; the smallest reachable p is 2/(draws+1)")

CONTRACT_REPORT_BASENAME = "hurst_gate_calibration.md"

INTERIM_LOOK_DISCLOSURE = (
    "INTERIM LOOK, disclosed before the run, and it is the worst kind. The "
    "SIGN this study was built to test was already visible: #1424's committed "
    "run reports a confirmatory separation of -0.005 efficiency units and "
    "-0.12 pp of net return on the `momentum` family, measured on the very "
    "rows this study re-scores. That look is not merely prior exposure — it is "
    "what MOTIVATED the hypothesis that Hurst might sort these trades the "
    "other way. No pre-registration can be claimed over rows chosen because of "
    "what they already showed. The study therefore takes OPTION 2 from the "
    "issue: it is EXPLORATORY-ONLY over the full pool, it states that cost "
    "here, and its verdict machinery is structurally unable to recommend "
    "anything. A reader should treat even a SIGNIFICANT reversal from this run "
    "as hypothesis-generating for a future pre-registered test on tape that "
    "does not yet exist, and never as a licence to act.")

COHORT_DECISION_STATEMENT = (
    "COHORT DECISION: Option 2 — exploratory-only, full pool. Option 1 "
    "(reserve rows #1424 never scored and make those the confirmatory cohort) "
    "is not available here. #1424's windows already run open-ended through the "
    "tape at run time, so almost no unscored calendar remains; its committed "
    "feasibility probes record every other reachable venue as INFEASIBLE "
    "(binance-global HTTP 451, Kraken ignores a deep `since`, TopStep exposes "
    "no `since` at all, IBKR exposes no OHLCV), so an unscored (venue, window) "
    "cell for a scored base asset is the same market's tape — the double count "
    "the window-ownership matrix exists to forbid. And effective N is set by "
    "independent calendar clusters rather than rows, so any strict subset "
    "carries a WORSE detection limit than the 0.013 measured on the full "
    "7,992-row family. Option 1 would buy a pre-registration guaranteed to be "
    "inconclusive. THE COST OF OPTION 2 IS REAL AND IT IS ENFORCED: no "
    "confirmatory claim is available from this run, no configuration can win, "
    "and no threshold ships.")
...
CONTRACT_PATH_STATEMENT = (
    ...
    "unconditionally. The supersede clause is shared with #1427 and #1428, and "
    "at most one of the three may claim it; #1426 is not that one.")

KEY_RISK_PREDICTION = (
    "The two-sided re-test is expected to return INCONCLUSIVE, and the reason "
    "matters more than the word. #1424 measured |-0.005| against a row-matched "
    "limit of 0.013, and a two-sided limit can only be at or above its "
    "one-sided counterpart at the same alpha, so the separation is expected to "
    "stay BELOW the limit and the validity gate is expected to FAIL. A failed "
    "gate carries NO bound — not in one direction and not in both. What this "
    "study buys is therefore narrower than more power and it should not be "
    "oversold: it removes the BLIND SPOT that made #1424's observation "
    "unusable, so the reversed direction becomes testable at all, and it "
    "reports what that test found. A both-directions BOUND is a different "
    "outcome with a different name — `resolved_null`, reachable only if the "
    "gate passes — and predicting it alongside INCONCLUSIVE would be "
...
VERDICT_LABELS = {
    VERDICT_SORT_DETECTED: "HURST SORTS THESE TRADES (EXPLORATORY FINDING)",
...
def two_sided_permutation_pvalue_group_diff(values: Sequence[float],
...
def two_sided_permutation_pvalue_weighted(returns: Sequence[float],
...
def two_sided_cluster_permutation_pvalue_group_diff(
    ...
    return {"p": doubled_tail_p(ge, le, draws), "n_draws": draws,
...
def two_sided_cluster_permutation_pvalue_weighted(
        trades: Sequence[dict],
...
def two_sided_min_detectable_effect_on_grid(trades: Sequence[dict],
    ...
    if floor > bar:
        raise ValueError(
            ...
            f"{bar:g} for a family of {family_size} under the TWO-SIDED "
            f"doubled-tail convention (smallest reachable p is {floor:g}); "
            f"raise --n-perm-mde to at least {int(math.ceil(2.0 / bar))}")
    ...
    def _p_one_direction(d: float) -> Optional[float]:
        ...
        if cluster:
            return two_sided_cluster_permutation_pvalue_group_diff(
                trades, shifted, mask, n_perm=n_perm, seed=seed).get("p")
        return two_sided_permutation_pvalue_group_diff(
            shifted, mask, n_perm=n_perm, seed=seed)
...
def _direction_phrase(separation: float) -> str:
    return ("the KEPT trades did better, the direction the gate's own "
            ...
            "hypothesis — a gate built on it would have HURT")
...
def decide_recommendation(configs: Sequence[dict], mde: dict) -> dict:
    ...
    head = (f"On the confirmatory family (`{gate.get('family', PRIMARY_FAMILY)}`, "
            ...
            f"zero-injection cluster p is {p_text} against a rank-1 "
            f"Benjamini-Hochberg bar of {bar:g}")

    if significant:
        verdict = VERDICT_SORT_DETECTED
        detail = (
            f"{head}, so the contrast is REJECTED in a test that could have "
            f"rejected it either way. Hurst at entry SORTS these trades: "
            f"the separation is {_fmt_signed(sep, 3)} efficiency units, which "
            f"means {_direction_phrase(sep if sep is not None else 0.0)}.")
        if gate["passed"]:
            detail += (
                f" The validity gate also PASSED — the two-sided limit of "
            ...
            detail += (
                " The validity gate did NOT pass, so the effect sits below the "
                "size this design resolves; treat the significance as fragile "
                "and the magnitude as unestimated.")
    elif gate["passed"]:
        verdict = VERDICT_RESOLVED_NULL
        detail = (
            f"{head}, so the contrast is NOT rejected. The validity gate PASSED: "
            f"the two-sided limit on those same rows is {gate['limit']:.3f} "
            f"efficiency units and their separation is "
            f"{_fmt_signed(sep, 3)}, whose magnitude the design can resolve. "
            f"So this is a statement about the data and not about the test: "
            f"Hurst at entry does not sort these trades IN EITHER DIRECTION at "
            f"an effect size this design can see.")
    else:
        verdict = VERDICT_INCONCLUSIVE
        if gate["reason"]:
            detail = (f"{head}. The validity gate FAILED: {gate['reason']}, so "
                      f"the run carries no bound in either direction.")
        else:
            detail = (
                f"{head}. The validity gate FAILED: the two-sided limit on "
                f"those rows is {gate['limit']:.3f} efficiency units while they "
                f"separate by only {_fmt_signed(sep, 3)}, whose magnitude sits "
                f"BELOW the limit. Nothing that small is visible to this "
                f"design, so the run bounds any sorting effect — in EITHER "
                f"direction, which is what a symmetric test buys and what "
                f"#1424's one-sided run could not offer — from above at "
                f"{gate['limit']:.3f} efficiency units, and says nothing about "
                f"anything smaller.")
    ...
    return {
        ...
        "justification": (
            f"{detail} {EXPLORATORY_ONLY_SENTENCE} Across the {tested} "
            f"primary-cohort "
            f"{'configuration' if tested == 1 else 'configurations'} swept, "
            f"{n_significant} reached Benjamini-Hochberg significance on the "
            f"two-sided cluster permutation and {n_untestable} were untestable; "
            f"neither count can promote a configuration, because this study has "
            f"no branch that promotes one."
        ).strip(),
    }
...
def decision_payload(decision: dict) -> dict:
    return {
        ...
        "cohort_option": decision["cohort_option"],
...
def render_recommendation(decision: dict, mde: dict,
    ...
    if decision["verdict"] == VERDICT_SORT_DETECTED:
        lines.append(
            "The prediction did NOT hold, and that is the interesting outcome. "
            "A two-sided test rejected the contrast, so Hurst at entry sorts "
            "these trades by an amount this design registers — a result the "
            "one-sided predecessors could not have produced whichever way the "
            "data pointed. Under Option 2 it is a HYPOTHESIS, not a finding to "
            "act on: the sign was known before the design was fixed, so the "
            "next step is a pre-registered test on tape this study did not "
            "score, and there is none available today.")
    elif decision.get("key_risk_held"):
        lines.append(
            "The prediction HELD in its useful half. The contrast is not "
            "rejected AND the validity gate passed, so the run bounds any "
            "sorting effect in BOTH directions at the measured limit. That is "
            "the concrete thing #1426 was built to produce: #1424's observation "
            "was unusable because its design could not speak to the direction "
            "the data pointed, and this run replaces it with a symmetric bound. "
            "The bound is economically negligible at this resolution.")
    else:
        lines.append(
            "The prediction is UNRESOLVED. The contrast is not rejected, but "
            "the validity gate did not pass either, so the run carries no bound "
            "— in either direction. A symmetric test removes the BLIND SPOT that "
            "made #1424's observation unusable; it does not manufacture power. "
            "Anyone reopening this question needs a design that resolves a "
            "smaller effect, and the calendar and the venues this repository can "
            "reach are already exhausted.")
    lines.append("")
    lines.append(
        "**No configuration is recommended, and none could be.** " +
        EXPLORATORY_ONLY_SENTENCE + " `decide_recommendation` has no branch "
        "that promotes a configuration and this module defines no "
        "configuration verdict at all; the per-family table below reports how "
        "many configs WOULD have passed #1424's acceptance rule purely as a "
...
def render_report(payload: dict) -> str:
    ...
    out.append("")
    out.append(
        "#1410, #1422 and #1424 all tested ONE hypothesis — that trades taken "
        "at high Hurst persistence beat trades taken at low persistence — and "
        "all three tested it ONE-SIDED. #1424 measured the consequence: on its "
        "confirmatory family the row-matched detection limit is 0.013 "
        "efficiency units while those same rows separate by **-0.005**. The "
        "sign is the finding. `mean(kept) - mean(suppressed)` going NEGATIVE "
        "means the trades the gate would have SUPPRESSED did BETTER, and that "
        "is the one direction a one-sided design cannot detect at any "
        "magnitude. The observation was therefore neither a rejection nor a "
        "confirmation. This study re-runs the same machinery with the "
        "inference made SYMMETRIC, so a reversed effect becomes a finding "
        "instead of a blind spot.")
    out.append("")
    out.append(
        f"Generated by `backtest/research/hurst_1426_two_sided_sort.py` (schema "
        f"{payload['schema_version']}). Every number below is rendered from "
        f"`hurst_1426_two_sided_sort.json`, produced by the same run.")
    out.append("")

    out.append("## Verdict at a glance")
    out.append("")
    out.append(f"- Cohort decision: **Option 2 — exploratory-only, full pool**. "
               f"No confirmatory claim is available from this run.")
    ...
    out.append(f"- Separation on those same rows: "
               f"**{_fmt_signed(gate.get('largest_separation'), 4)}** efficiency "
               f"units — " + (
    ...
    out.append(f"- Contract path: **deferred to #1424**; siblings "
               f"{', '.join('#' + str(n) for n in SIBLING_DEFERRAL)} may still "
               f"claim it.")
    out.append("")
    ...
    out.append(
        ...
        "is a DOUBLED ONE-TAILED permutation p:")
    out.append("")
    out.append("```")
    ...
    out.append("")
    out.append(
        "The doubled tail is used rather than the `|stat| >= |obs|` form "
        "because neither statistic is guaranteed symmetric under its own null: "
        "the group difference is not, under unequal group sizes, and the "
        "weighted (sizing) statistic is not centred at zero at all. Doubling "
        "the smaller tail is valid-conservative for both, uniformly. Both tails "
        "are counted over the draws the null ACTUALLY produced, never over the "
        "requested count — a cluster rotation discards draws that collapse the "
        "split.")
    out.append("")
    out.append(
        f"The smallest reachable p is therefore `2/(draws+1)`, and the "
        f"detection-limit search checks THAT floor rather than `1/(n+1)`: a run "
        f"with too few draws would find every effect undetectable and publish "
        f"\"no power\" when the truth is \"no permutations\". It RAISES instead. "
        f"At {pre['n_perm_mde']} detection-limit draws the floor is "
        f"{2.0 / (float(pre['n_perm_mde']) + 1.0):.5f}. The bar it has to clear "
        f"is not the confirmatory one: the #1410 and exploratory pools are "
        f"scored against a family of 30, whose rank-1 bar is "
        f"{ALPHA / 30.0:.5f}, and that is the BINDING constraint on this run's "
        f"draw count. The confirmatory family's own bar is "
        f"{ALPHA / max(1, PRIMARY_FAMILY_SIZE):g} and is far easier to resolve. "
        f"Doubling the tail doubles the required draws against #1424's "
        f"one-sided floor, which is why a draw count that sufficed there can "
        f"fail loudly here.")
    out.append("")
    out.append(
        f"The DETECTION LIMIT injects both directions at every grid point — the "
        f"suppressed values shifted down by `d`, then up by `d` — and scores "
        f"the point by `max(p2(+d), p2(-d))`. The published limit is therefore "
        f"the smallest effect this design would catch WHICHEVER WAY it points. "
        f"Taking the minimum instead would publish the easier direction as the "
        f"test's resolution, which is the one-sided error in a new costume. The "
        f"grids are #1424's verbatim ({MDE_EFF_GRID_STEP:g} to "
        f"{MDE_EFF_GRID_MAX:g} with a {MDE_EFF_REFINE_STEP:g} refinement on the "
        f"primary target; #1422's {MDE_PP_GRID_STEP:g}-to-{MDE_PP_GRID_MAX:g} pp "
        f"grid on the continuity target), so the limits stay directly "
        f"comparable and the only thing a reader has to account for is the "
        f"symmetry.")
    out.append("")
    out.append(
        "ONE DELIBERATE EXCEPTION, disclosed rather than buried: the joint "
        "ADX x Hurst **Stage 0** verdict in Part D is inherited from #1422 "
        "VERBATIM and stays ONE-SIDED on net return. #1424 pinned it that way "
        "so the verdict recorded against #1412 stays comparable across studies. "
        "It sits outside the confirmatory path — nothing in the config sweep, "
        "the detection limits, the validity gate or the verdict calls it — and "
        "a test pins that.")
    out.append("")
    ...
    out.append(
        ...
        f"family the single pinned hypothesis belongs to) falls at or below the "
        f"MAGNITUDE of that same family's observed separation, both in "
        f"efficiency units and both measured on the IDENTICAL rows.")
    out.append("")
    out.append(
        "The gate reads a MAGNITUDE here, and #1424's read a SIGNED value. That "
        "is not a relaxation: #1424's nulls and its injection were one-sided in "
        "the `mean(kept) - mean(suppressed)` orientation, so a negative "
        "separation pointed the way its design could not detect at any size and "
        "no limit spoke to it. Here the null is two-sided and the injection is "
        "applied in both directions with the limit taken as the harder of the "
        "two, so the limit describes an effect detectable whichever way it "
        "points. The magnitude comparison is legitimate ONLY because of that, "
        "and a future edit that makes any p on this path one-sided again must "
        "restore the signed comparison in the same change.")
    out.append("")
    out.append(
        "The SIGN is still carried and still reported everywhere, because it is "
        "what says WHICH way a finding points. And the gate still refuses the "
        "POOLED limit: that number spans both families and would resolve a "
        "smaller effect purely by holding more trades, so pairing it with one "
        "family's separation is the whole-study-versus-sub-cohort mismatch the "
        "pool-matched tables exist to prevent, and it biases the gate toward "
        "passing.")
    out.append("")
    ...
    out.append("")
    out.append(
        "Inherited from #1424 unless the two-sided section above names it. "
        "These constants are imported from that module rather than restated, so "
        "a drift there fails loud here instead of silently rescoring different "
        "cells.")
    out.append("")
    out.append(
    ...
    out.append(
        f"- PRIMARY TARGET: `{PRIMARY_TARGET}` over a fixed {HORIZON_HOURS}-hour "
        f"horizon, bounded in `[-1, 1]`. CONTINUITY TARGET: "
        f"`{CONTINUITY_TARGET}`, scored on the SAME rows so the two describe one "
        f"pool.")
    out.append(
        f"- PINNED HYPOTHESIS: `{PRIMARY_CONFIG_ID}`, inherited from #1424, "
        f"which derived it mechanically as the smallest raw p over all 30 "
        f"configs in the committed `hurst_1410_gate_calibration.json` and "
        f"re-derives it at run time with a hard assert. The Benjamini-Hochberg "
        f"denominator for the confirmatory family is {PRIMARY_FAMILY_SIZE}. "
        f"Under Option 2 this is a PINNED hypothesis and not a pre-registered "
        f"one: the sign it is tested against was already visible.")
    ...
    out.append(f"- Fee model: `{pre['fee_platform']}` on every venue, decoupled "
               f"from the data-source exchange exactly as in #1424.")
    out.append(
    ...
    out.append(
        ...
        f"rotation offset {MIN_OFFSET_DAYS} days; {pre['n_perm_mde']} draws for "
        f"the detection limits. Benjamini-Hochberg at alpha={ALPHA}, applied "
        f"SEPARATELY to the primary and exploratory cohorts.")
    out.append(f"- Joint table: ADX period {ADX_PERIOD} (Wilder), split at "
               f"{ADX_SPLIT:g}, scored ONE-SIDED on net return — the disclosed "
               f"exception above.")
    out.append("")
    ...
    out.append("")
    out.append(
        "Unchanged, and this study touches none of it. Rolling H at bar `i` "
        "uses closes `[i-W+1, i]`. The DECISION series is that shifted one bar, "
        "so a signal at bar N reads H through N-1; the backtester fills a bar-N "
        "signal at N+1's open, so the FILL-BAR stamp is the bar-close series "
        "shifted twice. Entry ADX uses the SAME two shifts, with warm-up bars "
        "masked to NaN. The primary target reads bars AFTER the fill, which is "
        "what an outcome does; it enters no decision anywhere.")
    out.append("")
    ...
    out.append(
        ...
        f"owned cells were DROPPED, listed below; a further "
    ...
    out.append(
        ...
        "daily-return correlation when two trades' holding periods OVERLAP and "
        "0 when they do not; same-asset pairs take 1.0 whatever the quote "
    ...
    out.append("")
    out.append(
        "The smallest per-trade effect each pool could have detected WHICHEVER "
        "WAY IT POINTS, under the two-sided cluster null at the rank-1 "
        "Benjamini-Hochberg threshold. The separation column is that same "
        "contrast measured on the SAME rows. `Resolvable?` compares the "
        "MAGNITUDE of the separation with the limit — which is a legitimate "
        "comparison here and was not in #1424, for the reason the validity-gate "
        "section gives.")
    out.append("")
    out.append(
        "**Separations still carry their SIGN, and the sign still decides what "
        "a finding MEANS.** A separation is `mean(kept) - mean(suppressed)`. A "
        "POSITIVE value means the kept trades did better, the direction the "
        "gate's hypothesis claims. A NEGATIVE value means the SUPPRESSED trades "
        "did better — under this study's symmetric test that is a detectable "
        "finding rather than a blind spot, and it says a gate built on the "
        "hypothesis would have HURT. The `Largest separation` column reports the "
        "largest MAGNITUDE with its own sign attached, so neither the size nor "
    ...
    out.append("| Pool | Rows | BH denominator | 2-sided cluster MDE (eff) | "
               "2-sided free MDE (eff) | Largest separation ON THAT POOL (eff) | "
               "By family (signed) | Resolvable? | 2-sided cluster p at zero "
               "injection |")
    out.append("|------|-----:|---------------:|--------------------------:|"
               "-----------------------:|--------------------------------------:|"
               "--------------------|:-----------:|"
    ...
    out.append("| Pool | 2-sided cluster MDE (pp/trade) | Largest separation ON "
               "THAT POOL (pp/trade) | By family (signed) | Resolvable? | "
    ...
    out.append("")
    out.append(
        "**The numbers the validity gate and the verdict actually read** are "
        f"neither of the pooled rows above. Both are evaluated on the "
        f"CONFIRMATORY family (`{PRIMARY_FAMILY}`) alone, on that family's own "
        f"rows. The pooled primary limit spans BOTH families and would resolve a "
        f"smaller effect purely because it holds more trades, so reading it "
        f"against one family's separation would make the gate easier to pass "
        f"than its own row-matched rule allows.")
    out.append("")
    ...
    out.append("| Family | Rows | 2-sided cluster MDE (eff) | Separation (eff, "
               "signed) | 2-sided p at zero injection | 2-sided cluster MDE (pp) "
               "| Separation (pp, signed) | Reads the gate? |")
    out.append("|--------|-----:|--------------------------:|"
               "-------------------------:|----------------------------:|"
               "-------------------------:|------------------------:|"
               ":---------------:|")
    ...
    out.append(
        ...
        "while the gate is disarmed (closes never masked); their drawdowns are "
        "bar-level. `size` rows re-compound the same ungated trade sequence "
        "with the size multiplier; their drawdowns are trade-granular. Never "
        "compare a `gate` drawdown to a `size` drawdown. `dd` and `chop` are "
        "MAGNITUDE deltas (arm minus ungated) averaged over that window's legs "
        "— negative means improvement. Every p column is TWO-SIDED. The "
        "`#1424 rule` column shows whether the config would have passed that "
        "study's acceptance rule; under Option 2 it cannot win here either way.")
    out.append("")
    ...
    out.append(
        ...
        "Under Option 2 the whole study is exploratory, so this grid differs "
        "from the one above only in its denominator and its hypothesis count.")
    out.append("")
    ...
    out.append(
        ...
        "alone? Scored ONE-SIDED on NET RETURN, inherited from #1422 verbatim. "
        "This is the study's single disclosed exception to two-sidedness, kept "
        "so the verdict recorded against #1412 stays comparable with the one "
        "already on the record. It sits outside the confirmatory path.")
    out.append("")
    ...
    for family in FAMILIES:
        ...
        if v.get("separated"):
            out.append(
                ...
                f"(one-sided cluster p={_fmt_p(v.get('p_cluster'))}, Bonferroni "
                f"bar {JOINT_ALPHA:g}; detection limit "
    ...
    if any(all_sep):
        out.append(
            "At least one family separates on the joint buckets on this run. "
            "That is Stage 0's own one-sided question and it is reported here "
            "for continuity; it is not this study's verdict.")
    else:
        out.append(
            f"`{NO_JOINT_SEPARATION}` on every family, re-discharging Stage 0 on "
            f"this pool exactly as #1424 did. #1412 stays closed.")
    out.append("")
    ...
    out.append("")
    out.append(
        "1. It cannot CONFIRM anything. The sign was seen before the design was "
        "fixed, and it is what motivated the hypothesis. Even a significant "
        "result is hypothesis-generating.")
    out.append(
        "2. It cannot recommend a threshold, a gate or a size rule. No code "
        "path here promotes a configuration.")
    out.append(
        "3. It cannot claim more independent information than #1424 had. The "
        "pool is the same tape, so the LIMITS are of the same order — a "
        "two-sided test costs power against a one-sided one at the same alpha, "
        "so this study's limit can only be at or above #1424's.")
    out.append(
        "4. It does not supersede #1424 as the live evidence, and it never "
        "writes the contract path.")
    out.append("")
    out.append(
        "What it CAN say, and what #1424 could not: whether the observed "
        "contrast is distinguishable from chance IN EITHER DIRECTION, and — "
        "when the validity gate passes — a bound that holds both ways.")
    out.append("")
    ...
    out.append(f"- Warm-up lead before each dataset's own earliest scored window: "
               f"min {warm.get('min_lead_bars', 0)} bars, required "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if (os.path.abspath(args.report_out)
        ...
        raise SystemExit(
            f"[1426] this study is EXPLORATORY-ONLY (Option 2) and DEFERS the "
            f"live-evidence contract path {CONTRACT_REPORT_BASENAME}; "
            f"hurst_1424_gate_resolution.py owns it. Its own render belongs at "
    ...
    if args.render_only:
        ...
        if is_committed:
            ...
            if not stamp.get("complete"):
                raise SystemExit(
                    f"[1426] {args.json_out} is not stamped as a complete run, "
                    f"so it may not be rendered to the committed report "
                    f"{_DEFAULT_REPORT_OUT}.")
            if not stamp.get("pre_registered_inference"):
                raise SystemExit(
                    f"[1426] {args.json_out} is not stamped as having run the "
                    f"pre-registered inference settings and verification, so it "
                    f"may not be rendered to the committed report "
                    f"{_DEFAULT_REPORT_OUT}.")
            if not args.write_report:
                raise SystemExit(
                    "[1426] writing the committed report needs --write-report, "
    ...
    print(f"[1426] coverage: {coverage['n_kept']}/{coverage['n_cells']} owned cells "
    ...
    if not warmup["sufficient"]:
        print(f"[1426] WARNING: warm-up shortfall on "
        ...
        print(f"[1426] warm-up OK: min lead {warmup['min_lead_bars']} bars before "
              f"each dataset's own earliest scored window "
              f"(need {warmup['required_bars']}).")

    print(f"[1426] computing rolling Hurst for {len(usable_datasets)}x"
    ...
    print(f"[1426] scoring {len(units)} legs "
    ...
    payload = {
        ...
        "pre_registered": {
            ...
            "cohort_option": COHORT_OPTION,
            "cohort_decision_statement": COHORT_DECISION_STATEMENT,
