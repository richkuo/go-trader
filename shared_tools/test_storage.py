...
def test_ohlcv_cache_env_override_is_import_time_default(tmp_path):
    ...
    proc = subprocess.run([sys.executable, '-c', 'import storage; print(storage.DB_PATH)'], cwd=os.path.dirname(__file__), env=env, text=True, capture_output=True, check=False)
...
def test_ohlcv_cache_env_override_refuses_blank_value():
    ...
    proc = subprocess.run([sys.executable, '-c', 'import storage'], cwd=os.path.dirname(__file__), env=env, text=True, capture_output=True, check=False)
...
def test_ohlcv_cache_env_override_fails_loudly_when_unwritable(tmp_path):
    ...
    proc = subprocess.run([sys.executable, '-c', 'import storage; storage.get_connection()'], cwd=os.path.dirname(__file__), env=env, text=True, capture_output=True, check=False)
    assert proc.returncode != 0
...
def _sample_ohlcv_df():
    return pd.DataFrame({'timestamp': [1700000000000, 1700003600000, 1700007200000], 'open': [35000.0, 35200.0, 35600.0], 'high': [35500.0, 35800.0, 36000.0], 'low': [34800.0, 35100.0, 35400.0], 'close': [35200.0, 35600.0, 35900.0], 'volume': [100.0, 120.0, 90.0]})

def _sample_backtest_result():
    return {'strategy_name': 'sma_crossover', 'symbol': 'BTC/USDT', 'timeframe': '1h', 'start_date': '2023-01-01', 'end_date': '2023-12-31', 'initial_capital': 10000.0, 'final_capital': 12500.0, 'total_return_pct': 25.0, 'annual_return_pct': 25.0, 'sharpe_ratio': 1.5, 'sortino_ratio': 2.0, 'max_drawdown_pct': 8.0, 'win_rate': 0.55, 'profit_factor': 1.8, 'total_trades': 42, 'params': {'fast_period': 10, 'slow_period': 50}, 'trades': [{'side': 'buy', 'price': 35000, 'qty': 0.1}]}
...
class TestOhlcvRoundTrip:
    ...
    def test_upsert_on_duplicate_timestamp(self, db_path):
        df = _sample_ohlcv_df()
        store_ohlcv(df, 'binanceus', 'BTC/USDT', '1h', db_path=db_path)
        df2 = pd.DataFrame({'timestamp': [1700000000000], 'open': [35000.0], 'high': [35500.0], 'low': [34800.0], 'close': [99999.0], 'volume': [100.0]})
    ...
    def test_filter_by_start_ts(self, db_path):
        df = _sample_ohlcv_df()
        store_ohlcv(df, 'binanceus', 'BTC/USDT', '1h', db_path=db_path)
        loaded = load_ohlcv('binanceus', 'BTC/USDT', '1h', start_ts=1700003600000, db_path=db_path)
        assert len(loaded) == 2

    def test_filter_by_end_ts(self, db_path):
        df = _sample_ohlcv_df()
        store_ohlcv(df, 'binanceus', 'BTC/USDT', '1h', db_path=db_path)
        loaded = load_ohlcv('binanceus', 'BTC/USDT', '1h', end_ts=1700003600000, db_path=db_path)
        assert len(loaded) == 2

    def test_filter_by_date_range(self, db_path):
        df = _sample_ohlcv_df()
        store_ohlcv(df, 'binanceus', 'BTC/USDT', '1h', db_path=db_path)
        loaded = load_ohlcv('binanceus', 'BTC/USDT', '1h', start_ts=1700003600000, end_ts=1700003600000, db_path=db_path)
        assert len(loaded) == 1
    ...
    def test_ordered_by_timestamp(self, db_path):
        df = pd.DataFrame({'timestamp': [1700007200000, 1700000000000, 1700003600000], 'open': [1.0, 2.0, 3.0], 'high': [1.0, 2.0, 3.0], 'low': [1.0, 2.0, 3.0], 'close': [1.0, 2.0, 3.0], 'volume': [1.0, 2.0, 3.0]})
        store_ohlcv(df, 'binanceus', 'BTC/USDT', '1h', db_path=db_path)
        loaded = load_ohlcv('binanceus', 'BTC/USDT', '1h', db_path=db_path)
...
class TestBacktestRoundTrip:
    ...
    def test_missing_fields_use_defaults(self, db_path):
        result = {'strategy_name': 'test', 'symbol': 'BTC/USDT', 'timeframe': '1d', 'start_date': '2023-01-01', 'end_date': '2023-12-31'}
        store_backtest_result(result, db_path=db_path)
        df = get_backtest_results(db_path=db_path)
        assert len(df) == 1
