...
import pandas as pd
from strategy_composition import compose_signal, evaluate_open_close, finalize_decision, max_close_fraction, validate_close_strategy_names
...
def test_max_close_fraction_clamps_and_is_order_independent():
    fraction, strategy = max_close_fraction([type('E', (), {'strategy': 'a', 'close_fraction': 0.25})(), type('E', (), {'strategy': 'b', 'close_fraction': 1.2})(), type('E', (), {'strategy': 'c', 'close_fraction': -1})()])
    assert fraction == 1.0
    assert strategy == 'b'
...
def test_evaluate_open_close_reuses_legacy_strategy_once():
    ...
    def apply_strategy(name, data, params=None):
        ...
        result['signal'] = [0, -1]
        return result
    evaluation = evaluate_open_close(apply_strategy, get_strategy, df, positional_strategy='legacy', open_strategy=None, close_strategies=None, position_side='long')
...
def test_evaluate_open_close_passes_position_ctx_to_close_only():
    ...
    def apply_strategy(name, data, params=None):
        ...
        return result
    evaluation = evaluate_open_close(apply_strategy, get_strategy, df, positional_strategy='legacy', open_strategy='open', close_strategies=['close'], position_side='long', params={'open_only': 1}, position_ctx={'side': 'long', 'avg_cost': 100, 'current_quantity': 0.5, 'initial_quantity': 1.0, 'entry_atr': 12.5})
    decision = finalize_decision(evaluation, position_side='long')
    assert calls[0] == ('open', {'open_only': 1})
    assert calls[1] == ('close', {'side': 'long', 'avg_cost': 100, 'current_quantity': 0.5, 'initial_quantity': 1.0, 'entry_atr': 12.5})
    assert decision['close_fraction'] == 1.0
    assert decision['signal'] == -1
...
def test_evaluate_open_close_uses_close_registry_before_open_fallback():
    ...
    evaluation = evaluate_open_close(apply_strategy, get_strategy, df, positional_strategy='legacy', open_strategy='open', close_strategies=['tiered_tp_pct'], position_side='long', params={'lookback': 5}, position_ctx={'side': 'long', 'avg_cost': 100, 'current_quantity': 1.0}, close_evaluate=close_evaluate, market_ctx={'mark_price': 106})
    decision = finalize_decision(evaluation, position_side='long')
    assert calls == [('open', 'open', {'lookback': 5}), ('close', 'tiered_tp_pct', {'side': 'long', 'avg_cost': 100, 'current_quantity': 1.0}, {'mark_price': 106}, {})]
...
def test_validate_close_strategy_names_reports_both_registries():
    ...
    validate_close_strategy_names(['tp_at_pct', 'legacy_open_close'], get_open_strategy, get_close_strategy, lambda: ['legacy_open_close'], lambda: ['tiered_tp_pct'])
    import pytest
    with pytest.raises(ValueError, match='Available close strategies.*fallback open strategies'):
        validate_close_strategy_names(['missing'], get_open_strategy, get_close_strategy, lambda: ['legacy_open_close'], lambda: ['tiered_tp_pct'])
...
def test_evaluate_open_close_reruns_same_strategy_when_close_params_differ():
    ...
    def apply_strategy(name, data, params=None):
        ...
        result['close_fraction'] = 1.0 if params and params.get('avg_cost') == 100 else 0.0
        return result
    evaluation = evaluate_open_close(apply_strategy, get_strategy, df, positional_strategy='same', open_strategy=None, close_strategies=None, position_side='long', params={'lookback': 5}, position_ctx={'avg_cost': 100})
...
def test_tp_at_pct_position_aware_close_handles_missing_and_hit():
    ...
    df = pd.DataFrame({'close': [100, 106]})
    legacy = open_mod.apply_strategy('sma_crossover', df, {'side': 'long', 'avg_cost': 100, 'current_quantity': 0.5})
    assert 'signal' in legacy.columns
    hit = mod.evaluate('tp_at_pct', {'side': 'long', 'avg_cost': 100, 'current_quantity': 0.5, 'initial_quantity': 1.0, 'entry_atr': 12.5}, {'mark_price': 106}, {'pct': 0.05})
...
def test_evaluate_open_close_injects_avwap_from_open_result():
    ...
    evaluate_open_close(apply_strategy, get_strategy, df, positional_strategy='legacy', open_strategy='open', close_strategies=['avwap_stop'], position_side='long', position_ctx={'side': 'long', 'avg_cost': 100, 'current_quantity': 1.0}, close_evaluate=close_evaluate, market_ctx=caller_market)
...
def test_evaluate_open_close_skips_avwap_when_nan_or_absent():
    ...
    def apply_nan_avwap(name, data, params=None):
        ...
        return result
    evaluate_open_close(apply_nan_avwap, get_strategy, df, positional_strategy='legacy', open_strategy='open', close_strategies=['avwap_stop'], position_side='long', close_evaluate=close_evaluate, market_ctx={'mark_price': 106})

    def apply_no_avwap(name, data, params=None):
        result = data.copy()
        result['signal'] = [0, 0]
        return result
    evaluate_open_close(apply_no_avwap, get_strategy, df, positional_strategy='legacy', open_strategy='open', close_strategies=['avwap_stop'], position_side='long', close_evaluate=close_evaluate, market_ctx={'mark_price': 106})
...
def _run_avwap_open_close(apply_strategy, close_strategies):
    df = pd.DataFrame({'close': [100, 106]})
    evaluate_open_close(apply_strategy, lambda name: None, df, positional_strategy='legacy', open_strategy='open', close_strategies=close_strategies, position_side='long', close_evaluate=_noop_close_evaluate, market_ctx={'mark_price': 106})
...
def test_validate_close_strategy_names_rejects_backtest_only_open_fallback():
    ...
    def get_close_strategy(name):
        raise ValueError('close missing')
    with pytest.raises(ValueError, match='backtest_only'):
        validate_close_strategy_names(['research_open'], get_open_strategy, get_close_strategy)
