...
def test_cli_list_json_emits_same_shape_as_python_call():
    result = subprocess.run([sys.executable, str(_LOADER_PATH), '--list-json'], capture_output=True, text=True, check=True)
...
def test_cli_without_list_json_flag_errors_with_json_on_stdout():
    result = subprocess.run([sys.executable, str(_LOADER_PATH)], capture_output=True, text=True)
