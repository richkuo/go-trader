...
class StubAdapter:
    ...
    def __init__(self, start_ms, hours):
        self.records = [{'rate': 1e-05 * (i % 5 - 2), 'time': start_ms + i * _HOUR_MS} for i in range(hours)]
        self.calls = 0

    def get_funding_history_range(self, coin, start_ms, end_ms=None):
        self.calls += 1
        return [r for r in self.records if r['time'] >= start_ms and (end_ms is None or r['time'] <= end_ms)]
...
def test_fetch_then_cache_hit():
    ...
    first = load_cached_funding('BTC', '2026-01-01', '2026-01-03', adapter=stub, db_path=db)
    assert len(first) > 0
    assert stub.calls == 1
    again = load_cached_funding('BTC', '2026-01-01', '2026-01-03', adapter=stub, db_path=db)
...
def test_uncovered_range_refetches():
    ...
    assert stub.calls == 1
    wider = load_cached_funding('BTC', '2026-01-01', '2026-01-09', adapter=stub, db_path=db)
...
def test_late_listed_coin_cached_after_first_fetch():
    ...
    first = load_cached_funding('LATECOIN', '2026-01-01', '2026-02-04', adapter=stub, db_path=db)
    assert stub.calls == 1
    assert int(first['timestamp'].iloc[0]) == listed_at
    again = load_cached_funding('LATECOIN', '2026-01-01', '2026-02-04', adapter=stub, db_path=db)
...
def test_partial_fetch_does_not_claim_tail_coverage():
    ...
    load_cached_funding('BTC', '2026-01-01', '2026-01-01 20:00', adapter=stub, db_path=db)
    assert stub.calls == 2
    os.unlink(db)
...
def test_disjoint_fetches_do_not_poison_middle():
    ...
    load_cached_funding('BTC', '2026-01-01', '2026-01-10', adapter=stub, db_path=db)
    assert stub.calls == 2
    middle = load_cached_funding('BTC', '2026-03-01', '2026-03-10', adapter=stub, db_path=db)
...
def test_adjacent_fetches_merge_into_one_covered_interval():
    ...
    assert stub.calls == 2
    spanning = load_cached_funding('BTC', '2026-01-05', '2026-01-15', adapter=stub, db_path=db)
...
def test_gap_spanning_request_backfills_and_heals_coverage():
    ...
    load_cached_funding('BTC', '2026-07-20', '2026-07-30', adapter=stub, db_path=db)
    assert stub.calls == 2
    spanning = load_cached_funding('BTC', '2026-01-05', '2026-07-25', adapter=stub, db_path=db)
    assert stub.calls == 3
    assert not spanning.empty
    again = load_cached_funding('BTC', '2026-01-05', '2026-07-25', adapter=stub, db_path=db)
...
def test_timestamp_end_date_accepted():
    ...
    out = load_cached_funding('BTC', '2026-01-01', naive_end, adapter=stub, db_path=db)
    assert stub.calls == 1 and (not out.empty)
    out2 = load_cached_funding('BTC', '2026-01-01', aware_end, adapter=stub, db_path=db)
...
def test_empty_api_returns_cached_or_empty():
    ...
    out = load_cached_funding('NEWCOIN', '2026-01-01', '2026-01-03', adapter=stub, db_path=db)
    assert out.empty
    os.unlink(db)
...
def test_attach_tz_naive_bars():
    df = _bars(3, tz=None)
    f = _funding_frame([_BASE_MS], [3e-05])
    out = attach_funding_column(df, f)
    assert out['funding_rate'].iloc[0] == 3e-05

def test_accrual_1h_is_per_bar_event():
    df = _bars(4, freq='1h')
    rates = [1e-05, 2e-05, 3e-05, 4e-05]
    ...
    assert acc[0] == 0.0
    assert acc[1] == pytest.approx(2e-05)
    assert acc[2] == pytest.approx(3e-05)
    assert acc[3] == pytest.approx(4e-05)
...
def test_accrual_4h_sums_the_interval():
    ...
    acc = out['funding_accrual'].tolist()
    assert acc[0] == 0.0
    assert acc[1] == pytest.approx(4e-05)
    assert acc[2] == pytest.approx(4e-05)

def test_accrual_total_equals_held_funding():
    df = _bars(5, freq='1h')
    times = [_BASE_MS + i * _HOUR_MS for i in range(5)]
    rates = [1e-05, 2e-05, 3e-05, 4e-05, 5e-05]
    out = attach_funding_accrual_column(df, _funding_frame(times, rates))
    assert out['funding_accrual'].sum() == pytest.approx(2e-05 + 3e-05 + 4e-05 + 5e-05)
...
def test_accrual_event_at_bar_is_right_closed():
    df = _bars(3, freq='1h')
    out = attach_funding_accrual_column(df, _funding_frame([_BASE_MS + _HOUR_MS], [9e-05]))
    acc = out['funding_accrual'].tolist()
    assert acc[1] == pytest.approx(9e-05)
    assert acc[2] == 0.0
