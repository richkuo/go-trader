...
SAMPLE_CANDLES = [[1700000000000, 35000.0, 35500.0, 34800.0, 35200.0, 100.0], [1700003600000, 35200.0, 35800.0, 35100.0, 35600.0, 120.0], [1700007200000, 35600.0, 36000.0, 35400.0, 35900.0, 90.0], [1700010800000, 35900.0, 36200.0, 35700.0, 36100.0, 110.0], [1700014400000, 36100.0, 36300.0, 35900.0, 36000.0, 80.0]]
...
class TestFetchFullHistory:
    ...
    def test_paginates_through_data(self, mock_sleep, mock_store, mock_get_ex):
        page1 = [[1700000000000, 35000, 35500, 34800, 35200, 100], [1700003600000, 35200, 35800, 35100, 35600, 120], [1700007200000, 35600, 36000, 35400, 35900, 90]]
        mock_ex = _make_mock_exchange()
    ...
    def test_deduplicates_candles(self, mock_sleep, mock_store, mock_get_ex):
        page1 = [[1700000000000, 35000, 35500, 34800, 35200, 100], [1700003600000, 35200, 35800, 35100, 35600, 120]]
        page2 = [[1700003600000, 35200, 35800, 35100, 35600, 120], [1700007200000, 35600, 36000, 35400, 35900, 90]]
        mock_ex = _make_mock_exchange()
    ...
    def test_rate_limit_retry(self, mock_sleep, mock_store, mock_get_ex):
        mock_ex = _make_mock_exchange()
        mock_ex.fetch_ohlcv.side_effect = [ccxt.RateLimitExceeded('rate limited'), SAMPLE_CANDLES[:2], []]
        mock_ex.parse8601.return_value = 1700000000000
        mock_ex.milliseconds.return_value = 1700100000000
        mock_get_ex.return_value = mock_ex
        df = fetch_full_history('BTC/USDT', '1h', since='2023-11-14', store=False)
    ...
    def test_network_error_retry(self, mock_sleep, mock_store, mock_get_ex):
        mock_ex = _make_mock_exchange()
        mock_ex.fetch_ohlcv.side_effect = [ccxt.NetworkError('timeout'), SAMPLE_CANDLES[:2], []]
        mock_ex.parse8601.return_value = 1700000000000
        mock_ex.milliseconds.return_value = 1700100000000
        mock_get_ex.return_value = mock_ex
        df = fetch_full_history('BTC/USDT', '1h', since='2023-11-14', store=False)
...
class TestLoadCachedData:
    ...
    def test_returns_cached_when_available(self, mock_fetch, mock_load):
        cached_df = pd.DataFrame({'timestamp': [1700000000000], 'open': [35000], 'high': [35500], 'low': [34800], 'close': [35200], 'volume': [100]})
    ...
    def test_fetches_when_cache_empty(self, mock_fetch, mock_load):
        ...
        mock_fetch.return_value = pd.DataFrame({'timestamp': [1700000000000], 'open': [35000], 'high': [35500], 'low': [34800], 'close': [35200], 'volume': [100]})
        df = load_cached_data('BTC/USDT', '1d')
        mock_fetch.assert_called_once()

    @patch('data_fetcher.load_ohlcv')
    def test_date_range_filtering(self, mock_load):
        mock_load.return_value = pd.DataFrame({'timestamp': [1700000000000], 'open': [35000], 'high': [35500], 'low': [34800], 'close': [35200], 'volume': [100], 'datetime': [pd.Timestamp('2023-11-14')]}).set_index('datetime')
