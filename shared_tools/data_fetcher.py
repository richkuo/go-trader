...
def fetch_ohlcv(symbol: str='BTC/USDT', timeframe: str='1d', since: Optional[str]=None, limit: int=500, exchange_id: str='binanceus', store: bool=True) -> pd.DataFrame:
...
def fetch_full_history(symbol: str='BTC/USDT', timeframe: str='1d', since: str='2020-01-01', exchange_id: str='binanceus', store: bool=True, limit: int=500) -> pd.DataFrame:
    exchange = get_exchange(exchange_id)
    since_ts = exchange.parse8601(since + 'T00:00:00Z')
    now_ts = exchange.milliseconds()
    all_candles = []
    current_since = since_ts
    tf_ms = {'1m': 60000, '5m': 300000, '15m': 900000, '1h': 3600000, '4h': 14400000, '1d': 86400000, '1w': 604800000}
    ...
    while current_since < now_ts:
        ...
        if last_ts == current_since:
            break
        current_since = last_ts + tf_ms.get(timeframe, 86400000)
    ...
    print(f"Fetched {len(df)} candles from {pd.to_datetime(df['timestamp'].iloc[0], unit='ms')} to {pd.to_datetime(df['timestamp'].iloc[-1], unit='ms')}")
    if store:
        store_ohlcv(df, exchange_id, symbol, timeframe)
    df['datetime'] = pd.to_datetime(df['timestamp'], unit='ms')
    df.set_index('datetime', inplace=True)
...
def load_cached_data(symbol: str='BTC/USDT', timeframe: str='1d', exchange_id: str='binanceus', start_date: Optional[str]=None, end_date: Optional[str]=None) -> pd.DataFrame:
    start_ts = None
    end_ts = None
    if start_date:
