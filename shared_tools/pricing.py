...
def bs_price(spot: float, strike: float, dte_days: float, vol: float, risk_free: float=0.05, option_type: str='call') -> float:
...
def bs_greeks(spot: float, strike: float, dte_days: float, vol: float, risk_free: float=0.05, option_type: str='call') -> dict:
    ...
    if option_type == 'call':
        delta = norm_cdf(d1)
        theta_annual = -(spot * pdf_d1 * vol) / (2 * sqrt_T) - risk_free * strike * math.exp(-risk_free * T) * norm_cdf(d2)
    else:
        delta = norm_cdf(d1) - 1
        theta_annual = -(spot * pdf_d1 * vol) / (2 * sqrt_T) + risk_free * strike * math.exp(-risk_free * T) * norm_cdf(-d2)
...
def bs_price_and_greeks(spot: float, strike: float, dte_days: float, vol: float, risk_free: float=0.05, option_type: str='call') -> tuple:
...
if __name__ == '__main__':
    spot, strike, dte, vol = (95000, 95000, 30, 0.8)
    for opt in ('call', 'put'):
