...
try:
    ...
    _ADX_TREND_PATH = Path(__file__).resolve().parents[1] / 'shared_strategies' / 'open' / 'adx_trend.py'
...
try:
    ...
    import importlib.util
    from pathlib import Path
    _INDICATORS_CORE_PATH = Path(__file__).resolve().parents[1] / 'shared_strategies' / 'open' / 'indicators_core.py'
    _HURST_SPEC = importlib.util.spec_from_file_location('_regime_indicators_core', _INDICATORS_CORE_PATH)
...
VALID_LABELS_COMPOSITE = frozenset({'trending_up_clean', 'trending_up_choppy', 'trending_down_clean', 'trending_down_choppy', 'ranging_quiet', 'ranging_volatile', 'ranging_directional', 'ranging_directional_up', 'ranging_directional_down'})
...
VALID_REGIME_GATE_ON_FAILURE = frozenset({REGIME_GATE_ON_FAILURE_OPEN, REGIME_GATE_ON_FAILURE_CLOSED})
...
def normalize_regime_gate_on_failure(value) -> str:
    ...
    if v not in VALID_REGIME_GATE_ON_FAILURE:
        raise ValueError(f"regime_gate_on_failure must be 'open' or 'closed', got {value!r}")
    return v
...
def regime_label_allows_entry(allowed, current: str, on_failure: str='open') -> bool:
    ...
    if current in RANGING_DIRECTIONAL_SUBS and RANGING_DIRECTIONAL_BARE in allowed:
        return True
    return False
_DEFAULT_COMPOSITE_THRESHOLDS = {'return_eff': 0.05, 'range_eff': 0.03, 'adx': 25.0, 'efficiency': 0.5}
_COMPOSITE_THRESHOLD_ALIASES = {'return_pct': 'return_eff', 'range_pct': 'range_eff'}
...
def normalize_composite_thresholds(thresholds: dict[str, Any] | None, *, with_defaults: bool=True) -> dict[str, float]:
...
def _composite_efficiency_metrics(window: pd.DataFrame, atr_val: float, period: int) -> dict:
    ...
    return {'return_eff': net / denom if denom > 0 else 0.0, 'range_eff': (hi - lo) / denom if denom > 0 else 0.0, 'efficiency': abs(net) / path if path > 0 else 0.0, 'close_end': close_end}

def map_composite_label(return_eff: float, adx_val: float, range_eff: float, efficiency: float, thresholds: dict[str, float]) -> str:
...
def latest_regime_composite(df: pd.DataFrame, period: int, thresholds: dict[str, float] | None=None) -> dict:
    ...
    if atr_val <= 0:
        return {'regime': 'ranging_quiet', 'score': 0.0, 'classifier': CLASSIFIER_COMPOSITE, 'metrics': dict(_DEFAULT_METRICS)}
    ...
    metrics = {'adx': adx_val, 'return_eff': round(eff['return_eff'], 4), 'range_eff': round(eff['range_eff'], 4), 'efficiency': round(eff['efficiency'], 4), 'atr_pct': round(atr_val / close_end * 100.0, 4) if close_end else 0.0}
    hurst_val = hurst_exponent(df['close'])
    if pd.notna(hurst_val):
        metrics['hurst'] = round(hurst_val, 4)
    return {'regime': label, 'score': score, 'classifier': CLASSIFIER_COMPOSITE, 'metrics': metrics}

def classify_window(df: pd.DataFrame, spec: dict[str, Any], *, default_adx_threshold: float=20.0) -> dict:
...
def compute_regime(df: pd.DataFrame, period: int=14, adx_threshold: float=20.0) -> pd.DataFrame:
    result = df.copy()
    n = len(result)
    ...
    if n == 0 or n <= period:
        return result
    components = _compute_adx_components(result['high'].values, result['low'].values, result['close'].values, period)
...
def compute_regime_composite(df: pd.DataFrame, period: int, thresholds: dict[str, float] | None=None) -> pd.DataFrame:
    th = normalize_composite_thresholds(thresholds)
    result = df.copy()
    n = len(result)
...
def composite_feature_matrix(df: pd.DataFrame, period: int, thresholds: dict[str, float] | None=None) -> pd.DataFrame:
    th = normalize_composite_thresholds(thresholds)
...
def latest_regime(df: pd.DataFrame, period: int=14, adx_threshold: float=20.0) -> dict:
    if len(df) == 0:
    ...
    return {'regime': str(last['regime']), 'score': float(last['regime_score']), 'metrics': {'adx': float(last['adx']), 'plus_di': float(last['plus_di']), 'minus_di': float(last['minus_di']), 'atr_pct': round(atr_pct, 4)}}

def compute_multi_regime(df: pd.DataFrame, windows: dict[str, Any], adx_threshold: float=20.0, *, default_adx_threshold: float | None=None) -> dict[str, dict]:
    if not windows:
    ...
    for name in sorted(windows.keys()):
        ...
        if trimmed.lower() == 'regime':
            raise ValueError("window name 'regime' is reserved (conflicts with legacy regime snapshot JSON)")
        value = windows[name]
...
def regime_payload_for_config(df: pd.DataFrame, *, period: int=14, adx_threshold: float=20.0, windows: dict[str, dict[str, Any]] | None=None) -> dict | str:
    if windows:
        return compute_multi_regime(df, windows, default_adx_threshold=adx_threshold)
    return classify_window(df, {'classifier': CLASSIFIER_ADX, 'period': period, 'adx_threshold': adx_threshold}, default_adx_threshold=adx_threshold)
...
def required_ohlcv_limit(period: int=14, windows: dict[str, dict[str, Any]] | None=None, *, base_limit: int=200, margin: int=10) -> int:
    max_period = period
    if windows:
...
def regime_from_injected_payload(raw: str | None, *, atr_window: str='') -> tuple[dict | str, str, dict]:
    ...
    if not windows:
        return ('', '', disabled)
    primary_key = REGIME_PRIMARY_WINDOW_KEY if REGIME_PRIMARY_WINDOW_KEY in windows else sorted(windows.keys())[0]
...
def prepare_check_regime(df: pd.DataFrame, *, regime_enabled: bool, period: int=14, adx_threshold: float=20.0, windows: dict[str, dict[str, Any]] | None=None, windows_spec: dict[str, dict[str, Any]] | None=None, atr_window: str='', injected_payload_json: str | None=None) -> tuple[dict | str, str, dict]:
    disabled = {'regime': '', 'score': 0.0, 'metrics': dict(_DEFAULT_METRICS)}
    ...
    if spec_map:
        ...
        primary_key = REGIME_PRIMARY_WINDOW_KEY if REGIME_PRIMARY_WINDOW_KEY in spec_map else sorted(spec_map.keys())[0]
    ...
    legacy = classify_window(df, {'classifier': CLASSIFIER_ADX, 'period': period, 'adx_threshold': adx_threshold}, default_adx_threshold=adx_threshold)
...
def parse_regime_windows_spec_json(raw: str | None) -> dict[str, dict[str, Any]] | None:
    ...
    for name, value in parsed.items():
        ...
        if key.lower() == 'regime':
            raise ValueError("regime window name 'regime' is reserved (conflicts with legacy regime snapshot JSON)")
        if isinstance(value, int) and (not isinstance(value, bool)):
...
def ensure_regime_columns(df: pd.DataFrame, period: int=14, adx_threshold: float=20.0, *, classifier: str=CLASSIFIER_ADX, thresholds: dict[str, float] | None=None, windows_spec: dict[str, dict[str, Any]] | None=None, gate_window: str='') -> pd.DataFrame:
