...
_spec = importlib.util.spec_from_file_location('regime', pathlib.Path(__file__).parent / 'regime.py')
...
_CHECK_REGIME_PATH = pathlib.Path(__file__).parent.parent / 'shared_scripts' / 'check_regime.py'
...
def _make_uptrend(n: int=120, noise: float=0.5) -> pd.DataFrame:
    ...
    return pd.DataFrame({'open': close - noise * 0.3, 'high': close + noise, 'low': close - noise, 'close': close, 'volume': np.ones(n) * 1000.0})
_SPEC_MULTI = {'short': {'classifier': 'adx', 'period': 10, 'adx_threshold': 20.0}, 'medium': {'classifier': 'adx', 'period': 14, 'adx_threshold': 20.0}, 'macro': {'classifier': 'composite', 'period': 30, 'thresholds': {'return_eff': 0.05, 'range_eff': 0.03, 'adx': 25.0, 'efficiency': 0.5}}}

def test_injected_payload_matches_inline_triple():
    df = _make_uptrend()
    inline = prepare_check_regime(df, regime_enabled=True, windows_spec=_SPEC_MULTI, atr_window='macro')
...
def test_prepare_check_regime_skips_compute_when_injected():
    ...
    stdout_regime, live, snap = prepare_check_regime(None, regime_enabled=True, windows_spec={'default': {'classifier': 'adx', 'period': 14}}, injected_payload_json=json.dumps(payload))
    assert stdout_regime == payload
    assert live == 'trending_up'
    assert snap['regime'] == 'trending_up'
    stdout_regime, live, snap = prepare_check_regime(None, regime_enabled=True, windows_spec=_SPEC_MULTI, injected_payload_json='')
    assert (stdout_regime, live, snap['regime']) == ('', '', '')

def test_prepare_check_regime_disabled_ignores_injection():
    out = prepare_check_regime(None, regime_enabled=False, injected_payload_json='{"default":{"regime":"x"}}')
...
def test_bundle_adx3_view_is_full_period_classifier():
    df = _make_uptrend()
    spec = {'macro': {'classifier': 'composite', 'period': 30, 'thresholds': {'return_eff': 0.05, 'range_eff': 0.03, 'adx': 25.0, 'efficiency': 0.5}}}
...
def test_bundle_roundtrip_through_injection():
    ...
    injected = prepare_check_regime(df, regime_enabled=True, windows_spec=_SPEC_MULTI, atr_window='short', injected_payload_json=raw)
    inline = prepare_check_regime(df, regime_enabled=True, windows_spec=_SPEC_MULTI, atr_window='short')
    assert injected == inline
