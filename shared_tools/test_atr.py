...
import pandas as pd
import numpy as np
spec = importlib.util.spec_from_file_location('atr', pathlib.Path(__file__).parent / 'atr.py')
...
def test_standard_atr_hand_computed():
    n = 10
    df = pd.DataFrame({'open': [100.0] * n, 'high': [102.0] * n, 'low': [98.0] * n, 'close': [100.0] * n, 'volume': [1.0] * n})
    result = standard_atr(df, period=3)
    assert math.isclose(result.iloc[3], 4.0, rel_tol=1e-09)
    for i in range(3, n):
        assert math.isclose(result.iloc[i], 4.0, rel_tol=1e-09)
...
def test_latest_atr_strict_positive():
    n = 30
    df = pd.DataFrame({'open': [100.0] * n, 'high': [100.0] * n, 'low': [100.0] * n, 'close': [100.0] * n, 'volume': [1.0] * n})
    assert latest_atr(df) == 0.0
...
def test_regime_classifier_pinned_to_simple():
    spec_r = importlib.util.spec_from_file_location('_t_regime_1277', pathlib.Path(__file__).parent / 'regime.py')
