...
def list_strategies_detailed() -> list[dict]:
    registry = _load_registry().STRATEGIES
    return [{'name': name, 'description': registry[name]['description'], 'default_params': registry[name]['default_params'], 'platforms': list(registry[name]['platforms'])} for name in sorted(registry.keys())]
