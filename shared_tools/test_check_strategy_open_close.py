...
def _load_check_strategy():
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'shared_scripts', 'check_strategy.py')
...
def test_check_strategy_open_close_uses_close_registry_loader(monkeypatch, capsys):
    ...
    df = pd.DataFrame({'open': closes, 'high': [price + 1 for price in closes], 'low': [price - 1 for price in closes], 'close': closes, 'volume': [100.0] * len(closes)}, index=idx)
    ...
    monkeypatch.setattr(sys, 'argv', ['check_strategy.py', 'sma_crossover', 'BTC/USDT', '1h', '--open-strategy', 'sma_crossover', '--close-strategies', 'tp_at_pct', '--position-side', 'long', '--position-avg-cost', '100', '--position-qty', '1'])
    try:
        mod.main()
