...
import numpy as np
import pandas as pd
from storage import load_funding_coverage, load_funding_rates, store_funding_coverage, store_funding_rates
_HOUR_MS = 3600000
...
def load_cached_funding(coin: str, start_date, end_date=None, exchange: str='hyperliquid', adapter=None, db_path: Optional[str]=None) -> pd.DataFrame:
...
def attach_funding_column(df: pd.DataFrame, funding: pd.DataFrame) -> pd.DataFrame:
    ...
    right = pd.DataFrame({'ts': pd.to_datetime(funding['timestamp'], unit='ms', utc=True).astype('datetime64[ns, UTC]'), 'funding_rate': funding['rate'].astype(float).values}).sort_values('ts')
