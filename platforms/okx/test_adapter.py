...
class TestProperties:
    ...
    def test_live_mode(self):
        mock_ex = MagicMock()
        with patch.dict(os.environ, {'OKX_API_KEY': 'key', 'OKX_API_SECRET': 'secret', 'OKX_PASSPHRASE': 'pass'}):
            orig = _mod.ccxt.okx
            _mod.ccxt.okx = MagicMock(return_value=mock_ex)
            try:
                a = OKXExchangeAdapter()
...
class TestMarketData:
    ...
    def test_get_spot_price_tries_multiple_suffixes(self, adapter):
        a, mock_ex = adapter
        mock_ex.fetch_ticker.side_effect = [Exception('not found'), {'last': 67000.0}]
        price = a.get_spot_price('BTC')
    ...
    def test_get_ohlcv_closes(self, adapter):
        a, mock_ex = adapter
        mock_ex.fetch_ohlcv.return_value = [[1700000000000, 100, 110, 90, 105, 50], [1700003600000, 105, 115, 95, 110, 60]]
    ...
    def test_get_funding_history(self, adapter):
        a, mock_ex = adapter
        mock_ex.fetch_funding_rate_history.return_value = [{'fundingRate': 0.0001, 'timestamp': 1700000000000}, {'fundingRate': 0.0002, 'timestamp': 1700003600000}]
...
class TestOrderExecution:
    ...
    def test_market_open_spot(self, adapter):
        ...
        mock_ex.create_market_order.assert_called_once_with('BTC/USDT', 'buy', 0.5, params={'tdMode': 'cash'})
    ...
    def test_market_open_swap(self, adapter):
        ...
        mock_ex.create_market_order.assert_called_once_with('BTC/USDT:USDT', 'sell', 1.0, params={'tdMode': 'cross'})

    def test_market_close_with_position(self, adapter):
        a, mock_ex = adapter
        a._is_live = True
        mock_ex.fetch_positions.return_value = [{'contracts': '1.5', 'side': 'long'}]
        mock_ex.create_market_order.return_value = {'id': '789'}
        result = a.market_close('BTC')
        assert result == {'id': '789'}
        mock_ex.create_market_order.assert_called_once_with('BTC/USDT:USDT', 'sell', 1.5, params={'tdMode': 'cross', 'reduceOnly': True})
    ...
    def test_market_close_hedge_mode_closes_both(self, adapter):
        a, mock_ex = adapter
        a._is_live = True
        mock_ex.fetch_positions.return_value = [{'contracts': '1.5', 'side': 'long'}, {'contracts': '0.8', 'side': 'short'}]
...
class TestOptionsProtocol:
    ...
    def test_get_real_expiry_with_markets(self, adapter):
        ...
        mock_ex.markets = {'BTC-30D-100000-C': {'type': 'option', 'base': 'BTC', 'active': True, 'expiry': int(exp1.timestamp() * 1000)}, 'BTC-60D-100000-C': {'type': 'option', 'base': 'BTC', 'active': True, 'expiry': int(exp2.timestamp() * 1000)}}
    ...
    def test_get_real_strike_with_markets(self, adapter):
        ...
        mock_ex.markets = {'BTC-30D-65000-C': {'type': 'option', 'base': 'BTC', 'optionType': 'call', 'active': True, 'strike': 65000.0, 'expiry': exp_start + 3600000}, 'BTC-30D-70000-C': {'type': 'option', 'base': 'BTC', 'optionType': 'call', 'active': True, 'strike': 70000.0, 'expiry': exp_start + 3600000}}
        mock_ex.load_markets.return_value = mock_ex.markets
    ...
    def test_get_premium_and_greeks_fallback(self, adapter):
        ...
        mock_ex.markets = {}
        mock_ex.load_markets.return_value = {}
        pct, usd, greeks = a.get_premium_and_greeks('BTC', 'call', 70000, '2026-05-01', 30, 67000, 0.6)
    ...
    def test_get_premium_and_greeks_live_quote(self, adapter):
        ...
        exp_str = exp.strftime('%Y-%m-%d')
        exp_start = int(datetime.strptime(exp_str, '%Y-%m-%d').replace(tzinfo=timezone.utc).timestamp() * 1000)
        mock_ex.markets = {'BTC-30D-70000-C': {'type': 'option', 'base': 'BTC', 'optionType': 'call', 'active': True, 'strike': 70000, 'expiry': exp_start + 3600000}}
        mock_ex.load_markets.return_value = mock_ex.markets
        mock_ex.fetch_ticker.return_value = {'last': 0.05, 'close': 0.05, 'info': {'delta': '0.45', 'gamma': '0.001', 'theta': '-10', 'vega': '50'}}
        pct, usd, greeks = a.get_premium_and_greeks('BTC', 'call', 70000, exp_str, 30, 67000, 0.6)
        assert pct == 0.05
        assert usd == 0.05 * 67000
        assert greeks['delta'] == 0.45

def _ledger_entry(bill_id, ts, balchg='1', ccy='USDT', btype='2', sub='', pnl='0', fee='0', inst='BTC-USDT-SWAP', trade='t1'):
    return {'id': bill_id, 'timestamp': ts, 'info': {'billId': bill_id, 'ts': str(ts), 'ccy': ccy, 'type': btype, 'subType': sub, 'balChg': str(balchg), 'pnl': str(pnl), 'fee': str(fee), 'instId': inst, 'tradeId': trade}}
...
class TestNormalizeOKXBill:
    ...
    def test_full_bill(self):
        ...
        assert out == {'bill_id': 'b1', 'ts_ms': 1700000000000, 'ccy': 'USDT', 'type': '2', 'sub_type': '', 'bal_chg': 19.7, 'pnl': 20.0, 'fee': 0.3, 'inst_id': 'BTC-USDT-SWAP', 'trade_id': 't1'}
...
class TestGetAccountEquityAndUPnL:
    ...
    def test_coherent_eq_and_upnl(self, adapter):
        a, mock_ex = adapter
        a._is_live = True
        mock_ex.fetch_balance.return_value = {'total': {'USDT': 1000.0}, 'info': {'data': [{'details': [{'ccy': 'USDT', 'cashBal': '900.0'}]}]}}
...
class TestGetAccountBills:
    ...
    def test_single_page_ascending(self, adapter):
        a, mock_ex = adapter
        a._is_live = True
        mock_ex.fetch_ledger.return_value = [_ledger_entry('b2', 200, '19.7'), _ledger_entry('b1', 100, '-0.5')]
    ...
    def test_same_ms_straddle_across_page_boundary_is_captured(self, adapter):
        ...
        page2 = [_ledger_entry('p-99', 199, '1'), _ledger_entry('p-100', 199, '1'), _ledger_entry('p-105', 205, '1')]
        mock_ex.fetch_ledger.side_effect = [page1, page2]
        bills, capped = a.get_account_bills(since_ms=0, page_limit=100)
