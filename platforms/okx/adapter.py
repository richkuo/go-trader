...
def _normalize_okx_bill(entry: dict) -> dict:
    ...
    return {'bill_id': str(info.get('billId') or entry.get('id') or ''), 'ts_ms': int(_bill_float(ts)), 'ccy': str(info.get('ccy') or ''), 'type': str(info.get('type') or ''), 'sub_type': str(info.get('subType') or ''), 'bal_chg': _bill_float(info.get('balChg')), 'pnl': _bill_float(info.get('pnl')), 'fee': _bill_float(info.get('fee')), 'inst_id': str(info.get('instId') or ''), 'trade_id': str(info.get('tradeId') or '')}
...
class OKXExchangeAdapter:
    ...
    def get_funding_history(self, symbol: str, days: int=7) -> list:
        try:
            ...
            return [{'rate': float(r.get('fundingRate', 0) or 0), 'time': int(r.get('timestamp', 0))} for r in records]
        except Exception:
            return []

    def fetch_open_positions(self) -> list:
        if not self._is_live:
            raise RuntimeError('fetch_open_positions requires live mode (set OKX_API_KEY, OKX_API_SECRET, OKX_PASSPHRASE)')
        return self._exchange.fetch_positions() or []

    def market_open(self, symbol: str, is_buy: bool, size: float, inst_type: str='spot') -> dict:
        if not self._is_live:
            raise RuntimeError('market_open requires live mode (set OKX_API_KEY, OKX_API_SECRET, OKX_PASSPHRASE)')
    ...
    def market_close(self, symbol: str, sz: float | None=None) -> dict:
        if not self._is_live:
            raise RuntimeError('market_close requires live mode (set OKX_API_KEY, OKX_API_SECRET, OKX_PASSPHRASE)')
        pair = f'{symbol}/USDT:USDT'
        ...
        for pos in positions:
            ...
            if contracts > 0:
                ...
                if sz is not None:
                    ...
                    if close_sz <= 0:
                        continue
                results.append(self._exchange.create_market_order(pair, close_side, close_sz, params={'tdMode': 'cross', 'reduceOnly': True}))
        return results[0] if results else {}

    def get_account_balance(self) -> float:
        if not self._is_live:
            raise RuntimeError('get_account_balance requires live mode (set OKX_API_KEY, OKX_API_SECRET, OKX_PASSPHRASE)')
    ...
    def get_account_equity_and_upnl(self) -> Tuple[float, float]:
        if not self._is_live:
            raise RuntimeError('get_account_equity_and_upnl requires live mode (set OKX_API_KEY, OKX_API_SECRET, OKX_PASSPHRASE)')
        bal = self._exchange.fetch_balance()
        total = bal.get('total') or {}
        try:
    ...
    def get_account_bills(self, since_ms: int=0, page_limit: int=100, max_bills: int=10000) -> Tuple[list, bool]:
        if not self._is_live:
            raise RuntimeError('get_account_bills requires live mode (set OKX_API_KEY, OKX_API_SECRET, OKX_PASSPHRASE)')
        collected = {}
    ...
    def get_real_expiry(self, underlying: str, target_dte: int) -> Tuple[str, int]:
        ...
        for market in self._exchange.markets.values():
            if market.get('type') == 'option' and market.get('base', '').upper() == underlying.upper() and market.get('active', True):
    ...
    def get_real_strike(self, underlying: str, expiry: str, option_type: str, target_strike: float) -> float:
        ...
        strikes = []
        for market in self._exchange.markets.values():
            if market.get('type') == 'option' and market.get('base', '').upper() == underlying.upper() and (market.get('optionType') == option_type) and market.get('active', True):
    ...
    def get_premium_and_greeks(self, underlying: str, option_type: str, strike: float, expiry: str, dte: float, spot: float, vol: float) -> Tuple[float, float, dict]:
        try:
            ...
            for sym, market in self._exchange.markets.items():
                if market.get('type') == 'option' and market.get('base', '').upper() == underlying.upper() and (market.get('optionType') == option_type) and (float(market.get('strike') or 0) == strike) and market.get('active', True):
                    mkt_exp = market.get('expiry')
                    if mkt_exp and exp_start <= int(mkt_exp) < exp_end:
                        ...
                        if mark and mark > 0:
                            ...
                            greeks = {'delta': ticker.get('info', {}).get('delta', 0), 'gamma': ticker.get('info', {}).get('gamma', 0), 'theta': ticker.get('info', {}).get('theta', 0), 'vega': ticker.get('info', {}).get('vega', 0)}
