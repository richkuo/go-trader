...
def _load_hl_adapter(mock_info_cls=None, mock_exchange_cls=None, mock_api_cls=None):
    ...
    saved = {}
    mod_names = ('hyperliquid', 'hyperliquid.info', 'hyperliquid.exchange', 'hyperliquid.api', 'hyperliquid.utils', 'hyperliquid.utils.error')
...
class TestMarketData:
    ...
    def test_get_ohlcv(self, monkeypatch):
        ...
        adapter, mock_info = self._make_adapter()
        mock_info.candles_snapshot.return_value = [{'T': 1700000000000, 'o': '100', 'h': '110', 'l': '90', 'c': '105', 'v': '50'}, {'T': 1700003600000, 'o': '105', 'h': '115', 'l': '95', 'c': '110', 'v': '60'}]
    ...
    def test_get_ohlcv_uses_t_key_fallback(self, monkeypatch):
        monkeypatch.setenv('GO_TRADER_HL_OHLCV_CACHE', '0')
        adapter, mock_info = self._make_adapter()
        mock_info.candles_snapshot.return_value = [{'t': 1700000000000, 'o': '100', 'h': '110', 'l': '90', 'c': '105', 'v': '50'}]
    ...
    def test_get_ohlcv_widens_window_for_gap_margin(self, monkeypatch):
        ...
        assert interval == '1h'
        assert end_ms - start_ms == 3600000 * (200 + 50)

    def test_get_ohlcv_trims_to_limit_when_api_returns_extra(self, monkeypatch):
        monkeypatch.setenv('GO_TRADER_HL_OHLCV_CACHE', '0')
        adapter, mock_info = self._make_adapter()
        mock_info.candles_snapshot.return_value = [{'T': 1700000000000 + i * 3600000, 'o': '100', 'h': '110', 'l': '90', 'c': str(100 + i), 'v': '50'} for i in range(5)]
    ...
    def test_get_ohlcv_warns_on_shortfall_below_limit(self, monkeypatch, capsys):
        monkeypatch.setenv('GO_TRADER_HL_OHLCV_CACHE', '0')
        adapter, mock_info = self._make_adapter()
        mock_info.candles_snapshot.return_value = [{'T': 1700000000000 + i * 3600000, 'o': '100', 'h': '110', 'l': '90', 'c': str(100 + i), 'v': '50'} for i in range(3)]
    ...
    def _gappy_candles(n):
        return [{'T': 1700000000000 + i * 3600000, 'o': '100', 'h': '110', 'l': '90', 'c': str(100 + i), 'v': '50'} for i in range(n)]
    ...
    def test_get_ohlcv_extends_window_until_limit_reached(self, monkeypatch):
        ...
        def side_effect(symbol, interval, start_ms, end_ms):
            span = round((end_ms - start_ms) / 3600000)
        ...
        spans = [round((c.args[3] - c.args[2]) / 3600000) for c in mock_info.candles_snapshot.call_args_list]
    ...
    def test_get_ohlcv_does_not_stop_on_single_interior_gap_widen(self, monkeypatch):
        ...
        mock_info.candles_snapshot.side_effect = lambda *a, **kw: self._gappy_candles(next(returns))
        result = adapter.get_ohlcv('BTC', '1h', 200)
        assert mock_info.candles_snapshot.call_count == 3
        assert len(result) == 200
    ...
    def test_get_ohlcv_extend_passes_are_bounded(self, monkeypatch):
        ...
        mock_info.candles_snapshot.side_effect = side_effect
        adapter.get_ohlcv('BTC', '1h', 10000)
    ...
    def test_get_funding_rate_found(self):
        adapter, mock_info = self._make_adapter()
        mock_info.meta_and_asset_ctxs.return_value = [{'universe': [{'name': 'BTC'}, {'name': 'ETH'}]}, [{'funding': '0.0001'}, {'funding': '0.0002'}]]
        rate = adapter.get_funding_rate('BTC')
        assert rate == 0.0001

    def test_get_funding_rate_not_found(self):
        adapter, mock_info = self._make_adapter()
        mock_info.meta_and_asset_ctxs.return_value = [{'universe': [{'name': 'ETH'}]}, [{'funding': '0.0002'}]]
    ...
    def test_get_funding_history(self):
        adapter, mock_info = self._make_adapter()
        mock_info.funding_history.return_value = [{'fundingRate': '0.0001', 'time': 1700000000000}, {'fundingRate': '0.0002', 'time': 1700003600000}]
...
class TestAccountData:
    ...
    def test_get_open_positions(self):
        ...
        mock_info.user_state.return_value = {'assetPositions': [{'position': {'coin': 'BTC', 'szi': '0.5', 'entryPx': '67000', 'unrealizedPnl': '100.50'}}, {'position': {'coin': 'ETH', 'szi': '0', 'entryPx': '3500', 'unrealizedPnl': '0'}}]}
...
class TestStopLossPlacement:
    ...
    def test_place_stop_loss_high_priced_asset_uses_per_asset_px_decimals(self):
        ...
        assert order_type['trigger']['triggerPx'] == round(order_type['trigger']['triggerPx'], 0) or order_type['trigger']['triggerPx'] == round(order_type['trigger']['triggerPx'], -1)
    ...
    def test_open_order_oids_filters_by_symbol(self):
        ...
        adapter._info.open_orders.return_value = [{'coin': 'ETH', 'oid': 111}, {'coin': 'BTC', 'oid': 222}, {'coin': 'ETH', 'oid': '333'}]
...
class TestLookupFillFeeByOID:
    ...
    def test_aggregates_fee_and_pnl_across_partial_fills(self):
        adapter, mock_info = self._make_adapter()
        mock_info.user_fills_by_time.return_value = [{'oid': 100, 'fee': '0.50', 'closedPnl': '1.25'}, {'oid': 100, 'fee': '0.30', 'closedPnl': '0.75'}, {'oid': 999, 'fee': '5.00', 'closedPnl': '10.00'}]
        result = adapter.lookup_fill_fee_by_oid(100, since_ms=1000)
        assert result['fee'] == pytest.approx(0.8)
        assert result['closed_pnl'] == pytest.approx(2.0)
        assert result['count'] == 2

    def test_handles_string_oid_in_response(self):
        adapter, mock_info = self._make_adapter()
        mock_info.user_fills_by_time.return_value = [{'oid': '100', 'fee': '0.42', 'closedPnl': '0'}]
        result = adapter.lookup_fill_fee_by_oid(100, since_ms=1000)
    ...
    def test_retries_until_indexer_catches_up(self, monkeypatch):
        adapter, mock_info = self._make_adapter()
        mock_info.user_fills_by_time.side_effect = [[], [{'oid': 999, 'fee': '1', 'closedPnl': '0'}], [{'oid': 100, 'fee': '0.65', 'closedPnl': '0'}]]
        sleeps = []
    ...
    def test_swallows_sdk_exceptions_and_retries(self, monkeypatch):
        adapter, mock_info = self._make_adapter()
        mock_info.user_fills_by_time.side_effect = [Exception('network blip'), [{'oid': 100, 'fee': '0.10', 'closedPnl': '0'}]]
        monkeypatch.setattr('time.sleep', lambda s: None)
        result = adapter.lookup_fill_fee_by_oid(100, since_ms=1000, max_retries=4, retry_delay_s=0.0)
        assert result['fee'] == pytest.approx(0.1)
...
class TestFillsSummaryByOID:
    ...
    def test_sums_size_and_size_weighted_vwap(self):
        adapter, mock_info = self._make_adapter()
        mock_info.user_fills_by_time.return_value = [{'oid': 100, 'sz': '0.4', 'px': '2000', 'fee': '0.20'}, {'oid': 100, 'sz': '0.6', 'px': '2010', 'fee': '0.30'}, {'oid': 999, 'sz': '5', 'px': '1', 'fee': '9'}]
        out = adapter.fills_summary_by_oid(100, since_ms=1000)
        assert out['filled_size'] == pytest.approx(1.0)
        assert out['fee'] == pytest.approx(0.5)
        assert out['count'] == 2
...
class TestFundingHistoryRange:
    ...
    def _paged_info(self, total_hours, page_size=500, base_ms=1700000000000):
        all_records = [{'fundingRate': str((i % 7 - 3) * 1e-05), 'time': base_ms + i * self._HOUR} for i in range(total_hours)]
...
class TestLazyExchangeInit:
    ...
    def _sample_meta(self):
        return ({'universe': [{'index': 0, 'name': 'USDC/USDC', 'tokens': [0, 0]}], 'tokens': [{'name': 'USDC', 'szDecimals': 0}]}, {'universe': [{'name': 'BTC', 'szDecimals': 5}]})
    ...
    def test_exchange_init_429_does_not_fail_adapter_init(self, monkeypatch):
        ...
        mod, adapter, mock_info, _, _ = self._live_adapter_mod(monkeypatch, mock_exchange_cls=MagicMock(side_effect=err))
        adapter._info = mock_info
        assert adapter._exchange is None
