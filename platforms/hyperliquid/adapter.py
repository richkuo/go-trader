...
def _ohlcv_cache_path(symbol: str, interval: str, limit: int, cache_dir: str=None) -> str:
...
def _normalize_spot_meta(spot_meta):
    ...
    for entry in universe:
        ...
        if isinstance(pair, list) and len(pair) == 2 and all((isinstance(x, int) and x in by_index for x in pair)):
            clean_universe.append(entry)
        else:
            dropped.append(entry.get('name') if isinstance(entry, dict) else None)
    aligned = max_index + 1 == len(tokens) and all((isinstance(t, dict) and t.get('index') == i for i, t in enumerate(tokens)))
    ...
    if dropped:
        print(f'[WARN] hl spotMeta: dropped {len(dropped)} unresolvable spot pair(s): {dropped[:5]}', file=sys.stderr)
...
class HyperliquidExchangeAdapter:
    ...
    def __init__(self):
        if not _SDK_AVAILABLE:
            raise ImportError('hyperliquid-python-sdk not installed. Run: uv sync')
    ...
    def _build_info(self, base_url: str, allow_cache: bool):
        ...
        if cached is not None:
            ...
            return _HLInfo(base_url=base_url, skip_ws=True, meta=meta, spot_meta=_normalize_spot_meta(spot_meta))
        try:
    ...
    def _ensure_exchange(self):
        ...
        with self._exchange_lock:
            if self._exchange is not None:
                return self._exchange
            if self._exchange_init_error is not None and now < self._exchange_backoff_until:
                raise RuntimeError(f'Failed to initialize Hyperliquid Exchange client: {self._exchange_init_error}')
            try:
                kwargs = {'base_url': self._base_url, 'account_address': self._account_address}
                ...
                raise RuntimeError(f'Failed to initialize Hyperliquid Exchange client: {e}') from e

    def _require_exchange(self, caller: str='live trading'):
        if self._wallet is None:
            raise RuntimeError(f'{caller} requires live mode (set HYPERLIQUID_SECRET_KEY)')
    ...
    def get_ohlcv(self, symbol: str, interval: str='1h', limit: int=200) -> list:
        interval_ms_map = {'1m': 60000, '3m': 180000, '5m': 300000, '15m': 900000, '30m': 1800000, '1h': 3600000, '2h': 7200000, '4h': 14400000, '8h': 28800000, '12h': 43200000, '1d': 86400000, '3d': 259200000, '1w': 604800000}
        ...
        for _ in range(OHLCV_MAX_EXTEND_PASSES):
            ...
            result = []
            for c in candles:
                result.append([int(c.get('T', c.get('t', 0))), float(c['o']), float(c['h']), float(c['l']), float(c['c']), float(c['v'])])
            if not result or len(result) >= limit or len(result) >= OHLCV_MAX_CANDLES:
                break
        ...
        if len(result) > limit:
            ...
            print(f"[WARN] hl ohlcv shortfall for {symbol} {interval}: got {len(result)} of {limit} requested after extending the window to the symbol's available history", file=sys.stderr)
    ...
    def get_funding_history(self, symbol: str, days: int=7) -> list:
        try:
            ...
            return [{'rate': float(r['fundingRate']), 'time': int(r['time'])} for r in records]
        except Exception:
            return []

    def get_funding_history_range(self, symbol: str, start_ms: int, end_ms: int=None) -> list:
        if end_ms is None:
            end_ms = int(time.time() * 1000)
        out = []
        seen = set()
    ...
    def get_open_positions(self) -> list:
        ...
        try:
            ...
            for asset_pos in user_state.get('assetPositions', []):
                ...
                if szi == 0:
                    continue
                positions.append({'coin': pos.get('coin', ''), 'size': szi, 'entry_price': float(pos.get('entryPx', 0) or 0), 'unrealized_pnl': float(pos.get('unrealizedPnl', 0) or 0)})
            return positions
        except Exception:
            return []
    ...
    def limit_open(self, symbol: str, is_buy: bool, size: float, limit_px: float, tif: str='Alo') -> dict:
        ...
        return exchange.order(symbol, is_buy, size, limit_px, order_type, reduce_only=False)
    ...
    def lookup_fill_fee_by_oid(self, oid: int, since_ms: int, max_retries: int=4, retry_delay_s: float=0.5) -> dict:
        if not self._account_address:
            return {}
        attempt = 0
        while attempt < max_retries:
            try:
                ...
                if getattr(exc, 'status_code', None) == 429:
                    print(f'[WARN] userFills lookup got HTTP 429 for oid={oid}; not retrying', file=sys.stderr)
                    return {}
                fills = None
            except Exception:
                fills = None
            if isinstance(fills, list):
                ...
                if matched:
                    ...
                    return {'fee': fee_total, 'closed_pnl': pnl_total, 'count': len(matched)}
            attempt += 1
            if attempt < max_retries:
                time.sleep(retry_delay_s)
        return {}

    def fills_summary_by_oid(self, oid: int, since_ms: int, max_retries: int=4, retry_delay_s: float=0.5) -> dict:
        ...
        while attempt < max_retries:
            try:
                ...
                if getattr(exc, 'status_code', None) == 429:
                    print(f'[WARN] fills_summary lookup got HTTP 429 for oid={oid}; not retrying', file=sys.stderr)
                    return {}
                fills = None
            except Exception:
                fills = None
            if isinstance(fills, list):
                ...
                if matched:
                    ...
                    return {'filled_size': size_total, 'avg_px': avg_px, 'fee': fee_total, 'count': len(matched)}
            attempt += 1
            if attempt < max_retries:
                time.sleep(retry_delay_s)
        return {}
    ...
    def place_stop_loss(self, symbol: str, sz: float, trigger_px: float, is_buy: bool, limit_slippage_pct: float=5.0) -> dict:
        ...
        return exchange.order(symbol, is_buy, sz, limit_px, order_type, reduce_only=True)

    def place_take_profit_limit(self, symbol: str, sz: float, limit_px: float, is_buy: bool) -> dict:
    ...
    def get_position_leverage(self, symbol: str) -> dict | None:
        ...
        try:
            ...
            print(f'[WARN] HL get_position_leverage({symbol}) user_state failed: {exc}', file=sys.stderr)
            return None
        for asset_pos in user_state.get('assetPositions', []):
