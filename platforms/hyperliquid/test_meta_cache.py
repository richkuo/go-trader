...
def _sample_meta():
    return ({'universe': [{'index': 0, 'name': 'USDC/USDC', 'tokens': [0, 0]}], 'tokens': [{'name': 'USDC', 'szDecimals': 0}]}, {'universe': [{'name': 'BTC', 'szDecimals': 5}, {'name': 'ETH', 'szDecimals': 4}]})
...
def test_load_rejects_empty_universe(adapter_mod, cache_path):
    payload = {'ts': time.time(), 'spot_meta': {'universe': [], 'tokens': []}, 'meta': {'universe': []}}
    with open(cache_path, 'w') as f:
        json.dump(payload, f)
    assert adapter_mod._load_meta_cache(path=cache_path) is None
...
def _make_live_adapter(adapter_mod, monkeypatch):
    ...
    monkeypatch.setattr(adapter_mod, '_load_meta_cache', lambda *a, **kw: ({'universe': [], 'tokens': []}, {'universe': [{'name': 'BTC', 'szDecimals': 5}]}))
...
def test_lookup_fill_fee_returns_real_fill_on_match(adapter_mod, monkeypatch):
    a = _make_live_adapter(adapter_mod, monkeypatch)
    a._info.user_fills_by_time = MagicMock(return_value=[{'oid': 42, 'fee': '0.10', 'closedPnl': '1.50'}, {'oid': 42, 'fee': '0.05', 'closedPnl': '0.75'}, {'oid': 999, 'fee': '0.99', 'closedPnl': '9.99'}])
...
def _sparse_spot_meta():
    ...
    tokens += [{'name': 'WARS', 'szDecimals': 2, 'index': 479}, {'name': 'ZZZ', 'szDecimals': 1, 'index': 513}]
    universe = [{'index': 0, 'name': 'PURR/USDC', 'tokens': [1, 0]}, {'index': 367, 'name': '@367', 'tokens': [479, 0]}]
...
def test_normalize_drops_unresolvable_pairs(adapter_mod):
    spot_meta = {'universe': [{'index': 0, 'name': 'GOOD/USDC', 'tokens': [1, 0]}, {'index': 1, 'name': 'BAD/USDC', 'tokens': [999, 0]}, {'index': 2, 'name': 'MALFORMED'}], 'tokens': [{'name': 'USDC', 'szDecimals': 8, 'index': 0}, {'name': 'GOOD', 'szDecimals': 2, 'index': 1}]}
    normalized = adapter_mod._normalize_spot_meta(spot_meta)
...
def test_normalize_passes_through_aligned_meta_unchanged(adapter_mod):
    spot_meta = {'universe': [{'index': 0, 'name': 'P/USDC', 'tokens': [1, 0]}], 'tokens': [{'name': 'USDC', 'szDecimals': 8, 'index': 0}, {'name': 'P', 'szDecimals': 2, 'index': 1}]}
    normalized = adapter_mod._normalize_spot_meta(spot_meta)
...
def test_build_info_normalizes_before_sdk(adapter_mod, monkeypatch):
    spot_meta = _sparse_spot_meta()
    monkeypatch.setattr(adapter_mod, '_load_meta_cache', lambda *a, **kw: (spot_meta, {'universe': [{'name': 'BTC', 'szDecimals': 5}]}))
    captured = {}
