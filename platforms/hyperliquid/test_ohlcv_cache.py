...
def _sample_candles():
    return [[1700000000000, 100.0, 110.0, 90.0, 105.0, 50.0], [1700003600000, 105.0, 115.0, 95.0, 110.0, 60.0]]
...
def test_ttl_caps_fast_intervals_to_half_bar(adapter_mod):
    assert adapter_mod._ohlcv_cache_ttl(60000) == 30

def test_ttl_caps_slow_intervals_at_default(adapter_mod):
    assert adapter_mod._ohlcv_cache_ttl(3600000) == adapter_mod.OHLCV_CACHE_TTL_S
...
def test_second_call_hits_cache_and_skips_snapshot(adapter_mod, monkeypatch, tmp_path):
    ...
    rows = [_raw(1700000000000 + i * 3600000) for i in range(200)]
...
def test_cache_stores_trimmed_rows_not_widened_fetch(adapter_mod, monkeypatch, tmp_path):
    monkeypatch.setattr(adapter_mod, 'OHLCV_CACHE_DIR', str(tmp_path))
    extra = [_raw(1700000000000 + i * 3600000) for i in range(adapter_mod.OHLCV_GAP_MARGIN + 5)]
