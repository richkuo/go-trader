...
class OptionContract:
    ...
    def to_dict(self) -> dict:
        return {'symbol': self.symbol, 'underlying': self.underlying, 'strike': self.strike, 'expiry': self.expiry.isoformat(), 'option_type': self.option_type.value, 'bid': self.bid, 'ask': self.ask, 'last': self.last, 'mid_price': self.mid_price, 'usd_price': self.usd_price, 'volume': self.volume, 'open_interest': self.open_interest, 'dte': round(self.dte, 1), 'moneyness': self.moneyness, 'greeks': self.greeks.to_dict()}
...
class OptionPosition:
    ...
    def to_dict(self) -> dict:
        return {'id': self.id, 'symbol': self.symbol, 'underlying': self.underlying, 'strike': self.strike, 'expiry': self.expiry.isoformat(), 'option_type': self.option_type.value, 'side': self.side.value, 'quantity': self.quantity, 'entry_price': self.entry_price, 'entry_price_usd': self.entry_price_usd, 'current_price': self.current_price, 'pnl_usd': round(self.pnl_usd, 2), 'pnl_pct': round(self.pnl_pct, 2), 'dte': round(self.dte, 1), 'greeks': self.greeks.to_dict(), 'leg_group': self.leg_group}
...
def bs_greeks(S: float, K: float, T: float, r: float, sigma: float, option_type: OptionType) -> Greeks:
    if T <= 0 or sigma <= 0:
        ...
        delta = 1.0 if intrinsic > 0 and option_type == OptionType.CALL else -1.0 if intrinsic > 0 and option_type == OptionType.PUT else 0.0
...
def implied_volatility(market_price: float, S: float, K: float, T: float, r: float, option_type: OptionType, tol: float=1e-06, max_iter: int=100) -> float:
...
class DeribitOptionsAdapter:
    ...
    def __init__(self, api_key: Optional[str]=None, api_secret: Optional[str]=None, sandbox: bool=True, initial_balance_usd: float=10000.0):
        self.sandbox = sandbox
        self.initial_balance_usd = initial_balance_usd
        config = {'sandbox': sandbox, 'enableRateLimit': True}
    ...
    def load_markets(self, force: bool=False):
        ...
        self._option_markets = {k: v for k, v in markets.items() if v.get('type') == 'option' and v.get('active', True)}
    ...
    def get_option_chain(self, underlying: str='BTC', min_dte: float=0, max_dte: float=365, max_entries: int=500) -> List[OptionContract]:
        self.load_markets()
        ...
        for symbol, market in self._option_markets.items():
            ...
            if option_type_str not in ('call', 'put'):
                continue
            expiry = datetime.utcfromtimestamp(expiry_ts / 1000) if expiry_ts > 10000000000.0 else datetime.utcfromtimestamp(expiry_ts)
            dte = (expiry - now).total_seconds() / 86400
            if dte < min_dte or dte > max_dte:
                continue
            opt_type = OptionType.CALL if option_type_str == 'call' else OptionType.PUT
            contract = OptionContract(symbol=symbol, underlying=underlying, strike=float(strike), expiry=expiry, option_type=opt_type, spot_price=spot)
    ...
    def enrich_contract(self, contract: OptionContract) -> OptionContract:
        try:
            ...
            if mid > 0 and contract.spot_price > 0 and (contract.time_to_expiry > 0):
                ...
                iv = implied_volatility(market_price_usd, contract.spot_price, contract.strike, contract.time_to_expiry, RISK_FREE_RATE, contract.option_type)
                contract.greeks = bs_greeks(contract.spot_price, contract.strike, contract.time_to_expiry, RISK_FREE_RATE, iv, contract.option_type)
                ...
                self._iv_history[key] = [(t, v) for t, v in self._iv_history[key] if t > cutoff]
        except Exception as e:
            pass
        return contract

    def find_options(self, underlying: str, option_type: OptionType, min_dte: float=7, max_dte: float=60, moneyness: str='ATM', max_results: int=10) -> List[OptionContract]:
    ...
    def get_atm_iv(self, underlying: str, dte_target: float=30) -> float:
        calls = self.find_options(underlying, OptionType.CALL, min_dte=dte_target - 10, max_dte=dte_target + 10, moneyness='ATM', max_results=3)
        if not calls:
            return 0.0
        for c in calls:
    ...
    def buy_option(self, contract: OptionContract, quantity: float=1.0, leg_group: Optional[str]=None) -> Optional[OptionPosition]:
        ...
        pos = OptionPosition(id=self._next_order_id(), symbol=contract.symbol, underlying=contract.underlying, strike=contract.strike, expiry=contract.expiry, option_type=contract.option_type, side=OptionSide.BUY, quantity=quantity, entry_price=price, entry_price_usd=price * contract.spot_price, entry_time=datetime.utcnow(), entry_spot=contract.spot_price, current_price=price, current_spot=contract.spot_price, greeks=contract.greeks, leg_group=leg_group)
        self._positions[pos.id] = pos
        self._trades.append({'action': 'BUY', 'position_id': pos.id, 'symbol': contract.symbol, 'option_type': contract.option_type.value, 'strike': contract.strike, 'price': price, 'price_usd': cost_usd, 'quantity': quantity, 'commission': commission, 'timestamp': datetime.utcnow().isoformat()})
        return pos

    def sell_option(self, contract: OptionContract, quantity: float=1.0, leg_group: Optional[str]=None) -> Optional[OptionPosition]:
        contract = self.enrich_contract(contract)
        ...
        pos = OptionPosition(id=self._next_order_id(), symbol=contract.symbol, underlying=contract.underlying, strike=contract.strike, expiry=contract.expiry, option_type=contract.option_type, side=OptionSide.SELL, quantity=quantity, entry_price=price, entry_price_usd=price * contract.spot_price, entry_time=datetime.utcnow(), entry_spot=contract.spot_price, current_price=price, current_spot=contract.spot_price, greeks=contract.greeks, leg_group=leg_group)
        self._positions[pos.id] = pos
        self._trades.append({'action': 'SELL', 'position_id': pos.id, 'symbol': contract.symbol, 'option_type': contract.option_type.value, 'strike': contract.strike, 'price': price, 'price_usd': premium_usd, 'quantity': quantity, 'commission': commission, 'timestamp': datetime.utcnow().isoformat()})
        return pos
    ...
    def close_position(self, position_id: str) -> Optional[dict]:
        ...
        if pos.side == OptionSide.BUY:
            ...
            self._cash_usd -= cost_usd + commission
        pnl = pos.pnl_usd
        trade = {'action': 'CLOSE', 'position_id': position_id, 'symbol': pos.symbol, 'close_price': close_price, 'pnl_usd': pnl, 'commission': commission, 'timestamp': datetime.utcnow().isoformat()}
        ...
        for pid in expired_ids:
            ...
            if intrinsic > 0:
                ...
                if pos.side == OptionSide.BUY:
                    ...
                    self._cash_usd -= settlement_usd
                self._trades.append({'action': 'EXERCISED', 'position_id': pid, 'symbol': pos.symbol, 'settlement_usd': settlement_usd, 'intrinsic': intrinsic, 'timestamp': datetime.utcnow().isoformat()})
            else:
                self._trades.append({'action': 'EXPIRED', 'position_id': pid, 'symbol': pos.symbol, 'timestamp': datetime.utcnow().isoformat()})
        ...
        for pos in self._positions.values():
            try:
                ...
                if pos.current_price > 0 and pos.current_spot > 0:
                    ...
                    if T > 0:
                        iv = implied_volatility(market_usd, pos.current_spot, pos.strike, T, RISK_FREE_RATE, pos.option_type)
                        pos.greeks = bs_greeks(pos.current_spot, pos.strike, T, RISK_FREE_RATE, iv, pos.option_type)
            except Exception:
                pass

    def open_spread(self, buy_contract: OptionContract, sell_contract: OptionContract, quantity: float=1.0, name: str='spread') -> Optional[str]:
    ...
    def open_straddle(self, underlying: str, dte_target: float=30, side: OptionSide=OptionSide.BUY, quantity: float=1.0) -> Optional[str]:
        calls = self.find_options(underlying, OptionType.CALL, min_dte=dte_target - 7, max_dte=dte_target + 7, moneyness='ATM', max_results=1)
        puts = self.find_options(underlying, OptionType.PUT, min_dte=dte_target - 7, max_dte=dte_target + 7, moneyness='ATM', max_results=1)
    ...
    def open_strangle(self, underlying: str, dte_target: float=30, otm_pct: float=0.05, side: OptionSide=OptionSide.BUY, quantity: float=1.0) -> Optional[str]:
        calls = self.find_options(underlying, OptionType.CALL, min_dte=dte_target - 7, max_dte=dte_target + 7, moneyness='OTM', max_results=5)
        puts = self.find_options(underlying, OptionType.PUT, min_dte=dte_target - 7, max_dte=dte_target + 7, moneyness='OTM', max_results=5)
        if not calls or not puts:
            return None
        spot = self.get_spot_price(underlying)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if chain:
            sample = chain[0]
            print(f'Sample: {sample.symbol} | Strike: {sample.strike} | Type: {sample.option_type.value} | DTE: {sample.dte:.0f}')
    except Exception as e:
...
class DeribitExchangeAdapter:
    ...
    def get_real_strike(self, underlying: str, expiry: str, option_type: str, target_strike: float) -> float:
        try:
    ...
    def get_premium_and_greeks(self, underlying: str, option_type: str, strike: float, expiry: str, dte: float, spot: float, vol: float) -> Tuple[float, float, dict]:
        try:
