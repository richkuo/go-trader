...
class TestFetchAvailableExpiries:
    ...
    def _mock_instruments(self, days_list):
        ...
        for d in days_list:
            ...
            instruments.append({'expiration_timestamp': int(exp_time.timestamp() * 1000), 'instrument_name': f'BTC-TEST-C'})
        return instruments
...
class TestFindClosestExpiry:
    ...
    def test_returns_none_outside_tolerance(self):
        now = datetime.now(timezone.utc)
        instruments = [{'expiration_timestamp': int((now + timedelta(days=60)).timestamp() * 1000), 'instrument_name': 'BTC-TEST-C'}]
        mock_resp = MagicMock()
        mock_resp.json.return_value = {'result': instruments}
        mock_resp.raise_for_status = MagicMock()
        with patch('requests.get', return_value=mock_resp):
...
class TestFetchAvailableStrikes:
    ...
    def test_fetches_strikes_for_expiry(self):
        ...
        instruments = [{'expiration_timestamp': exp_ts, 'instrument_name': 'BTC-65000-C', 'strike': 65000}, {'expiration_timestamp': exp_ts, 'instrument_name': 'BTC-70000-C', 'strike': 70000}, {'expiration_timestamp': exp_ts, 'instrument_name': 'BTC-70000-P', 'strike': 70000}, {'expiration_timestamp': exp_ts + 86400 * 7 * 1000, 'instrument_name': 'BTC-80000-C', 'strike': 80000}]
        mock_resp = MagicMock()
        mock_resp.json.return_value = {'result': instruments}
        mock_resp.raise_for_status = MagicMock()
        with patch('requests.get', return_value=mock_resp):
...
class TestFindClosestStrike:
    ...
    def test_finds_closest(self):
        ...
        exp_ts = int(exp_time.timestamp() * 1000)
        expiry_str = target_date.strftime('%Y-%m-%d')
        instruments = [{'expiration_timestamp': exp_ts, 'instrument_name': 'BTC-65000-C', 'strike': 65000}, {'expiration_timestamp': exp_ts, 'instrument_name': 'BTC-70000-C', 'strike': 70000}, {'expiration_timestamp': exp_ts, 'instrument_name': 'BTC-75000-C', 'strike': 75000}]
        mock_resp = MagicMock()
        mock_resp.json.return_value = {'result': instruments}
        mock_resp.raise_for_status = MagicMock()
        with patch('requests.get', return_value=mock_resp):
...
class TestGetLiveQuote:
    ...
    def test_returns_quote(self):
        mock_resp = MagicMock()
        mock_resp.json.return_value = {'result': {'mark_price': 0.045, 'underlying_price': 67000, 'greeks': {'delta': 0.55, 'gamma': 0.0001, 'theta': -20.0, 'vega': 50.0}}}
        mock_resp.raise_for_status = MagicMock()
        with patch('requests.get', return_value=mock_resp):
...
class TestGetLivePremium:
    ...
    def test_returns_mark_price(self):
        mock_resp = MagicMock()
        mock_resp.json.return_value = {'result': {'mark_price': 0.055, 'underlying_price': 67000, 'greeks': {'delta': 0.5, 'gamma': 0, 'theta': 0, 'vega': 0}}}
        mock_resp.raise_for_status = MagicMock()
        with patch('requests.get', return_value=mock_resp):
