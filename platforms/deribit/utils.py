...
def find_closest_expiry(underlying: str, target_dte: int, max_tolerance_days: int=7) -> Optional[Tuple[str, int]]:
    ...
    if abs(actual_dte - target_dte) > max_tolerance_days:
        print(f'No Deribit expiry within {max_tolerance_days}d of target {target_dte} DTE (closest: {expiry_str} at {actual_dte} DTE, diff={abs(actual_dte - target_dte)}d)', file=sys.stderr)
        return None
    return best
...
def fetch_available_strikes(underlying: str, expiry_str: str, option_type: str='call') -> List[float]:
    try:
        ...
        resp.raise_for_status()
        data = resp.json()
        target_time = datetime.strptime(expiry_str, '%Y-%m-%d').replace(hour=8, minute=0, second=0, microsecond=0, tzinfo=timezone.utc)
...
def get_live_quote(underlying: str, option_type: str, strike: float, expiry_str: str) -> Optional[Dict[str, Any]]:
    try:
        ...
        return {'mark_price': float(mark_price), 'underlying_price': float(result.get('underlying_price') or 0), 'greeks': {'delta': float(greeks.get('delta', 0)), 'gamma': float(greeks.get('gamma', 0)), 'theta': float(greeks.get('theta', 0)), 'vega': float(greeks.get('vega', 0))}}
    except Exception as e:
