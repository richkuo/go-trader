...
_spec = importlib.util.spec_from_file_location('deribit_adapter', _adapter_path, submodule_search_locations=[os.path.dirname(_adapter_path)])
...
class TestOptionContract:
    ...
    def test_mid_price(self):
        c = OptionContract(symbol='BTC-100000-C', underlying='BTC', strike=100000, expiry=datetime.utcnow() + timedelta(days=30), option_type=OptionType.CALL, bid=0.05, ask=0.06, spot_price=67000)
    ...
    def test_dte_positive(self):
        c = OptionContract(symbol='BTC-100000-C', underlying='BTC', strike=100000, expiry=datetime.utcnow() + timedelta(days=30), option_type=OptionType.CALL)
        assert c.dte > 29

    def test_moneyness_itm_call(self):
        c = OptionContract(symbol='X', underlying='BTC', strike=60000, expiry=datetime.utcnow() + timedelta(days=30), option_type=OptionType.CALL, spot_price=67000)
        assert c.moneyness == 'ITM'

    def test_moneyness_otm_call(self):
        c = OptionContract(symbol='X', underlying='BTC', strike=75000, expiry=datetime.utcnow() + timedelta(days=30), option_type=OptionType.CALL, spot_price=67000)
        assert c.moneyness == 'OTM'

    def test_moneyness_atm_call(self):
        c = OptionContract(symbol='X', underlying='BTC', strike=67000, expiry=datetime.utcnow() + timedelta(days=30), option_type=OptionType.CALL, spot_price=67000)
...
class TestDeribitExchangeAdapter:
    ...
    def test_get_premium_and_greeks_bs_fallback(self):
        adapter = DeribitExchangeAdapter()
        with patch.dict(sys.modules, {'utils': None}):
            pct, usd, greeks = adapter.get_premium_and_greeks('BTC', 'call', 70000, '2026-05-01', 30, 67000, 0.6)
            assert usd > 0
