...
class TestMarketDataPaper:
    ...
    def test_get_ohlcv_paper_yahoo(self):
        ...
        data = {'Open': [5490, 5495, 5500, 5505, 5510], 'High': [5495, 5500, 5505, 5510, 5515], 'Low': [5485, 5490, 5495, 5500, 5505], 'Close': [5492, 5498, 5503, 5508, 5512], 'Volume': [100, 200, 150, 180, 120]}
...
class TestCashflowJournalFeeds:
    ...
    def test_equity_and_upnl_missing_cash_degrades_to_zero_upnl(self):
        session = MagicMock()
        resp = MagicMock()
        resp.json.return_value = {'equity': 1000.0}
        session.get.return_value = resp
        adapter = _live_adapter_with_session(session)
        equity, upnl = adapter.get_account_equity_and_upnl()
    ...
    def test_equity_and_upnl_missing_equity_raises(self):
        session = MagicMock()
        resp = MagicMock()
        resp.json.return_value = {'cashBalance': 50000.0}
        session.get.return_value = resp
        adapter = _live_adapter_with_session(session)
    ...
    def test_get_account_fills_shapes_and_sorts(self):
        session = MagicMock()
        resp = MagicMock()
        resp.json.return_value = {'fills': [{'id': 'f2', 'timestamp': 200, 'symbol': 'es', 'kind': 'TRADE', 'realizedPnl': 20, 'fee': 0.3}, {'id': 'f1', 'timestamp': 100, 'symbol': 'ES', 'kind': '', 'realizedPnl': 0, 'commission': 0.5}]}
        session.get.return_value = resp
        adapter = _live_adapter_with_session(session)
    ...
    def test_get_account_fills_drains_across_pages(self):
        page1 = {'fills': [{'id': 'a', 'timestamp': 100, 'symbol': 'ES', 'kind': 'trade', 'realizedPnl': 1, 'fee': 0}, {'id': 'b', 'timestamp': 200, 'symbol': 'ES', 'kind': 'trade', 'realizedPnl': 2, 'fee': 0}]}
        page2 = {'fills': [{'id': 'b', 'timestamp': 200, 'symbol': 'ES', 'kind': 'trade', 'realizedPnl': 2, 'fee': 0}, {'id': 'c', 'timestamp': 300, 'symbol': 'ES', 'kind': 'trade', 'realizedPnl': 3, 'fee': 0}]}
        page3 = {'fills': []}
        session = MagicMock()
    ...
    def test_get_account_fills_single_ms_overflow_caps(self):
        same_ms = {'fills': [{'id': 'x', 'timestamp': 500, 'symbol': 'ES', 'kind': 'trade', 'realizedPnl': 1, 'fee': 0}, {'id': 'y', 'timestamp': 500, 'symbol': 'ES', 'kind': 'trade', 'realizedPnl': 1, 'fee': 0}]}
        session = MagicMock()
        resp = MagicMock()
