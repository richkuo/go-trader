...
YAHOO_SYMBOL_MAP = {'ES': 'ES=F', 'NQ': 'NQ=F', 'MES': 'MES=F', 'MNQ': 'MNQ=F', 'CL': 'CL=F', 'GC': 'GC=F'}
CONTRACT_SPECS = {'ES': {'tick_size': 0.25, 'tick_value': 12.5, 'multiplier': 50, 'margin': 15400, 'type': 'index'}, 'NQ': {'tick_size': 0.25, 'tick_value': 5.0, 'multiplier': 20, 'margin': 21000, 'type': 'index'}, 'MES': {'tick_size': 0.25, 'tick_value': 1.25, 'multiplier': 5, 'margin': 1540, 'type': 'index'}, 'MNQ': {'tick_size': 0.25, 'tick_value': 0.5, 'multiplier': 2, 'margin': 2100, 'type': 'index'}, 'CL': {'tick_size': 0.01, 'tick_value': 10.0, 'multiplier': 1000, 'margin': 7500, 'type': 'energy'}, 'GC': {'tick_size': 0.1, 'tick_value': 10.0, 'multiplier': 100, 'margin': 11000, 'type': 'metals'}}
...
def _normalize_topstep_fill(f):
    try:
        ...
        return None
    return {'fill_id': str(f.get('id', f.get('fill_id', ''))), 'ts_ms': ts_ms, 'symbol': (f.get('symbol') or '').upper(), 'kind': (f.get('kind') or '').lower(), 'realized_pnl': float(f.get('realizedPnl', f.get('realized_pnl', 0)) or 0), 'fee': float(f.get('fee', f.get('commission', 0)) or 0)}
...
class TopStepExchangeAdapter:
    ...
    def __init__(self, mode='paper'):
        ...
        if mode == 'live':
            if not self._api_key or not self._api_secret or (not self._account_id):
                raise RuntimeError('Live mode requires TOPSTEP_API_KEY, TOPSTEP_API_SECRET, and TOPSTEP_ACCOUNT_ID environment variables')
            try:
                import requests
                self._session = requests.Session()
                self._session.headers.update({'X-API-Key': self._api_key, 'X-API-Secret': self._api_secret, 'Content-Type': 'application/json'})
            except ImportError:
    ...
    def get_price(self, symbol: str) -> float:
        ...
        try:
            resp = self._session.get(f'{API_BASE_URL}/v1/market/quote', params={'symbol': symbol, 'accountId': self._account_id}, timeout=10)
    ...
    def get_ohlcv(self, symbol: str, interval: str='1h', limit: int=200) -> list:
        ...
        try:
            resp = self._session.get(f'{API_BASE_URL}/v1/market/candles', params={'symbol': symbol, 'interval': interval, 'limit': limit, 'accountId': self._account_id}, timeout=15)
            resp.raise_for_status()
            candles = resp.json().get('candles', [])
            result = []
            for c in candles:
                result.append([int(c.get('timestamp', 0)), float(c.get('open', 0)), float(c.get('high', 0)), float(c.get('low', 0)), float(c.get('close', 0)), float(c.get('volume', 0))])
            return result
        except Exception as e:
    ...
    def get_open_positions_raise(self) -> list:
        ...
        if self._session is None:
            raise RuntimeError('TopStep live session not initialized — missing TOPSTEP_API_KEY / TOPSTEP_API_SECRET / TOPSTEP_ACCOUNT_ID')
        return self._fetch_open_positions()

    def _fetch_open_positions(self) -> list:
        resp = self._session.get(f'{API_BASE_URL}/v1/account/positions', params={'accountId': self._account_id}, timeout=10)
        resp.raise_for_status()
        positions = []
        for pos in resp.json().get('positions', []):
            ...
            if qty == 0:
                continue
            positions.append({'symbol': pos.get('symbol', ''), 'quantity': qty, 'avg_price': float(pos.get('avgPrice', 0)), 'side': 'long' if qty > 0 else 'short', 'unrealized_pnl': float(pos.get('unrealizedPnl', 0))})
        return positions

    def get_account_equity_and_upnl(self):
        if not self.is_live:
            raise RuntimeError('get_account_equity_and_upnl requires live mode')
        resp = self._session.get(f'{API_BASE_URL}/v1/account/balance', params={'accountId': self._account_id}, timeout=10)
        resp.raise_for_status()
        data = resp.json()
    ...
    def get_account_fills(self, since_ms=0, page_limit=100, max_fills=10000):
        ...
        for _ in range(max(1, max_fills // max(1, page_limit)) + 2):
            resp = self._session.get(f'{API_BASE_URL}/v1/account/fills', params={'accountId': self._account_id, 'sinceMs': cursor, 'limit': int(page_limit)}, timeout=15)
            resp.raise_for_status()
    ...
    def _get_yahoo_ohlcv(self, symbol: str, interval: str='1h', limit: int=200) -> list:
        ...
        try:
            ...
            for idx, row in hist.iterrows():
                ...
                result.append([ts_ms, float(row['Open']), float(row['High']), float(row['Low']), float(row['Close']), float(row.get('Volume', 0))])
            return result[-limit:]
        except ImportError:
    ...
    def market_open(self, symbol: str, is_buy: bool, contracts: int) -> dict:
        ...
        resp = self._session.post(f'{API_BASE_URL}/v1/order/market', json={'accountId': self._account_id, 'symbol': symbol, 'side': 'buy' if is_buy else 'sell', 'quantity': contracts}, timeout=10)
        resp.raise_for_status()
        return resp.json()
    ...
    def market_close(self, symbol: str) -> dict:
        ...
        resp = self._session.post(f'{API_BASE_URL}/v1/order/close', json={'accountId': self._account_id, 'symbol': symbol}, timeout=10)
        resp.raise_for_status()
        return resp.json()
