...
YAHOO_CRYPTO_MAP = {'BTC': 'BTC-USD', 'ETH': 'ETH-USD', 'SOL': 'SOL-USD', 'DOGE': 'DOGE-USD', 'AVAX': 'AVAX-USD', 'LINK': 'LINK-USD', 'ADA': 'ADA-USD', 'DOT': 'DOT-USD', 'MATIC': 'MATIC-USD', 'SHIB': 'SHIB-USD'}
STRIKE_INTERVALS = [(100, 1), (500, 5), (float('inf'), 10)]
...
class RobinhoodExchangeAdapter:
    ...
    def _login(self):
        ...
        if not username or not password or (not totp_secret):
            if self._mode == 'live':
                raise RuntimeError('Live mode requires ROBINHOOD_USERNAME, ROBINHOOD_PASSWORD, and ROBINHOOD_TOTP_SECRET environment variables')
            return
    ...
    def get_real_strike(self, underlying: str, expiry: str, option_type: str, target_strike: float) -> float:
        if self._logged_in:
            try:
                import robin_stocks.robinhood as rh
                options = rh.options.find_tradable_options(underlying, expirationDate=expiry, optionType=option_type)
                if options:
                    best_strike = None
                    best_diff = float('inf')
                    for opt in options:
    ...
    def get_premium_and_greeks(self, underlying: str, option_type: str, strike: float, expiry: str, dte: float, spot: float, vol: float) -> Tuple[float, float, dict]:
        if self._logged_in:
            try:
                import robin_stocks.robinhood as rh
                options = rh.options.find_tradable_options(underlying, expirationDate=expiry, strikePrice=str(strike), optionType=option_type)
                if options:
                    ...
                    if opt_id:
                        ...
                        if market_data and isinstance(market_data, list) and market_data[0]:
                            ...
                            greeks = {'delta': float(md.get('delta', 0) or 0), 'gamma': float(md.get('gamma', 0) or 0), 'theta': float(md.get('theta', 0) or 0), 'vega': float(md.get('vega', 0) or 0)}
    ...
    def get_crypto_positions_strict(self) -> list:
        if not self._logged_in:
            raise RuntimeError('Robinhood adapter not logged in — cannot fetch crypto positions')
        import robin_stocks.robinhood as rh
        ...
        for pos in positions:
            ...
            result.append({'symbol': symbol, 'quantity': qty, 'avg_price': avg_price})
        return result
