...
| `tune_live.py` | One-command live-strategy tuner: resolves each config strategy's effective live params (reusing `run_backtest.load_strategy_config`), searches a bounded neighborhood (registry `constraints` + `DEFAULT_PARAM_RANGES`, operator `--param`/`--overrides`) via stage-1 walk-forward, then gates survivors through `auto_suggest` under selection-aware inference — stage-1 data sliced disjoint from the stage-2 windows, BH family size = searched N (`correction.family_size`). Emits a schema-versioned artifact + patches + progress JSON (#1339–#1341). **Suggest-only — never writes a config/default/PR.** | active | #1338 |
