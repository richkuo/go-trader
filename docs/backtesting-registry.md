...
| `research/hurst_1422_gate_power.py` | POWER study that supersedes #1410 as the live Hurst evidence. Measures the MINIMUM DETECTABLE EFFECT of both designs and reports it beside the separation on the SAME rows, so an underpowered null is never published as evidence of absence, adds pre-2023 history and non-audit symbols, replaces the free-shuffle null with a shared circular calendar rotation so correlated concurrent trades move together, reports EFFECTIVE N beside nominal (and applies the volume floors to it), scores a small pre-registered primary hypothesis set only on cells #1410 never saw, and renders the joint ADX x Hurst bucket section #1412's Stage 0 gate reads. Report-only: OWNS the contract path `hurst_gate_calibration.md` plus `hurst_1422_gate_power.json`, and touches no scheduler/config/live path. `--render-only` re-renders from the committed JSON; `--fetch-only` backfills history. | one-shot | #1422 |
