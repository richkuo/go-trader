...
| `research/hurst_1410_gate_calibration.py` | Calibration evidence for a POSSIBLE future Hurst entry gate — buckets entry outcomes by H at entry (#1409 `hurst_exponent`, NaN is its own bucket), sweeps hysteresis gate pairs as real masked-entry `Backtester` re-runs and a size-multiplier variant, over the M1 windows/datasets under one Benjamini–Hochberg correction. Report-only: writes `hurst_gate_calibration.md` + `hurst_1410_gate_calibration.json` and touches no scheduler/config/live path (honours the #1409 advisory-only invariant). `--render-only` re-renders the report from the committed JSON. | one-shot | #1410 |
