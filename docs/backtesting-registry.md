...
| `parity_diff.py` | Backtest-vs-live parity diff — enforces the simulator↔scheduler contract. `--batched` (#1442, off by default) adds a `solo_*` vs `batch_*` dimension: each compared bar is evaluated through the Hyperliquid signal evaluator alone and as one slot of a multi-slot batch, so a batching defect surfaces per bar on real candles. | active | #906, #1442 |
...
| `research/hurst_1426_two_sided_sort.py` | TWO-SIDED re-test of the hypothesis #1410, #1422 and #1424 all tested ONE-SIDED. #1424 measured a confirmatory separation of -0.005 efficiency units against a row-matched limit of 0.013: the SIGN is the finding, because a negative `mean(kept) - mean(suppressed)` means the SUPPRESSED trades did better, the one direction a one-sided null cannot reject at any magnitude, so the observation was neither a rejection nor a confirmation. This study inherits #1424's design WHOLESALE (datasets, windows and ownership matrix, cohorts, estimator, targets, cluster-rotation null internals, effective N, coverage floors, warm-up audit, acceptance rule) and changes only the DIRECTION of the inference: every confirmatory p becomes a doubled one-tailed permutation p `min(1, 2*min(p_ge, p_le))` over the surviving draws, the detection limit injects BOTH directions and takes `max(p(+d), p(-d))` so the published limit is symmetric, and the validity gate compares the MAGNITUDE of the separation — legitimate only because the null underneath it is now symmetric — while staying ROW-MATCHED on the confirmatory family and preserving the sign. `MODE_REVERSED` ceases to exist. Because the sign was seen before the design was fixed, the study takes the issue's OPTION 2 and is EXPLORATORY-ONLY over the full pool: there is no `VERDICT_CONFIG` and no winner branch, so no configuration can be recommended. It DEFERS the live-evidence contract path to #1424 and refuses `hurst_gate_calibration.md` unconditionally; the supersede clause stays available to #1427 and #1428. One disclosed exception to two-sidedness: the joint ADX x Hurst Stage 0 verdict is inherited one-sided on net return so the verdict recorded against #1412 stays comparable, and it sits outside the confirmatory path. Report-only: writes `hurst_1426_two_sided_sort.md` + `hurst_1426_two_sided_sort.json` and touches no scheduler/config/live path. `--render-only` re-renders from the committed JSON; `--fetch-only` backfills history. | one-shot | #1426 |
| `research/hurst_1427_change_sort.py` | Tests whether the CHANGE in the Hurst exponent sorts entry outcomes, where #1410, #1422, #1424 and #1426 all bucket its LEVEL. The change series is a DIFFERENCE of the same rolling H those studies compute (`dH(i) = H(i) - H(i - W//2)`, no new estimator, `hurst_exponent` stays the #1409 SSoT), and it inherits H's look-ahead convention exactly (decision series shifted one bar, fill-bar stamp shifted twice), with NaN PROPAGATING from either endpoint rather than being filled: an undefined change is its own bucket, never 0, it holds the gate state, and it sizes at exactly 1. The three parameters a change study invents are all pre-registered as named constants and rendered from the run's JSON: ONE lookback rule (`W // 2`, the smallest lookback at which half the newer estimate's sample is new, and never swept, so it adds no Benjamini-Hochberg multiplicity), the bucket edges and gate pairs (#1410's committed level landmarks RE-CENTRED by subtracting 0.5, derived and asserted at import time), and the DIRECTION of the inference (TWO-SIDED, chosen because #1424's only measured effect on these rows pointed the way a one-sided design cannot detect). The pinned confirmatory hypothesis is #1424's own pin re-centred, so nothing in this study's data chose it. Everything else is #1424's, imported rather than restated: datasets, windows and the ownership matrix, cohorts, targets, #1426's two-sided cluster null, effective N, coverage floors and acceptance rule. The warm-up audit and the coverage floor account for the lookback ON TOP of the Hurst window (`W + W//2 + margin`), and every dropped cell is listed with its reason. Report-only and exploratory with respect to the live evidence: it DEFERS the live-evidence contract path `hurst_gate_calibration.md` to #1424 and refuses it unconditionally, because `scheduler/hurst_gate.go` reads the LEVEL of H and never its change, so a positive result licenses a follow-up DESIGN rather than a threshold. The supersede clause passes to #1428. #1426 defers for its own reason. It takes NO #1412 Stage 0 verdict, because that question is defined on the level, so its joint ADX x change-in-H table is descriptive only. Committed run: verdict `change_sorts`, two-sided confirmatory cluster p=0.010995 against a 0.05 bar on 7,971 row-matched `momentum` rows with a separation of +0.009863 efficiency units, but the validity gate PASSES TRIVIALLY (limit 0.000, the degenerate value the search returns when the zero-injection contrast already clears the bar) so the effect SIZE is unestimated, and the continuity target separates +0.550492 pp of net return at p=0.814593 BELOW its 1.10 pp limit. NO threshold ships and no configuration can be recommended structurally. Writes `hurst_1427_change_sort.md` + `hurst_1427_change_sort.json` and touches no scheduler/config/live path; `--render-only` re-renders byte-for-byte from the committed JSON, `--fetch-only` backfills history. | one-shot | #1427 |
