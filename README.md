...
**Folding a paper instance into the live process.** Stop both units, then `bash scripts/merge-paper-instance.sh --live <live> --paper <paper>` (dry run) and, once it prints `VERDICT: READY`, the same command with `--apply`. Both state files stay where they are: the live config gains `paper_db_file`, colliding paper ids get a `-paper` suffix with `storage_strategy_id` keeping the stored identity, and a systemd drop-in grants the paper database directory. The script holds both databases' locks for the run, verifies the merged config with the live binary's own `inspect --all --json` and `storage-inspect --json`, and never opens a database for writing. Back up both files first, then `daemon-reload`, disable the paper unit, start the live unit and retire the paper status port. `--rollback` restores the previous config and drop-in. See `scheduler/config.live-paper.example.json` and SKILL.md § Storage Ownership.
