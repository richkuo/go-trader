...
- **`--restart-mode systemd`** (default) — `sudo systemctl restart <unit>`, polls `systemctl is-active` + `/health`. If the unit is not loaded (systemctl exit 5), the script falls back to the signal path when `go-trader.pid` and `run.sh` exist (#785).
